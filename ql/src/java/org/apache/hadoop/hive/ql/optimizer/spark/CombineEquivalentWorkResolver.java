...
block|}
decl_stmt|;
comment|// maps from a work to the DPPs it contains -- used to combine equivalent DPP sinks
specifier|private
name|Map
argument_list|<
name|BaseWork
...
argument_list|()
decl_stmt|;
comment|// maps from unique id to DPP sink -- used to update the DPP sinks when
comment|// target map works are removed
specifier|private
name|Map
argument_list|<
name|String
...
block|}
block|}
comment|// Remove the map work from DPP sinks that have it as a target
specifier|private
name|void
name|removeTargetFromDPP
parameter_list|(
...
name|sink
argument_list|,
literal|"Unable to find DPP sink whose target work is removed."
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// If the target can be removed, it means there's another MapWork that shares the same
comment|// DPP sink, and therefore it cannot be the only target.
name|Preconditions
operator|.
name|checkState
argument_list|(
...
argument_list|()
argument_list|,
literal|"The removed target work is the only target."
argument_list|)
expr_stmt|;
block|}
block|}
...
return|;
block|}
comment|// Checks whether two MapWorks will be pruned by the same DPP sink
specifier|private
name|boolean
name|targetsOfSameDPPSink
parameter_list|(
...
return|;
block|}
comment|// check whether each DPP sink target same columns
for|for
control|(
name|String
name|source
