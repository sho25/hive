...
argument_list|)
decl_stmt|;
comment|// extract drop privileges
name|DropPrivilegeExtractor
name|privExtractor
init|=
operator|new
name|DropPrivilegeExtractor
...
name|privExtractor
operator|.
name|getWriteReqPriv
argument_list|()
expr_stmt|;
comment|// authorize drops if there was a drop privilege requirement
if|if
condition|(
name|privExtractor
operator|.
...
throw|;
block|}
comment|// extract any drop privileges out of required privileges
name|DropPrivilegeExtractor
name|privExtractor
init|=
operator|new
...
argument_list|()
expr_stmt|;
comment|// if CREATE or DROP priv requirement is there, the owner should have WRITE permission on
comment|// the database directory
if|if
condition|(
name|privExtractor
operator|.
...
expr_stmt|;
block|}
comment|// If the user has specified a location - external or not, check if the user
comment|// has the permissions on the table dir
if|if
condition|(
name|path
operator|!=
...
block|}
block|}
comment|/**    *    * @param privs    * @return true, if set of given privileges privs contain CREATE privilege    */
specifier|private
name|boolean
name|requireCreatePrivilege
parameter_list|(
...
block|{
class|class
name|MustCheckTablePermissions
block|{
comment|// For closure.
specifier|public
name|boolean
...
argument_list|)
decl_stmt|;
comment|// Get partition paths. Filter out null-partitions, and partitions without data-locations.
name|Iterator
argument_list|<
name|Partition
argument_list|>
name|nonNullPartitions
init|=
name|Iterators
operator|.
name|filter
...
argument_list|(
literal|"Could not find location for partition: "
operator|+
name|partition
argument_list|,
name|exception
...
condition|)
block|{
comment|// At least one partition was null, or had a non-existent path. So check table-permissions, once.
comment|// Partition path can be null in the case of a new create partition - in this case,
comment|// we try to default to checking the permissions of the parent table.
comment|// Partition itself can also be null, in cases where this gets called as a generic
comment|// catch-all call in cases like those with CTAS onto an unpartitioned table (see HIVE-1887)
