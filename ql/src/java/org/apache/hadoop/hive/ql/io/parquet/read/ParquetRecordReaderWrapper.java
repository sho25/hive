...
comment|// TODO: This line is left due to incorrect Predicate push down results (parquet_ppd_char,parquet_ppd_varchar).
comment|// The problem is that Parquet PPD is set on getSplit() function called above, but the old code used this
comment|// line to overwrite such configuration. I'm adding a fix to timestamp issues only, so we should follow up
comment|// this issue in another JIRA.
name|JobConf
name|conf
init|=
operator|new
...
argument_list|)
decl_stmt|;
comment|// Set the TimeZone conversion in case the file has timestamp columns.
