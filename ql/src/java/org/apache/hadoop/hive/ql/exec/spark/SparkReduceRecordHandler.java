...
argument_list|()
decl_stmt|;
comment|// CONSIDER: Cleaning up this code and eliminating the arrays.  Vectorization only handles
comment|// one operator tree.
name|Preconditions
operator|.
name|checkState
argument_list|(
...
expr_stmt|;
block|}
comment|/**    * A reusable dummy iterator that has only one value.    *    */
specifier|private
specifier|static
class|class
name|DummyIterator
...
argument_list|(
literal|"Iterator.remove() is not implemented/supported"
argument_list|)
throw|;
block|}
block|}
...
argument_list|()
decl_stmt|;
comment|/**    * Process one row using a dummy iterator.  Or, add row to vector batch.    */
annotation|@
name|Override
specifier|public
name|void
...
condition|)
block|{
comment|// Flush current group batch as last batch of group.
if|if
condition|(
name|batch
operator|.
...
throw|;
block|}
comment|// And, mark group keys as repeating.
for|for
control|(
name|int
name|i
...
block|{
comment|// Reset key and value columns; and batch.size
name|batch
operator|.
name|reset
argument_list|()
