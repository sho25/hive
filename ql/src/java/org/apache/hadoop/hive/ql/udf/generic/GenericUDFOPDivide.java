...
block|{
return|return
name|deriveResultExactTypeInfoAnsiSql
argument_list|()
return|;
block|}
return|return
name|deriveResultExactTypeInfoBackwardsCompat
argument_list|()
return|;
block|}
specifier|protected
...
parameter_list|()
block|{
comment|// Preserve existing return type behavior for division:
comment|// Non-decimal division should return double
if|if
condition|(
name|leftOI
operator|.
...
specifier|static
name|int
name|DEFAULT_DEC_DIGITS
init|=
literal|18
decl_stmt|;
comment|/**    * A balanced way to determine the precision/scale of decimal division result. Integer digits and    * decimal digits are computed independently. However, when the precision from above reaches above    * HiveDecimal.MAX_PRECISION, integer digit and decimal digits are shrunk equally to fit.    */
annotation|@
name|Override
specifier|protected
name|DecimalTypeInfo
