...
block|}
block|}
comment|// selected[] points to the valid/filtered/selected records at row level.
comment|// for MultiValuedColumnVector such as ListColumnVector one record of vector points to multiple nested records.
comment|// In child vectors we get these records in exploded manner i.e. the number of records in child vectors can have size more
comment|// than actual the VectorizedRowBatch, consequently selected[] also needs to be readjusted.
comment|// This method creates a shallow copy of VectorizedRowBatch with corrected size and selected[]
specifier|private
specifier|static
name|VectorizedRowBatch
name|correctSelectedAndSize
parameter_list|(
name|VectorizedRowBatch
name|sourceVrb
parameter_list|,
