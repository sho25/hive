...
argument_list|)
expr_stmt|;
comment|// Get the table name from which the min-max values and bloom filter will come.
name|Operator
argument_list|<
name|?
argument_list|>
...
condition|)
block|{
comment|// If hints map has no entry that would imply that user enforced
comment|// no runtime filtering.
if|if
condition|(
name|hints
operator|.
...
name|SemanticException
block|{
comment|// If semijoin hint is enforced, make sure hint is provided
if|if
condition|(
name|parseContext
operator|.
...
block|}
block|}
comment|// If hint is provided and only hinted semijoin optimizations should be
comment|// created, then skip other columns on the table
if|if
condition|(
name|parseContext
operator|.
