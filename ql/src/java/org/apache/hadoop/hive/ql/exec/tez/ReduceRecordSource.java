...
decl_stmt|;
comment|// Flush the last record when reader is out of records
specifier|private
name|boolean
name|flushLastRecord
init|=
literal|false
decl_stmt|;
name|void
...
condition|)
block|{
comment|// For now, we don't go higher than the default batch size unless we do more work
comment|// to verify every vectorized operator downstream can handle a larger batch size.
name|vectorizedTestingReducerBatchSize
operator|=
name|VectorizedRowBatch
operator|.
...
condition|)
block|{
comment|// Batch is full AND we have at least 1 more row...
name|batch
operator|.
name|size
operator|=
