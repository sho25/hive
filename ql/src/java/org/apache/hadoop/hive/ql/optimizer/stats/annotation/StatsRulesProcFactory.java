...
argument_list|)
decl_stmt|;
comment|// the idea is to measure unmatche rows in outer joins by figuring out how many rows didn't match
comment|// mismatched rows are figured using denomUnmatched which is the difference of denom used for computing
comment|// join cardinality minus the ndv which wasn't used. This number (mismatched rows) is then subtracted from
comment|/// join cardinality to get the rows which didn't match
name|long
name|unMatchedRows
init|=
name|Math
...
condition|)
block|{
comment|// if unmatched denominator is zero we take it as all rows will match
name|unMatchedRows
operator|=
literal|0
expr_stmt|;
...
name|LEFT_OUTER_JOIN
case|:
comment|//if this column is coming from right input only then we update num nulls
if|if
condition|(
name|pos
operator|==
...
block|{
comment|// TODO: oldNumNulls should be scaled instead of taken as it is
name|newNumNulls
operator|=
name|Math
operator|.
