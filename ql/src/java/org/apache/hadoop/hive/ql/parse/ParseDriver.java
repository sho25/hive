...
comment|// for
comment|// the lexical analysis part of antlr. By converting the token stream into
comment|// upper case
comment|// at the time when lexical rules are checked, this class ensures that the
comment|// lexical rules
comment|// need to just match the token with upper case letters as opposed to
comment|// combination of upper
comment|// case and lower case characteres. This is purely used for matching lexical
comment|// rules. The
comment|// actual token text is stored in the same way as the user input without
comment|// actually converting
comment|// it into an upper case. The token values are generated by the consume()
comment|// function of the
comment|// super class ANTLRStringStream. The LA() function is the lookahead funtion
comment|// and is purely
comment|// used for matching lexical rules. This also means that the grammar will only
comment|// accept
comment|// capitalized tokens in case it is run from other tools like antlrworks which
comment|// do not
