...
expr_stmt|;
block|}
comment|// Hash Set iteration isn't ordered, but force string sorted order
comment|// to get a consistent test run.
specifier|private
name|Collection
argument_list|<
name|ExprNodeDescEqualityWrapper
...
condition|)
block|{
comment|// normal case - sorting is wasted for an IN()
return|return
name|valuesExpr
return|;
block|}
...
parameter_list|)
block|{
comment|// fail if you find nulls (this is a test-code section)
if|if
condition|(
name|w1
operator|.
...
argument_list|)
decl_stmt|;
comment|// extract pre-conditions for the tuple expressions
comment|// (a,b) IN ((1,2),(2,3)) ->
comment|//          ((a) IN (1,2) and b in (2,3)) and (a,b) IN ((1,2),(2,3))
for|for
control|(
name|String
name|keyString
...
name|left
decl_stmt|;
comment|// generate a partial IN clause, if the column is a partition column
if|if
condition|(
name|lookupCol
operator|.
...
condition|)
block|{
comment|// optimize only nDV reductions
specifier|final
name|List
argument_list|<
name|ExprNodeDesc
...
block|}
block|}
comment|// loop complete, inspect the sub expressions generated
if|if
condition|(
name|subExpr
operator|.
...
condition|)
block|{
comment|// add the newPredicate to the end& produce an AND clause
name|subExpr
operator|.
name|add
argument_list|(
...
expr_stmt|;
block|}
comment|// else, newPredicate is unmodified
block|}
return|return
name|newPredicate
return|;
