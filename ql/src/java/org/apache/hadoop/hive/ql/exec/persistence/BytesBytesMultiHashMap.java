...
literal|1024
decl_stmt|;
comment|/** Make sure maxCapacity has a lower limit */
specifier|private
specifier|final
specifier|static
name|int
name|DEFAULT_MIN_MAX_CAPACITY
init|=
literal|16
operator|*
literal|1024
...
expr_stmt|;
block|}
comment|/**      * Return the thread-safe read position.      */
specifier|public
name|WriteBuffers
operator|.
name|Position
...
expr_stmt|;
block|}
comment|/**    * Finds a key.  Values can be read with the supplied result object.    *    * Important Note: The caller is expected to pre-allocate the hashMapResult and not    * share it among other threads.    *    * @param key Key buffer.    * @param offset the offset to the key in the buffer    * @param hashMapResult The object to fill in that can read the values.    * @param matchTracker Opitional object for tracking key matches.    * @return The state byte.    */
specifier|public
name|byte
name|getValueResult
parameter_list|(
...
expr_stmt|;
block|}
comment|/**    * Finds the next non matched Small Table key and value.  Supports FULL OUTER MapJoin.    *    * @param currentSlotNum  Start by specifying -1; the return index from the previous call.    * @param keyRef          If the return value is not -1, a reference to the key bytes.    * @param hashMapResult   If the return value is not -1, the key's values.    * @param matchTracker    The object that tracks matches (non-shared).    * @return The current index of the non-matched key; or -1 if no more.    */
specifier|public
name|int
...
argument_list|)
expr_stmt|;
comment|// 11 primitive1 fields, 2 refs above with alignment
name|size
operator|+=
name|JavaDataModel
operator|.
...
argument_list|(
literal|"Attempting to expand the hash table to "
operator|+
name|capacity
operator|+
literal|" that overflows maximum array size. For this query, you may want to disable "
operator|+
name|ConfVars
operator|.
...
condition|)
block|{
comment|// Support for FULL OUTER MapJoin.  Track matches of the slot table entry.
name|matchTracker
operator|.
name|trackMatch
argument_list|(
