...
return|;
block|}
comment|/**    * Processes the provided task logs to extract stack traces.    * @return A list of lists of strings where each list of strings represents a stack trace    */
specifier|public
name|List
argument_list|<
name|List
...
control|)
block|{
comment|// Open the log file, and read the lines, parse out stack traces
name|URL
name|taskAttemptLogUrl
decl_stmt|;
try|try
...
literal|null
decl_stmt|;
comment|// Patterns that match the middle/end of stack traces
name|Pattern
...
condition|)
block|{
comment|// We are in a stack trace
if|if
condition|(
name|stackTrace
operator|==
...
condition|)
block|{
comment|// This is the first time we have realized we are in a stack trace.  In this case,
comment|// the previous line was the error message, add that to the stack trace as well
name|stackTrace
operator|=
operator|new
name|ArrayList
...
condition|)
block|{
comment|// The last line didn't match a pattern, it is probably an error message, part of
comment|// a string of stack traces related to the same error message so add it to the stack
comment|// trace
name|stackTrace
operator|.
name|add
...
condition|)
block|{
comment|// If the last line didn't match the patterns either, the stack trace is definitely
comment|// over
name|stackTraces
operator|.
name|add
