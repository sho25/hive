...
expr_stmt|;
block|}
comment|/**    * Get the value based on the key. this GET method will directly    * return the value from jdbm storage.    * @param key    * @return Value corresponding to the key. If the key is not found, return    *         null.    */
comment|/*   public V getMapJoinValueObject(K key) throws HiveException{     if(pHash == null) {       LOG.warn("the jdbm object is not ready!");       throw new HiveException();     }     try{       V value = (V)pHash.get(key);       return value;     }catch(Exception e){       throw new HiveException(e);     }   }*/
comment|/*    * In this get operation, the jdbm should read only    */
specifier|public
name|V
...
comment|//pHash.put(tail.key, tail.value);
comment|//pHash.remove(key);
comment|//recman.commit();
...
try|try
block|{
comment|//if no persistent cache file; create a new one
if|if
condition|(
name|pHash
operator|==
...
argument_list|()
decl_stmt|;
comment|//no data in the memory cache
if|if
condition|(
name|mm_size
operator|==
...
return|;
block|}
comment|//iterate the memory hash table and put them into persistent file
for|for
control|(
name|Map
operator|.
...
expr_stmt|;
block|}
comment|//commit to the persistent file
name|recman
operator|.
name|commit
argument_list|()
...
name|warn
argument_list|(
literal|"initiliaze the hash table by jdbm file Error!"
argument_list|)
expr_stmt|;
throw|throw
operator|new
