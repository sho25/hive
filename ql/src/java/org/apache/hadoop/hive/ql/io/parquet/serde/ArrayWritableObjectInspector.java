...
name|fieldsByName
decl_stmt|;
comment|// Whether this OI is for the column-level schema (as opposed to nested column fields).
specifier|private
specifier|final
name|boolean
name|isRoot
...
name|index
decl_stmt|;
comment|// This is the adjusted index after nested column pruning.
comment|// For instance, given the struct type: s:<struct<a:int, b:boolean>>
comment|// If only 's.b' is used, the pruned type is: s:<struct<b:boolean>>.
comment|// Here, the index of field 'b' is changed from 1 to 0.
comment|// When we look up the data from Parquet, index needs to be adjusted accordingly.
comment|// Note: currently this is only used in the read path.
specifier|final
name|int
name|adjustedIndex
decl_stmt|;
