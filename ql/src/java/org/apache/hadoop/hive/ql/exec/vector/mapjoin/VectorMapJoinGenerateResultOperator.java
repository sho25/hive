...
specifier|final
name|int
name|CHECK_INTERRUPT_PER_OVERFLOW_BATCHES
init|=
literal|10
decl_stmt|;
comment|//------------------------------------------------------------------------------------------------
...
control|)
block|{
comment|// We will only do interrupt checking in the lowest-level operator for multiple joins.
if|if
condition|(
name|child
operator|instanceof
...
condition|)
block|{
comment|// For [FULL] OUTER MapJoin, copy Big Table keys to Small Table area within
comment|// same batch by reference.
comment|//
name|outerSmallTableKeyVectorCopy
operator|.
...
condition|)
block|{
comment|// For non-[FULL] OUTER MapJoin, copy non-retained Big Table keys to the Big Table area
comment|// across to overflow batch by value so Small Key projection will see its keys...
comment|//
name|nonOuterSmallTableKeyVectorCopy
operator|.
name|copyByValue
...
condition|)
block|{
comment|// For [FULL] OUTER MapJoin, copy Big Table keys to Small Table area across
comment|// to overflow batch by value.
comment|//
name|outerSmallTableKeyVectorCopy
operator|.
name|copyByValue
...
argument_list|)
expr_stmt|;
comment|// NOTE: The closeOp call on super MapJoinOperator can trigger Hybrid Grace additional
comment|// NOTE: processing and also FULL OUTER MapJoin non-match Small Table result generation. So,
comment|// NOTE: we flush the overflowBatch after the call.
comment|//
if|if
condition|(
operator|!
