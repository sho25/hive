...
control|)
block|{
comment|// For dynamic partitioned hash join, ReduceSinkMapJoinProc rule may not get run for all
comment|// of the ReduceSink parents, because the parents of the MapJoin operator get
comment|// removed later on in this method. Keep track of the parent to mapjoin mapping
comment|// so we can later run the same logic that is run in ReduceSinkMapJoinProc.
if|if
condition|(
name|mj
operator|.
...
condition|)
block|{
comment|// Since this is a dynamic partitioned hash join, the work for this join should be a ReduceWork
name|ReduceWork
name|reduceWork
init|=
operator|(
...
argument_list|)
expr_stmt|;
comment|// Use context.mapJoinParentMap to get the original RS parents, because
comment|// the MapJoin's parents may have been replaced by dummy operator.
name|List
argument_list|<
name|Operator
argument_list|<
...
name|SemanticException
argument_list|(
literal|"Unexpected error - context.mapJoinParentMap did not have an entry for "
operator|+
name|mj
argument_list|)
throw|;
...
control|)
block|{
comment|// This processing only needs to happen for the small tables
if|if
condition|(
name|pos
operator|==
...
name|parentOp
decl_stmt|;
comment|// TableDesc needed for dynamic partitioned hash join
name|GenMapRedUtils
operator|.
...
argument_list|)
expr_stmt|;
comment|// For small table RS parents that have already been processed, we need to
comment|// add the tag to the RS work to the reduce work that contains this map join.
comment|// This was not being done for normal mapjoins, where the small table typically
comment|// has its ReduceSink parent removed.
if|if
condition|(
operator|!
name|context
...
condition|)
block|{
comment|// This reduce sink has been processed already, so the work for the parentRS exists
name|BaseWork
name|parentWork
init|=
name|ReduceSinkMapJoinProc
...
argument_list|)
expr_stmt|;
comment|// For dynamic partitioned hash join, run the ReduceSinkMapJoinProc logic for any
comment|// ReduceSink parents that we missed.
name|MapJoinOperator
name|mj
init|=
name|context
...
condition|)
block|{
comment|// Only need to run the logic for tables we missed
if|if
condition|(
name|context
operator|.
...
condition|)
block|{
comment|// ReduceSinkMapJoinProc logic does not work unless the ReduceSink is connected as
comment|// a parent of the MapJoin, but at this point we have already removed all of the
comment|// parents from the MapJoin.
comment|// Try temporarily adding the RS as a parent
name|ArrayList
argument_list|<
name|Operator
argument_list|<
...
argument_list|)
expr_stmt|;
comment|// ReduceSink also needs MapJoin as child
name|List
argument_list|<
name|Operator
argument_list|<
...
argument_list|)
expr_stmt|;
comment|// Since the MapJoin has had all of its other parents removed at this point,
comment|// it would be bad here if processReduceSinkToHashJoin() tries to do anything
comment|// with the RS parent based on its position in the list of parents.
name|ReduceSinkMapJoinProc
operator|.
...
argument_list|)
expr_stmt|;
comment|// Remove any parents from MapJoin again
name|mj
operator|.
name|removeParents
argument_list|()
expr_stmt|;
comment|// TODO: do we also need to remove the MapJoin from the list of RS's children?
block|}
block|}
if|if
condition|(
