...
parameter_list|)
block|{
comment|// Bail out if there is nothing to push
if|if
condition|(
name|predsToPushDown
operator|.
...
return|;
block|}
comment|// Build map to not convert multiple times, further remove already included predicates
name|Map
argument_list|<
name|String
argument_list|,
...
return|;
block|}
comment|// Finally exclude preds that are already in the subtree as given by the metadata provider
comment|// Note: this is the last step, trying to avoid the expensive call to the metadata provider
comment|//       if possible
name|Set
argument_list|<
name|String
argument_list|>
name|predicatesInSubtree
init|=
name|Sets
operator|.
name|newHashSet
...
specifier|static
class|class
name|DeterMinisticFuncVisitorImpl
extends|extends
name|RexVisitorImpl
argument_list|<
name|Void
...
block|{
name|boolean
name|deterministicFuncOnLiterals
init|=
literal|true
decl_stmt|;
name|RexVisitor
...
name|RexNode
argument_list|>
name|getDeterministicFuncWithSingleInputRef
parameter_list|(
name|List
argument_list|<
name|RexNode
...
block|{
name|boolean
name|deterministicFuncWithSingleInputRef
init|=
literal|true
decl_stmt|;
name|RexVisitor
...
return|;
block|}
comment|/**    * Is the expression usable for query materialization.    */
specifier|public
specifier|static
name|boolean
...
return|;
block|}
comment|/**    * Check if the expression is usable for query materialization, returning the first failing expression.    */
specifier|public
specifier|static
name|RexCall
name|checkMaterializable
...
parameter_list|)
block|{
comment|// non-deterministic functions as well as runtime constants are not materializable.
name|SqlOperator
name|op
init|=
name|call
...
return|;
block|}
comment|// this will create a project which will project out the column in positions
specifier|public
specifier|static
name|HiveProject
...
parameter_list|)
block|{
comment|// it seems that it is not used by anything.
return|return
literal|false
return|;
block|}
