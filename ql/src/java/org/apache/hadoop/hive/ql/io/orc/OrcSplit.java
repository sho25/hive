...
literal|0
decl_stmt|;
comment|/**    * This contains the synthetic ROW__ID offset and bucket properties for original file splits in an ACID table.    */
specifier|private
name|OffsetAndBucketProperty
name|syntheticAcidProps
decl_stmt|;
specifier|static
specifier|final
name|int
name|HAS_SYNTHETIC_ACID_PROPS_FLAG
init|=
literal|32
decl_stmt|;
specifier|static
...
return|;
block|}
comment|/**    * @return {@code true} if file schema doesn't have Acid metadata columns    * Such file may be in a delta_x_y/ or base_x due to being added via    * "load data" command.  It could be at partition|table root due to table having    * been converted from non-acid to acid table.  It could even be something like    * "warehouse/t/HIVE_UNION_SUBDIR_15/000000_0" if it was written by an    * "insert into t select ... from A union all select ... from B"    */
specifier|public
name|boolean
name|isOriginal
parameter_list|()
...
name|info
argument_list|(
literal|"Skipping Llap IO based on the following: [vectorized={}, hive.llap.io.acid={}] for {}"
argument_list|,
name|Utilities
operator|.
name|getIsVectorized
...
block|}
block|}
comment|/**    * Note: this is the write id as seen in the file name that contains this split    * For files that have min/max writeId, this is the starting one.      * @return    */
specifier|public
name|long
name|getWriteId
parameter_list|()
...
return|;
block|}
comment|/**    * Note: this is the bucket number as seen in the file name that contains this split.    * Hive 3.0 encodes a bunch of info in the Acid schema's bucketId attribute.    * See: {@link org.apache.hadoop.hive.ql.io.BucketCodec#V1} for details.    * @return    */
specifier|public
name|int
name|getBucketId
parameter_list|()
