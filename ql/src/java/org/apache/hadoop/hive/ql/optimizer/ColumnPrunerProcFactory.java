...
return|;
block|}
comment|/*      * reconstruct Column names& types list based on the prunedCols list.      */
specifier|private
name|void
name|setSerdePropsOfShape
parameter_list|(
...
argument_list|)
decl_stmt|;
comment|/*        * fieldNames in OI are lower-cased. So we compare lower cased names for now.        */
name|prunedCols
operator|=
name|getLowerCasePrunedCols
argument_list|(
...
condition|)
block|{
comment|/*          * For RS that are followed by Extract& PTFOp for windowing          * - do the same thing as above. Reconstruct ValueColumn list based on what is required          *   by the PTFOp.          */
assert|assert
name|parentOperators
operator|.
name|size
...
condition|)
block|{
comment|// this SEL is SEL(*) for LV
name|lvJoin
operator|=
operator|(
name|LateralViewJoinOperator
...
argument_list|()
decl_stmt|;
comment|// ReduceSinkOperators that precede GroupByOperators have the keys in the schema in addition
comment|// to the values.  These are not pruned.
name|List
argument_list|<
name|ColumnInfo
argument_list|>
