...
name|conf
decl_stmt|;
comment|/*    * Create a new type for handling precision conversions from Decimal -> Double/Float    *     * The type is only relevant to boxLiteral and all other functions treat it identically.    */
specifier|private
specifier|static
enum|enum
name|BoxType
...
constructor|FLOAT(PredicateLeaf.Type.FLOAT
block|)
enum|,
comment|// float
name|DOUBLE
...
operator|,
comment|// double
constructor|STRING(PredicateLeaf.Type.STRING
block|)
operator|,
...
condition|)
block|{
comment|// HiveDecimal -> Number -> Double
name|dbl
operator|=
operator|(
operator|(
...
condition|)
block|{
comment|// HiveDecimal -> Float -> Number
name|fl
operator|=
operator|(
call|(
...
begin_comment
comment|/**    * Find the child that is the literal.    * @param expr the parent node to check    * @param boxType the type of the expression    * @return the literal boxed if found or null    */
end_comment
...
condition|)
block|{
comment|// Both children in the expression should not be literal
if|if
condition|(
name|result
operator|!=
...
name|warn
argument_list|(
literal|"Exception thrown during SARG creation. Returning YES_NO_NULL."
operator|+
literal|" Exception: "
operator|+
name|e
...
name|SearchArgument
argument_list|>
name|initializeAndGetSargsCache
parameter_list|(
name|Configuration
name|conf
parameter_list|)
...
argument_list|()
expr_stmt|;
comment|// Can't use CacheLoader because SearchArguments may be built either from Kryo strings,
comment|// or from expressions.
block|}
return|return
name|sargsCache
return|;
...
specifier|static
name|SearchArgument
name|getSearchArgumentFromString
parameter_list|(
name|Configuration
name|conf
parameter_list|,
...
specifier|static
name|SearchArgument
name|getSearchArgumentFromExpression
parameter_list|(
name|Configuration
name|conf
parameter_list|,
