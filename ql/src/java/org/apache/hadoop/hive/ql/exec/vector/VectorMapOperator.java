...
expr_stmt|;
block|}
comment|/*    * Flush a partially full deserializerBatch.    * @return Return true if the operator tree is not done yet.    */
specifier|private
name|boolean
name|flushDeserializerBatch
parameter_list|()
throws|throws
name|HiveException
block|{
...
condition|)
block|{
comment|// Operator tree is now done.
return|return;
block|}
...
condition|)
block|{
comment|// No ACID in code path -- set ROW__ID to NULL.
name|setRowIdentiferToNull
argument_list|(
name|deserializerBatch
argument_list|)
...
return|;
block|}
comment|/*    * Deliver a vector batch to the operator tree.    *    * The Vectorized Input File Format reader has already set the partition column    * values, reset and filled in the batch, etc.    *    * We pass the VectorizedRowBatch through here.    *    * @return Return true if the operator tree is not done yet.    */
specifier|private
name|boolean
name|deliverVectorizedRowBatch
parameter_list|(
name|Writable
name|value
parameter_list|)
...
condition|)
block|{
comment|/*            * This case can happen with LLAP.  If it is able to deserialize and cache data from the            * input format, it will deliver that cached data to us as VRBs.            */
comment|/*            * Clear out any rows we may have processed in row-mode for the current partition..            */
if|if
condition|(
operator|!
name|flushDeserializerBatch
