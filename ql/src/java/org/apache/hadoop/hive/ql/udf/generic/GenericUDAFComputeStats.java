...
literal|1
argument_list|,
literal|"Exactly 2 (col + hll) or 3 (col + fm + #bitvectors) arguments are expected."
argument_list|)
throw|;
block|}
if|if
...
return|return
operator|new
name|GenericUDAFDecimalStatsEvaluator
argument_list|()
return|;
case|case
name|DATE
...
return|return
operator|new
name|GenericUDAFDateStatsEvaluator
argument_list|()
return|;
case|case
name|TIMESTAMP
...
return|return
operator|new
name|GenericUDAFTimestampStatsEvaluator
argument_list|()
return|;
default|default:
throw|throw
...
literal|0
argument_list|,
literal|"Only integer/long/timestamp/date/float/double/string/binary/boolean/decimal type argument "
operator|+
...
literal|0
decl_stmt|;
comment|// func may be null when GBY op is closing.
comment|// see mvn test -Dtest=TestMiniTezCliDriver -Dqfile=explainuser_3.q
comment|// original behavior is to create FMSketch
name|func
operator|=
name|parameters
index|[
...
block|{
comment|// Update max length if new length is greater than the ones seen so
comment|// far
name|int
name|len
init|=
...
block|}
block|}
comment|/**    * GenericUDAFDateStatsEvaluator    * High/low value will be saved in stats DB as long value representing days since epoch.    */
specifier|public
specifier|static
class|class
name|GenericUDAFDateStatsEvaluator
...
block|}
block|}
comment|/**    * GenericUDAFTimestampStatsEvaluator    * High/low value will be saved in stats DB as long value representing seconds since epoch.    */
specifier|public
specifier|static
class|class
name|GenericUDAFTimestampStatsEvaluator
...
parameter_list|)
block|{
comment|// TimestampWritableV2 is mutable, TimestampStatsAgg needs its own copy
name|TimestampWritableV2
name|v
init|=
operator|new
...
literal|16
expr_stmt|;
comment|// HiveConf hive.stats.ndv.error default produces 16
comment|// vectors
block|}
if|if
condition|(
