...
literal|null
decl_stmt|;
comment|// Column values from previous row. Used to compare with current row for the case of COUNT(DISTINCT).
name|long
name|value
decl_stmt|;
annotation|@
...
block|}
block|}
comment|// Skip the counting if the values are the same for COUNT(DISTINCT) case
if|if
condition|(
name|countThisRow
operator|&&
...
expr_stmt|;
block|}
comment|// We need to keep a copy of values from previous row.
if|if
condition|(
name|countThisRow
condition|)
