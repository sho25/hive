...
expr_stmt|;
block|}
comment|// If file is already present in base directory, then remove it from the list.
comment|// Check  HIVE-21197 for more detail
specifier|private
name|void
name|updateSrcFileListForDupCopy
parameter_list|(
name|FileSystem
name|dstFs
parameter_list|,
...
name|error
argument_list|(
literal|"File listing failed for "
operator|+
name|toPath
argument_list|,
name|e
...
name|debug
argument_list|(
literal|"Path {} does not exist, will be created before copy"
argument_list|,
name|toPath
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// if more than one base directory is present, then it means one or more replace operation is done. Any duplicate
comment|// check after that may cause data loss as the check will happen with the first base directory
comment|// which is no more valid.
name|LOG
operator|.
name|info
argument_list|(
literal|"Number of base directory {} in path {} is more than one. Duplicate check should not be done."
argument_list|,
name|statuses
argument_list|,
name|toPath
...
name|filePath
operator|+
literal|" is already present in base directory. So removing it from the list."
argument_list|)
expr_stmt|;
name|iter
operator|.
...
specifier|private
name|void
name|renameFileCopiedFromCmPath
parameter_list|(
name|Path
name|toPath
parameter_list|,
...
argument_list|)
decl_stmt|;
comment|// if the new file exist then delete it before renaming, to avoid rename failure. If the copy is done
comment|// directly to table path (bypassing staging directory) then there might be some stale files from previous
comment|// incomplete/failed load. No need of recycle as this is a case of stale file.
if|if
condition|(
name|dstFs
operator|.
...
name|newDestFile
operator|+
literal|" is deleted before renaming"
argument_list|)
expr_stmt|;
name|dstFs
operator|.
...
name|info
argument_list|(
literal|"All files are already present in the base directory. Skipping copy task."
argument_list|)
expr_stmt|;
return|return
literal|0
...
block|}
block|}
comment|// If direct (move optimized) copy is triggered for data to a migrated transactional table, then it
comment|// should have a write ID allocated by parent ReplTxnTask. Use it to create the base or delta directory.
comment|// The toPath received in ReplCopyWork is pointing to table/partition base location.
comment|// So, just need to append the base or delta directory.
...
name|printError
argument_list|(
literal|"ReplCopyTask : Write id is not set in the config by open txn task for migration"
argument_list|)
expr_stmt|;
return|return
literal|6
...
argument_list|)
expr_stmt|;
comment|// in case of move optimization, file is directly copied to destination. So we need to clear the old content, if
comment|// its a replace (insert overwrite ) operation.
if|if
condition|(
name|work
operator|.
...
name|rcwork
operator|.
name|setCopyToMigratedTxnTable
argument_list|(
name|copyToMigratedTxnTable
argument_list|)
expr_stmt|;
comment|// For replace case, duplicate check should not be done. The new base directory will automatically make the older
comment|// data invisible. Doing duplicate check and ignoring copy will cause consistency issue if there are multiple
comment|// replace events getting replayed in the first incremental load.
name|rcwork
operator|.
name|setCheckDuplicateCopy
argument_list|(
name|replicationSpec
operator|.
