...
return|;
block|}
comment|//I think this is a bug in Calcite where Aggregate seems to always expect
comment|// correlated variable in nodes underneath it which is not true for queries such as
comment|// select p.empno, li.mgr from (select distinct empno as empno from emp) p join emp li on p.empno= li.empno where li.sal = 1
comment|//    and li.deptno in (select deptno from emp where JOB = 'AIR' AND li.mgr=mgr)
...
comment|// There could be situation e.g. multiple correlated variables refering to
comment|//  same outer variable, in which case Project will be created with multiple
comment|// fields with same name. Hive doesn't allow HiveProject with multiple fields
comment|//  having same name. So to avoid that we keep a set of all fieldnames and
comment|// on encountering an existing one a new field/column name is generated
specifier|final
name|Set
argument_list|<
name|String
