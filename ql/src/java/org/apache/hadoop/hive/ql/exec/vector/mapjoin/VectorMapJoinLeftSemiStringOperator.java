...
begin_comment
comment|/*  * Specialized class for doing a vectorized map join that is an left semi join on a Single-Column String  * using a hash set.  */
end_comment
...
block|}
comment|//---------------------------------------------------------------------------
comment|// Process Single-Column String Left-Semi Join on a vectorized row batch.
comment|//
annotation|@
name|Override
specifier|public
...
comment|/*          * Get our Single-Column String hash set information for this specialized class.          */
name|hashSet
operator|=
operator|(
name|VectorMapJoinBytesHashSet
...
name|MATCH
case|:
comment|// We have extracted the existence from the hash set result, so we don't keep it.
break|break;
case|case
name|SPILL
case|:
comment|// We keep the hash set result for its spill information.
name|hashSetResultCount
operator|++
expr_stmt|;
break|break;
...
literal|true
expr_stmt|;
comment|/*              * Single-Column String specific save key and lookup.              */
name|saveKeyBatchIndex
operator|=
name|batchIndex
expr_stmt|;
