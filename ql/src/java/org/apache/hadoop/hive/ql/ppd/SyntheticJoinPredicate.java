...
block|}
block|}
comment|// Handle non-equi joins like<,<=,>, and>=
name|List
argument_list|<
name|ExprNodeDesc
argument_list|>
...
condition|)
block|{
comment|// Either srcPos or targetPos is larger than 1, making this filter a complex one.
for|for
control|(
name|ExprNodeDesc
name|filter
...
name|filter
decl_stmt|;
comment|// filter should be of type<,>,<= or>=
if|if
condition|(
name|getFuncText
argument_list|(
...
continue|continue;
block|}
comment|// Create non-equi function.
name|List
argument_list|<
name|ExprNodeDesc
argument_list|>
...
name|sourceKey
init|=
name|getRSColExprFromResidualFilter
argument_list|(
name|sourceChild
argument_list|,
name|join
...
argument_list|)
decl_stmt|;
comment|// TODO : deduplicate the code below.
if|if
condition|(
name|LOG
operator|.
...
name|debug
argument_list|(
literal|" Non-Equi Join Predicate "
operator|+
name|funcExpr
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// TODO : HIVE-21098 : Support for extended predicates
if|if
condition|(
name|andArgs
operator|.
...
argument_list|()
decl_stmt|;
comment|// use name to get the alias pos of parent and name in parent
specifier|final
name|int
name|aliasPos
init|=
...
argument_list|)
decl_stmt|;
comment|// Get the correct parent
specifier|final
name|ReduceSinkOperator
name|parentRS
init|=
...
argument_list|)
decl_stmt|;
comment|// Fetch the colExpr from parent
return|return
name|parentRS
operator|.
name|getColumnExprMap
...
comment|// 1. As the name suggests, provides inverted function text for a given function text
comment|// 2. If inversion fails, it can be inferred that the given function is not supported.
name|String
name|getFuncText
parameter_list|(
name|String
