...
argument_list|)
expr_stmt|;
comment|// Command should be redacted before passing it to the QueryPlan in order
comment|// to avoid returning sensitive data
name|String
name|queryStr
init|=
name|HookUtils
...
block|}
block|}
comment|/**    * @param hiveLocks    *          list of hive locks to be released Release all the locks specified. If some of the    *          locks have already been released, ignore them    * @param commit if there is an open transaction and if true, commit,    *               if false rollback.  If there is no open transaction this parameter is ignored.    *    **/
specifier|private
name|void
