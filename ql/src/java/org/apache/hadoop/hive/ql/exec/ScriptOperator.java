...
literal|null
decl_stmt|;
comment|/**    * Timer to send periodic reports back to the tracker.    */
specifier|transient
name|Timer
name|rpTimer
decl_stmt|;
...
condition|)
block|{
comment|/* Timer that reports every 5 minutes to the jobtracker. This ensures that even if            the user script is not returning rows for greater than that duration, a progress            report is sent to the tracker so that the tracker does not think that the job             is dead.         */
name|Integer
name|expInterval
init|=
name|Integer
