...
block|}
block|}
comment|/**      * Get the size of compressed and uncompressed data in the stream's buffers.      * @return the number of bytes in the buffers.      */
name|long
name|getSize
parameter_list|()
block|{
...
name|IOException
block|{
comment|// Traverse the red-black tree writing out the bytes and lengths; and
comment|// creating the map from the original order to the final sorted order.
specifier|final
name|int
index|[]
name|dumpOrder
...
argument_list|()
decl_stmt|;
comment|// need to build the first index entry out here, to handle the case of
comment|// not having any values.
if|if
condition|(
name|buildIndex
condition|)
...
block|}
block|}
comment|// once every 1000 rows, check the size to see if we should spill
if|if
condition|(
name|rowsInStripe
operator|%
