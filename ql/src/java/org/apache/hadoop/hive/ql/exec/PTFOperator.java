...
condition|)
block|{
comment|/*            * this is needed because during Translation we are still assuming that rows            * are collected into a PTFPartition.            * @Todo make translation handle the case when the first PTF is Streaming.            */
name|row
operator|=
name|ObjectInspectorUtils
operator|.
