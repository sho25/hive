...
argument_list|()
decl_stmt|;
comment|// Find the PrivRequirements that match on IOType and ActionType, and add
comment|// the privilege
comment|// required to reqPrivs
for|for
control|(
name|PrivRequirement
name|opPriv
...
condition|)
block|{
comment|// if action in PrivRequirement is null, it means that
comment|// the privileges are required irrespective of hObj's action type
comment|// If it is not null, action type has to match
if|if
condition|(
name|hObj
operator|.
