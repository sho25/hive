...
expr_stmt|;
block|}
comment|/**      * @param stmtIds delta dir suffixes when a single txn writes> 1 delta in the same partition      * @param visibilityTxnId maybe 0, if the dir name didn't have it.  txnid:0 is always visible      */
name|DeltaMetaData
parameter_list|(
name|long
name|minWriteId
...
function_decl|;
block|}
comment|/**    * Get a record reader that provides the user-facing view of the data after    * it has been merged together. The key provides information about the    * record's identifier (write id, bucket, record id).    * @param split the split to read    * @param options the options to read with    * @return a record reader    * @throws IOException    */
specifier|public
name|RowReader
argument_list|<
name|VALUE
...
function_decl|;
block|}
comment|/**    * Get a reader that returns the raw ACID events (insert, update, delete).    * Should only be used by the compactor.    * @param conf the configuration    * @param collapseEvents should the ACID events be collapsed so that only    *                       the last version of the row is kept.    * @param bucket the bucket to read    * @param validWriteIdList the list of valid write ids to use    * @param baseDirectory the base directory to read or the root directory for    *                      old style files    * @param deltaDirectory a list of delta files to include in the merge    * @return a record reader    * @throws IOException    */
name|RawReader
argument_list|<
name|VALUE
argument_list|>
