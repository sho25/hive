...
argument_list|)
decl_stmt|;
comment|// Because we are visiting the operators recursively, the last limit op that
comment|// calls the following function will set the global property.
if|if
condition|(
name|this
operator|.
...
return|;
block|}
comment|// use this function to make the union "flat" for both execution and explain
comment|// purpose
specifier|private
name|List
argument_list|<
...
name|SemanticException
argument_list|(
literal|"In CBO return path, genReduceSinkAndBacktrackSelect is expecting only one SelectOp but there is "
operator|+
name|input
operator|.
name|getSchema
...
control|)
block|{
comment|// 2. Backtracking from RS
name|ReduceSinkOperator
name|inputRS
init|=
operator|(
...
argument_list|)
expr_stmt|;
comment|// 4. Create and populate filter map
name|int
index|[]
index|[]
name|filterMap
