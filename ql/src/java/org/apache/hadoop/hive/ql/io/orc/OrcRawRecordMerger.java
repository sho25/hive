...
return|;
block|}
comment|/**      * The cumulative number of row in all files of the logical bucket that precede the file      * represented by {@link #getRecordReader()}      */
specifier|abstract
name|long
name|getRowIdOffset
parameter_list|()
...
condition|)
block|{
comment|/*whatever SARG maybe applicable to base it's not applicable to delete_delta since it has no       * user columns       * HIVE-17320: we should compute a SARG to push down min/max key to delete_delta*/
name|Reader
operator|.
name|Options
...
condition|)
block|{
comment|//it's not wrong to take all delete events for bucketed tables but it's more efficient
comment|//to only take those that belong to the 'bucket' assuming we trust the file name
comment|//un-bucketed table - get all files
name|FileSystem
name|fs
init|=
name|deltaDirectory
...
expr_stmt|;
block|}
comment|//todo: need a test where we actually have more than 1 file
return|return
name|deltaFiles
return|;
block|}
comment|//if here it must be delta_x_y - insert events only, so we must be compacting
assert|assert
name|mergerOptions
operator|.
name|isCompacting
argument_list|()
operator|:
literal|"Expected to be called as part of compaction"
assert|;
name|Path
name|deltaFile
init|=
