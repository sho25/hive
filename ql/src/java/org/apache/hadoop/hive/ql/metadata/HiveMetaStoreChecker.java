...
argument_list|)
decl_stmt|;
comment|// Check if too low config is provided for move files. 2x CPU is reasonable max count.
name|poolSize
operator|=
name|poolSize
operator|==
...
argument_list|)
expr_stmt|;
comment|// Fixed thread pool on need basis
specifier|final
name|ThreadPoolExecutor
name|pool
init|=
...
argument_list|()
decl_stmt|;
comment|// Check if thread pool can be used.
name|boolean
name|useThreadPool
init|=
literal|false
...
init|)
block|{
comment|// In case of recursive calls, it is possible to deadlock with TP. Check TP usage here.
if|if
condition|(
name|pool
operator|.
...
name|debug
argument_list|(
literal|"Not using threadPool as active count:"
operator|+
name|pool
operator|.
name|getActiveCount
