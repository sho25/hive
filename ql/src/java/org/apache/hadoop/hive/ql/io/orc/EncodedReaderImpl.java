...
block|}
block|}
comment|/** DiskRange containing encoded, uncompressed data from cache. */
annotation|@
name|VisibleForTesting
specifier|public
specifier|static
...
condition|)
block|{
comment|// We are missing a section at the end of the part...
name|lastUncompressed
operator|=
...
expr_stmt|;
block|}
comment|// We will take care of this at the end of the part, or if we find a gap.
name|next
operator|=
name|current
operator|.
...
continue|continue;
block|}
comment|// We don't have the entire part; just copy to an allocated buffer. We could try to
comment|// optimize a bit if we have contiguous buffers with gaps, but it's probably not needed.
name|lastUncompressed
operator|=
