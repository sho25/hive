...
block|}
block|}
comment|/**    * Get dequoted name from a table/column node.    * @param tableOrColumnNode the table or column node    * @return for table node, db.tab or tab. for column node column.    * @throws SemanticException    */
specifier|public
specifier|static
name|String
...
return|;
block|}
comment|/**    * Get the name reference of a DB table node.    * @param tabNameNode    * @return a {@link TableName}, not null. The catalog will be missing from this.    * @throws SemanticException    */
specifier|public
specifier|static
name|TableName
...
name|SemanticException
block|{
comment|// Ideally this would be removed, once the catalog is accessible in all use cases
return|return
name|getQualifiedTableName
argument_list|(
name|tabNameNode
...
return|;
block|}
comment|/**    * Get the name reference of a DB table node.    * @param tabNameNode    * @param catalogName the catalog of the DB/object    * @return a {@link TableName}, not null. The catalog will be missing from this.    * @throws SemanticException    */
specifier|public
specifier|static
name|TableName
name|getQualifiedTableName
...
name|getMsg
argument_list|(
literal|"Subqueries are not allowed "
operator|+
literal|"in Check Constraints"
argument_list|)
argument_list|)
throw|;
block|}
...
name|getMsg
argument_list|(
literal|"Temporary UDFs are not allowed "
operator|+
literal|"in Check Constraints"
argument_list|)
argument_list|)
...
name|getMsg
argument_list|(
literal|"Window functions are not allowed "
operator|+
literal|"in Check Constraints"
argument_list|)
argument_list|)
...
return|;
block|}
comment|/**    * Validate and get the default value from the AST    * @param defaultValueAST AST node corresponding to default value    * @return retrieve the default value and return it as string    * @throws SemanticException    */
specifier|private
specifier|static
name|String
...
name|defaultValueText
operator|+
literal|". Please make sure that the type is compatible with column type: "
operator|+
name|colTypeInfo
operator|.
name|getTypeName
...
block|}
block|}
comment|/**    * Process the foreign keys from the AST and populate the foreign keys in the SQLForeignKey list    * @param tName catalog/db/table name reference    * @param child Foreign Key token node    * @param foreignKeys SQLForeignKey list    * @throws SemanticException    */
specifier|protected
specifier|static
name|void
...
argument_list|)
expr_stmt|;
comment|// not that elegant, but hard to refactor
block|}
if|if
condition|(
name|ast
...
condition|)
block|{
comment|// if user hasn't specify partition spec generate it from table's partition spec
comment|// do this only if it is INSERT/INSERT INTO/INSERT OVERWRITE/ANALYZE
name|List
argument_list|<
name|FieldSchema
argument_list|>
...
condition|)
block|{
comment|// getTable needs a refactor with all ~50 occurences
throw|throw
operator|new
name|SemanticException
argument_list|(
...
return|;
block|}
comment|/**    * Create a FetchTask for a given schema.    */
specifier|protected
name|FetchTask
...
name|SemanticException
argument_list|(
literal|"Cannot recognize keystore "
operator|+
name|keystore
operator|+
literal|", only JavaKeyStoreProvider is "
operator|+
literal|"supported"
argument_list|)
throw|;
...
block|}
block|}
comment|/**    * Unparses the analyzed statement    */
specifier|protected
name|void
