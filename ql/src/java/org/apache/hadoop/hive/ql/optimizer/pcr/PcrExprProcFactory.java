...
literal|false
decl_stmt|;
comment|// Whether any of the node outputs is unknown
name|boolean
name|allDivided
init|=
literal|true
decl_stmt|;
comment|// Whether all of the node outputs are divided
name|List
argument_list|<
name|NodeInfoWrapper
argument_list|>
name|newNodeOutputsList
init|=
operator|new
name|ArrayList
argument_list|<
...
block|}
block|}
comment|// If all of them were true, return true
if|if
condition|(
name|newNodeOutputsList
operator|.
...
return|;
block|}
comment|// If we are left with a single child, return the child
if|if
condition|(
name|newNodeOutputsList
operator|.
...
block|}
block|}
comment|// If all of them were false, return false
if|if
condition|(
name|newNodeOutputsList
operator|.
...
condition|)
block|{
comment|// It is an IN clause on a column
if|if
condition|(
operator|(
operator|(
...
condition|)
block|{
comment|// It is a partition column, we can proceed
name|removePredElem
operator|=
literal|true
expr_stmt|;
...
condition|)
block|{
comment|// We should not remove the dynamic partition pruner generated synthetic predicates.
for|for
control|(
name|int
name|i
...
condition|)
block|{
comment|// It is an IN clause on a struct
comment|// Make sure that the generic udf is deterministic
if|if
condition|(
name|FunctionRegistry
operator|.
...
block|{
name|boolean
name|hasOnlyPartCols
init|=
literal|true
decl_stmt|;
name|boolean
name|hasDynamicListDesc
init|=
literal|false
decl_stmt|;
for|for
...
control|)
block|{
comment|// Check if the current field expression contains only
comment|// partition column or a virtual column or constants.
comment|// If yes, this filter predicate is a candidate for this optimization.
if|if
condition|(
operator|!
operator|(
...
block|}
block|}
comment|// If we have non-partition columns, we cannot remove the predicate.
if|if
condition|(
name|hasOnlyPartCols
condition|)
...
block|}
block|}
comment|// If removePredElem is set to true, return true as this is a potential candidate
comment|// for partition condition remover. Else, set the WalkState for this node to unknown.
return|return
name|removePredElem
condition|?
operator|new
...
return|;
block|}
comment|// Try to fold, otherwise return the expression itself
specifier|final
name|ExprNodeGenericFuncDesc
name|desc
init|=
