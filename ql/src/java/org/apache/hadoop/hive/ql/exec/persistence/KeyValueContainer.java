...
begin_comment
comment|/**  * An eager key/value container that puts every row directly to output stream.  * Kryo is used for the serialization/deserialization.  * When reading, we load IN_MEMORY_NUM_ROWS rows from input stream to memory batch by batch.  */
end_comment
...
name|info
argument_list|(
literal|"KeyValueContainer created temp file "
operator|+
name|tmpFile
operator|.
...
argument_list|)
expr_stmt|;
comment|// Since hashCode is not used, just put an arbitrary number
name|output
operator|.
name|writeInt
argument_list|(
...
name|RuntimeException
argument_list|(
literal|"Failed to load key/value pairs from disk"
argument_list|,
name|e
argument_list|)
throw|;
