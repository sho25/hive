...
argument_list|)
expr_stmt|;
comment|// Special date formatting functions
name|system
operator|.
name|registerUDF
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// following mapping is to enable UDFName to UDF while generating expression for default value (in operator tree)
comment|//  e.g. cast(4 as string) is serialized as UDFToString(4) into metastore, to allow us to generate appropriate UDF for
comment|//  UDFToString we need the following mappings
comment|// Rest of the types e.g. DATE, CHAR, VARCHAR etc are already registered
comment|// TODO: According to vgarg, these function mappings are no longer necessary as the default value logic has changed.
name|system
operator|.
name|registerUDF
argument_list|(
...
block|}
block|}
comment|/**    * Find a common type for union-all operator. Only the common types for the same    * type group will resolve to a common type. No implicit conversion across different    * type groups will be done.    */
specifier|public
specifier|static
name|TypeInfo
...
condition|)
block|{
comment|// Equal groups, return what we can handle
switch|switch
condition|(
name|pgA
condition|)
...
block|}
block|}
comment|// Handle date-string common category and numeric-string common category
if|if
condition|(
name|pgA
operator|==
...
return|;
block|}
comment|// We could not find a common category, return null
return|return
literal|null
return|;
block|}
...
return|;
block|}
comment|/**    * Returns whether a GenericUDF is a runtime constant or not.    */
specifier|public
specifier|static
name|boolean
...
return|;
block|}
comment|/**    * Returns whether the expression, for a single query, returns the same result given    * the same arguments/children. This includes deterministic functions as well as runtime    * constants (which may not be deterministic across queries).    */
specifier|public
specifier|static
name|boolean
