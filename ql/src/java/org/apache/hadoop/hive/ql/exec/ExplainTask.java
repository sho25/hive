...
specifier|static
name|String
name|trueCondNameVectorizationEnabled
init|=
name|HiveConf
operator|.
name|ConfVars
...
specifier|static
name|String
name|falseCondNameVectorizationEnabled
init|=
name|HiveConf
operator|.
name|ConfVars
...
decl_stmt|;
name|String
name|isVectorizationEnabledCondName
init|=
operator|(
name|isVectorizationEnabled
condition|?
...
name|String
argument_list|>
name|isVectorizationEnabledCondList
init|=
name|Arrays
operator|.
name|asList
...
block|}
name|boolean
name|suppressOthersForVectorization
init|=
literal|false
decl_stmt|;
if|if
...
condition|)
block|{
comment|// Suppress the STAGES if vectorization is off.
name|suppressOthersForVectorization
operator|=
operator|!
name|planVecPair
...
condition|)
block|{
comment|// Because of the implementation of the JsonParserFactory, we are sure
comment|// that we can get a TezJsonParser.
name|JsonParser
name|jsonParser
init|=
name|JsonParserFactory
...
parameter_list|)
block|{
comment|// if there is anything wrong happen, we bail out.
name|LOG
operator|.
name|error
argument_list|(
...
condition|)
block|{
comment|// use the parser to get the output operators of RS
name|JsonParser
name|jsonParser
init|=
name|JsonParserFactory
...
return|;
block|}
comment|/**    * Retruns a map which have either primitive or string keys.    *    * This is neccessary to discard object level comparators which may sort the objects based on some non-trivial logic.    *    * @param mp    * @return    */
specifier|private
name|TreeMap
argument_list|<
name|Object
...
control|)
block|{
comment|// discard possibly type related sorting order and replace with alphabetical
name|ret
operator|.
name|put
argument_list|(
...
decl_stmt|;
comment|// Assume.
comment|// The EXPLAIN VECTORIZATION option was specified.
specifier|final
name|boolean
name|desireOnly
init|=
...
name|NON_VECTORIZED
case|:
comment|// Display all non-vectorized leaf objects unless ONLY.
if|if
condition|(
name|desireOnly
condition|)
...
condition|)
block|{
comment|// This detail not desired.
name|invokeFlag
operator|=
literal|false
expr_stmt|;
...
condition|)
block|{
comment|// Suppress headers and all objects below.
name|invokeFlag
operator|=
literal|false
expr_stmt|;
...
argument_list|(
literal|"Unknown EXPLAIN vectorization "
operator|+
name|vectorization
argument_list|)
throw|;
...
name|OPERATOR_PATH
case|:
comment|// Always include headers since they contain non-vectorized objects, too.
break|break;
default|default:
throw|throw
operator|new
...
return|;
block|}
comment|/**    * Returns EXPLAIN EXTENDED output for a semantically analyzed query.    *    * @param sem semantic analyzer for analyzed query    * @param plan query plan    * @param astTree AST tree dump    */
specifier|public
specifier|static
name|String
