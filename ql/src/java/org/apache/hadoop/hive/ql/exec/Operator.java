...
specifier|public
name|void
name|allocateParentOperatorsInitArray
parameter_list|()
block|{
if|if
condition|(
...
expr_stmt|;
block|}
comment|/**     * The default implementation assumes that the first inspector in the array is the output inspector as well. Specific operators can override this    * to pass their own output object inspectors.     */
specifier|public
name|void
...
argument_list|)
expr_stmt|;
comment|// Copy operators from List to Array for faster access
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|// If all the parents have been initialied, go ahead
for|for
control|(
name|ParentInit
name|par
...
name|info
argument_list|(
literal|"done Initializing "
operator|+
name|id
argument_list|)
expr_stmt|;
block|}
comment|/**    * Process the row.    * @param row  The object representing the row.    * @param rowInspector  The inspector for the row object, will be deprecated soon.    * @param tag  The tag of the row usually means which parent this row comes from.    *             Rows with the same tag should have exactly the same rowInspector all the time.    */
specifier|public
specifier|abstract
name|void
name|process
