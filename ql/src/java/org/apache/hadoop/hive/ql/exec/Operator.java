...
expr_stmt|;
block|}
comment|// bytesWritableGroupKey is only used when a query plan is optimized by CorrelationOptimizer.
comment|// CorrelationLocalSimulativeReduceSinkOperator will use this variable to determine when it needs to start or end the group
comment|// for its child operator.
specifier|protected
name|BytesWritable
...
return|;
block|}
comment|// The number of current row
specifier|protected
name|long
name|rowNumber
decl_stmt|;
...
argument_list|()
operator|+
literal|" row number initialized to 0"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|info
argument_list|(
literal|"Initializing row numbers of children of "
operator|+
name|id
operator|+
literal|" "
