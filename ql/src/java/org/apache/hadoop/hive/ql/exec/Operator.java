...
decl_stmt|;
comment|// Data structures specific for vectorized operators.
specifier|private
specifier|transient
name|boolean
name|multiChildren
...
literal|0
expr_stmt|;
comment|// initializeOp can be overridden
comment|// Initializing data structures for vectorForward
name|this
operator|.
name|selected
operator|=
...
block|{
comment|// TODO: ugly hack because Java does not have dtors and Tez input
comment|// hangs on shutdown.
if|if
condition|(
operator|!
name|isInitOk
...
name|debug
argument_list|(
literal|"Initialization Done - Reset: {}"
argument_list|,
name|this
argument_list|)
expr_stmt|;
...
literal|null
decl_stmt|;
comment|// Wait for all futures to complete. Check for an abort while waiting for each future. If any of the futures is cancelled / aborted - cancel all subsequent futures.
name|boolean
name|cancelAll
init|=
literal|false
...
control|)
block|{
comment|// If aborted - break out of the loop, and cancel all subsequent futures.
if|if
condition|(
name|cancelAll
condition|)
...
block|{
comment|// Wait for the current future.
while|while
condition|(
literal|true
condition|)
...
try|try
block|{
comment|// Await future result with a timeout to check the abort field occasionally.
comment|// It's possible that the interrupt which comes in along with an abort, is suppressed
comment|// by some other operator.
name|Object
name|futureResult
init|=
name|f
...
parameter_list|)
block|{
comment|// Expected if the operation takes time. Continue the loop, and wait for op completion.
block|}
catch|catch
parameter_list|(
...
control|)
block|{
comment|// It's ok to send a cancel to an already completed future. Is a no-op
name|f
operator|.
name|cancel
argument_list|(
...
name|HiveException
argument_list|(
literal|"Async Initialization failed. abortRequested="
operator|+
name|abortOp
operator|.
name|get
...
condition|)
block|{
comment|// Determine the next power of 2 larger than the requested index
name|int
name|newLength
init|=
literal|2
...
name|trace
argument_list|(
literal|"No children operators; start group done"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|trace
argument_list|(
literal|"No children operators; end group done"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|HiveException
block|{   }
comment|// Recursive flush to flush all the tree operators
specifier|public
name|void
name|flushRecursive
parameter_list|()
...
argument_list|)
expr_stmt|;
comment|// closeOp can be overriden
if|if
condition|(
name|conf
operator|!=
...
condition|)
block|{
name|publishRunTimeStats
argument_list|()
expr_stmt|;
block|}
name|LongWritable
name|runTimeRowsWritable
init|=
operator|new
name|LongWritable
argument_list|(
...
block|}
block|}
comment|/*    * Forward a VectorizedRowBatch to the children operators.    */
specifier|protected
name|void
name|vectorForward
parameter_list|(
...
return|return;
block|}
comment|// Data structures to store original values
specifier|final
name|int
name|size
init|=
...
name|warn
argument_list|(
literal|"Cannot set traits when there is no descriptor: {}"
argument_list|,
name|this
argument_list|)
expr_stmt|;
...
name|warn
argument_list|(
literal|"Cannot set stats when there is no descriptor: {}"
argument_list|,
name|this
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|/** @return Compilation operator context. Only available during compilation. */
specifier|public
name|CompilationOpContext
...
block|}
block|}
comment|/**    * Decides whether two operators are logically the same.    * This can be used to merge same operators and avoid repeated computation.    */
specifier|public
name|boolean
name|logicalEquals
parameter_list|(
...
return|;
block|}
comment|/**    * Compares the whole operator tree with the other.    */
comment|// Currently only used during re-optimization related parts.
comment|// FIXME: HIVE-18703 should probably move this method somewhere else
specifier|public
specifier|final
name|boolean
...
parameter_list|)
block|{
comment|// XXX: this could easily become a hot-spot
if|if
condition|(
operator|!
name|logicalEquals
