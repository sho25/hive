...
throw|;
block|}
comment|// This is a hack for generating the correct ObjectInspector.
comment|// In the future, we should use DynamicSerde and initialize it using the type info.
if|if
condition|(
name|keyObjectInspector
operator|==
...
condition|)
block|{
comment|// Directly create ObjectInspector here because we didn't know the number of cols till now.
name|keyObjectInspector
operator|=
name|MetadataListStructObjectInspector
operator|.
...
name|RuntimeException
argument_list|(
literal|"Reduce operator close failed during initialize"
argument_list|,
name|e
argument_list|)
throw|;
