...
decl_stmt|;
comment|/**    * This method returns the big-endian representation of value.    * @param value    * @param length    * @return    */
specifier|public
specifier|static
name|FastBitSet
...
name|info
argument_list|(
literal|"Max hash table memory: {} bytes"
argument_list|,
name|maxHashTblMemory
argument_list|)
expr_stmt|;
...
parameter_list|)
block|{
comment|// exactly one reducer should emit the summary row
if|if
condition|(
operator|!
name|firstReducer
...
return|;
block|}
comment|// empty keyset is basically ()
if|if
condition|(
name|desc
operator|.
...
argument_list|()
decl_stmt|;
comment|// groupingSets are known at map/reducer side; but have to do real processing
comment|// hence grouppingSetsPresent is true only at map side
if|if
condition|(
name|groupingSetPosition
operator|>=
