...
return|;
block|}
comment|/*      * Returns absolute offset of the match      */
specifier|public
name|int
name|index
parameter_list|(
...
block|}
block|}
comment|/**    * Matches a chained sequence of checkers.    *    * This has 4 chain scenarios cases in it (has no escaping or single char wildcards)    *    * 1) anchored left "abc%def%"    * 2) anchored right "%abc%def"    * 3) unanchored "%abc%def%"    * 4) anchored on both sides "abc%def"    */
specifier|protected
specifier|static
specifier|final
class|class
...
literal|0
decl_stmt|;
comment|// at least 2 checkers always
name|BeginChecker
name|left
init|=
literal|null
...
condition|)
block|{
comment|// %% is folded in the .*?.*? regex usually into .*?
continue|continue;
block|}
name|len
operator|+=
...
block|{
comment|// no pos change - no need since we've shrunk the string with same pos
name|mark
operator|-=
name|endLen
expr_stmt|;
block|}
comment|// loop for middles
for|for
control|(
name|int
name|i
