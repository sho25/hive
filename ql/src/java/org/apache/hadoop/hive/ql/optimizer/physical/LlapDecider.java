...
name|debug
argument_list|(
literal|"Disabling hybrid grace hash join in case of LLAP "
operator|+
literal|"and non-dynamic partition hash join."
argument_list|)
expr_stmt|;
name|graceMapJoinOp
operator|.
...
block|{
return|return;
comment|// Not based on ARP and cannot assume uniform distribution, bail.
block|}
name|clusterState
operator|.
...
name|warn
argument_list|(
literal|"Cannot determine LLAP cluster information"
argument_list|)
expr_stmt|;
name|executorCount
operator|=
...
argument_list|)
expr_stmt|;
comment|// We only increase the targets here, but we stay below maxReducers
if|if
condition|(
name|reduceWork
operator|.
...
argument_list|)
expr_stmt|;
comment|// TODO: is this correct? based on the same logic as HIVE-14200
name|reduceWork
operator|.
name|getEdgePropRef
argument_list|()
...
block|{
comment|// UNIFORM || AUTOPARALLEL (maxed out)
name|reduceWork
operator|.
name|setNumReduceTasks
argument_list|(
...
argument_list|()
operator|+
literal|" (permanent functions are disabled)"
argument_list|)
expr_stmt|;
return|return
literal|false
...
name|info
argument_list|(
literal|"Not a built-in or permanent function: "
operator|+
name|cur
operator|.
name|getExprString
