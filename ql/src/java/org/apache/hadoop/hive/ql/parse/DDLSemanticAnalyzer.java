...
throw|;
block|}
comment|// If the table is partitioned, the number of buckets cannot be changed
comment|// (unless the table is empty).
comment|// The hive code uses bucket information from the table, and changing the
comment|// number of buckets can lead to wrong results for bucketed join/sampling
comment|// etc. This should be fixed as part of HIVE-3283.
comment|// Once the above jira is fixed, this error check/message should be removed
if|if
condition|(
name|tab
operator|.
...
expr_stmt|;
block|}
comment|// Handle xpath correctly
name|String
