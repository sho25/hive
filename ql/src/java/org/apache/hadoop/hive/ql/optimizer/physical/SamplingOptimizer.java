...
condition|)
block|{
comment|// Not supported for MM tables - sampler breaks separate MM dirs into splits, resulting in
comment|// mismatch when the downstream task looks at them again assuming they are MM table roots.
comment|// We could somehow unset the MM flag for the main job when the sampler succeeds, since the
comment|// sampler will limit the input to the the correct directories, but we don't care about MR.
continue|continue;
block|}
name|ReduceSinkOperator
name|child
