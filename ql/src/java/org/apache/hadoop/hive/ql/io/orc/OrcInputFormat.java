...
name|isTransactionalTableScan
init|=
comment|//this never seems to be set correctly
name|HiveConf
operator|.
name|getBoolVar
argument_list|(
...
expr_stmt|;
name|boolean
name|isFastVectorizedReaderAvailable
init|=
...
condition|)
block|{
comment|// Faster vectorized ACID row batch reader is available that avoids row-by-row stitching.
return|return
operator|(
name|org
operator|.
...
condition|)
block|{
comment|// Vectorized regular ACID reader that does row-by-row stitching.
return|return
operator|(
name|org
operator|.
...
argument_list|)
decl_stmt|;
comment|// The deltas are decided based on whether split-update has been turned on for the table or not.
comment|// When split-update is turned off, everything in the delta_x_y/ directory should be treated
comment|// as delta. However if split-update is turned on, only the files in delete_delta_x_y/ directory
comment|// need to be considered as delta, because files in delta_x_y/ will be processed as base files
comment|// since they only have insert events in them.
specifier|final
name|Path
index|[]
name|deltas
...
operator|)
decl_stmt|;
comment|// When no baseFiles, we will just generate a single split strategy and return.
name|List
argument_list|<
name|HdfsFileStatusWithId
argument_list|>
...
block|{
comment|/**we don't know which file to start reading -          * {@link OrcRawRecordMerger.OriginalReaderPairToCompact} does*/
name|isOriginal
operator|=
literal|true
expr_stmt|;
