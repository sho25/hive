...
return|;
block|}
comment|/**    * Check if the given file is original orc file or an ACID file.    * @param file The file reader to check    * @return<code>false</code> if an ACID file,<code>true</code> if a simple orc file    */
specifier|public
specifier|static
name|boolean
name|isOriginal
...
return|;
block|}
comment|/**    * Check if the given file is original orc file or an ACID file.    * @param footer The footer of the given file to check    * @return<code>false</code> if an ACID file,<code>true</code> if a simple orc file    */
specifier|public
specifier|static
name|boolean
name|isOriginal
...
condition|)
block|{
comment|// This assumes all struct cols immediately follow struct
name|List
argument_list|<
name|TypeDescription
argument_list|>
...
return|;
block|}
comment|// Mostly dup of genIncludedColumns
specifier|public
specifier|static
name|TypeDescription
index|[]
...
name|AssertionError
argument_list|(
literal|"Inconsistent includes: "
operator|+
name|included
operator|.
name|length
operator|+
literal|" elements; found column ID "
operator|+
name|id
argument_list|)
throw|;
...
name|AssertionError
argument_list|(
literal|"Inconsistent includes: root column IDs are ["
operator|+
name|id
operator|+
literal|", "
...
index|]
operator|+
literal|", which is different "
operator|+
literal|" from the previous IDs of the same root column."
argument_list|)
throw|;
block|}
name|isIncluded
...
return|;
block|}
comment|/**    * Take the configuration and figure out which columns we need to include.    * @param readerSchema the types for the reader    * @param conf the configuration    */
specifier|static
name|boolean
index|[]
name|genIncludedColumns
...
name|debug
argument_list|(
literal|"Turning off hive.orc.splits.ms.footer.cache.enabled since it is not fully supported yet"
argument_list|)
expr_stmt|;
block|}
name|useExternalCache
...
block|}
block|}
comment|// Determine the transactional_properties of the table from the job conf stored in context.
...
literal|"isAcid: {} "
operator|+
literal|"isVectorMode: {} "
operator|+
literal|"sarg: {} "
operator|+
literal|"minSplitSize: {} "
operator|+
literal|"maxSplitSize: {} "
operator|+
literal|"splitStrategy: {} "
operator|+
literal|"footerInSplits: {} "
operator|+
literal|"numBuckets: {} "
operator|+
literal|"numThreads: {} "
operator|+
literal|"cacheMemSize: {} "
operator|+
literal|"cacheStripeDetails: {} "
operator|+
literal|"useSoftReference: {} "
operator|+
literal|"writeIdList: {} "
operator|+
literal|"isTransactionalTable: {} "
operator|+
literal|"txnProperties: {} "
argument_list|,
name|isAcid
argument_list|,
name|isVectorMode
...
name|deleteEvents
decl_stmt|;
comment|/**      * No (qualifying) data files found in {@link #splitPath}      * @return      */
name|boolean
name|isEmpty
parameter_list|()
block|{
...
condition|)
block|{
comment|//todo: what is this checking????
return|return
operator|(
name|files
operator|.
...
name|vectorMode
decl_stmt|;
comment|/**      * @param dir - root of partition dir      */
specifier|public
name|BISplitStrategy
parameter_list|(
name|Context
...
name|debug
argument_list|(
literal|"Blob storage detected for BI split strategy. Splitting files at boundary {}.."
argument_list|,
name|splitSize
argument_list|)
expr_stmt|;
...
comment|// This is a huge performance benefit of split-update. And the reason why we are able to
comment|// do so is because the 'deltas' here are actually only the delete_deltas. All the insert_deltas
comment|// with valid user payload data has already been considered as base for the covered buckets.
comment|// Hence, the uncovered buckets do not have any relevant data and we can just ignore them.
if|if
condition|(
name|acidOperationalProperties
operator|!=
...
name|fs
decl_stmt|;
comment|/**      * For plain or acid tables this is the root of the partition (or table if not partitioned).      * For MM table this is delta/ or base/ dir.  In MM case applying of the ValidTxnList that      * {@link AcidUtils#getAcidState(Path, Configuration, ValidWriteIdList)} normally does has already      * been done in {@link HiveInputFormat#processPathsForMmRead(List, JobConf, ValidWriteIdList)}.      */
specifier|private
specifier|final
name|Path
name|dir
...
condition|)
block|{
comment|// See the class comment - HIF handles MM for all input formats, so if we try to handle it
comment|// again, in particular for the non-recursive originals-only getSplits call, we will just
comment|// get confused. This bypass was not necessary when MM tables didn't support originals. Now
comment|// that they do, we use this path for anything MM table related, although everything except
comment|// the originals could still be handled by AcidUtils like a regular non-txn table.
name|boolean
name|isRecursive
init|=
name|context
...
return|;
block|}
comment|//todo: shouldn't ignoreEmptyFiles be set based on ExecutionEngine?
name|AcidUtils
operator|.
name|Directory
name|dirInfo
...
condition|)
block|{
comment|// For non-acid tables (or paths), all data files are in getOriginalFiles() list
for|for
control|(
name|HdfsFileStatusWithId
name|fileId
...
block|}
block|}
comment|// Find the parsed deltas- some of them containing only the insert delta events
comment|// may get treated as base if split-update is enabled for ACID. (See HIVE-14035 for details)
name|List
argument_list|<
name|ParsedDelta
argument_list|>
...
condition|)
block|{
comment|// If we have split-update turned on for this table, then the delta events have already been
comment|// split into two directories- delta_x_y/ and delete_delta_x_y/.
comment|// When you have split-update turned on, the insert events go to delta_x_y/ directory and all
comment|// the delete events go to delete_x_y/. An update event will generate two events-
comment|// a delete event for the old record that is put into delete_delta_x_y/,
comment|// followed by an insert event for the updated record put into the usual delta_x_y/.
comment|// Therefore, everything inside delta_x_y/ is an insert event and all the files in delta_x_y/
comment|// can be treated like base files. Hence, each of these are added to baseOrOriginalFiles list.
for|for
control|(
name|ParsedDelta
name|parsedDelta
...
condition|)
block|{
comment|//delta/ with files in raw format are a result of Load Data (as opposed to compaction
comment|//or streaming ingest so must have interval length == 1.
throw|throw
operator|new
name|IllegalStateException
argument_list|(
...
name|ORIGINAL_BASE
operator|+
literal|" format but txnIds are out of range: "
operator|+
name|parsedDelta
operator|.
name|getPath
...
throw|;
block|}
comment|// This is a normal insert delta, which only has insert events and hence all the files
comment|// in this delta directory can be considered as a base.
name|Boolean
name|val
init|=
name|useFileIds
...
literal|true
expr_stmt|;
comment|// The call succeeded, so presumably the API is there.
block|}
continue|continue;
comment|// move on to process to the next parsedDelta.
block|}
catch|catch
parameter_list|(
name|Throwable
...
block|}
block|}
comment|// Fall back to regular API and create statuses without ID.
name|List
argument_list|<
name|FileStatus
argument_list|>
...
block|{
comment|/*         We already handled all delete deltas above and there should not be any other deltas for         any table type.  (this was acid 1.0 code path).          */
assert|assert
name|dirInfo
operator|.
name|getCurrentDirectories
...
argument_list|()
assert|;
comment|// When split-update is not enabled, then all the deltas in the current directories
comment|// should be considered as usual.
name|parsedDeltas
operator|.
name|addAll
argument_list|(
...
name|evolution
decl_stmt|;
comment|//this is the root of the partition in which the 'file' is located
specifier|private
specifier|final
name|Path
name|rootDir
...
name|IOException
block|{
comment|// When reading the file for first time we get the orc tail from the orc reader and cache it
comment|// in the footer cache. Subsequent requests will get the orc tail from the cache (if file
comment|// length and modification time is not changed) and populate the split info. If the split info
comment|// object contains the orc tail from the cache then we can skip creating orc reader avoiding
comment|// filesystem calls.
if|if
condition|(
name|orcTail
operator|==
...
condition|)
block|{
comment|// The SchemaEvolution class has added the ACID metadata columns.  Let's update our
comment|// readerTypes so PPD code will work correctly.
name|readerTypes
operator|=
name|OrcUtils
operator|.
...
argument_list|)
decl_stmt|;
comment|// Sharing this state assumes splits will succeed or fail to get it together (same FS).
comment|// We also start with null and only set it to true on the first call, so we would only do
comment|// the global-disable thing on the first failure w/the API error, not any random failure.
name|Ref
argument_list|<
name|Boolean
argument_list|>
...
name|debug
argument_list|(
literal|"Generate splits schema evolution property "
operator|+
name|isSchemaEvolution
operator|+
...
continue|continue;
block|}
comment|// We have received a new directory information, make split strategies.
operator|--
name|resultsLeft
expr_stmt|;
if|if
...
condition|)
block|{
comment|//no files found, for example empty table/partition
continue|continue;
block|}
comment|// The reason why we can get a list of split strategies here is because for ACID split-update
comment|// case when we have a mix of original base files& insert deltas, we will produce two
comment|// independent split strategies for them. There is a global flag 'isOriginal' that is set
comment|// on a per split strategy basis and it has to be same for all the files in that strategy.
name|List
argument_list|<
name|SplitStrategy
argument_list|<
...
condition|)
block|{
comment|//why is this checking for deltas.isEmpty() - HIVE-18110
return|return
operator|new
name|ETLSplitStrategy
argument_list|(
...
name|info
argument_list|(
literal|"getSplits finished (#splits: {}). duration: {} ms"
argument_list|,
name|result
operator|.
name|size
...
name|IOException
block|{
comment|//CombineHiveInputFormat may produce FileSplit that is not OrcSplit
name|boolean
name|vectorMode
init|=
name|Utilities
...
argument_list|)
expr_stmt|;
comment|//if here we are now doing an Acid read so must have OrcSplit.  CombineHiveInputFormat is
comment|// disabled for acid path
name|OrcSplit
name|split
init|=
operator|(
...
argument_list|)
decl_stmt|;
comment|// Non-vectorized regular ACID reader.
return|return
operator|new
...
name|IllegalStateException
argument_list|(
literal|"Expected SpliUpdate table: "
operator|+
name|split
operator|.
name|getPath
...
name|warn
argument_list|(
literal|"Can't determine bucket ID for "
operator|+
name|split
operator|.
name|getPath
...
literal|false
operator|:
literal|"We should never have a split w/o base in acid 2.0 for full acid: "
operator|+
name|split
operator|.
name|getPath
...
assert|;
block|}
comment|//todo: createOptionsForReader() assumes it's !isOriginal.... why?
specifier|final
name|Reader
operator|.
name|Options
...
name|debug
argument_list|(
literal|"getReader:: Read ValidWriteIdList: "
operator|+
name|validWriteIdList
operator|.
name|toString
...
name|debug
argument_list|(
literal|"Creating merger for {} and {}"
argument_list|,
name|split
operator|.
name|getPath
...
argument_list|)
expr_stmt|;
comment|//todo: last param is bogus. why is this hardcoded?
name|OrcInputFormat
operator|.
name|setSearchArgument
argument_list|(
...
index|]
decl_stmt|;
comment|// if row count is 0 and where there are no nulls it means index is disabled and we don't have stats
if|if
condition|(
name|stats
operator|.
...
name|debug
argument_list|(
literal|"Dynamic values are not available here {}"
argument_list|,
name|dve
operator|.
name|getMessage
...
argument_list|)
decl_stmt|;
comment|// if forceSynthetic == true, then assume it is not a defaultFS
name|boolean
name|isDefaultFs
init|=
operator|(
...
argument_list|()
decl_stmt|;
comment|// Separate the base files into acid schema and non-acid(original) schema files.
for|for
control|(
name|AcidBaseFileInfo
name|acidBaseFileInfo
...
block|}
block|}
comment|// Generate split strategy for non-acid schema original files, if any.
if|if
condition|(
operator|!
name|originalSchemaFiles
...
block|}
block|}
comment|// Generate split strategy for acid schema files, if any.
if|if
condition|(
operator|!
name|acidSchemaFiles
...
block|}
block|}
comment|/**    *    * @param bucket bucket/writer ID for this split of the compaction job    */
annotation|@
name|Override
specifier|public
name|RawReader
...
condition|)
block|{
comment|//this is NULL for minor compaction
comment|//it may also be null if there is no base - only deltas
name|mergerOptions
operator|.
name|baseDir
argument_list|(
...
block|{
comment|//since we have no base, there must be at least 1 delta which must a result of acid write
comment|//so it must be immediate child of the partition
name|mergerOptions
operator|.
name|rootPath
argument_list|(
...
name|fileId
decl_stmt|;
comment|// used by external cache
name|Path
name|path
decl_stmt|;
comment|// used by local cache
name|FooterCacheKey
parameter_list|(
name|Long
name|fileId
