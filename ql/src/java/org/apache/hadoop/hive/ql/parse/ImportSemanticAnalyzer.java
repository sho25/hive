...
argument_list|)
expr_stmt|;
comment|//TODO : isPathOwnedByHive is hard coded to true, need to get it from repl dump metadata.
name|TableType
name|tableType
init|=
name|TableType
...
argument_list|)
expr_stmt|;
comment|// we should set this to null so default location for external tables is chosen on target
name|tblDesc
operator|.
name|setLocation
argument_list|(
...
condition|)
block|{
comment|// If the user is explicitly importing a new external table, clear txn flags from the spec.
name|AcidUtils
operator|.
...
comment|// In replication flow, no need to allocate write id. It will be allocated using the alloc write id event.
if|if
condition|(
name|x
operator|.
...
name|debug
argument_list|(
literal|"partition {} has data location: {}"
argument_list|,
name|partition
argument_list|,
name|location
...
name|OVERWRITE_EXISTING
expr_stmt|;
comment|//Replication scope the write id will be invalid
name|boolean
name|useStagingDirectory
init|=
operator|!
name|AcidUtils
operator|.
...
condition|)
block|{
comment|// During replication, by the time we are applying commit transaction event, we expect
comment|// the partition/s to be already added or altered by previous events. So no need to
comment|// create add partition event again.
name|addPartTask
operator|=
name|TaskFactory
operator|.
...
condition|)
block|{
comment|/*         we use isExternal and not tableType() method since that always gives type as managed table.         we don't do anything since for external table partitions the path is already set correctly         in {@link org.apache.hadoop.hive.ql.parse.repl.load.message.TableHandler}        */
if|if
condition|(
name|replicationSpec
operator|.
...
condition|)
block|{
comment|// at this point the table.getDataLocation() should be set already for external tables
comment|// using the correct values of default warehouse external table location on target.
name|partSpec
operator|.
name|setLocation
argument_list|(
...
name|debug
argument_list|(
literal|"partition spec {} has location set to {} for a table migrating to external table"
operator|+
literal|" from managed table"
argument_list|,
name|StringUtils
operator|.
name|join
...
name|AssertionError
argument_list|(
literal|"Internal error: transactional properties not set properly"
operator|+
name|tblDesc
operator|.
name|getTblProps
...
expr_stmt|;
block|}
comment|// During replication, by the time we replay a commit transaction event, the table should
comment|// have been already created when replaying previous events. So no need to create table
comment|// again.
if|if
condition|(
name|x
...
block|{
comment|// We should not require to create a drop table task when replaying a commit transaction
comment|// event. That should have been done when replaying create table event itself.
assert|assert
name|dropTblTask
operator|==
literal|null
assert|;
comment|// Add all the tasks created above directly
if|if
condition|(
name|dependentTasks
operator|!=
