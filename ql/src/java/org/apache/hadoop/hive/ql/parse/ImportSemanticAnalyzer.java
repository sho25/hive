...
condition|)
block|{
comment|// We could import everything as is - directories and IDs, but that won't work with ACID
comment|// txn ids in future. So, let's import everything into the new MM directory with ID == 0.
name|mmWriteId
operator|=
literal|0l
expr_stmt|;
block|}
comment|//todo due to master merge on May 4, tblDesc has been changed from CreateTableDesc to ImportTableDesc
comment|// which may result in Import test failure
comment|/*     if (mmWriteId != null) {       tblDesc.setInitialMmWriteId(mmWriteId);     }     */
if|if
condition|(
operator|!
name|replicationSpec
