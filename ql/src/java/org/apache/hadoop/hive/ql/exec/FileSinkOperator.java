...
expr_stmt|;
block|}
comment|// bucket number will exists only in PARTITION_BUCKET_SORTED mode
if|if
condition|(
name|conf
operator|.
...
argument_list|)
expr_stmt|;
comment|// when dynamic partition sorting is not used, the DPSortState will be NONE
comment|// in which we will fall back to old method of file system path creation
comment|// i.e, having as many record writers as distinct values in partition column
if|if
condition|(
name|conf
operator|.
