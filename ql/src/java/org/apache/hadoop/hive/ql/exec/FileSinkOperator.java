...
init|=
literal|"TOTAL_TABLE_ROWS_WRITTEN"
decl_stmt|;
comment|/**    * Counters.    */
specifier|public
specifier|static
...
specifier|private
name|Path
name|taskOutputTempPathRoot
decl_stmt|;
name|Path
index|[]
name|outPaths
decl_stmt|;
comment|// The bucket files we successfully wrote to in this writer
name|Path
index|[]
name|outPathsCommitted
decl_stmt|;
name|Path
index|[]
name|finalPaths
...
block|{
comment|/**            * For direct write to final path during ACID insert, we create the delta directories            * later when we create the RecordUpdater using AcidOutputFormat.Options            */
name|subdirForTxn
operator|=
literal|null
expr_stmt|;
...
argument_list|()
decl_stmt|;
comment|// Ignore 0 row files except in case of insert overwrite
if|if
condition|(
name|isDirectInsert
operator|&&
...
name|HiveException
argument_list|(
literal|"Unable to commit output from: "
operator|+
name|outPaths
index|[
name|idx
...
argument_list|()
operator|+
literal|" exists. Trying to delete it."
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|HiveException
argument_list|(
literal|"Unable to delete existing target output: "
operator|+
name|finalPaths
index|[
name|idx
...
parameter_list|)
block|{
comment|// this assumes all paths are bucket names (which means no lookup is needed)
name|int
name|writerOffset
init|=
name|bucketNum
...
argument_list|()
assert|;
comment|// Not supported for temp tables.
specifier|final
name|Path
name|outputPath
init|=
...
expr_stmt|;
comment|//Counter for number of rows that are associated with a destination table in FileSinkOperator.
comment|//This count is used to get total number of rows in an insert query.
if|if
condition|(
name|conf
operator|.
...
parameter_list|)
block|{
comment|// There are 2 cases where we increment CREATED_DYNAMIC_PARTITIONS counters
comment|// 1) Insert overwrite (all partitions are newly created)
comment|// 2) Insert into table which creates new partitions (some new partitions)
if|if
condition|(
name|bDynParts
operator|&&
...
argument_list|)
decl_stmt|;
comment|// For MM tables, directory structure is
comment|//<table-dir>/<partition-dir>/<delta-dir>/
comment|// For Non-MM tables, directory structure is
comment|//<table-dir>/<staging-dir>/<partition-dir>
comment|// if UNION ALL insert, for non-mm tables subquery creates another subdirectory at the end for each union queries
comment|//<table-dir>/<staging-dir>/<partition-dir>/<union-dir>
comment|// for non-MM tables, the final destination partition directory is created during move task via rename
comment|// for MM tables and ACID insert, the final destination partition directory is created by the tasks themselves
try|try
block|{
if|if
condition|(
...
block|{
comment|// outPath will be
comment|// non-union case:<table-dir>/<staging-dir>/<partition-dir>/<taskid>
comment|// union case:<table-dir>/<staging-dir>/<partition-dir>/<union-dir>/<taskid>
name|Path
name|dpStagingDir
init|=
name|fsp
...
name|warn
argument_list|(
literal|"Skipping to increment CREATED_DYNAMIC_PARTITIONS counter.Exception: {}"
argument_list|,
name|e
operator|.
name|getMessage
...
argument_list|)
expr_stmt|;
comment|// move task will create dp final path
if|if
condition|(
name|reporter
operator|!=
...
condition|)
block|{
comment|//bucket_num_reducers_acid.q, TestTxnCommands.testMoreBucketsThanReducers()
if|if
condition|(
operator|!
name|bucketMap
...
argument_list|(
literal|"Found bucketNum="
operator|+
name|bucketNum
operator|+
literal|" from data but no mapping in 'bucketMap'."
operator|+
name|extraMsg
argument_list|)
throw|;
...
argument_list|()
assert|;
comment|//this is not used for DELETE commands (partitionEval is not set up correctly
comment|// (or needed) for that
name|Object
index|[]
...
name|trace
argument_list|(
literal|"creating new paths {} for {}, childSpec {}: tmpPath {},"
operator|+
literal|" task path {}"
argument_list|,
name|System
...
block|{
comment|// check # of dp
comment|// TODO: add an option to skip this if number of partitions checks is done by Triggers via
comment|// CREATED_DYNAMIC_PARTITION counter
if|if
condition|(
name|valToPaths
operator|.
...
condition|)
block|{
comment|//Because LLAP arrow output depends on the ThriftJDBCBinarySerDe code path
comment|//this is required for 0 row outputs
comment|//i.e. we need to write a 0 size batch to signal EOS to the consumer
for|for
control|(
name|FSPaths
name|fsPaths
...
return|;
block|}
comment|/**    * Check if nested column paths is set for 'conf'.    * If set, create a copy of 'conf' with this property unset.    */
specifier|private
name|Configuration
name|unsetNestedColumnPaths
parameter_list|(
