...
control|)
block|{
comment|// Extract the buckedID from pathFilesMap, this is more accurate method,
comment|// however. it may not work in certain cases where buckets are named
comment|// after files used while loading data. In such case, fallback to old
comment|// potential inaccurate method.
comment|// The accepted file names are such as 000000_0, 000001_0_copy_1.
name|String
name|bucketIdStr
init|=
name|Utilities
...
name|info
argument_list|(
literal|"Fallback to using older sort based logic to assign "
operator|+
literal|"buckets to splits."
argument_list|)
expr_stmt|;
name|bucketIds
operator|.
...
condition|)
block|{
comment|// This is the old logic which assumes that the filenames are sorted in
comment|// alphanumeric order and mapped to appropriate bucket number.
for|for
control|(
name|Map
operator|.
...
condition|)
block|{
comment|// The fallback mechanism kicked in which only works correctly if
comment|// there exists a file for each bucket, else it may result in wrong
comment|// result. Throw an error
block|}
if|if
condition|(
name|inputNumBuckets
...
condition|)
block|{
comment|// Need to send the splits to multiple buckets
for|for
control|(
name|int
name|i
