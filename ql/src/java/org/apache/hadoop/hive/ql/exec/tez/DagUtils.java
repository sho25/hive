...
block|}
block|}
comment|// we cannot currently allow grouping of splits where each split is a different input format
comment|// or has different deserializers similar to the checks in CombineHiveInputFormat. We do not
comment|// need the check for the opList because we will not process different opLists at this time.
comment|// Long term fix would be to have a custom input format
comment|// logic that groups only the splits that share the same input format
name|Class
argument_list|<
name|?
argument_list|>
...
name|debug
argument_list|(
literal|"Current input format class = "
operator|+
name|currentInputFormatClass
operator|+
literal|", previous input format class = "
operator|+
name|previousInputFormatClass
operator|+
literal|", verifying "
operator|+
literal|" current deserializer class = "
operator|+
name|currentDeserializerClass
operator|+
literal|" previous deserializer class = "
operator|+
name|previousDeserializerClass
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// if it is the case of different input formats for different partitions, we cannot group
comment|// in the custom vertex for now. Long term, this can be improved to group the buckets that
comment|// share the same input format.
if|if
condition|(
name|useTezGroupedSplits
operator|==
