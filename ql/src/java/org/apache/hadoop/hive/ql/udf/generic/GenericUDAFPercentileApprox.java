...
comment|// method.
comment|// The return type for FINAL and COMPLETE is a full aggregation result, which is also a
comment|// list of DoubleWritables with the requested quantile values. The only exception is
comment|// when a single double, as opposed to an array of doubles, is passed as a parameter. In
comment|// that case, just return a single double value.
if|if
condition|(
name|m
operator|==
