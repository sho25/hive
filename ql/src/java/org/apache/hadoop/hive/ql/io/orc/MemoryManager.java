...
argument_list|)
decl_stmt|;
comment|/**    * How often should we check the memory sizes? Measured in rows added    * to all of the writers.    */
specifier|private
specifier|static
specifier|final
name|int
...
name|Callback
block|{
comment|/**      * The writer needs to check its memory usage      * @param newScale the current scale factor for memory allocations      * @return true if the writer was over the limit      * @throws IOException      */
name|boolean
name|checkMemory
parameter_list|(
name|double
...
argument_list|)
decl_stmt|;
comment|// this should always be null, but we handle the case where the memory
comment|// manager wasn't told that a writer wasn't still in use and the task
comment|// starts writing to the same path.
if|if
condition|(
name|oldVal
operator|==
...
block|{
comment|// handle a new writer that is writing to the same path
name|totalAllocation
operator|+=
name|requestedAllocation
operator|-
...
name|debug
argument_list|(
literal|"Notifying writers after "
operator|+
name|rowsAddedSinceCheck
argument_list|)
expr_stmt|;
