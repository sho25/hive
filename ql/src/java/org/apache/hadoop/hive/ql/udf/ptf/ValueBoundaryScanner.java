...
name|HiveException
function_decl|;
comment|/**    * Checks and maintains cache content - optimizes cache window to always be around current row    * thereby makes it follow the current progress.    * @param rowIdx current row    * @param p current partition for the PTF operator    * @throws HiveException    */
specifier|public
name|void
name|handleCache
parameter_list|(
...
return|return;
block|}
comment|//No need to setup/fill cache.
if|if
condition|(
name|start
operator|.
...
condition|)
block|{
comment|//We can wait with cache eviction until we're at the end of currently known ranges.
name|Map
operator|.
name|Entry
argument_list|<
...
block|{
comment|//Starting from current row, all previous ranges can be evicted.
name|checkIfCacheCanEvict
argument_list|(
name|rowIdx
argument_list|,
name|p
...
block|{
comment|//We either evict when we're at the end of currently known ranges, or if not there yet and
comment|// END is of FOLLOWING type: we should remove ranges preceding the current range beginning.
name|Map
operator|.
name|Entry
argument_list|<
...
condition|)
block|{
comment|//Starting from current row, all previous ranges before the previous range can be evicted.
name|checkIfCacheCanEvict
argument_list|(
name|rowIdx
argument_list|,
...
expr_stmt|;
block|}
comment|/**    * Retrieves the range for rowIdx, then removes all previous range entries before it.    * @param rowIdx row index.    * @param p partition.    * @param willScanFwd false: removal is started only from the previous previous range.    */
specifier|private
name|void
name|checkIfCacheCanEvict
parameter_list|(
...
block|}
block|}
comment|/**    * Inserts values into cache starting from rowIdx in the current partition p. Stops if cache    * reaches its maximum size or we get out of rows in p.    * @param rowIdx    * @param p    * @throws HiveException    */
specifier|private
name|void
name|fillCacheUntilEndOrFull
parameter_list|(
...
literal|null
decl_stmt|;
comment|//If we continue building cache
name|Map
operator|.
name|Entry
argument_list|<
...
expr_stmt|;
block|}
comment|//Signaling end of all rows in a partition
if|if
condition|(
name|cache
operator|.
...
block|}
block|}
comment|/**    * Uses cache content to jump backwards if possible. If not, it steps one back.    * @param r    * @param p    * @return pair of (row we stepped/jumped onto ; row value at this position)    * @throws HiveException    */
specifier|protected
name|Pair
argument_list|<
name|Integer
...
return|;
block|}
comment|/**    * Uses cache content to jump forward if possible. If not, it steps one forward.    * @param r    * @param p    * @return pair of (row we stepped/jumped onto ; row value at this position)    * @throws HiveException    */
specifier|protected
name|Pair
argument_list|<
name|Integer
...
return|;
block|}
comment|/**    * Uses cache to lookup row value. Computes it on the fly on cache miss.    * @param r    * @param p    * @return row value.    * @throws HiveException    */
specifier|protected
name|Object
name|computeValueUseCache
parameter_list|(
name|int
name|r
parameter_list|,
...
argument_list|(
literal|"Only unbounded ranges supported"
argument_list|)
throw|;
block|}
block|}
