...
name|debug
argument_list|(
literal|"findMinMaxKeys(original split) - ignoring"
argument_list|)
expr_stmt|;
return|return
operator|new
...
condition|)
block|{
comment|//the last condition is for when both splitStart and splitEnd are in
comment|// the same stripe
name|lastStripeIndex
operator|=
name|i
expr_stmt|;
...
name|warn
argument_list|(
literal|"Could not find keyIndex or length doesn't match ("
operator|+
name|firstStripeIndex
operator|+
literal|","
...
name|MAX_VALUE
operator|:
literal|"was bucketProper changed to a long ("
operator|+
name|bucketProperty
operator|.
name|getMinimum
