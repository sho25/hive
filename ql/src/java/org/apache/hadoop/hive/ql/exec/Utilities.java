...
parameter_list|)
block|{
comment|// TODO# this will probably break with directories cause buckets would be above (or not?)
name|String
name|taskID2
init|=
name|replaceTaskId
...
return|;
block|}
comment|/**    * Check if LLAP IO supports the column type that is being read    * @param conf - configuration    * @return false for types not supported by vectorization, true otherwise    */
specifier|public
specifier|static
name|boolean
...
name|warn
argument_list|(
literal|"Column names ({}) or types ({}) is null. Skipping type checking for LLAP IO."
argument_list|,
name|columnNames
argument_list|,
name|columnTypes
...
return|;
block|}
comment|/**    * Check if LLAP IO supports the column type that is being read    * @param readColumnNames - columns that will be read from the table/partition    * @param allColumnNames - all columns    * @param allColumnTypes - all column types    * @return false for types not supported by vectorization, true otherwise    */
specifier|public
specifier|static
name|boolean
name|checkLlapIOSupportedTypes
...
name|warn
argument_list|(
literal|"Unsupported column type encountered ({}). Disabling LLAP IO."
argument_list|,
name|readColumnType
argument_list|)
expr_stmt|;
...
name|warn
argument_list|(
literal|"readColumnTypes is null. Skipping type checking for LLAP IO. "
operator|+
literal|"readColumnNames: {} allColumnNames: {} allColumnTypes: {} readColumnTypes: {}"
argument_list|,
name|readColumnNames
argument_list|,
name|allColumnNames
...
argument_list|)
expr_stmt|;
comment|// TODO: we could add exact mm prefix here
name|Utilities
operator|.
name|LOG14535
operator|.
...
name|warn
argument_list|(
literal|"Unknown file found, deleting: "
operator|+
name|path
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|// Delete the manifest directory if we only created it for manifests; otherwise the
comment|// dynamic partition loader will find it and try to load it as a partition... what a mess.
if|if
condition|(
name|manifestDir
operator|!=
