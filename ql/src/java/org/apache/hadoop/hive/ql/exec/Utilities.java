...
return|;
block|}
comment|/**    * @param conf the configuration used to derive the filesystem to create the path    * @param mkdir the path to be created    * @param fsPermission ignored if it is hive server session and doAs is enabled    * @return true if successfully created the directory else false    * @throws IOException if hdfs experiences any error conditions    */
specifier|public
specifier|static
name|boolean
name|createDirsWithPermission
...
argument_list|)
expr_stmt|;
comment|// we reset the permission in case of hive server and doAs enabled because
comment|// currently scratch directory uses /tmp/hive-hive as the scratch directory.
comment|// However, with doAs enabled, the first user to create this directory would
comment|// own the directory and subsequent users cannot access the scratch directory.
comment|// The right fix is to have scratch dir per user.
name|fsPermission
operator|=
operator|new
name|FsPermission
...
expr_stmt|;
block|}
comment|// if we made it so far without exception we are good!
return|return
name|createDirsWithPermission
argument_list|(
name|conf
...
parameter_list|)
block|{
comment|// do nothing - double failure
block|}
block|}
return|return
name|retval
