...
name|Utilities
block|{
comment|/**    * Mapper to use to serialize/deserialize JSON objects ().    */
specifier|public
specifier|static
specifier|final
name|ObjectMapper
...
decl_stmt|;
comment|// all common whitespaces as defined in Character.isWhitespace(char)
comment|// Used primarily as a workaround until TEXT-175 is released
specifier|public
specifier|static
specifier|final
name|char
...
specifier|public
interface|interface
name|SupplierWithCheckedException
parameter_list|<
name|T
parameter_list|,
name|X
...
argument_list|)
decl_stmt|;
comment|// since we are doing RPC creating a directory is un-necessary
name|fs
operator|.
name|mkdirs
argument_list|(
...
comment|//   1) Rename tmpPath to a new directory name to prevent additional files
comment|//      from being added by runaway processes.
comment|// this is only done for all statements except SELECT, CTAS and Create MV
name|Path
...
name|debug
argument_list|(
literal|"shouldAvoidRename is false therefore moving/renaming "
operator|+
name|tmpPathOriginal
operator|+
literal|" to "
...
condition|)
block|{
comment|// for CTAS or Create MV statements
name|perfLogger
operator|.
name|PerfLogBegin
argument_list|(
literal|"FileSinkOperator"
argument_list|,
literal|"moveSpecifiedFileStatus"
argument_list|)
expr_stmt|;
name|LOG
operator|.
name|debug
argument_list|(
literal|"CTAS/Create MV: Files being renamed:  "
operator|+
name|filesKept
operator|.
name|toString
...
block|{
comment|// for rest of the statement e.g. INSERT, LOAD etc
name|perfLogger
operator|.
name|PerfLogBegin
argument_list|(
...
name|debug
argument_list|(
literal|"Final renaming/moving. Source: "
operator|+
name|tmpPath
operator|+
literal|" .Destination: "
...
expr_stmt|;
block|}
comment|/**    * Check the existence of buckets according to bucket specification. Create empty buckets if    * needed.    *    * @param hconf The definition of the FileSink.    * @param paths A list of empty buckets to create    * @param reporter The mapreduce reporter object    * @throws HiveException    * @throws IOException    */
specifier|static
name|void
name|createEmptyBuckets
parameter_list|(
...
name|IOException
block|{
comment|// listStatus is not required to be called to check if we need to delete the directory or not,
comment|// delete does that internally. We are getting the file list as it is used by the caller.
name|FileStatus
index|[]
name|items
init|=
...
condition|)
block|{
comment|// delete() returns false in only two conditions
comment|//  1. Tried to delete root
comment|//  2. The file wasn't actually there (or deleted by some other thread)
comment|//  So return value is not checked for delete.
name|fs
operator|.
name|delete
argument_list|(
...
return|;
block|}
comment|// Returns the list of non empty sub-directories, deletes the empty sub sub-directories.
specifier|private
specifier|static
name|Map
argument_list|<
...
name|error
argument_list|(
literal|"Exception in getting dir status"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// If file is already deleted by some other task, just ignore the failure with a warning.
name|LOG
operator|.
name|warn
argument_list|(
...
name|IOException
block|{
comment|// Pick the one with mewest attempt ID. For sanity, check the file sizes too.
comment|// If the file size of newest attempt is less than that for older one,
comment|// Throw an exception as it maybe a correctness issue causing it.
comment|// This breaks speculative execution if it ends prematurely.
name|FileStatus
name|toDelete
init|=
literal|null
...
argument_list|)
decl_stmt|;
comment|// Files may come in any order irrespective of their attempt IDs
if|if
condition|(
name|existingFileAttemptId
operator|>
...
name|filePath
operator|+
literal|" with newer attempt ID "
operator|+
name|fileAttemptId
operator|+
literal|" is smaller than the file "
operator|+
name|existingFile
operator|.
name|getPath
...
return|;
block|}
comment|// This function recurisvely fetches the size of all the files in given directory
specifier|private
specifier|static
name|long
name|getFileSizeRecursively
...
argument_list|)
decl_stmt|;
comment|// Recursively fetch sizes of each file
for|for
control|(
name|FileStatus
name|file
...
name|IOException
argument_list|(
literal|"Unable to fetch files in directory "
operator|+
name|src
operator|.
name|getPath
...
return|;
block|}
comment|/**    * Remove elements from the classpath, if possible. This will only work if the current thread context class loader is    * an UDFClassLoader (i.e. if we have created it).    *    * @param pathsToRemove    *          Array of classpath elements    */
specifier|public
specifier|static
name|void
...
argument_list|()
decl_stmt|;
comment|// If current class loader is NOT UDFClassLoader, then it is a system class loader, we should not mess with it.
if|if
condition|(
operator|!
operator|(
...
name|warn
argument_list|(
literal|"Ignoring attempt to manipulate {}; probably means we have closed more UDF loaders than opened."
argument_list|,
name|currentLoader
operator|==
literal|null
...
return|return;
block|}
comment|// Otherwise -- for UDFClassLoaders -- we close the current one and create a new one, with more limited class path.
name|UDFClassLoader
name|loader
init|=
operator|(
...
return|;
block|}
comment|/**    * Extract db and table name from dbtable string.    * @param defaultDb    * @param dbtable    * @return String array with two elements, first is db name, second is table name    * @throws SemanticException    * @deprecated use {@link TableName} or {@link org.apache.hadoop.hive.ql.parse.HiveTableName} instead    */
annotation|@
name|Deprecated
specifier|public
specifier|static
...
block|}
block|}
comment|/**    * Copy job credentials to table properties    * @param tbl    */
specifier|public
specifier|static
name|void
...
name|info
argument_list|(
literal|"Not using thread pool for getContentSummary"
argument_list|)
expr_stmt|;
name|executor
operator|=
...
return|;
block|}
comment|/**    * Performs a ContentSummary lookup over a set of paths using 1 or more    * threads. The 'summary' argument is directly modified.    *    * @param ctx    * @param pathNeedProcess    * @param work    * @param summary    * @param executor    * @throws IOException    */
annotation|@
name|VisibleForTesting
specifier|static
name|void
...
name|trace
argument_list|(
literal|"Looking at committed file: {}"
argument_list|,
name|nextFile
argument_list|)
expr_stmt|;
...
condition|)
block|{
name|cleanDirectInsertDirectoriesConcurrently
argument_list|(
name|directInsertDirectories
argument_list|,
name|committed
...
argument_list|>
argument_list|>
name|cleanTaskFutures
init|=
name|submitCleanTasksForExecution
argument_list|(
name|executor
argument_list|,
name|directInsertDirectories
...
argument_list|)
decl_stmt|;
name|waitForCleanTasksToComplete
argument_list|(
name|executor
argument_list|,
name|cleanTaskFutures
...
name|setNameFormat
argument_list|(
literal|"Clean-Direct-Insert-Dirs-Thread-%d"
argument_list|)
operator|.
name|build
argument_list|()
...
condition|)
block|{
comment|// We need to recurse into some LB directories. We don't check the directories themselves
comment|// for matches; if they are empty they don't matter, and we do will delete bad files.
comment|// This recursion is not the most efficient way to do this but LB is rarely used.
if|if
condition|(
name|child
operator|.
...
name|trace
argument_list|(
literal|"Recursion into LB directory {}; levels remaining "
argument_list|,
name|childPath
argument_list|,
name|lbLevels
...
name|HiveException
argument_list|(
literal|"LB FSOP has commited "
operator|+
name|childPath
operator|+
literal|" outside of LB directory levels "
operator|+
name|lbLevels
argument_list|)
throw|;
...
continue|continue;
block|}
comment|// No more LB directories expected.
if|if
condition|(
name|unionSuffix
operator|==
...
name|HiveException
argument_list|(
literal|"Union FSOP has an unknown directory "
operator|+
name|childPath
operator|+
literal|" outside of union directory "
...
return|;
block|}
comment|/**    * Checks if path passed in exists and has writable permissions.    * The path will be created if it does not exist.    * @param rootHDFSDirPath    * @param conf    */
specifier|public
specifier|static
name|void
...
block|}
block|}
comment|// Get the bucketing version stored in the string format
specifier|public
specifier|static
name|int
...
return|;
block|}
comment|/**    * Logs the class paths of the job class loader and the thread context class loader to the passed logger.    * Checks both loaders if getURLs method is available; if not, prints a message about this (instead of the class path)    *    * Note: all messages will always be logged with INFO log level.    */
specifier|public
specifier|static
name|void
...
name|info
argument_list|(
literal|"{} class path = unavailable for {}"
argument_list|,
name|prefix
argument_list|,
name|loader
