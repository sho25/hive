...
argument_list|)
expr_stmt|;
name|createReplLoadCompleteAckTask
argument_list|()
expr_stmt|;
block|}
catch|catch
...
return|;
block|}
comment|/**    * If replication policy is changed between previous and current load, then the excluded tables in    * the new replication policy will be dropped.    * @throws HiveException Failed to get/drop the tables.    */
specifier|private
name|void
name|dropTablesExcludedInReplScope
parameter_list|(
name|ReplScope
name|replScope
parameter_list|)
...
name|HiveException
block|{
comment|// If all tables are included in replication scope, then nothing to be dropped.
if|if
condition|(
operator|(
name|replScope
...
argument_list|()
decl_stmt|;
comment|// List all the tables that are excluded in the current repl scope.
name|Iterable
argument_list|<
name|String
argument_list|>
...
name|info
argument_list|(
literal|"Tables in the Database: {} that are excluded in the replication scope are dropped."
argument_list|,
name|dbName
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|//All repl load tasks are executed and status is 0, create the task to add the acknowledgement
...
try|try
block|{
comment|// If replication policy is changed between previous and current repl load, then drop the tables
comment|// that are excluded in the new replication policy.
name|dropTablesExcludedInReplScope
argument_list|(
name|work
operator|.
...
expr_stmt|;
block|}
comment|// If there are no more events to be applied, add a task to update the last.repl.id of the
comment|// target database to the event id of the last event considered by the dump. Next
comment|// incremental cycle won't consider the events in this dump again if it starts from this id.
if|if
condition|(
operator|!
name|builder
...
condition|)
block|{
comment|// The name of the database to be loaded into is either specified directly in REPL LOAD
comment|// command i.e. when dbNameToLoadIn has a valid dbname or is available through dump
comment|// metadata during table level replication.
name|String
name|dbName
init|=
name|work
...
block|}
block|}
comment|// If we are replicating to multiple databases at a time, it's not
comment|// possible to know which all databases we are replicating into and hence we can not
comment|// update repl id in all those databases.
if|if
condition|(
name|StringUtils
operator|.
...
name|debug
argument_list|(
literal|"Added task to set last repl id of db "
operator|+
name|dbName
operator|+
literal|" to "
