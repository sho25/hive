...
return|return
operator|new
name|GenericUDAFHypotheticalSetCumeDistEvaluator
argument_list|()
return|;
block|}
specifier|public
...
block|}
block|}
comment|/**    * Evaluator for calculating the cumulative distribution.    * SELECT cume_dist(expression) WITHIN GROUP (ORDER BY col1)    * Implementation is based on hypothetical rank calculation: (rank + 1) / (count + 1)    * Differences:    * - rows which has equal column value with the specified expression value should be counted in the rank    * - the return value type of this function is double.    */
specifier|public
specifier|static
class|class
name|GenericUDAFHypotheticalSetCumeDistEvaluator
