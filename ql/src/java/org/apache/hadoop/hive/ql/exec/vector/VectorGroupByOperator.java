...
name|forwardCache
decl_stmt|;
comment|/**    * Interface for processing mode: global, hash or streaming    */
specifier|private
specifier|static
interface|interface
...
block|}
block|}
comment|/**    * Streaming processing mode. Intermediate values are flushed each time key changes.    * In this mode we're relying on the MR shuffle and merge the intermediates in the reduce.    */
specifier|private
class|class
...
name|ProcessingModeBase
block|{
comment|/**       * The aggreagation buffers used in streaming mode      */
specifier|private
name|VectorAggregationBufferRow
...
name|info
argument_list|(
literal|"using streaming aggregation processing mode"
argument_list|)
expr_stmt|;
block|}
annotation|@
...
block|{
comment|//TODO: consider if parent can offer order guarantees
comment|// If input is sorted, is more efficient to use the streaming mode
name|processingMode
operator|=
name|this
operator|.
