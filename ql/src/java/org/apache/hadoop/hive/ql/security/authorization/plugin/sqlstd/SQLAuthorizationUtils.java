...
argument_list|)
decl_stmt|;
comment|// There are a couple of possibilities to consider here to see if we should recurse or not.
comment|// a) Path is a regex, and may match multiple entries - if so, this is likely a load and
comment|//        we should listStatus for all relevant matches, and recurse-check each of those.
comment|//        Simply passing the filestatus on as recurse=true makes sense for this.
comment|// b) Path is a singular directory/file and exists
comment|//        recurse=true to check all its children if applicable
comment|// c) Path is a singular entity that does not exist
comment|//        recurse=false to check its parent - this is likely a case of
comment|//        needing to create a dir that does not exist yet.
if|if
condition|(
operator|(
name|fileMatches
...
name|debug
argument_list|(
literal|"Checking fs privileges for multiple files that matched {}"
argument_list|,
name|filePath
operator|.
name|toString
...
operator|)
decl_stmt|;
comment|// did we find the file/dir itself?
if|if
condition|(
name|pickParent
condition|)
...
name|debug
argument_list|(
literal|"Checking fs privileges for parent path {} for nonexistent {}"
argument_list|,
name|path
operator|.
name|toString
...
name|debug
argument_list|(
literal|"Checking fs privileges for path itself {}, originally specified as {}"
argument_list|,
name|path
operator|.
name|toString
...
name|Exception
block|{
comment|// We need to obtain an intersection of all the privileges
if|if
condition|(
name|fileStatuses
operator|.
...
name|debug
argument_list|(
literal|"addPrivilegesFromFS:[{}] asked for privileges on [{}] with recurse={} and obtained:[{}]"
argument_list|,
name|userName
argument_list|,
name|fileStatus
