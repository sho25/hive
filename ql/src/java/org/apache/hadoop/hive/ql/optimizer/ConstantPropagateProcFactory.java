...
return|;
block|}
comment|/**    * Cast type from expression type to expected type ti.    *    * @param desc constant expression    * @param ti expected type info    * @param performSafeTypeCast when true then don't perform typecast because it could be unsafe (loosing leading zeroes etc.)    * @return cast constant, or null if the type cast failed.    */
specifier|private
specifier|static
name|ExprNodeConstantDesc
name|typeCast
...
return|;
block|}
comment|/**    * Fold input expression desc.    *    * @param desc folding expression    * @param constants current propagated constant map    * @param cppCtx    * @param op processing operator    * @param propagate if true, assignment expressions will be added to constants.    * @return fold expression    * @throws UDFArgumentException    */
specifier|private
specifier|static
name|ExprNodeDesc
name|foldExpr
...
return|;
block|}
comment|/**    * Combines the logical not() operator with the child operator if possible.    * @param desc the expression to be evaluated    * @return  the new expression to be replaced    * @throws UDFArgumentException    */
specifier|private
specifier|static
name|ExprNodeDesc
name|foldNegative
...
block|{
comment|// For operator like if() that cannot be handled, leave not() as it
comment|// is and continue processing the children
name|List
argument_list|<
name|ExprNodeDesc
argument_list|>
...
name|UDFArgumentException
block|{
comment|// Combine NOT operator with the child operator. Otherwise, the following optimization
comment|// from bottom up could lead to incorrect result, such as not(x> 3 and x is not null),
comment|// should not be optimized to not(x> 3), but (x<=3 or x is null).
name|desc
operator|=
name|foldNegative
argument_list|(
...
return|;
block|}
comment|/**    * Can the UDF be used for constant folding.    */
specifier|private
specifier|static
name|boolean
name|isConstantFoldableUdf
...
parameter_list|)
block|{
comment|// Runtime constants + deterministic functions can be folded.
if|if
condition|(
operator|!
name|FunctionRegistry
...
condition|)
block|{
comment|// unix_timestamp is polymorphic (ignore class annotations)
return|return
literal|true
return|;
block|}
...
condition|)
block|{
comment|// we didn't find case or when udf
return|return
literal|null
return|;
block|}
...
condition|)
block|{
comment|// Try to fold (key<op> 86) and (key is not null) to (key<op> 86)
comment|// where<op> can be "=", ">=", "<=", ">", "<".
comment|// Note: (key<> 86) and (key is not null) cannot be folded
name|ExprNodeColumnDesc
name|colDesc
init|=
name|ExprNodeDescUtils
...
block|}
block|}
comment|// Remove unnecessary expressions
name|int
name|pos
init|=
literal|0
...
condition|)
block|{
comment|// In general, when can have unlimited # of branches,
comment|// we currently only handle either 1 or 2 branch.
return|return
literal|null
return|;
block|}
...
condition|)
block|{
comment|// both branches are null.
return|return
name|thenExpr
return|;
block|}
...
condition|)
block|{
comment|// we can still fold, since here null is equivalent to false.
return|return
name|Boolean
operator|.
name|TRUE
...
block|{
comment|// can't do much, expression is not in context of filter, so we can't treat null as equivalent to false here.
return|return
literal|null
return|;
block|}
...
condition|)
block|{
comment|// HIVE-9644 Attempt to fold expression like :
comment|// where (case ss_sold_date when '1998-01-01' then 1=1 else null=1 end);
comment|// where ss_sold_date= '1998-01-01' ;
if|if
condition|(
operator|!
operator|(
...
condition|)
block|{
comment|// In general case can have unlimited # of branches,
comment|// we currently only handle either 1 or 2 branch.
return|return
literal|null
return|;
...
condition|)
block|{
comment|// nested complex types cannot be folded cleanly
return|return
literal|null
return|;
block|}
...
condition|)
block|{
comment|// do not fold named_struct, only struct()
name|ConstantObjectInspector
name|coi
init|=
operator|(
...
argument_list|)
expr_stmt|;
comment|// See if we can set the tabAlias this was folded from as well.
name|ExprNodeDesc
name|colExpr
init|=
name|colList
