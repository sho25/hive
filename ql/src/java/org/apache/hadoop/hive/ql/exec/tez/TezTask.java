...
expr_stmt|;
block|}
comment|// We only need a username for UGI to use for groups; getGroups will fetch the groups
comment|// based on Hadoop configuration, as documented at
comment|// https://hadoop.apache.org/docs/r2.8.0/hadoop-project-dist/hadoop-common/GroupsMapping.html
name|String
name|userName
init|=
name|getUserNameForGroups
argument_list|(
name|ss
argument_list|)
decl_stmt|;
...
argument_list|)
decl_stmt|;
comment|// DAG scratch dir. We get a session from the pool so it may be different from Tez one.
comment|// TODO: we could perhaps reuse the same directory for HiveResources?
name|Path
name|scratchDir
init|=
name|utils
...
comment|// This would refresh any conf resources and also local resources.
...
argument_list|)
expr_stmt|;
comment|// This is a combination of the jar stuff from conf, and not from conf.
name|List
argument_list|<
name|LocalResource
argument_list|>
...
argument_list|)
expr_stmt|;
comment|// Note: we no longer call addTaskLocalFiles because all the resources are correctly
comment|//       updated in the session resource lists now, and thus added to vertices.
comment|//       If something breaks, dag.addTaskLocalFiles might need to be called here.
...
parameter_list|)
block|{
comment|// This should be removed when authenticator and the 2-username mess is cleaned up.
if|if
condition|(
name|ss
operator|.
...
condition|)
block|{
comment|// Note: the only sane case where this can happen is the non-pool one. We should get rid
comment|//       of it, in non-pool case perf doesn't matter so we might as well open at get time
comment|//       and then call update like we do in the else.
...
name|TezRuntimeConfiguration
operator|.
name|TEZ_RUNTIME_IO_SORT_MB_DEFAULT
expr_stmt|;
block|}
else|else
block|{
...
name|Exception
block|{
comment|// Note: we don't pass the config to reopen. If the session was already open, it would
comment|//       have kept running with its current config - preserve that behavior.
name|TezSessionState
name|newSession
init|=
name|oldSession
