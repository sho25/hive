...
block|}
block|}
comment|// test if we need total order, if so, we can either use MR shuffle + set #reducer to 1,
comment|// or we can use SHUFFLE_SORT
if|if
condition|(
name|edgeProperty
operator|.
...
block|}
block|}
comment|// set to groupby-shuffle if it's still NONE
