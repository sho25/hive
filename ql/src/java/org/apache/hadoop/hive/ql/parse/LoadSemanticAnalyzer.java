...
name|SemanticException
argument_list|(
literal|"FileFormat should contain both input format and Serde"
argument_list|)
throw|;
block|}
try|try
...
name|SemanticException
argument_list|(
literal|"FileFormat inputFormatClassName or serDeClassName is incorrect"
argument_list|)
throw|;
block|}
block|}
...
name|SemanticException
argument_list|(
literal|"Load Data: Failed to set inputFormat or SerDe"
argument_list|)
throw|;
block|}
block|}
comment|// Make the columns list for the temp table (input data file).
...
argument_list|)
expr_stmt|;
comment|// inpPartSpec is a mapping from partition column name to its value.
name|Map
argument_list|<
name|String
argument_list|,
...
literal|null
decl_stmt|;
comment|// Partition spec was already validated by caller when create TableSpec object.
comment|// So, need not validate inpPartSpec here.
name|List
argument_list|<
name|FieldSchema
argument_list|>
...
comment|// Not stripping quotes here as we need to use it as it is while framing PARTITION clause
comment|// in INSERT query.
name|partVal
operator|=
name|partSpecValNode
operator|.
...
expr_stmt|;
block|}
comment|// Add only dynamic partition columns to the temp table (input data file).
comment|// For static partitions, values would be obtained from partition(key=value...) clause.
for|for
control|(
name|FieldSchema
name|fs
...
argument_list|()
decl_stmt|;
comment|// If a partition value is not there, then it is dynamic partition key.
if|if
condition|(
name|inpPartSpec
operator|.
...
block|{
comment|// No static partitions specified and hence all are dynamic partition keys and need to be part
comment|// of temp table (input data file).
name|colList
operator|.
name|addAll
argument_list|(
...
expr_stmt|;
block|}
comment|// Set columns list for temp table.
name|tempTableObj
operator|.
name|setFields
argument_list|(
