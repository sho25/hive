...
specifier|private
name|void
name|throwInvalidSubqueryError
parameter_list|(
specifier|final
name|ASTNode
name|comparisonOp
...
throw|;
block|}
comment|//<>ANY and =ALL is not supported
specifier|private
name|RexNode
name|convertSubquerySomeAll
parameter_list|(
specifier|final
name|ExprNodeSubQueryDesc
name|subQueryDesc
...
argument_list|)
expr_stmt|;
comment|// Adjust branch types by inserting explicit casts if the actual is ambigous
name|childRexNodeLst
operator|=
name|adjustCaseBranchTypes
argument_list|(
name|childRexNodeLst
argument_list|,
name|retType
...
return|;
block|}
comment|/**    * Adds explicit casts if Calcite's type system could not resolve the CASE branches to a common type.    *    * Calcite is more stricter than hive w.r.t type conversions.    * If a CASE has branches with string/int/boolean branch types; there is no common type.    */
specifier|private
name|List
argument_list|<
name|RexNode
...
condition|)
block|{
comment|// conversion is possible; not changes are neccessary
return|return
name|nodes
return|;
block|}
