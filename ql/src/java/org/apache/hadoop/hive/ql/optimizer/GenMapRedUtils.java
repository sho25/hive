...
name|MoveTask
operator|&&
name|areMoveTasksOnSameBlobStorage
argument_list|(
name|hconf
argument_list|,
operator|(
...
block|}
block|}
comment|/**    * Compare if moveTask1 source path is on the same filesystem as moveTask2 destination path.    *    * @param hconf Configuration object    * @param moveTask1 First MoveTask where the source will be compared.    * @param moveTask2 Second MoveTask where the destination will be compared.    * @return True if source/destination are on the same filesystem; False otherwise.    */
specifier|private
specifier|static
name|boolean
name|areMoveTasksOnSameBlobStorage
...
argument_list|()
decl_stmt|;
comment|// Let's not merge the tasks in case both file and table work are present. This should not
comment|// be configured this way, but the API allows you to do that.
if|if
condition|(
name|moveWork1
operator|.
...
block|{
comment|// Multi-files is not supported on this optimization
return|return
literal|false
return|;
block|}
...
return|;
block|}
comment|/**    * Creates a new MoveTask that uses the moveTask1 source and moveTask2 destination as new    * source/destination paths. This function is useful when two MoveTask are found on the    * execution plan, and they are join each other.    *    * @param moveTask1 First MoveTask where the source path will be used.    * @param moveTask2 Second MoveTask where the destination path will be used.    */
specifier|private
specifier|static
name|void
name|mergeMoveTasks
...
argument_list|)
expr_stmt|;
comment|// Link task2 dependent tasks to MoveTask1
if|if
condition|(
name|moveTask2
operator|.
