...
argument_list|)
expr_stmt|;
comment|// Create and attach the filesink for the merge.
name|OperatorFactory
operator|.
...
block|}
block|}
comment|/**    * Returns the table location path from a TableDesc object.    *    * @param hconf Configuration object.    * @param tableDesc Table description from where to get the table name.    * @return The path where the table is located.    */
specifier|private
specifier|static
name|Path
name|getTableLocationPath
parameter_list|(
specifier|final
name|HiveConf
name|hconf
...
name|warn
argument_list|(
literal|"Unable to get the table location path for: "
operator|+
name|tableDesc
operator|.
name|getTableName
...
argument_list|(
literal|"unexpected; table should be present already..: "
operator|+
name|tableName
argument_list|,
name|e
...
name|debug
argument_list|(
literal|"can't pre-create table for CTAS"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|debug
argument_list|(
literal|"can't pre-create table for MV"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|RuntimeException
argument_list|(
literal|"unexpected; this should be a CTAS or a CREATE/REBUILD MV - however no desc present"
argument_list|)
throw|;
block|}
block|}
...
return|;
block|}
comment|/**    * Checks whether the given input/output paths and a linked MoveWork should be merged into one only MoveWork.    * This is an optimization for BlobStore systems to avoid doing two renames/copies that are not necessary.    *    * @param conf A HiveConf object to check if BlobStorage optimizations are enabled.    * @param condInputPath A path that the ConditionalTask uses as input for its sub-tasks.    * @param condOutputPath A path that the ConditionalTask uses as output for its sub-tasks.    * @param linkedMoveWork A MoveWork that the ConditionalTask uses to link to its sub-tasks.    * @return True if both Conditional input/output paths and the linked MoveWork should be merged.    */
annotation|@
name|VisibleForTesting
specifier|protected
specifier|static
...
block|{
name|Path
name|linkedSourcePath
decl_stmt|,
name|linkedTargetPath
decl_stmt|;
if|if
condition|(
name|linkedMoveWork
...
argument_list|(
literal|"Merging a path with a MoveWork with multi-files work is not allowed."
argument_list|)
throw|;
block|}
name|newWork
...
condition|)
block|{
comment|// If a new MoveWork was created, then we should link all dependent tasks from the MoveWork to link.
if|if
condition|(
name|moveTaskToLink
operator|.
