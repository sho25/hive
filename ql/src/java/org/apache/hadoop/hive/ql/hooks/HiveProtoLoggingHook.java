...
name|info
argument_list|(
literal|"Event per file enabled: {}"
argument_list|,
name|eventPerFile
argument_list|)
expr_stmt|;
...
return|return;
block|}
comment|// Note: same hookContext object is used for all the events for a given query, if we try to
comment|// do it async we have concurrency issues and when query cache is enabled, post event comes
comment|// before we start the pre hook processing and causes inconsistent events publishing.
name|QueryPlan
name|plan
init|=
name|hookContext
...
try|try
block|{
comment|// ScheduledThreadPoolExecutor uses an unbounded queue which cannot be replaced with a bounded queue.
comment|// Therefore checking queue capacity manually here.
if|if
condition|(
name|logWriter
operator|.
...
try|try
block|{
name|maybeRolloverWriterForDay
argument_list|()
expr_stmt|;
block|}
catch|catch
...
name|error
argument_list|(
literal|"Got IOException while trying to rollover: "
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// Day change over case, reset the logFileCount.
name|logFileCount
operator|=
literal|0
expr_stmt|;
...
expr_stmt|;
block|}
comment|// increment log file count, if creating a new writer.
name|writer
operator|=
name|logger
operator|.
...
parameter_list|)
block|{
comment|// Something wrong with writer, lets close and reopen.
name|IOUtils
operator|.
name|closeQuietly
argument_list|(
...
name|warn
argument_list|(
literal|"Error writing proto message for query {}, eventType: {}, retryCount: {},"
operator|+
literal|" error: {} "
argument_list|,
name|event
...
try|try
block|{
comment|// 0 seconds, for first retry assuming fs object was closed and open will fix it.
name|Thread
operator|.
name|sleep
argument_list|(
...
name|warn
argument_list|(
literal|"Got interrupted in retry sleep."
argument_list|,
name|e1
argument_list|)
expr_stmt|;
