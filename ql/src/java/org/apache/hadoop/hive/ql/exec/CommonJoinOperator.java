...
name|joinFilters
decl_stmt|;
comment|/**    * List of evaluators for conditions which appear on on-clause and needs to be    * evaluated before emitting rows. Currently, relevant only for outer joins.    *    * For instance, given the query:    *     select * from t1 right outer join t2 on t1.c1 + t2.c2&gt; t1.c3;    * The expression evaluator for t1.c1 + t2.c2&gt; t1.c3 will be stored in this list.    */
specifier|protected
specifier|transient
name|List
argument_list|<
...
name|ObjectInspector
argument_list|>
name|residualJoinFiltersOIs
decl_stmt|;
comment|/**    * Will be true depending on content of residualJoinFilters.    */
specifier|protected
specifier|transient
name|boolean
name|needsPostEvaluation
decl_stmt|;
comment|/**    * This data structure is used to keep track of rows on which residualFilters    * evaluated to false. We will iterate on this container afterwards and emit    * rows appending NULL values if it was not done. Key is relation index.    */
specifier|protected
specifier|transient
name|Map
argument_list|<
...
index|[]
argument_list|>
name|rowContainerPostFilteredOuterJoin
init|=
literal|null
decl_stmt|;
...
block|}
block|}
comment|// Create post-filtering evaluators if needed
if|if
condition|(
name|conf
operator|.
...
expr_stmt|;
block|}
comment|/**    * Determine the frequency with which to emit a log message instead of    * one for every for every event.    *    * @param sz The current number of events    * @return The next event count to emit a log message    */
specifier|protected
name|long
name|getNextSize
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// If no logging is configured, log every 1, 10, 100, 1000, ..., 100000
if|if
condition|(
name|this
operator|.
...
comment|// returns whether a record was forwarded
specifier|private
name|boolean
...
condition|)
block|{
comment|// If it is not an outer join, or the post-condition filters
comment|// are empty or the row passed them
...
condition|)
block|{
comment|// Initialize container to use for storing tuples before emitting them
name|rowContainerPostFilteredOuterJoin
operator|=
operator|new
name|HashMap
...
expr_stmt|;
block|}
comment|// Consolidation for outer joins
if|if
condition|(
name|needsPostEvaluation
operator|&&
...
condition|)
block|{
comment|// If it is a RIGHT / FULL OUTER JOIN, we need to iterate through the row container
comment|// that contains all the right records that did not produce results. Then, for each
comment|// of those records, we replace the left side with NULL values, and produce the
comment|// records.
comment|// Observe that we only enter this block when we have finished iterating through
comment|// all the left and right records (aliasNum == numAliases - 2), and thus, we have
comment|// tried to evaluate the post-filter condition on every possible combination.
comment|// NOTE: the left records that do not produce results (for LEFT / FULL OUTER JOIN)
comment|// will always be caught in the genObject method
name|Arrays
operator|.
name|fill
argument_list|(
...
condition|)
block|{
comment|// This is only executed for outer joins with residual filters
name|boolean
name|forward
init|=
name|createForwardJoinObject
...
condition|)
block|{
comment|// This record produced a result this time, remove it from the storage
comment|// as it will not need to produce a result with NULL values anymore
name|rowContainerPostFilteredOuterJoin
operator|.
name|put
argument_list|(
...
block|{
comment|// We need to store this record (if it is not done yet) in case
comment|// we should produce a result
if|if
condition|(
operator|!
name|rowContainerPostFilteredOuterJoin
...
condition|)
block|{
comment|// If it is a LEFT / FULL OUTER JOIN and the left record did not produce
comment|// results, we need to take that record, replace the right side with NULL
comment|// values, and produce the records
name|int
name|i
init|=
name|numAliases
