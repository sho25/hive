...
condition|)
block|{
comment|// Already processed, skip
continue|continue;
block|}
if|if
condition|(
...
name|warn
argument_list|(
literal|"markSemiJoinForDPP : Null pointer exception caught while accessing semijoin operators"
argument_list|)
expr_stmt|;
assert|assert
literal|false
...
try|try
block|{
comment|// If stats are not available, just assume its a useful edge
name|Statistics
name|stats
init|=
name|selOp
...
continue|continue;
block|}
comment|// Skip setting if the bucketing version is not set in FileSinkOp.
if|if
condition|(
name|fsOp
operator|.
