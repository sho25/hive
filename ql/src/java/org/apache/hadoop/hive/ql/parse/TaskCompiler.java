...
condition|)
block|{
comment|// file format is changed so that IF file sink provides list of files to fetch from (instead
comment|// of whle directory) list status is done on files (which is what HiveSequenceFileInputFormat do)
name|resultTab
operator|=
name|PlanUtils
operator|.
...
expr_stmt|;
block|}
comment|// The idea here is to keep an object reference both in FileSink and in FetchTask for list of files
comment|// to be fetched. During Job close file sink will populate the list and fetch task later will use it
comment|// to fetch the results.
name|Collection
argument_list|<
name|Operator
argument_list|<
...
argument_list|()
decl_stmt|;
comment|// Perform Final chores on generated Map works
comment|//   1.  Intern the table descriptors
comment|//   2.  Derive final explain attributes based on previous compilation.
name|GenMapRedUtils
operator|.
...
literal|0
decl_stmt|;
comment|// CTAS or CMV cannot be part of multi-txn stmt
name|FileSinkDesc
name|dataSink
init|=
literal|null
...
operator|)
assert|;
comment|// Target task is supposed to be the last task
name|Task
argument_list|<
name|?
argument_list|>
...
condition|)
block|{
comment|// For partitioned CTAS, we need to create the table before the move task
comment|// as we need to create the partitions in metastore and for that we should
comment|// have already registered the table
name|interleaveTask
argument_list|(
name|parentOfStatsTask
argument_list|,
...
block|}
block|}
comment|/**    * Makes dependentTask dependent of task.    */
specifier|private
name|void
name|interleaveTask
parameter_list|(
...
name|SemanticException
function_decl|;
comment|/*    * Called for dynamic partitioning sort optimization.    */
specifier|protected
name|void
