...
argument_list|()
decl_stmt|;
comment|// Don't filter out MetaDataFile.METADATA_FILE
if|if
condition|(
name|name
operator|.
...
return|;
block|}
comment|// Don't filter out OrcAcidVersion.ACID_FORMAT
if|if
condition|(
name|name
operator|.
...
argument_list|()
decl_stmt|;
comment|// We don't want to filter out temp tables
if|if
condition|(
name|dirPathStr
operator|.
...
block|}
block|}
comment|/**    * Determine if a table is used during query based compaction.    * @param tblProperties table properties    * @return true, if the tblProperties contains {@link AcidUtils#COMPACTOR_TABLE_PROPERTY}    */
specifier|public
specifier|static
name|boolean
...
return|;
block|}
comment|/**    * Determine if a table is used during query based compaction.    * @param parameters table properties map    * @return true, if the parameters contains {@link AcidUtils#COMPACTOR_TABLE_PROPERTY}    */
specifier|public
specifier|static
name|boolean
name|isCompactionTable
...
block|{
return|return
literal|"Aborted Directories: "
operator|+
name|abortedDirectories
operator|+
literal|"; isBaseInRawFormat: "
operator|+
name|isBaseInRawFormat
operator|+
literal|"; original: "
...
block|{
comment|// Okay, we're going to need these originals.
comment|// Recurse through them and figure out what we really need.
comment|// If we already have the original list, do nothing
comment|// If dirSnapshots != null, we would have already populated "original"
if|if
condition|(
name|dirSnapshots
operator|==
...
expr_stmt|;
block|}
comment|// We're not filtering out the metadata file and acid format file, as they represent parts of a valid snapshot
comment|// We're not using the cached values downstream, but we can potentially optimize more in a follow-up task
if|if
condition|(
name|fStatus
operator|.
...
block|}
block|}
comment|/**    * DFS dir listing.     * Captures a dir and the corresponding list of files it contains,    * with additional properties about the dir (like isBase etc)    *    */
specifier|public
specifier|static
interface|interface
name|HdfsDirSnapshot
...
parameter_list|)
function_decl|;
comment|// FileStatus of this HDFS directory
specifier|public
name|FileStatus
name|getFileStatus
parameter_list|()
function_decl|;
comment|// Get the list of files if any within this directory
specifier|public
name|List
argument_list|<
name|FileStatus
...
specifier|private
name|FileStatus
name|metadataFStatus
init|=
literal|null
decl_stmt|;
specifier|private
name|FileStatus
name|orcAcidFormatFStatus
init|=
literal|null
decl_stmt|;
specifier|private
...
condition|)
block|{
comment|// if the candidateDirectory is itself a delta directory, we need to add originals in that directory
comment|// and return. This is the case when compaction thread calls getChildState.
for|for
control|(
name|FileStatus
name|fileStatus
...
return|;
block|}
comment|/**      * Checks if the files in base/delta dir are a result of Load Data/Add Partition statement      * and thus do not have ROW_IDs embedded in the data.      * This is only meaningful for full CRUD tables - Insert-only tables have all their data      * in raw format by definition.      * @param baseOrDeltaDir base or delta file.      * @param dirSnapshot       */
specifier|public
specifier|static
name|boolean
name|isRawFormat
...
block|}
block|}
comment|/**    * Determines transaction type based on query AST.    * @param tree AST    */
specifier|public
specifier|static
name|TxnType
name|getTxnType
...
function|.noneMatch
parameter_list|(
function|pattern ->             astSearcher.simpleBreadthFirstSearch
parameter_list|(
name|tree
parameter_list|,
name|pattern
...
name|debug
argument_list|(
literal|"DirCache got initialized already"
argument_list|)
expr_stmt|;
return|return;
block|}
...
begin_comment
comment|/**    * Tries to get directory details from cache. For now, cache is valid only    * when base directory is available and no deltas are present. This should    * be used only in BI strategy and for ACID tables.    *    * @param fileSystem file system supplier    * @param candidateDirectory the partition directory to analyze    * @param conf the configuration    * @param writeIdList the list of write ids that we are reading    * @param useFileIds    * @param ignoreEmptyFiles    * @param tblproperties    * @param generateDirSnapshots    * @return directory state    * @throws IOException on errors    */
end_comment
...
name|debug
argument_list|(
literal|"dirCache is not enabled"
argument_list|)
expr_stmt|;
return|return
name|getAcidState
...
expr_stmt|;
block|}
comment|/*      * Cache for single case, where base directory is there without deltas.      * In case of changes, cache would get invalidated based on      * open/aborted list      */
comment|//dbName + tableName + dir
name|String
name|key
init|=
name|writeIdList
...
argument_list|)
decl_stmt|;
comment|// in case of open/aborted txns, recompute dirInfo
name|long
index|[]
name|exceptions
init|=
...
name|info
argument_list|(
literal|"invalidating cache entry for key: {}"
argument_list|,
name|key
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"writeIdList: {} from cache: {} is not matching "
operator|+
literal|"for key: {}"
argument_list|,
name|writeIdList
...
block|}
block|}
comment|// compute and add to cache
if|if
condition|(
name|recompute
operator|||
...
name|info
argument_list|(
literal|"Got {} from cache, cache size: {}"
argument_list|,
name|key
argument_list|,
name|dirCache
...
condition|)
block|{
comment|// Cache base directory contents
name|List
argument_list|<
name|HdfsFileStatusWithId
argument_list|>
