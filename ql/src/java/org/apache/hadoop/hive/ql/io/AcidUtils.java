...
return|;
block|}
comment|/**    * Get the write id from a base directory name.    * @param path the base directory name    * @return the maximum write id that is included    */
specifier|public
specifier|static
name|long
name|parseBase
...
condition|)
block|{
comment|/**        * If here, it means there was a base_x (> 1 perhaps) but none were suitable for given        * {@link writeIdList}.  Note that 'original' files are logically a base_Long.MIN_VALUE and thus        * cannot have any data for an open txn.  We could check {@link deltas} has files to cover        * [1,n] w/o gaps but this would almost never happen...*/
name|long
index|[]
name|exceptions
init|=
...
argument_list|)
decl_stmt|;
comment|// Handle aborted deltas. Currently this can only happen for MM tables.
if|if
condition|(
name|tblproperties
operator|!=
...
argument_list|()
expr_stmt|;
comment|//don't ask - some ByteBuffer weridness. w/o this, empty buffer is written
name|writer
operator|.
