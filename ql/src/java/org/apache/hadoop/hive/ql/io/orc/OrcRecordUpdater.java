...
name|IOException
block|{
comment|/*     * 1. need to know bucket we are writing to     * 2. need to know which delta dir it's in     * Then,     * 1. find the same bucket file in previous (insert) delta dir for this txn     *    (Note: in case of split_update, we can ignore the delete_delta dirs)     * 2. read the footer and get AcidStats which has insert count      * 2.1 if AcidStats.inserts>0 add to the insert count.      *  else go to previous delta file      *  For example, consider insert/update/insert case...*/
if|if
condition|(
name|options
operator|.
...
block|{
comment|// Just remove insert delta paths, when there are no insert events.
name|fs
operator|.
name|delete
argument_list|(
