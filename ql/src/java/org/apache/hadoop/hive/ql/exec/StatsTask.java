...
expr_stmt|;
block|}
comment|/**    *    * Statistics for a Partition or Unpartitioned Table    *    */
class|class
name|Statistics
block|{
name|Map
...
argument_list|)
expr_stmt|;
comment|// Make sure that it is either an ANALYZE, INSERT OVERWRITE or CTAS command
name|short
...
condition|)
block|{
comment|// Key is of the form dbName.tblName/
name|aggKey
operator|=
name|tableFullName
operator|+
...
condition|)
block|{
comment|// stat-Agg-key is of form : dbName.tblName/p1=v1/p2=val2/
name|partitionID
operator|=
name|Utilities
operator|.
...
argument_list|)
expr_stmt|;
comment|// there is no need to aggregate stats in this case, but this should also work.
comment|// also check non-partitioned code path.
block|}
else|else
block|{
