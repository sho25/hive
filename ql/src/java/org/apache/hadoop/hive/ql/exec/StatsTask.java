...
name|numFiles
decl_stmt|;
comment|// number of files in the partition
name|long
name|numRows
decl_stmt|;
comment|// number of rows in the partition
name|long
name|size
decl_stmt|;
comment|// total size in bytes of the partition
specifier|public
name|PartitionStatistics
parameter_list|()
block|{
...
decl_stmt|;
comment|//
comment|// For partitioned table get the old table statistics for incremental update
comment|//
if|if
condition|(
name|table
...
argument_list|()
decl_stmt|;
comment|// In that case of a partition, the key for stats temporary store is "rootDir/[dynamic_partition_specs/]%"
name|String
name|partitionID
init|=
name|work
...
argument_list|)
expr_stmt|;
comment|// Adding the path separator to avoid an Id being a prefix of another ID
block|}
block|}
block|}
end_class
