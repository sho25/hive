...
argument_list|)
expr_stmt|;
comment|// 3. Appy Join Order Optimizations using Hep Planner (MST Algorithm)
name|List
argument_list|<
name|RelMetadataProvider
argument_list|>
...
condition|)
block|{
comment|// run rules to aid in translation from Optiq tree -> Hive tree
name|hepPgm
operator|=
operator|new
name|HepProgramBuilder
...
comment|// Further, we insert a project operator on top to remove the grouping
comment|// boolean associated to each column in Calcite; this will avoid
comment|// recalculating all column positions when we go back from Calcite to Hive
if|if
condition|(
name|groupingSets
operator|!=
...
argument_list|)
expr_stmt|;
comment|// 6.1 List of columns to keep from groupBy operator
name|List
argument_list|<
name|RelDataTypeField
argument_list|>
...
block|}
block|}
comment|// 6.2 Add column for grouping_id function
name|String
name|field
init|=
name|getColumnInternalName
...
argument_list|)
expr_stmt|;
comment|// 6.3 Compute column for grouping_id function in Calcite
name|List
argument_list|<
name|RexNode
argument_list|>
