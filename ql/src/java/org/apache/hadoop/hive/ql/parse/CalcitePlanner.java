...
argument_list|)
decl_stmt|;
comment|/**    * This is the list of operators that are specifically used in Hive.    */
specifier|private
specifier|static
specifier|final
name|List
...
condition|)
block|{
comment|// Enrich explain with information derived from CBO
...
condition|)
block|{
comment|// Include cost as provided by Calcite
name|newPlan
operator|.
name|getCluster
argument_list|()
...
name|SemanticException
argument_list|(
literal|"Unexpected condition in incremental rewriting"
argument_list|)
throw|;
block|}
name|ASTNode
...
argument_list|)
expr_stmt|;
comment|// VARCHAR(INTEGER.MAX) -> STRING
name|sql
operator|=
name|PATTERN_TIMESTAMP
operator|.
...
name|calciteOptimizedPlan
operator|=
name|applyJoinOrderingTransform
argument_list|(
name|calcitePreCboPlan
argument_list|,
name|mdProvider
...
expr_stmt|;
block|}
comment|// 5. Apply post-join order optimizations
name|calciteOptimizedPlan
operator|=
name|applyPostJoinOrderingTransform
argument_list|(
name|calciteOptimizedPlan
argument_list|,
name|mdProvider
...
comment|// 5. Try to remove limit and order by
name|generatePartialProgram
argument_list|(
name|program
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// 9. Get rid of sq_count_check if group by key is constant
if|if
condition|(
name|conf
operator|.
...
name|OPTIMIZER
argument_list|,
literal|"Calcite: Prejoin ordering transformation"
argument_list|)
expr_stmt|;
return|return
name|basePlan
...
specifier|final
name|boolean
name|useMaterializedViewsRegistry
init|=
operator|!
name|conf
operator|.
...
block|{
comment|// This is not a rebuild, we retrieve all the materializations.
comment|// In turn, we do not need to force the materialization contents to be up-to-date,
comment|// as this is not a rebuild, and we apply the user parameters
comment|// (HIVE_MATERIALIZED_VIEW_REWRITING_TIME_WINDOW) instead.
if|if
condition|(
name|useMaterializedViewsRegistry
condition|)
...
condition|)
block|{
comment|// There are no materializations, we can return the original plan
return|return
name|calcitePreMVRewritingPlan
return|;
block|}
...
condition|)
block|{
comment|// If it is a materialized view rebuild, we use the HepPlanner, since we only have
comment|// one MV and we would like to use it to create incremental maintenance plans
specifier|final
name|HepProgramBuilder
name|program
init|=
...
argument_list|)
expr_stmt|;
comment|// Add materialization for rebuild to planner
assert|assert
name|materializations
operator|.
name|size
...
block|{
comment|// If this is not a rebuild, we use Volcano planner as the decision
comment|// on whether to use MVs or not and which MVs to use should be cost-based
name|optCluster
operator|.
name|invalidateMetadataQuery
argument_list|()
...
name|Table
argument_list|>
name|materializedViewsUsedOriginalPlan
init|=
name|getMaterializedViewsUsed
argument_list|(
name|calcitePreMVRewritingPlan
argument_list|)
decl_stmt|;
...
name|Table
argument_list|>
name|materializedViewsUsedAfterRewrite
init|=
name|getMaterializedViewsUsed
argument_list|(
name|basePlan
...
condition|)
block|{
comment|// Materialized view-based rewriting did not happen, we can return the original plan
return|return
name|calcitePreMVRewritingPlan
return|;
block|}
comment|// A rewriting was produced, we will check whether it was part of an incremental rebuild
comment|// to try to replace INSERT OVERWRITE by INSERT or MERGE
if|if
condition|(
name|mvRebuildMode
operator|==
...
condition|)
block|{
comment|// Before proceeding we need to check whether materialized views used are up-to-date
comment|// wrt information in metastore
try|try
block|{
if|if
condition|(
...
name|warn
argument_list|(
literal|"Exception validating materialized views"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// Make a cost-based decision factoring the configuration property
name|optCluster
operator|.
name|invalidateMetadataQuery
argument_list|()
...
return|;
block|}
comment|/**      * Perform join reordering optimization.      *      * @param basePlan      *          original plan      * @param mdProvider      *          meta data provider      * @param executorProvider      *          executor      * @return      */
specifier|private
name|RelNode
name|applyJoinOrderingTransform
parameter_list|(
...
return|;
block|}
comment|/**      * Perform join reordering post-optimization.      *      * @param basePlan      *          original plan      * @param mdProvider      *          meta data provider      * @param executorProvider      *          executor      * @return      */
specifier|private
name|RelNode
name|applyPostJoinOrderingTransform
parameter_list|(
...
argument_list|()
decl_stmt|;
comment|// 1. Run other optimizations that do not need stats
name|generatePartialProgram
argument_list|(
name|program
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// 2. Run aggregate-join transpose (cost based)
...
expr_stmt|;
block|}
comment|// 3. Convert Join + GBy to semijoin
...
expr_stmt|;
block|}
comment|// 4. convert SemiJoin + GBy to SemiJoin
name|generatePartialProgram
argument_list|(
name|program
argument_list|,
...
expr_stmt|;
block|}
comment|// 6. Sort predicates in filter expressions
if|if
condition|(
name|conf
operator|.
...
expr_stmt|;
block|}
comment|// 7. Apply Druid transformation rules
name|generatePartialProgram
argument_list|(
name|program
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// 8. Apply JDBC transformation rules
if|if
condition|(
name|conf
operator|.
...
expr_stmt|;
block|}
comment|// 9. Run rules to aid in translation from Calcite tree to Hive tree
if|if
condition|(
name|HiveConf
operator|.
...
condition|)
block|{
comment|// 9.1. Merge join into multijoin operators (if possible)
name|generatePartialProgram
argument_list|(
name|program
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// 9.2.  Introduce exchange operators below join/multijoin operators
name|generatePartialProgram
argument_list|(
name|program
argument_list|,
...
name|OPTIMIZER
argument_list|,
literal|"Calcite: Postjoin ordering transformation"
argument_list|)
expr_stmt|;
return|return
name|basePlan
...
return|;
block|}
comment|/**      * Generate a HEP program with the given rule set.      *      * @param isCollection      * @param order      * @param rules      * @return HEP program      */
specifier|private
name|void
name|generatePartialProgram
parameter_list|(
...
name|retNodeRR
init|=
name|internalGenSelectLogicalPlan
argument_list|(
name|qb
argument_list|,
name|srcRel
...
if|if
condition|(
name|isSelectDistinct
argument_list|(
name|selExprList
argument_list|)
operator|&&
name|hasGroupBySibling
argument_list|(
name|selExprList
argument_list|)
condition|)
...
return|;
block|}
comment|/**      * NOTE: there can only be one select caluse since we don't handle multi destination insert.      * @param isAllColRefRewrite      *          when it is true, it means that it is called from group by *, where we use      *          genSelectLogicalPlan to rewrite *      * @return RelNode: the select relnode RowResolver: i.e., originalRR, the RR after select when there is an order by.      */
specifier|private
name|Pair
argument_list|<
name|RelNode
...
comment|//
comment|// If DISTINCT is present, it is not possible to ORDER BY unselected
comment|// columns, and in fact adding all columns would change the behavior of
comment|// DISTINCT, so we bypass this logic.
if|if
condition|(
name|obAST
operator|!=
...
argument_list|)
decl_stmt|;
comment|// This comes from genSelectLogicalPlan, must be a project assert srcRel instanceof HiveProject;
name|RowResolver
name|outputRR
init|=
name|srcNodeRR
...
return|;
block|}
comment|/**    * This method can be called at startup time to pre-register all the    * additional Hive classes (compared to Calcite core classes) that may    * be visited during the planning phase in the metadata providers    * and the field trimmer.    */
specifier|public
specifier|static
name|void
name|warmup
