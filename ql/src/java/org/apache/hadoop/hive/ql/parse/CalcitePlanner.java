...
argument_list|()
decl_stmt|;
comment|// currently semi-join optimization doesn't work with subqueries
comment|// so this will be turned off for if we find subqueries and will later be
comment|// restored to its original state
name|boolean
name|originalSemiOptVal
init|=
name|this
operator|.
name|conf
...
expr_stmt|;
block|}
comment|// restore semi-join opt flag
name|this
operator|.
name|conf
operator|.
...
argument_list|)
expr_stmt|;
comment|// 5. Materialized view based rewriting
...
expr_stmt|;
block|}
comment|// 8. Run rule to fix windowing issue when it is done over
...
expr_stmt|;
block|}
comment|// 9. Apply Druid transformation rules
name|perfLogger
operator|.
name|PerfLogBegin
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// 10. Run rules to aid in translation from Calcite tree to Hive tree
if|if
condition|(
name|HiveConf
operator|.
...
argument_list|)
expr_stmt|;
comment|// 10.1. Merge join into multijoin operators (if possible)
name|calciteOptimizedPlan
operator|=
name|hepPlan
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// 10.2.  Introduce exchange operators below join/multijoin operators
name|calciteOptimizedPlan
operator|=
name|hepPlan
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// 8. Merge, remove and reduce Project if possible
name|perfLogger
operator|.
name|PerfLogBegin
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Missing Check: Check.5.h :: For In and Not In the SubQuery must implicitly or
comment|// explicitly only contain one select item.
block|}
block|}
specifier|private
name|boolean
...
argument_list|)
expr_stmt|;
comment|// semi-join opt doesn't work with subqueries
name|conf
operator|.
name|setBoolVar
argument_list|(
...
condition|)
block|{
comment|// The basic idea for CBO support of UDTF is to treat UDTF as a special project.
comment|// In AST return path, as we just need to generate a SEL_EXPR, we just need to remember the expressions and the alias.
comment|// In OP return path, we need to generate a SEL and then a UDTF following old semantic analyzer.
name|outputRel
operator|=
name|genUDTFPlan
argument_list|(
