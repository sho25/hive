...
block|}
block|}
comment|// Only cache ZK connections (ie local clients); these are presumed to be used in HS2.
comment|// TODO: temporary before HIVE-13698.
specifier|private
specifier|static
specifier|final
name|Cache
...
name|boolean
name|useLocalTokenClient
init|=
name|isUsingLocalClient
argument_list|(
name|conf
argument_list|)
decl_stmt|;
...
argument_list|)
decl_stmt|;
comment|// We are initialized on first use inside TezSessionState::openInternal; assume the session
...
name|warn
argument_list|(
literal|"There's no session to check if we are in HS2"
argument_list|)
expr_stmt|;
block|}
return|return
