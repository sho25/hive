...
parameter_list|)
block|{
comment|// The caller has already stopped the session.
name|LOG
operator|.
name|error
argument_list|(
...
throw|;
block|}
comment|// Check interrupt at the last moment in case we get cancelled quickly.
comment|// This is not bulletproof but should allow us to close session in most cases.
if|if
condition|(
name|Thread
operator|.
...
name|info
argument_list|(
literal|"Interrupted while starting Tez session"
argument_list|)
expr_stmt|;
...
name|RuntimeException
argument_list|(
literal|"Initialization was interrupted"
argument_list|)
throw|;
block|}
name|this
