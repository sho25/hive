...
specifier|static
class|class
name|Decimal64StreamReader
extends|extends
name|Decimal64TreeReader
implements|implements
name|SettableTreeReader
block|{
specifier|private
...
name|IOException
block|{
comment|// Note: we only look at the schema here to deal with complex types. Somebody has set up the
comment|//       reader with whatever ideas they had to the schema and we just trust the reader to
comment|//       produce the CVBs that was asked for. However, we only need to look at top level columns.
name|int
name|includedCount
init|=
name|batch
operator|.
name|getColumnsWithDataCount
argument_list|()
decl_stmt|;
if|if
condition|(
...
expr_stmt|;
block|}
comment|// TODO: do we actually need this reader? the caller just extracts child readers.
return|return
...
name|DECIMAL
case|:
comment|// special handling for serde reader (text) in llap IO.
comment|// if file format version is null, then we are processing text IF in LLAP IO, in which case
comment|// we get vectors instead of streams. If vectors contain instance of Decimal64ColumnVector we
comment|// should use Decimal64StreamReader (which acts as a wrapper around vectors)
name|boolean
name|useDecimal64Reader
init|=
name|context
operator|.
name|getFileFormat
