...
name|hashResult
decl_stmt|;
comment|// the pre-created array for reducerHash results
specifier|public
name|VectorReduceSinkOperator
parameter_list|(
...
expr_stmt|;
block|}
comment|// First, evaluate the key - the way things stand we'd need it regardless.
for|for
control|(
name|int
name|i
...
argument_list|)
expr_stmt|;
comment|// TopN is not supported for multi-distinct currently. If we have more cachedKeys
comment|// than one for every input key horrible things will happen (OOB error on array likely).
assert|assert
operator|!
name|useTopN
operator|||
...
block|{
comment|// No TopN, just forward the key
name|keyWritable
operator|.
name|setHashCode
argument_list|(
...
expr_stmt|;
block|}
comment|// Compute everything now - we'd either store it, or forward it.
name|int
name|hashCode
init|=
name|computeHashCode
...
condition|)
block|{
comment|// Kinda hacky; see getVectorizedKeyToForward javadoc.
name|byte
index|[]
name|key
init|=
