...
name|SemanticException
block|{
comment|// This functions filters out all the events which are already replayed. This can be done only
comment|// for transaction related events as for other kind of events we can not gurantee that the last
comment|// repl id stored in the database/table is valid.
if|if
condition|(
operator|(
name|dumpType
