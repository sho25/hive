...
literal|null
decl_stmt|;
comment|// Source DB Name for REPL LOAD
specifier|private
name|String
...
name|TOK_REPL_TABLES
case|:
comment|//Accept TOK_REPL_TABLES for table level repl.Needn't do anything as dump path needs db only
break|break;
default|default:
throw|throw
operator|new
...
comment|//If repl status of target is greater than dumps, don't do anything as the load for the latest dump is done
if|if
condition|(
operator|!
name|isTargetAlreadyLoaded
...
condition|)
block|{
comment|//sort based on last modified. Recent one is at the end
name|Arrays
operator|.
name|sort
argument_list|(
...
block|{
name|String
name|currentReplStatusOfTarget
init|=
name|getReplStatus
argument_list|(
name|replScope
...
block|{
comment|//If dbname is null(in case of repl load *), can't get repl status of target, return unsupported
throw|throw
operator|new
...
name|debug
argument_list|(
literal|"ReplicationSemanticAnalyzer.analyzeReplStatus: writing repl.last.id={} out to {} using configuration {}"
argument_list|,
name|replLastId
argument_list|,
name|ctx
