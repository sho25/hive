...
return|;
block|}
comment|/** Class to store the return result of genExprNodeDesc.    *     *  TODO: In the future when we refactor the PartitionPruner code, we should    *  use the same code (GraphWalker) as it is now in TypeCheckProcFactory.     *  We should use NULL to represent a table name node, and the DOT operator    *  should descend into the sub tree for 2 levels in order to find out the    *  table name.  The benefit is that we get rid of another concept class -    *  here it is ExprNodeTempDesc - the return value of a branch in the     *  Expression Syntax Tree, which is different from the value of a branch in    *  the Expression Evaluation Tree.      *         */
specifier|static
class|class
name|ExprNodeTempDesc
block|{
specifier|public
name|ExprNodeTempDesc
parameter_list|(
...
expr_stmt|;
block|}
comment|// Is it a special case: table DOT column?
if|if
condition|(
name|expr
operator|.
...
case|:
block|{
comment|// Must be a column. If it's a table then it's already processed by "case DOT" below.
name|String
name|colName
init|=
name|BaseSemanticAnalyzer
...
block|}
block|}
comment|// else fall through to default
block|}
default|default:
block|{
name|boolean
