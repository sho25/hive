...
argument_list|)
decl_stmt|;
comment|// TODO: For object inspector fields, assigning 16KB for now. To better estimate the memory size every
comment|// object inspectors have to implement MemoryEstimate interface which is a lot of change with little benefit compared
comment|// to writing an instrumentation agent for object size estimation
specifier|public
specifier|static
specifier|final
name|long
name|DEFAULT_OBJECT_INSPECTOR_MEMORY_SIZE
init|=
literal|16
operator|*
literal|1024L
...
condition|)
block|{
comment|// Skip empty values just like MapJoinEagerRowContainer.read does.
return|return
operator|(
name|byte
operator|)
...
return|return
operator|new
name|NonMatchedSmallTableIteratorImpl
argument_list|(
name|matchTracker
argument_list|)
return|;
...
parameter_list|()
block|{
comment|// If another non-matched key is found, the key bytes will be referenced by keyRef, and
comment|// our ReusableRowContainer's BytesBytesMultiHashMap.Result will reference the value rows.
name|currentIndex
operator|=
name|hashMap
operator|.
