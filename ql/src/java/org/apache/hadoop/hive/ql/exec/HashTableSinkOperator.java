...
name|hconf
decl_stmt|;
comment|// Used as a differentiator for different files
comment|// in case multiple files are created for one operator.
specifier|private
name|int
name|fileIndex
init|=
...
return|;
block|}
comment|//  private static List<ObjectInspector>[] getStandardObjectInspectors(
comment|//      List<ObjectInspector>[] aliasToObjectInspectors, int maxTag) {
comment|//    @SuppressWarnings("unchecked")
comment|//    List<ObjectInspector>[] result = new List[maxTag];
comment|//    for (byte alias = 0; alias< aliasToObjectInspectors.length; alias++) {
comment|//      List<ObjectInspector> oiList = aliasToObjectInspectors[alias];
comment|//      if (oiList == null) {
comment|//        continue;
comment|//      }
comment|//      ArrayList<ObjectInspector> fieldOIList = new ArrayList<ObjectInspector>(oiList.size());
comment|//      for (int i = 0; i< oiList.size(); i++) {
comment|//        fieldOIList.add(ObjectInspectorUtils.getStandardObjectInspector(oiList.get(i),
comment|//            ObjectInspectorCopyOption.WRITABLE));
comment|//      }
comment|//      result[alias] = fieldOIList;
comment|//    }
...
argument_list|)
decl_stmt|;
comment|// For Spark, path is a folder. Let's create it now.
if|if
condition|(
name|HiveConf
operator|.
