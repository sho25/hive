...
comment|// so we first process distUDAF and then non-distUDAF.
comment|// But we need to remember the sequence of udafs.
name|List
argument_list|<
name|Integer
argument_list|>
...
condition|)
block|{
comment|// udafAttr.udafParamsIndxInGBInfoDistExprs is not quite useful
comment|// because distinctUDAF can also include group by key as an argument.
for|for
control|(
name|int
name|j
...
condition|)
block|{
comment|// distinctUDAF includes group by key as an argument or reuses distinct keys.
name|rsUDAFParamColInfo
operator|=
name|rsColInfoLst
operator|.
