...
condition|)
block|{
comment|// We replace if the existing MV is not newer
name|Table
name|existingMVTable
init|=
name|extractTable
argument_list|(
name|existingMV
...
block|{
comment|// Simply replace if it still does not exist
name|refreshMaterializedView
argument_list|(
name|db
operator|.
...
name|info
argument_list|(
literal|"Materialized views registry has been refreshed"
argument_list|)
expr_stmt|;
block|}
else|else
...
name|error
argument_list|(
literal|"Problem connecting to the metastore when refreshing the view registry"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**    * Parses and creates a materialization.    */
specifier|public
name|RelOptMaterialization
...
return|;
block|}
comment|/**    * Adds a newly created materialized view to the cache.    */
specifier|public
name|void
...
block|}
block|}
comment|/**    * Update the materialized view in the registry (if existing materialized view matches).    */
specifier|public
name|void
name|refreshMaterializedView
parameter_list|(
...
argument_list|()
operator|+
literal|" dropped; it is not rewrite enabled"
argument_list|)
expr_stmt|;
return|return;
block|}
...
condition|)
block|{
comment|// If it was not existing, we just create it
return|return
name|newMaterialization
return|;
block|}
name|Table
name|existingMaterializedViewTable
init|=
name|extractTable
argument_list|(
name|existingMaterialization
...
condition|)
block|{
comment|// If the old version is the same, we replace it
return|return
name|newMaterialization
return|;
block|}
comment|// Otherwise, we return existing materialization
return|return
name|existingMaterialization
return|;
block|}
...
name|debug
argument_list|(
literal|"Materialized view refreshed: "
operator|+
name|materializedViewTable
operator|.
name|getFullyQualifiedName
...
block|}
block|}
comment|/**    * Removes the materialized view from the cache (based on table object equality), if exists.    */
specifier|public
name|void
name|dropMaterializedView
parameter_list|(
...
condition|)
block|{
comment|// Delete only if the create time for the input materialized view table and the table
comment|// in the map match. Otherwise, keep the one in the map.
name|dbMap
operator|.
...
block|}
block|}
comment|/**    * Removes the materialized view from the cache (based on qualified name), if exists.    */
specifier|public
name|void
name|dropMaterializedView
parameter_list|(
...
block|}
block|}
comment|/**    * Returns all the materialized views in the cache.    *    * @return the collection of materialized views, or the empty collection if none    */
name|List
argument_list|<
name|RelOptMaterialization
argument_list|>
...
return|;
block|}
comment|/**    * Returns the materialized views in the cache for the given database.    *    * @return the collection of materialized views, or the empty collection if none    */
name|RelOptMaterialization
name|getRewritingMaterializedView
parameter_list|(
name|String
