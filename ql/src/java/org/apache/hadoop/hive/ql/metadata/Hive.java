...
condition|)
block|{
comment|// If the thread local rawStore is already cleaned by current thread, then remove from rawStoreMap.
name|rawStoreMap
operator|.
name|remove
argument_list|(
...
block|{
comment|// If same thread is re-used, then need to cleanup the latest thread local instance of rawStore.
comment|// So, overwrite the old one if exists in rawStoreMap.
name|rawStoreMap
operator|.
name|put
argument_list|(
