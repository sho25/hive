...
name|destFile
decl_stmt|;
comment|/* Append the source filename to the destination directory */
if|if
condition|(
name|destFs
operator|.
...
expr_stmt|;
block|}
comment|// Destination should be replaced, so we delete it first
if|if
condition|(
name|destFs
operator|.
...
name|format
argument_list|(
literal|"File to replace could not be deleted: %s"
argument_list|,
name|destFile
argument_list|)
argument_list|)
...
return|;
block|}
comment|/**    * If moving across different FileSystems or differnent encryption zone, need to do a File copy instead of rename.    * TODO- consider if need to do this for different file authority.    */
specifier|static
specifier|protected
name|boolean
name|needToCopy
...
specifier|public
name|long
name|getPermanenFunctionsChangeVersion
parameter_list|()
throws|throws
name|HiveException
block|{
