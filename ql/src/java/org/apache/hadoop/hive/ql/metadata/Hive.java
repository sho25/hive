...
argument_list|()
decl_stmt|;
comment|// Static class to store thread local Hive object.
specifier|private
specifier|static
class|class
...
condition|)
block|{
comment|// Remove/close current thread-local Hive object before overwriting with new Hive object.
name|remove
argument_list|()
expr_stmt|;
name|super
...
condition|)
block|{
comment|// Close the metastore connections before removing it from thread local hiveDB.
name|currentHive
operator|.
name|close
argument_list|(
...
block|}
block|}
comment|/**    * GC is attempting to destroy the object.    * No one references this Hive anymore, so HMS connection from this Hive object can be closed.    * @throws Throwable    */
annotation|@
name|Override
specifier|protected
name|void
...
expr_stmt|;
block|}
comment|/**    * Marks if the given Hive object is allowed to close metastore connections.    * @param allowClose    */
specifier|public
name|void
name|setAllowClose
parameter_list|(
...
return|;
block|}
comment|/**    * Closes the connection to metastore for the calling thread if allow to close.    * @param forceClose - Override the isAllowClose flag to forcefully close the MS connections.    */
specifier|public
name|void
name|close
parameter_list|(
...
name|HiveException
argument_list|(
literal|"Cannot drop table since it is used by at least one materialized view definition. "
operator|+
literal|"Please drop any materialized view that uses the table before dropping it"
argument_list|,
name|e
argument_list|)
throw|;
...
block|}
block|}
comment|/**    * Returns metadata of the table    *    * @param tableName    *          the tableName object    * @return the table    * @exception HiveException    *              if there's an internal error or if the table doesn't exist    */
specifier|public
name|Table
name|getTable
parameter_list|(
...
return|;
block|}
comment|/**    * Get materialized views for the specified database that match the provided regex pattern.    * @param dbName    * @param pattern    * @return List of materialized view table objects    * @throws HiveException    */
specifier|public
name|List
argument_list|<
name|Table
...
block|}
block|}
comment|/**    * Get the materialized views that have been enabled for rewriting from the    * cache (registry). It will preprocess them to discard those that are    * outdated and augment those that need to be augmented, e.g., if incremental    * rewriting is enabled.    *    * @return the list of materialized views available for rewriting from the registry    * @throws HiveException    */
specifier|public
name|List
argument_list|<
name|RelOptMaterialization
argument_list|>
name|getPreprocessedMaterializedViewsFromRegistry
parameter_list|(
name|List
argument_list|<
name|String
...
return|;
block|}
comment|// Add to final result
return|return
name|filterAugmentMaterializedViews
argument_list|(
name|materializedViews
argument_list|,
name|tablesUsed
...
comment|// This would be costly since we are doing it for every materialized view
comment|// that is outdated, but it only happens for more than one materialized view
comment|// if rewriting with outdated materialized views is enabled (currently
...
block|}
block|}
comment|/**    * Validate that the materialized views retrieved from registry are still up-to-date.    * For those that are not, the method loads them from the metastore into the registry.    *    * @return true if they are up-to-date, otherwise false    * @throws HiveException    */
specifier|public
name|boolean
name|validateMaterializedViewsFromRegistry
parameter_list|(
name|List
argument_list|<
name|Table
argument_list|>
name|cachedMaterializedViewTables
parameter_list|,
name|List
argument_list|<
name|String
...
control|)
block|{
comment|// Retrieve the materialized view table from the metastore
specifier|final
name|Table
name|materializedViewTable
init|=
...
condition|)
block|{
comment|// This could happen if materialized view has been deleted or rewriting has been disabled.
comment|// We remove it from the registry and set result to false.
name|HiveMaterializedViewsRegistry
operator|.
name|get
argument_list|()
...
continue|continue;
block|}
comment|// If the cached materialized view was not outdated wrt the query snapshot,
comment|// then we know that the metastore version should be either the same or
comment|// more recent. If it is more recent, snapshot isolation will shield us
comment|// from the reading its contents after snapshot was acquired, but we will
comment|// update the registry so we have most recent version.
comment|// On the other hand, if the materialized view in the cache was outdated,
comment|// we can only use it if the version that was in the cache is the same one
comment|// that we can find in the metastore.
if|if
condition|(
name|outdated
condition|)
...
condition|)
block|{
comment|// We ignore and update the registry
name|HiveMaterializedViewsRegistry
operator|.
name|get
argument_list|()
...
condition|)
block|{
comment|// We ignore (as it did not meet the requirements), but we do not need to update it in the
comment|// registry, since it is up-to-date
name|result
operator|=
literal|false
expr_stmt|;
...
name|RelOptMaterialization
argument_list|>
name|getPreprocessedMaterializedViews
parameter_list|(
name|List
argument_list|<
name|String
...
return|;
block|}
comment|/**    * Get the target materialized view from the metastore. Although it may load the plan    * from the registry, it is guaranteed that it will always return an up-to-date version    * wrt metastore.    *    * @return the materialized view for rebuild    * @throws HiveException    */
specifier|public
name|RelOptMaterialization
name|getMaterializedViewForRebuild
parameter_list|(
name|String
name|dbName
parameter_list|,
...
comment|// or it is not up to date. We need to add it
if|if
condition|(
name|LOG
operator|.
...
block|}
block|}
comment|/**    * Utility method that returns whether a materialized view is outdated (true), not outdated    * (false), or it cannot be determined (null). The latest case may happen e.g. when the    * materialized view definition uses external tables.    */
specifier|public
specifier|static
name|Boolean
name|isOutdatedMaterializedView
...
comment|// to the metastore and thus get AlreadyExistsException, because some earlier query just
comment|// created it (race condition).
...
name|debug
argument_list|(
literal|"Caught AlreadyExistsException, trying to add partitions one by one."
argument_list|)
expr_stmt|;
assert|assert
name|partitions
...
name|StringBuilder
argument_list|(
literal|"Altering existing partitions "
argument_list|)
decl_stmt|;
name|partitions
operator|.
...
argument_list|)
decl_stmt|;
comment|// calculate full path spec for each valid partition path
name|validPartitions
operator|.
name|forEach
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// fetch all the partitions matching the part spec using the partition iterable
comment|// this way the maximum batch size configuration parameter is considered
...
argument_list|()
decl_stmt|;
comment|// Match valid partition path to partitions
while|while
condition|(
name|iterator
operator|.
...
name|setNameFormat
argument_list|(
literal|"load-dynamic-partitionsToAdd-%d"
argument_list|)
operator|.
name|build
argument_list|()
...
name|debug
argument_list|(
literal|"Number of partitionsToAdd to be added is "
operator|+
name|futures
operator|.
name|size
...
expr_stmt|;
block|}
comment|// add new partitions in batch
name|addPartitionsToMetastore
argument_list|(
name|partitionDetailsMap
operator|.
...
name|append
argument_list|(
literal|"Exception when loading partitionsToAdd with parameters "
argument_list|)
expr_stmt|;
name|logMsg
operator|.
...
parameter_list|)
block|{
comment|// this means no partition exists for the given dbName and tblName
comment|// key value pairs - thrift cannot handle null return values, hence
comment|// listPartitionNames() throws NoSuchObjectException to indicate null partitions
return|return
name|Lists
operator|.
name|newArrayList
...
parameter_list|)
block|{
comment|// this means no partition exists for the given partition spec
comment|// key value pairs - thrift cannot handle null return values, hence
comment|// listPartitionNames() throws NoSuchObjectException to indicate null partitions
return|return
name|Lists
...
condition|)
block|{
comment|// rename cannot overwrite non empty destination directory, so deleting the destination before renaming.
name|destFs
operator|.
name|delete
argument_list|(
...
name|info
argument_list|(
literal|"Deleting destination file"
operator|+
name|destFile
operator|.
name|toUri
...
name|HiveException
argument_list|(
literal|"Cannot create a plan in a different NS; was "
operator|+
name|resourcePlan
operator|.
name|getNs
...
name|HiveException
argument_list|(
literal|"Cannot modify a plan in a different NS; was "
operator|+
name|resourcePlan
operator|.
name|getNs
...
name|HiveException
argument_list|(
literal|"Cannot create a trigger in a different NS; was "
operator|+
name|trigger
operator|.
name|getNs
...
name|HiveException
argument_list|(
literal|"Cannot modify a trigger in a different NS; was "
operator|+
name|trigger
operator|.
name|getNs
...
name|HiveException
argument_list|(
literal|"Cannot create a pool in a different NS; was "
operator|+
name|pool
operator|.
name|getNs
...
name|HiveException
argument_list|(
literal|"Cannot modify a pool in a different NS; was "
operator|+
name|pool
operator|.
name|getNs
...
name|HiveException
argument_list|(
literal|"Cannot create a mapping in a different NS; was "
operator|+
name|mapping
operator|.
name|getNs
...
name|HiveException
argument_list|(
literal|"Cannot modify a mapping in a different NS; was "
operator|+
name|mapping
operator|.
name|getNs
