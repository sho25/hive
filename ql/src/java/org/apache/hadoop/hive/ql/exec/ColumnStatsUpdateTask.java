...
argument_list|()
decl_stmt|;
comment|// We do not support stats replication for a transactional table yet. If we are converting
comment|// a non-transactional table to a transactional table during replication, we might get
comment|// column statistics but we shouldn't update those.
if|if
condition|(
name|work
operator|.
...
name|debug
argument_list|(
literal|"Skipped updating column stats for table "
operator|+
name|TableName
operator|.
name|getDbTable
...
argument_list|)
operator|+
literal|" because it is converted to a transactional table during replication."
argument_list|)
expr_stmt|;
return|return
literal|0
