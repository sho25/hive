...
name|MetaException
block|{
comment|// If we are replicating the stats, we don't need to construct those again.
if|if
condition|(
name|work
operator|.
...
name|debug
argument_list|(
literal|"Got stats through replication for "
operator|+
name|colStats
operator|.
name|getStatsDesc
...
argument_list|)
decl_stmt|;
comment|// Set writeId and validWriteId list for replicated statistics. getColStats() will return
comment|// non-null value only during replication.
if|if
condition|(
name|work
operator|.
...
argument_list|()
decl_stmt|;
comment|// If it's a transactional table on source and target, we will get a valid writeId
comment|// associated with it. Otherwise it's a non-transactional table on source migrated to a
comment|// transactional table on target, we need to craft a valid writeId here.
if|if
condition|(
name|AcidUtils
operator|.
...
expr_stmt|;
block|}
comment|// We need a valid writeId list to update column statistics for a transactional table. We
comment|// do not have a valid writeId list which was used to update the column stats on the
comment|// source. But we know for sure that the writeId associated with the stats was valid then
comment|// (otherwise column stats update would have failed on the source). So use a valid
...
name|debug
argument_list|(
literal|"Reading timestamp value as seconds since epoch: {}"
argument_list|,
name|timestampStr
argument_list|)
expr_stmt|;
