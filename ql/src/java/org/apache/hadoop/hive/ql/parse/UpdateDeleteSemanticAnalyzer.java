...
decl_stmt|;
block|{
comment|/**        * add an alter table task to set transactional props        * do it after populating temp table so that it's written as non-transactional table but        * update props before export so that export archive metadata has these props.  This way when        * IMPORT is done for this archive and target table doesn't exist, it will be created as Acid.        */
name|alterTblDesc
operator|=
operator|new
name|AlterTableDesc
...
expr_stmt|;
block|{
comment|/**        * Now make a task to drop temp table        * {@link DDLSemanticAnalyzer#analyzeDropTable(ASTNode ast, TableType expectedType)        */
