...
specifier|private
name|int
name|targetShardsPerGranularity
init|=
literal|0
decl_stmt|;
specifier|private
name|int
name|granularityKeyPos
init|=
operator|-
literal|1
decl_stmt|;
...
name|sortNullOrder
decl_stmt|;
comment|//Order matters, assuming later that __time_granularity comes first then __druidPartitionKey
if|if
condition|(
name|targetShardsPerGranularity
operator|>
...
name|ENGLISH
argument_list|,
literal|"Unknown Druid Granularity [%s], Accepted values are [YEAR, MONTH, WEEK, DAY, HOUR, MINUTE, SECOND]"
argument_list|,
name|segmentGranularity
argument_list|)
argument_list|)
...
condition|)
block|{
comment|// add another partitioning key based on floor(1/rand) % targetShardsPerGranularity
specifier|final
name|ColumnInfo
name|partitionKeyCi
init|=
