...
specifier|protected
name|int
name|inputArgNumSplits
decl_stmt|;
specifier|protected
name|boolean
name|schemaSplitOnly
decl_stmt|;
annotation|@
name|Override
specifier|public
...
parameter_list|(
name|boolean
name|generateLightWeightSplits
parameter_list|)
throws|throws
name|HiveException
throws|,
...
name|schemaSplit
argument_list|,
literal|"schema split cannot be null"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|planSplit
argument_list|,
literal|"plan split cannot be null"
argument_list|)
expr_stmt|;
block|}
name|LOG
...
name|info
argument_list|(
literal|"Generated {} splits for query {}. orderByQuery: {} forceSingleSplit: {}"
argument_list|,
name|splits
operator|.
name|length
...
argument_list|)
expr_stmt|;
comment|// spark-llap always wraps query under a subquery, until that is removed from spark-llap
comment|// hive compiler is going to remove inner order by. disable that optimization until then.
name|HiveConf
operator|.
name|setBoolVar
argument_list|(
...
block|}
block|}
comment|// generateLightWeightSplits - if true then
comment|// 1) schema and planBytes[] in each LlapInputSplit are not populated
comment|// 2) schemaSplit(contains only schema) and planSplit(contains only planBytes[]) are populated in SplitResult
specifier|private
name|SplitResult
name|getSplits
parameter_list|(
...
decl_stmt|;
name|int
name|numGroupedSplitsGenerated
init|=
name|eventList
operator|.
name|size
