...
block|{
comment|// Some HMS background tasks skip processing tables being replicated into. Set the
comment|// replication property while creating the table so that they can identify such tables right
comment|// from the beginning. Set it to 0, which is lesser than any eventId ever created. This will
comment|// soon be overwritten by an actual value.
if|if
condition|(
name|desc
operator|.
...
argument_list|)
expr_stmt|;
comment|// We did not create the table before moving the data files for a non-partitioned table i.e
comment|// we used load file instead of load table (see SemanticAnalyzer#getFileSinkPlan() for
comment|// more details). Thus could not add a write notification required for a transactional
comment|// table. Do that here, after we have created the table. Since this is a newly created
comment|// table, listing all the files in the directory and listing only the ones corresponding to
comment|// the given id doesn't have much difference.
if|if
condition|(
operator|!
name|createdTable
