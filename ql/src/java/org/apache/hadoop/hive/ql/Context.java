...
argument_list|()
decl_stmt|;
comment|// keeps track of result cache dir for the query, later cleaned up by context cleanup
specifier|private
name|Path
name|fsResultCacheDirs
init|=
literal|null
decl_stmt|;
specifier|private
...
name|tokenRewriteStream
decl_stmt|;
comment|// Holds the qualified name to tokenRewriteStream for the views
comment|// referenced by the query. This is used to rewrite the view AST
comment|// with column masking and row filtering policies.
specifier|private
specifier|final
name|Map
argument_list|<
...
decl_stmt|;
comment|/**    * Unparsing is not available in {@link SemanticAnalyzer} by default - because of performance reasons.    * After enabling unparsing before analysis - a valid query unparse can be done.    */
specifier|private
name|boolean
name|enableUnparse
decl_stmt|;
...
condition|)
block|{
comment|/**        * Views get inlined in the logical plan but not in the AST        * {@link org.apache.hadoop.hive.ql.parse.SemanticAnalyzer#replaceViewReferenceWithDefinition(QB, Table, String, String)}        * Since here we only care to identify clauses representing Update/Delete which are not        * possible inside a view/subquery, we can immediately return the default {@link DestClausePrefix.INSERT}        */
return|return
name|DestClausePrefix
operator|.
name|INSERT
...
return|;
block|}
comment|//if it is an 'interesting' op but it's a select it must be a sub-query or a derived table
comment|//it doesn't require a special Acid code path - the reset of the code here is to ensure
comment|//the tree structure is what we expect
name|boolean
name|thisIsInASubquery
init|=
literal|false
decl_stmt|;
name|parentLoop
...
case|:
comment|//this is a real subquery (foo IN (select ...))
case|case
name|HiveParser
operator|.
name|TOK_SUBQUERY
case|:
comment|//this is a Derived Table Select * from (select a from ...))
comment|//strictly speaking SetOps should have a TOK_SUBQUERY parent so next 6 items are redundant
case|case
name|HiveParser
operator|.
name|TOK_UNIONALL
...
name|HiveParser
operator|.
name|TOK_EXCEPTDISTINCT
case|:
case|case
name|HiveParser
operator|.
name|TOK_INTERSECTALL
case|:
case|case
name|HiveParser
operator|.
name|TOK_INTERSECTDISTINCT
case|:
name|thisIsInASubquery
operator|=
literal|true
...
argument_list|)
operator|+
literal|"' to be in sub-query or set operation."
argument_list|)
throw|;
block|}
return|return
...
name|IllegalStateException
argument_list|(
literal|"Found a node w/o branch mapping: '"
operator|+
name|getMatchedText
argument_list|(
name|insert
...
block|}
block|}
comment|/**    * Will make SemanticAnalyzer.Phase1Ctx#dest in subtree rooted at 'tree' use 'prefix'.  This to    * handle multi-insert stmt that represents Merge stmt and has insert branches representing    * update/delete/insert.    * @param pos ordinal index of specific TOK_INSERT as child of TOK_QUERY    * @return previous prefix for 'tree' or null    */
specifier|public
name|DestClausePrefix
...
parameter_list|)
block|{
comment|// This method creates a deep copy of context, but the copy is partial,
comment|// hence it needs to be used carefully. In particular, following objects
comment|// are ignored:
comment|// opContext, pathToCS, cboInfo, cboSucceeded, tokenRewriteStream, viewsTokenRewriteStreams,
comment|// rewrittenStatementContexts, cteTables, loadTableOutputMap, planMapper, insertBranchToNamePrefix
name|this
operator|.
name|isHDFSCleanup
operator|=
...
name|debug
argument_list|(
literal|"Deleting result cache dir: {}"
argument_list|,
name|p
argument_list|)
expr_stmt|;
...
name|warn
argument_list|(
literal|"Error Removing result cache dir: "
operator|+
name|StringUtils
operator|.
...
condition|)
block|{
comment|// delete only the paths which aren't result cache dir path
comment|// because that will be taken care by removeResultCacheDir
name|FileSystem
name|fs
init|=
name|p
...
specifier|public
name|TokenRewriteStream
name|getViewTokenRewriteStream
parameter_list|(
name|String
name|viewFullyQualifiedName
parameter_list|)
...
block|{
comment|// hierarchical; add def stats also here
return|return
name|EmptyStatsSource
operator|.
name|INSTANCE
