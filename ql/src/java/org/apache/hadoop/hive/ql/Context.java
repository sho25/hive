...
name|hiveTxnManager
decl_stmt|;
comment|// Used to track what type of acid operation (insert, update, or delete) we are doing.  Useful
comment|// since we want to change where bucket columns are accessed in some operators and
comment|// optimizations when doing updates and deletes.
specifier|private
name|AcidUtils
operator|.
name|Operation
