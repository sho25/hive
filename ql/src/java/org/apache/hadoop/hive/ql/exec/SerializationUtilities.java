...
block|}
block|}
comment|/**    * Supports sublists created via {@link ArrayList#subList(int, int)} since java7 and {@link LinkedList#subList(int, int)} since java9 (openjdk).    * This is from kryo-serializers package.    */
specifier|private
specifier|static
class|class
name|ArrayListSubListSerializer
...
name|forName
argument_list|(
literal|"java.util.ArrayList$SubList"
argument_list|)
decl_stmt|;
name|_parentField
operator|=
...
try|try
block|{
comment|// up to jdk8 (which also has an "offset" field (we don't need) - therefore we check "parentOffset" first
return|return
name|clazz
operator|.
name|getDeclaredField
argument_list|(
literal|"parentOffset"
argument_list|)
return|;
block|}
catch|catch
...
parameter_list|)
block|{
comment|// jdk9+ only has "offset" which is the parent offset
return|return
name|clazz
operator|.
name|getDeclaredField
...
block|}
block|}
comment|/**    * We use a custom {@link com.esotericsoftware.kryo.Serializer} for {@link MapWork} objects in    * order to invoke any string interning code present in the "setter" methods. The fields in {@link    * MapWork} often store paths that contain duplicate strings, so interning them can decrease    * memory significantly.    */
specifier|private
specifier|static
class|class
name|MapWorkSerializer
...
argument_list|)
decl_stmt|;
comment|// The set methods in MapWork intern the any duplicate strings which is why we call them
comment|// during de-serialization
name|mapWork
operator|.
...
block|}
block|}
comment|/**    * We use a custom {@link com.esotericsoftware.kryo.Serializer} for {@link PartitionDesc} objects    * in order to invoke any string interning code present in the "setter" methods. {@link    * PartitionDesc} objects are usually stored by {@link MapWork} objects and contain duplicate info    * like input format class names, partition specs, etc.    */
specifier|private
specifier|static
class|class
name|PartitionDescSerializer
...
argument_list|)
decl_stmt|;
comment|// The set methods in PartitionDesc intern the any duplicate strings which is why we call them
comment|// during de-serialization
name|partitionDesc
operator|.
