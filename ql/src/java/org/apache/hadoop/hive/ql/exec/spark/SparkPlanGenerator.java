...
control|)
block|{
comment|// Run the SparkDynamicPartitionPruner, we run this here instead of inside the
comment|// InputFormat so that we don't have to run pruning when creating a Record Reader
name|runDynamicPartitionPruner
argument_list|(
name|work
argument_list|)
expr_stmt|;
