...
control|)
block|{
comment|// if the fetch is not completed for the big table
if|if
condition|(
name|i
operator|==
...
condition|)
block|{
comment|// if we have never fetched, we need to fetch before we can do the join
if|if
condition|(
name|firstFetchHappened
operator|==
...
condition|)
block|{
comment|// we need to fetch all the needed ones at least once to ensure bootstrapping
if|if
condition|(
name|i
operator|==
...
expr_stmt|;
block|}
comment|// This is a bootstrap. The joinOneGroup automatically fetches the next rows.
name|fetchNextGroup
argument_list|(
operator|(
name|byte
