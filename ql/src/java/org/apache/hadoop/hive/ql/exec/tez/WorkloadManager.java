...
comment|//       sessions, so the pool itself could internally track the ses  sions it gave out, since
...
argument_list|()
expr_stmt|;
comment|// 4. Delete unneeded directories that were replaced by other ones via reopen.
for|for
control|(
specifier|final
name|Path
...
name|error
argument_list|(
literal|"Failed to delete an old path; ignoring "
operator|+
name|ex
operator|.
name|getMessage
...
block|}
block|}
comment|// 13. To record move events, we need to cluster fraction updates that happens at step 11.
for|for
control|(
name|Map
operator|.
...
argument_list|()
decl_stmt|;
comment|// Do not update metrics, we'd immediately add the session back if we are able to remove.
name|RemoveSessionResult
name|rr
init|=
name|checkAndRemoveSessionFromItsPool
...
argument_list|)
expr_stmt|;
comment|// Do not update metrics - see above.
name|sw
operator|.
name|start
argument_list|()
...
condition|)
block|{
comment|// NULL plan means WM is disabled via a command; it could still be reenabled.
name|LOG
operator|.
name|info
argument_list|(
literal|"Disabling workload management because the resource plan has been removed"
argument_list|)
expr_stmt|;
name|this
operator|.
...
block|}
block|}
comment|// TODO: in the current impl, triggers are added to RP. For tez, no pool triggers (mapping between trigger name and
comment|// pool name) will exist which means all triggers applies to tez. For LLAP, pool triggers has to exist for attaching
comment|// triggers to specific pools.
comment|// For usability,
comment|// Provide a way for triggers sharing/inheritance possibly with following modes
comment|// ONLY - only to pool
comment|// INHERIT - child pools inherit from parent
comment|// GLOBAL - all pools inherit
if|if
condition|(
name|plan
operator|!=
...
argument_list|)
expr_stmt|;
comment|// Do not update metrics - we didn't update on removal.
name|pool
operator|.
name|sessions
operator|.
...
condition|)
block|{
comment|// Update max executors now that cluster info is definitely available.
name|pool
operator|.
name|metrics
operator|.
...
name|error
argument_list|(
literal|"Session {} was not added to pool {}"
argument_list|,
name|session
argument_list|,
name|destPoolName
...
name|info
argument_list|(
literal|"Queued session for kill: {}"
argument_list|,
name|killQueryContext
operator|.
name|session
...
name|info
argument_list|(
literal|"Subscribed to counters: {}"
argument_list|,
name|wmContext
operator|.
...
name|fullName
expr_stmt|;
comment|// TODO: this actually calls the metrics system and getMetrics - that may be expensive.
comment|//       For now it looks like it should be ok to do on WM thread.
name|this
operator|.
name|metrics
operator|=
...
parameter_list|)
block|{
comment|// This should be validated at change time; let's fall back to a default here.
name|LOG
operator|.
name|error
argument_list|(
literal|"Unknown scheduling policy "
operator|+
name|schedulingPolicy
operator|+
literal|"; using FAIR"
...
control|)
block|{
name|updateSessionAllocationWithEvent
argument_list|(
name|session
argument_list|,
name|allocation
...
name|error
argument_list|(
literal|"Failed to retry; propagating original error. The new error is "
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// Don't pass in the pool set - not thread safe; if the user is trying to force us to
comment|// use a non-existent pool, we want to fail anyway. We will fail later during get.
name|String
name|mappedPool
init|=
name|mapping
...
name|info
argument_list|(
literal|"Mapping input: {} mapped to pool: {}"
argument_list|,
name|input
argument_list|,
name|mappedPool
...
argument_list|)
expr_stmt|;
comment|// The way this works is, a session in WM pool will move back to tez AM pool on a kill and will get
comment|// reassigned back to WM pool on GetRequest based on user pool mapping. Only if we remove the session from active
comment|// sessions list of its WM pool will the queue'd GetRequest be processed
name|String
name|poolName
init|=
name|toKill
