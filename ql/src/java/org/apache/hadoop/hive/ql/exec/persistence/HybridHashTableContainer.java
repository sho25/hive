...
block|{
comment|// To avoid having a huge BloomFilter we need to scale up False Positive Probability
name|double
name|fpp
init|=
name|calcFPP
...
literal|1
operator|:
literal|"Too many keys! BloomFilter False Positive Probability is 1!"
assert|;
if|if
condition|(
name|fpp
...
name|warn
argument_list|(
literal|"BloomFilter FPP is greater than 0.5!"
argument_list|)
expr_stmt|;
block|}
name|LOG
...
name|info
argument_list|(
literal|"BloomFilter is using FPP: "
operator|+
name|fpp
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**    * Calculate the proper False Positive Probability so that the BloomFilter won't grow too big    * @param keyCount number of keys    * @return FPP    */
specifier|private
name|double
name|calcFPP
parameter_list|(
...
literal|0.05
decl_stmt|;
comment|// Calculation below is consistent with BloomFilter.optimalNumOfBits().
comment|// Also, we are capping the BloomFilter size below 100 MB (800000000/8)
while|while
condition|(
operator|(
operator|-
...
name|SerDeException
block|{
comment|// Save the key and value contexts in case the MapJoinOperator needs them when creating
comment|// a stand alone spill MapJoinBytesTableContainer.
name|keyContext
operator|=
name|keyCtx
expr_stmt|;
...
name|valCtx
expr_stmt|;
comment|// Save the key serde for possible use by NonMatchedSmallTableIteratorImpl.
name|keySerde
operator|=
name|keyCtx
operator|.
