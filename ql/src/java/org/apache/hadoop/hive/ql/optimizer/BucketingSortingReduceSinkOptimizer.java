...
return|;
block|}
comment|// Return true if the partition is bucketed/sorted by the specified positions
...
return|;
block|}
comment|// Store the bucket path to bucket number mapping in the table scan operator.
...
comment|// also output the file corresponding to bucket 2 of the output.
name|storeBucketPathMapping
argument_list|(
name|tsOp
argument_list|,
...
comment|// Use BucketizedHiveInputFormat so that one mapper processes exactly one file
specifier|private
name|void
name|removeReduceSink
parameter_list|(
...
return|;
block|}
comment|// The output columns for the destination table should match with the join keys
comment|// This is to handle queries of the form:
comment|// insert overwrite table T3
comment|// select T1.key, T1.key2, UDF(T1.value, T2.value)
comment|// from T1 join T2 on T1.key = T2.key and T1.key2 = T2.key2
comment|// where T1, T2 and T3 are bucketized/sorted on key and key2
comment|// Assuming T1 is the table on which the mapper is run, the following is true:
comment|// . The number of buckets for T1 and T3 should be same
comment|// . The bucketing/sorting columns for T1, T2 and T3 should be same
comment|// . The sort order of T1 should match with the sort order for T3.
comment|// . If T1 is partitioned, only a single partition of T1 can be selected.
comment|// . The select list should contain with (T1.key, T1.key2) or (T2.key, T2.key2)
comment|// . After the join, only selects and filters are allowed.
specifier|private
name|boolean
name|validateSMBJoinKeys
parameter_list|(
name|SMBJoinDesc
name|smbJoinDesc
parameter_list|,
...
parameter_list|)
block|{
comment|// The sort-merge join creates the output sorted and bucketized by the same columns.
comment|// This can be relaxed in the future if there is a requirement.
if|if
condition|(
operator|!
name|sourceTableBucketCols
...
return|;
block|}
comment|// Get the total number of columns selected, and for each output column, store the
comment|// base table it points to. For
comment|// insert overwrite table T3
comment|// select T1.key, T1.key2, UDF(T1.value, T2.value)
comment|// from T1 join T2 on T1.key = T2.key and T1.key2 = T2.key2
comment|// the following arrays are created
comment|// [0, 0, 0, 1] --> [T1, T1, T1, T2] (table mapping)
comment|// [0, 1, 2, 0] --> [T1.0, T1.1, T1.2, T2.0] (table columns mapping)
name|Byte
index|[]
name|tagOrder
init|=
...
block|}
block|}
comment|// All output columns used for bucketing/sorting of the destination table should
comment|// belong to the same input table
comment|//   insert overwrite table T3
comment|//   select T1.key, T2.key2, UDF(T1.value, T2.value)
comment|//   from T1 join T2 on T1.key = T2.key and T1.key2 = T2.key2
comment|// is not optimized, whereas the insert is optimized if the select list is either changed to
comment|// (T1.key, T1.key2, UDF(T1.value, T2.value)) or (T2.key, T2.key2, UDF(T1.value, T2.value))
comment|// Get the input table and make sure the keys match
name|List
argument_list|<
name|String
argument_list|>
...
condition|)
block|{
comment|// Bucketing and sorting keys should exactly match
if|if
condition|(
operator|!
operator|(
...
argument_list|()
decl_stmt|;
comment|// join keys dont match the bucketing keys
name|List
argument_list|<
name|ExprNodeDesc
argument_list|>
...
expr_stmt|;
block|}
comment|// since it is a sort-merge join, only follow the big table
name|op
operator|=
name|op
operator|.
...
argument_list|()
decl_stmt|;
comment|// Iterate backwards, from the destination table to the top of the tree
comment|// Based on the output column names, get the new columns.
if|if
condition|(
operator|!
name|useBucketSortPositions
