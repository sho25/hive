...
throw|;
block|}
comment|// If the number of joins< number of input tables-1, this is not a star join.
if|if
condition|(
name|joinPredInfo
operator|.
...
return|;
block|}
comment|// Validate that the multi-join is a valid star join before returning it.
for|for
control|(
name|int
name|i
...
block|{
comment|// If we join on different keys on different tables, we can no longer apply
comment|// multi-join conversion as this is no longer a valid star join.
comment|// Bail out if this is the case.
if|if
condition|(
operator|!
name|joinKeys
