...
block|}
block|}
comment|/**    * Add default order spec if there is no order and validate order spec for valued based    * windowing since only one sort key is allowed.    * @param wFn Window function spec    * @throws SemanticException    */
specifier|private
name|void
name|setAndValidateOrderSpec
parameter_list|(
...
name|SemanticException
argument_list|(
literal|"Range based Window Frame needs to specify ORDER BY clause"
argument_list|)
throw|;
block|}
name|boolean
...
decl_stmt|;
name|boolean
name|defaultPrecedingFollowing
init|=
name|start
operator|.
name|getDirection
...
name|SemanticException
argument_list|(
literal|"Range value based Window Frame can have only 1 Sort Key"
argument_list|)
throw|;
block|}
block|}
...
name|format
argument_list|(
literal|"window(type=%s, start=%s, end=%s)"
argument_list|,
name|this
operator|.
name|windowType
...
block|}
empty_stmt|;
comment|// The types for ROWS BETWEEN or RANGE BETWEEN windowing spec
specifier|public
specifier|static
enum|enum
name|WindowType
