...
condition|)
block|{
comment|// if orc table, restrict changing column types. Only integer type promotion is supported.
comment|// smallint -> int -> bigint
if|if
condition|(
name|isOrc
operator|&&
operator|!
name|isSupportedTypeChange
argument_list|(
name|col
operator|.
name|getType
...
argument_list|)
decl_stmt|;
comment|// adding columns and limited integer type promotion is supported for ORC
if|if
condition|(
name|isOrc
condition|)
...
return|;
block|}
comment|// don't change the order of enums as ordinal values are used to check for valid type promotions
enum|enum
name|PromotableTypes
block|{
name|SMALLINT
block|,
name|INT
...
block|}
block|}
comment|// for ORC, only supported type promotions are smallint -> int -> bigint. No other
comment|// type promotions are supported at this point
specifier|private
name|boolean
name|isSupportedTypeChange
parameter_list|(
