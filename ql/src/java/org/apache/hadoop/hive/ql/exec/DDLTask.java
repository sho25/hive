...
condition|)
block|{
comment|// Don't delete the underlying data when dropping a table link.
name|db
operator|.
name|dropTable
argument_list|(
...
argument_list|)
decl_stmt|;
comment|// trying to drop partitions inside a har, disallow it.
if|if
condition|(
name|partAchiveLevel
operator|<
...
name|HiveException
argument_list|(
literal|"Cannot drop a subset of partitions in an archive, partition "
operator|+
name|p
operator|.
name|getName
...
return|;
block|}
comment|/**    * Create a table link to an existing table.    *    * @param db  The database in question.    * @param crtTblLinkDesc    * @return Returns 0 when execution succeeds and above 0 if it fails.    */
specifier|private
name|int
name|createTableLink
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// set owner and creation time
name|int
name|rc
init|=
name|setGenericTableAttributes
...
argument_list|()
decl_stmt|;
comment|// Most of the metadata of the target table is retained. However we don't want to
comment|// copy the Parameters of the target table because they include several things
comment|// that would be wrong to copy like numPartitions, transient_lastDdlTime, etc.
name|params
operator|.
name|clear
argument_list|()
