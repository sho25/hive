...
argument_list|)
expr_stmt|;
comment|// If the add partition was created with IF NOT EXISTS, then we should
comment|// not throw an error if the specified part does exist.
name|Partition
name|checkPart
init|=
name|db
...
block|}
block|}
comment|// Returns only the path component of the URI
specifier|private
name|String
name|getArchiveDirOnly
parameter_list|(
name|Path
name|parentDir
parameter_list|,
...
condition|)
block|{
comment|// If there were a failure right after the metadata was updated in an
comment|// archiving operation, it's possible that the original, unarchived files
comment|// weren't deleted.
name|Path
name|originalDir
init|=
operator|new
...
decl_stmt|;
name|Path
name|leftOverIntermediateOriginal
init|=
operator|new
name|Path
argument_list|(
...
name|leftOverIntermediateOriginal
operator|+
literal|" left over from a previous archiving operation"
argument_list|)
expr_stmt|;
name|deleteDir
argument_list|(
...
name|HiveException
argument_list|(
literal|"Specified partition is already archived"
argument_list|)
throw|;
block|}
name|Path
...
argument_list|)
decl_stmt|;
comment|// dirInArchive is the directory within the archive that has all the files
comment|// for this partition. With older versions of Hadoop, archiving a
comment|// a directory would produce the same directory structure
comment|// in the archive. So if you created myArchive.har of /tmp/myDir, the
comment|// files in /tmp/myDir would be located under myArchive.har/tmp/myDir/*
comment|// In this case, dirInArchive should be tmp/myDir
comment|// With newer versions of Hadoop, the parent directory could be specified.
comment|// Assuming the parent directory was set to /tmp/myDir when creating the
comment|// archive, the files can be found under myArchive.har/*
comment|// In this case, dirInArchive should be empty
name|String
name|dirInArchive
init|=
literal|""
...
name|HiveException
argument_list|(
literal|"ARCHIVE is for partitions only"
argument_list|)
throw|;
block|}
name|Map
...
literal|" left over "
operator|+
literal|"from a previous unarchiving operation"
argument_list|)
expr_stmt|;
name|deleteDir
argument_list|(
...
name|HiveException
argument_list|(
literal|"Specified partition is not archived"
argument_list|)
throw|;
block|}
name|Path
...
argument_list|)
expr_stmt|;
comment|// Verify that there are no files in the tmp dir, because if there are, it
comment|// would be copied to the partition
name|FileStatus
index|[]
name|filesInTmpDir
init|=
...
name|HiveException
argument_list|(
literal|"Location should refer to a HAR"
argument_list|)
throw|;
block|}
comment|// Clarification of terms:
comment|// - The originalLocation directory represents the original directory of the
comment|//   partition's files. They now contain an archived version of those files
comment|//   eg. hdfs:/warehouse/myTable/ds=1/
comment|// - The source directory is the directory containing all the files that
comment|//   should be in the partition. e.g. har:/warehouse/myTable/ds=1/myTable.har/
...
comment|// 4. Rename intermediate-extracted to the original partition dir
comment|// 5. Change the metadata
comment|// 6. Delete the archived partition files in intermediate-archive
if|if
condition|(
operator|!
name|pathExists
...
name|HiveException
argument_list|(
literal|"Error while copying files from archive"
argument_list|)
throw|;
block|}
name|console
...
name|HiveException
argument_list|(
literal|"Cannot alter a view with ALTER TABLE"
argument_list|)
throw|;
block|}
switch|switch
...
name|HiveException
argument_list|(
literal|"Cannot alter a base table with ALTER VIEW"
argument_list|)
throw|;
block|}
block|}
...
name|HiveException
argument_list|(
literal|"Cannot use ALTER TABLE on a non-native table"
argument_list|)
throw|;
block|}
block|}
...
return|;
block|}
comment|/**    * Write a list of the available databases to a file.    *    * @param showDatabases    *          These are the databases we're interested in.    * @return Returns 0 when execution succeeds and above 0 if it fails.    * @throws HiveException    *           Throws this exception if an unexpected error occurs.    */
specifier|private
name|int
name|showDatabases
parameter_list|(
...
block|{
comment|// This is actually an ALTER TABLE DROP PARTITION
if|if
condition|(
name|tbl
operator|!=
...
expr_stmt|;
block|}
comment|// get all partition names of the table
name|List
argument_list|<
name|String
argument_list|>
...
argument_list|)
decl_stmt|;
comment|//Build a hashtable of a "sorted partition name" to the original partition spec
comment|//We need a sorted partition name so we can compare partitions by their names
name|Hashtable
argument_list|<
name|String
argument_list|,
...
parameter_list|)
block|{
comment|//If the partition spec to remove is empty or null, just move on gracefully
name|LOG
operator|.
name|warn
argument_list|(
...
literal|null
decl_stmt|;
comment|//If the partition we wish to delete exists in our hashtable of all partition names,
comment|//get the partition and add it to our list of partitions to delete
if|if
condition|(
operator|(
name|origPart
...
return|;
block|}
comment|/**    * Update last_modified_by and last_modified_time parameters in parameter map.    *    * @param params    *          Parameters.    * @param user    *          user that is doing the updating.    */
specifier|private
name|boolean
name|updateModifiedParameters
parameter_list|(
...
block|}
block|}
comment|/**    * Create a Database    * @param db    * @param crtDb    * @return Always returns 0    * @throws HiveException    * @throws AlreadyExistsException    */
specifier|private
name|int
name|createDatabase
parameter_list|(
...
return|;
block|}
comment|/**    * Drop a Database    * @param db    * @param dropDb    * @return Always returns 0    * @throws HiveException    * @throws NoSuchObjectException    */
specifier|private
name|int
name|dropDatabase
parameter_list|(
...
name|rc
init|=
name|setGenericTableAttributes
argument_list|(
name|tbl
argument_list|)
decl_stmt|;
...
expr_stmt|;
block|}
comment|// we should reset table specific parameters including (stats, lastDDLTime etc.)
name|Map
argument_list|<
name|String
argument_list|,
