...
specifier|final
name|HiveAuthorizationTranslator
name|defaultAuthorizationTranslator
init|=
operator|new
name|DefaultHiveAuthorizationTranslator
argument_list|()
...
argument_list|(
literal|"Indexes unsupported for Tez execution engine"
argument_list|)
throw|;
block|}
name|db
...
name|HiveException
block|{
comment|// get the tables/views for the desired pattern - populate the output stream
name|List
argument_list|<
name|String
argument_list|>
...
block|}
block|}
comment|/**    * Write the description of a table to a file.    *    * @param db    *          The database in question.    * @param descTbl    *          This is the table we're interested in.    * @return Returns 0 when execution succeeds and above 0 if it fails.    * @throws HiveException    *           Throws this exception if an unexpected error occurs.    * @throws MetaException    */
specifier|private
name|int
name|describeTable
parameter_list|(
...
name|HiveException
block|{
comment|// Note: doesn't take LB into account; that is not presently supported here (throws above).
try|try
block|{
name|FileSystem
name|fs
...
condition|)
block|{
comment|// Assume no concurrent active writes - we rely on locks here. We could check and fail.
name|ensureDelete
argument_list|(
name|fs
argument_list|,
name|childPath
argument_list|,
literal|"an uncommitted directory"
argument_list|)
expr_stmt|;
block|}
else|else
...
name|path
operator|+
literal|"; cannot remove MM setting from the table"
decl_stmt|;
name|LOG
operator|.
name|error
...
argument_list|)
decl_stmt|;
comment|// TODO# this is hacky and will be gone with ACID. The problem is getting the write ID above
comment|//       modifies the table, but the table object above is preserved and modified without
comment|//       getting this change, so saving it will overwrite write ID. Ideally, when we save
comment|//       only specific fields, and not overwrite write ID every time we alter table.
comment|//       There's probably some way in DN to achieve that, but for now let's just update the
comment|//       original object here. This is safe due to DDL lock and the fact that converting
comment|//       the table to MM here from non-MM should mean no concurrent write ID updates.
name|tbl
operator|.
...
condition|)
block|{
comment|// TODO# this would be retrieved via ACID before the query runs; for now we rely on it
comment|//       being zero at start; we can't create a write ID before we create the table here.
name|long
name|initialWriteId
init|=
name|db
...
expr_stmt|;
block|}
comment|// Retain Parquet INT96 write zone property to keep Parquet timezone bugfixes.
if|if
condition|(
name|params
operator|.
...
block|{
comment|// This is a replace, so we need an exclusive lock
name|addIfAbsentByName
argument_list|(
operator|new
name|WriteEntity
...
expr_stmt|;
block|}
comment|// Short circuit the checks that the input format is valid, this is configured for all
comment|// materialized views and doesn't change so we don't need to check it constantly.
name|tbl
operator|.
name|getSd
argument_list|()
