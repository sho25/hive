...
literal|20089
decl_stmt|;
comment|// Range lower limit for timestamp type when not defined (seconds, heuristic): '1999-01-01 00:00:00'
specifier|private
specifier|static
specifier|final
name|long
name|TIMESTAMP_RANGE_LOWER_LIMIT
init|=
literal|915148800L
decl_stmt|;
comment|// Range upper limit for timestamp type when not defined (seconds, heuristic): '2024-12-31 23:59:59'
specifier|private
specifier|static
specifier|final
name|long
name|TIMESTAMP_RANGE_UPPER_LIMIT
init|=
literal|1735689599L
decl_stmt|;
...
return|;
block|}
comment|/**    * Returns number of rows if it exists. Otherwise it estimates number of rows    * based on estimated data size for both partition and non-partitioned table    * RelOptHiveTable's getRowCount uses this.    */
specifier|public
specifier|static
name|long
name|getNumRows
...
argument_list|()
decl_stmt|;
comment|// FIXME: this point will be lost after the factory; check that it's really a warning....cleanup/etc
if|if
condition|(
name|nr
operator|<=
...
expr_stmt|;
block|}
comment|//      long ds = shouldEstimateStats? getDataSize(conf, table): getRawDataSize(table);
name|basicStatsFactory
operator|.
name|addEnhancer
argument_list|(
...
argument_list|)
decl_stmt|;
comment|//      long nr = getNumRows(conf, schema, neededColumns, table, ds);
name|long
name|ds
init|=
name|basicStats
...
name|debug
argument_list|(
literal|"Stats for column {} in table {} could not be retrieved from cache"
argument_list|,
name|colName
argument_list|,
name|table
...
name|debug
argument_list|(
literal|"Stats for column {} in table {} retrieved from cache"
argument_list|,
name|colName
argument_list|,
name|table
...
argument_list|)
expr_stmt|;
comment|// epoch, seconds since epoch
name|cs
operator|.
name|setRange
argument_list|(
...
name|debug
argument_list|(
literal|"Interpreting constant ("
operator|+
name|constant
operator|+
literal|")  resulted in exception"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**    * Are the basic stats for the table up-to-date for query planning.    * Can run additional checks compared to the version in StatsSetupConst.    */
specifier|public
specifier|static
name|boolean
name|areBasicStatsUptoDateForQueryAnswering
parameter_list|(
name|Table
name|table
parameter_list|,
...
parameter_list|)
block|{
comment|// HIVE-19332: external tables should not be considered to have up-to-date stats.
if|if
condition|(
name|MetaStoreUtils
operator|.
...
return|;
block|}
comment|/**    * Are the column stats for the table up-to-date for query planning.    * Can run additional checks compared to the version in StatsSetupConst.    */
specifier|public
specifier|static
name|boolean
name|areColumnStatsUptoDateForQueryAnswering
parameter_list|(
name|Table
name|table
parameter_list|,
...
name|ndvValues
init|=
name|extractNDVGroupingColumns
argument_list|(
name|colStats
argument_list|,
name|parentStats
...
condition|)
block|{
comment|// No grouping columns, one row
return|return
literal|1L
return|;
block|}
