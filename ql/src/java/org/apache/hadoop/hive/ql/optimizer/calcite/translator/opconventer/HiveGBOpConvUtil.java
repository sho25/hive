...
argument_list|(
literal|"HiveGBOpConvUtil should not be instantiated!"
argument_list|)
throw|;
block|}
specifier|private
...
argument_list|()
decl_stmt|;
comment|// We store the position of the argument for the function in the input.
block|}
empty_stmt|;
specifier|private
specifier|static
...
control|)
block|{
comment|// NOTE: distinct expr can be part of of GB key
if|if
condition|(
name|udafAttrs
operator|.
...
condition|)
block|{
comment|// This is the case where distinct cols are part of GB Keys in which case
comment|// we still need to add it to out put col names
name|outputKeyColumnNames
operator|.
name|add
argument_list|(
...
control|)
block|{
comment|//gbInfo already has ExprNode for gbkeys
name|reduceKeys
operator|.
name|add
argument_list|(
...
expr_stmt|;
block|}
comment|// Note: GROUPING SETS are not allowed with map side aggregation set to false so we don't have to worry about it
name|int
name|keyLength
init|=
name|reduceKeys
...
argument_list|)
expr_stmt|;
comment|//this part of reduceKeys is later used to create column names strictly for non-distinct aggregates
comment|// with parameters same as distinct keys which expects _col0 at the end. So we always append
comment|// _col0 at the end instead of _col<i>
name|outputColName
operator|=
name|SemanticAnalyzer
operator|.
...
argument_list|()
decl_stmt|;
comment|// the positions in rsColInfoLst are as follows
comment|// --grpkey--,--distkey--,--values--
comment|// but distUDAF may be before/after some non-distUDAF,
comment|// i.e., their positions can be mixed.
comment|// so for all UDAF we first check to see if it is groupby key, if not is it distinct key
comment|// if not it should be value
name|Map
argument_list|<
name|Integer
argument_list|,
...
parameter_list|)
block|{
comment|//first see if it is gbkeys
name|int
name|gbKeyIndex
init|=
name|ExprNodeDescUtils
...
condition|)
block|{
comment|//then check if it is distinct key
name|int
name|distinctKeyIndex
init|=
name|ExprNodeDescUtils
operator|.
name|indexOf
...
condition|)
block|{
comment|// lastly it should be in deDupedNonDistIrefs
name|int
name|deDupValIndex
init|=
name|ExprNodeDescUtils
...
block|{
comment|//aggExpr is part of distinct key
return|return
name|gbInfo
operator|.
name|gbKeys
