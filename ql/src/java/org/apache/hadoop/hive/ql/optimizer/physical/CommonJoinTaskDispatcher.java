...
block|}
block|}
comment|/**    * Copy reducer configuration if the childTask also has a reducer.    *    * @param task    * @param childTask    */
specifier|private
name|void
name|copyReducerConf
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// Make sure the key configuration is correct, clear and regenerate.
name|work
operator|.
...
specifier|private
name|void
name|mergeMapJoinTaskWithMapReduceTask
parameter_list|(
name|MapRedTask
name|mapJoinTask
parameter_list|,
...
condition|)
block|{
comment|// Not a MR job, nothing to merge.
return|return;
block|}
comment|// Can this be merged
name|Map
...
return|return;
block|}
comment|// Locate leaf operator of the map-join task. Start by initializing leaf
comment|// operator to be root operator.
name|Operator
argument_list|<
name|?
extends|extends
...
condition|)
block|{
comment|// Sanity check, shouldn't happen.
return|return;
block|}
name|FileSinkOperator
...
return|return;
block|}
comment|// remove the unnecessary TableScan
if|if
condition|(
name|childAliasOp
operator|instanceof
...
block|}
block|}
comment|// Fill up stuff in local work
if|if
condition|(
name|mapJoinLocalWork
operator|!=
...
condition|)
block|{
comment|// Merging two map-join tasks
name|mergeMapJoinTaskWithChildMapJoinTask
argument_list|(
name|newTask
argument_list|,
...
expr_stmt|;
block|}
comment|// Converted the join operator into a map-join. Now see if it can
comment|// be merged into the following map-reduce job.
name|boolean
name|convertToSingleJob
init|=
name|HiveConf
...
condition|)
block|{
comment|// Try merging a map-join task with a mapreduce job to have a
comment|// single job.
name|mergeMapJoinTaskWithMapReduceTask
argument_list|(
name|newTask
