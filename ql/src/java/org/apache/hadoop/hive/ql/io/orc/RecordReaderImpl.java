...
condition|)
continue|continue;
comment|// TODO#: this code is relevant
name|int
name|posn
init|=
name|getIndexPosition
...
argument_list|)
decl_stmt|;
comment|// TODO## GET FROM METADATA? same for indexes, remove set... method
name|List
argument_list|<
name|OrcProto
operator|.
...
comment|// TODO# We could build RG x all cols batches here cheaper and avoid building them on higher
comment|//       level (except for HL cache data that higher level would add). Esp. useful before we
comment|//       implement high-level cache. We could even alloc one return object and not per column!
name|int
name|rgCount
init|=
operator|(
...
comment|// last one will unlock the buffers. Cheaper than locking the buffers 500 times.
name|cb
operator|.
name|incRef
argument_list|()
...
name|ucOffset
expr_stmt|;
comment|// We go by compression block, so this is always true.
name|long
name|startCOffset
init|=
name|cOffset
