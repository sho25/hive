...
block|}
comment|// If we send values to llap one by one, it will be hard for it to decide how to
comment|// store them wrt nulls. Therefore, we'll group values together and send in groups.
if|if
condition|(
name|values
operator|==
...
parameter_list|)
block|{
comment|// TODO: string support would be here
throw|throw
operator|new
...
parameter_list|)
block|{
comment|// TODO: timestamp support would be here
throw|throw
operator|new
name|UnsupportedOperationException
argument_list|()
...
parameter_list|)
block|{
comment|// TODO: decimal support would be here
throw|throw
operator|new
name|UnsupportedOperationException
argument_list|()
...
name|UnsupportedOperationException
argument_list|(
literal|"Non-primitives are not supported"
argument_list|)
throw|;
block|}
block|}
...
block|}
block|}
comment|/** Readers for each separate column; no nulls, just the columns being read. */
specifier|private
specifier|final
name|TreeReader
index|[]
name|readers
decl_stmt|;
comment|/** Remembered row offset after a partial read of one column from stripe. */
specifier|private
name|long
name|rowInStripe
init|=
literal|0
decl_stmt|;
comment|/** Next column to be read (index into readers). */
specifier|private
name|int
name|columnIx
init|=
literal|0
decl_stmt|;
comment|/** Remaining row count for current stripe; same for every column, so don't recompute. */
specifier|private
name|long
name|remainingToReadFromStart
init|=
operator|-
literal|1
decl_stmt|;
comment|/** Whether the next call will be the first for this column x stripe. TODO: derive? */
specifier|private
name|boolean
name|firstCall
init|=
...
block|}
name|long
name|rowInStripeGlobal
init|=
name|rowInStripe
decl_stmt|;
comment|// Remember the global state.
name|rowInStripe
operator|=
name|ctx
operator|.
...
condition|)
block|{
comment|// We are starting a new stripe - remember the number of rows to read (same for all cols).
comment|// Doesn't take into account space remaining in ChunkWriter.
name|ctx
operator|.
name|remainingToReadFromStart
operator|=
...
name|doneWithColumnStripe
expr_stmt|;
comment|// If we are not done, there will be more calls.
if|if
condition|(
operator|!
name|doneWithColumnStripe
condition|)
block|{
comment|// Note that we are only advancing the reader for the current column.
name|boolean
name|hasRows
init|=
name|advanceToNextRow
...
name|rowInStripe
expr_stmt|;
comment|// Remember the current value for next call.
if|if
condition|(
operator|!
name|hasRows
...
name|AssertionError
argument_list|(
literal|"No rows after advance; read "
operator|+
name|rowsRead
operator|+
literal|" out of "
...
block|{
comment|// Done with some column + stripe.
operator|++
name|ctx
operator|.
name|columnIx
...
condition|)
block|{
comment|// Done with the last column in this stripe; advance the global rowInStripe.
name|ctx
operator|.
name|columnIx
operator|=
...
block|{
comment|// Revert the state back to start of stripe.
name|ctx
operator|.
name|rowInStripe
operator|=
