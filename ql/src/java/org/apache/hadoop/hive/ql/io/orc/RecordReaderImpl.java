...
name|buffer
decl_stmt|;
comment|/** When we get (or allocate+put) memory buffer to cache, it's locked for us once. All is well      * if we unlock it once; but if we use the same buffer for 2+ RGs, we need to incRef again,      * or track our own separate refcount so we don't unlock more than once. We do the former.      * Every time we get or allocate, we put buffer in one cache chunk that is later used by all      * future lookups that happen to land within this DiskRange. When they call "setReused", they      * get back previous value. If we have not used this range for any RG yet, we don't need to      * notify cache; if it's called more than once, we are re-using this buffer and will incref.      */
specifier|private
name|boolean
name|isReused
init|=
...
name|lastRange
operator|=
name|addRgFilteredStreamToResult
argument_list|(
name|stream
argument_list|,
name|includedRowGroups
...
literal|1
operator|:
literal|"Reader is supposed to be per stripe"
assert|;
if|if
condition|(
name|currentStripe
...
comment|// TODO#: determine start offset, end offset from index; nexts can be end of stream.
comment|//        Either see getIndexPosition or
comment|//        https://cwiki.apache.org/confluence/display/Hive/LanguageManual+ORC#LanguageManualORC-ColumnEncodings
name|long
name|cOffset
init|=
literal|0
