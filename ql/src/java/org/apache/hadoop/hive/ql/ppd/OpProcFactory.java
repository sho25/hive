...
comment|// we assume that nothing can is pushed beyond this operator
name|List
argument_list|<
name|Operator
argument_list|<
...
condition|)
block|{
comment|// remove all the candidate filter operators
comment|// when we get to the TS
name|removeAllCandidates
argument_list|(
name|owi
argument_list|)
...
parameter_list|,
name|boolean
name|onlySyntheticJoinPredicate
parameter_list|,
name|Object
modifier|...
name|nodeOutputs
...
name|nd
decl_stmt|;
comment|// We try to push the full Filter predicate iff:
comment|// - the Filter is on top of a TableScan, or
comment|// - the Filter is on top of a PTF (between PTF and Filter, there might be Select operators)
comment|// Otherwise, we push only the synthetic join predicates
comment|// Note : pushing Filter on top of PTF is necessary so the LimitPushdownOptimizer for Rank
comment|// functions gets enabled
name|boolean
name|parentTableScan
init|=
name|filterOp
operator|.
name|getParentOperators
