...
block|}
block|}
comment|// Push where false filter transitively
name|Map
argument_list|<
name|String
argument_list|,
...
name|exprs
decl_stmt|;
comment|// where false is not associated with any alias in candidates
if|if
condition|(
literal|null
operator|!=
...
condition|)
block|{
comment|// push this 'where false' expr to all aliases
for|for
control|(
name|String
name|alias
...
expr_stmt|;
block|}
comment|// this filter is pushed, we can remove it from non-final candidates.
name|itr
operator|.
name|remove
argument_list|()
