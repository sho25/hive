...
name|streamAliases
decl_stmt|;
comment|// Note: there are two things in Hive called semi-joins - the left semi join construct,
comment|//       and also a bloom-filter based optimization that came later. This is for the latter.
comment|//       Everything else in this desc that says "semi-join" is for the former.
specifier|private
specifier|transient
name|Map
argument_list|<
...
name|semiJoinHints
decl_stmt|;
comment|// non-transient field, used at runtime to kill a task if it exceeded memory limits when running in LLAP
specifier|protected
name|long
...
name|displayName
operator|=
literal|"residual filter predicates"
argument_list|,
name|explainLevels
operator|=
block|{
...
specifier|public
name|String
name|getResidualFilterExprsString
parameter_list|()
block|{
if|if
condition|(
...
name|debug
argument_list|(
literal|"Setting semi-join hints to "
operator|+
name|semiJoinHints
argument_list|)
expr_stmt|;
