...
condition|)
block|{
comment|// Execute extended shared work optimization
name|sharedWorkExtendedOptimization
argument_list|(
name|pctx
argument_list|,
name|optimizerCache
...
argument_list|)
expr_stmt|;
comment|// We rank by size of table x number of reads
name|sortedTables
operator|=
name|rankTablesByAccumulatedSize
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Execute shared work optimization with semijoin removal
name|boolean
name|optimized
init|=
name|sharedWorkOptimization
...
condition|)
block|{
comment|// If it was further optimized, execute a second round of extended shared work optimizer
name|sharedWorkExtendedOptimization
argument_list|(
name|pctx
argument_list|,
...
name|debug
argument_list|(
literal|"After SharedWorkSJOptimizer:\n"
operator|+
name|Operator
operator|.
name|toString
...
condition|)
block|{
comment|// Try to reuse cache for broadcast side in mapjoin operators that
comment|// share same input.
comment|// First we group together all the mapjoin operators that share same
comment|// reduce sink operator.
specifier|final
name|Multimap
argument_list|<
name|Operator
...
condition|)
block|{
comment|// Already visited this work, we move on
continue|continue;
block|}
for|for
control|(
...
name|op
decl_stmt|;
comment|// Only allowed for mapjoin operator
if|if
condition|(
operator|!
name|mapJoinOp
...
block|}
block|}
comment|// For each group, set the cache key accordingly if there is more than one operator
comment|// and input RS operator are equal
for|for
control|(
name|Collection
argument_list|<
...
condition|)
block|{
comment|// Either it is the first map join operator or there was no equivalent RS,
comment|// hence generate cache key
name|cacheKey
operator|=
name|MapJoinDesc
operator|.
...
expr_stmt|;
block|}
comment|// Set in the conf of the map join operator
name|mapJoinOp
operator|.
name|getConf
argument_list|()
...
name|SemanticException
block|{
comment|// Boolean to keep track of whether this method actually merged any TS operators
name|boolean
name|mergedExecuted
init|=
literal|false
...
condition|)
block|{
comment|// We check if the two table scan operators can actually be merged modulo SJs.
comment|// Hence, two conditions should be met:
comment|// (i) the TS ops should be mergeable excluding any kind of DPP, and
comment|// (ii) the DPP branches (excluding SJs) should be the same
name|boolean
name|mergeable
init|=
name|areMergeable
...
name|validMerge
init|=
name|areMergeableExcludeSemijoinsExtendedCheck
argument_list|(
name|pctx
argument_list|,
name|optimizerCache
...
continue|continue;
block|}
comment|// If tests pass, we create the shared work optimizer additional information
comment|// about the part of the tree that can be merged. We need to regenerate the
comment|// cache because semijoin operators have been removed
name|sr
operator|=
...
operator|||
operator|!
name|areMergeableExtendedCheck
argument_list|(
name|pctx
argument_list|,
name|optimizerCache
...
block|{
name|ExprNodeDesc
name|newRetainableTsFilterExpr
init|=
literal|null
decl_stmt|;
name|List
...
condition|)
block|{
comment|// Gather SJ expressions and normal expressions
name|List
argument_list|<
name|ExprNodeDesc
argument_list|>
name|allExprNodesExceptSemijoin
init|=
operator|new
name|ArrayList
argument_list|<>
...
argument_list|)
expr_stmt|;
comment|// Create new expressions
if|if
condition|(
name|allExprNodesExceptSemijoin
operator|.
...
block|}
name|ExprNodeDesc
name|newDiscardableTsFilterExpr
init|=
literal|null
decl_stmt|;
if|if
...
comment|// and it means that we will merge filter expressions for it. Thus, we
comment|// might need to remove DPP predicates before doing that
name|List
argument_list|<
name|ExprNodeDesc
argument_list|>
...
expr_stmt|;
block|}
comment|// Remove and add semijoin filter from expressions
name|replaceSemijoinExpressions
argument_list|(
name|discardableTsOp
argument_list|,
name|semijoinExprNodes
...
block|}
block|}
comment|// Create expression node that will be used for the retainable table scan
if|if
condition|(
operator|!
name|semijoinExprNodes
...
block|}
block|}
comment|// Filter operators on top
if|if
condition|(
name|tsOp
operator|.
...
condition|)
block|{
comment|// DYNAMIC PARTITION PRUNING
return|return
literal|true
return|;
block|}
...
expr_stmt|;
block|}
comment|/**    * Obtain the RS input for a mapjoin operator.    */
specifier|private
specifier|static
name|ReduceSinkOperator
name|obtainBroadcastInput
...
return|;
block|}
comment|// If table properties do not match, we currently do not merge
if|if
condition|(
operator|!
name|Objects
...
name|SemanticException
block|{
comment|// We remove RS-based SJs from consideration, then we compare
name|List
argument_list|<
name|Operator
argument_list|<
...
condition|)
block|{
comment|// This is a hint, we should keep it, hence we bail out
return|return
literal|false
return|;
block|}
...
condition|)
block|{
comment|// TODO: We do not merge, since currently we only merge when one of the TS operators
comment|// are not targetted by a SJ edge
return|return
literal|false
return|;
block|}
...
condition|)
block|{
comment|// None of them are targetted by a SJ, we skip them
return|return
literal|false
return|;
block|}
...
condition|)
block|{
comment|// We cannot merge, we move to the next couple
return|return
literal|false
return|;
block|}
...
condition|)
block|{
comment|// We cannot merge, we move to the next group
name|equalBranches
operator|=
literal|false
expr_stmt|;
...
return|;
block|}
comment|// We reached here, other DPP is the same, these two could potentially be merged.
comment|// Hence, we perform the last check. To do this, we remove the SJ operators,
comment|// but we remember their position in the plan. After that, we will reintroduce
comment|// the SJ operator. If the checks were valid, we will merge and remove the semijoin.
comment|// If the rest of tests to merge do not pass, we will abort the shared scan optimization
...
condition|)
block|{
comment|// We are going to merge, hence we remove the semijoins completely
for|for
control|(
name|ReduceSinkOperator
name|semijoinRsOp
...
block|{
comment|// Otherwise, the put the semijoins back in the auxiliary data structures
name|optimizerCache
operator|.
name|tableScanToDPPSource
operator|.
...
argument_list|()
operator|&&
name|canDeduplicateReduceTraits
argument_list|(
name|op1Conf
argument_list|,
name|op2Conf
...
block|}
block|}
comment|// Combine filters trying to remove any duplicate nodes
name|boolean
name|isOpAndFilter
init|=
name|FunctionRegistry
...
name|n1
decl_stmt|;
comment|// NOTE: UNSET is exclusive from other traits, so FIXED is.
if|if
condition|(
name|x1
operator|||
...
condition|)
block|{
comment|// FIXED(x) + FIXED(x) = FIXED(x)
comment|// FIXED(x) + FIXED(y) = no deduplication (where x != y)
if|if
condition|(
name|n1
operator|==
...
block|{
comment|// FIXED(x) + others = FIXED(x)
name|dedup
operator|=
literal|true
expr_stmt|;
...
condition|)
block|{
comment|// UNIFORM(x) + UNIFORM(y) = UNIFORM(max(x, y))
name|dedup
operator|=
literal|true
expr_stmt|;
...
condition|)
block|{
comment|// AUTOPARALLEL(x) + AUTOPARALLEL(y) = AUTOPARALLEL(max(x, y))
name|dedup
operator|=
literal|true
expr_stmt|;
...
block|}
block|}
comment|// Gether the results into the retainable object
if|if
condition|(
name|apply
operator|&&
