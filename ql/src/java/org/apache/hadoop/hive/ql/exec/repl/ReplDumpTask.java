...
decl_stmt|;
name|Path
name|previousDumpMetaPath
init|=
name|getPreviousDumpMetadataPath
argument_list|(
name|dumpRoot
argument_list|)
decl_stmt|;
name|Path
name|previousHiveDumpPath
init|=
name|previousDumpMetaPath
operator|!=
literal|null
...
literal|null
decl_stmt|;
comment|//If no previous dump is present or previous dump was loaded, proceed
if|if
condition|(
name|shouldDump
argument_list|(
...
name|setEventFrom
argument_list|(
name|getEventFromPreviousDumpMetadata
argument_list|(
name|previousHiveDumpPath
argument_list|)
argument_list|)
...
argument_list|)
expr_stmt|;
name|writeDumpCompleteAck
argument_list|(
name|hiveDumpRoot
argument_list|)
expr_stmt|;
name|deleteAllPreviousDumpMeta
argument_list|(
name|dumpRoot
argument_list|,
name|currentDumpPath
...
name|warn
argument_list|(
literal|"Previous Dump is not yet loaded"
argument_list|)
expr_stmt|;
block|}
block|}
...
return|;
block|}
comment|//bootstrap case return event from
return|return
name|dmd
operator|.
name|getEventFrom
...
name|IOException
block|{
comment|//If no previous dump means bootstrap. So return true as there was no
comment|//previous dump to load
if|if
condition|(
name|previousDumpPath
operator|==
...
return|;
block|}
comment|/**    * Decide whether to dump external tables data. If external tables are enabled for replication,    * then need to dump it's data in all the incremental dumps.    * @return true if need to dump external table data and false if not.    */
specifier|private
name|boolean
name|shouldDumpExternalTableLocation
parameter_list|()
block|{
return|return
name|conf
...
return|;
block|}
comment|/**    * Decide whether to dump external tables.    * @param tableName - Name of external table to be replicated    * @return true if need to bootstrap dump external table and false if not.    */
specifier|private
name|boolean
name|shouldBootstrapDumpExternalTable
parameter_list|(
name|String
name|tableName
parameter_list|)
block|{
comment|// Note: If repl policy is replaced, then need to dump external tables if table is getting replicated
comment|// for the first time in current dump. So, need to check if table is included in old policy.
return|return
name|conf
operator|.
name|getBoolVar
...
return|;
block|}
comment|/**    * Decide whether to dump ACID tables.    * @param tableName - Name of ACID table to be replicated    * @return true if need to bootstrap dump ACID table and false if not.    */
specifier|private
name|boolean
name|shouldBootstrapDumpAcidTable
parameter_list|(
name|String
name|tableName
parameter_list|)
block|{
comment|// Note: If repl policy is replaced, then need to dump ACID tables if table is getting replicated
comment|// for the first time in current dump. So, need to check if table is included in old policy.
return|return
name|ReplUtils
operator|.
...
specifier|private
name|boolean
name|shouldBootstrapDumpTable
parameter_list|(
name|Table
name|table
parameter_list|)
block|{
comment|// Note: If control reaches here, it means, table is already included in new replication policy.
if|if
condition|(
name|TableType
operator|.
...
return|;
block|}
comment|// If replication policy is changed with new included/excluded tables list, then tables which
comment|// are not included in old policy but included in new policy should be bootstrapped along with
comment|// the current incremental replication dump.
comment|// Control reaches for Non-ACID tables.
return|return
operator|!
name|ReplUtils
operator|.
...
argument_list|()
decl_stmt|;
comment|// If we are bootstrapping ACID tables, we need to perform steps similar to a regular
comment|// bootstrap (See bootstrapDump() for more details. Only difference here is instead of
comment|// waiting for the concurrent transactions to finish, we start dumping the incremental events
comment|// and wait only for the remaining time if any.
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|// If repl policy is changed (oldReplScope is set), then pass the current replication policy,
comment|// so that REPL LOAD would drop the tables which are not included in current policy.
if|if
condition|(
name|work
operator|.
...
argument_list|()
expr_stmt|;
comment|// Examine all the tables if required.
if|if
condition|(
name|shouldExamineTablesToDump
argument_list|()
...
condition|)
block|{
comment|// If required wait more for any transactions open at the time of starting the ACID bootstrap.
if|if
condition|(
name|needBootstrapAcidTablesDuringIncrementalDump
argument_list|()
...
argument_list|)
decl_stmt|;
comment|// Dump external table locations if required.
if|if
condition|(
name|TableType
operator|.
...
expr_stmt|;
block|}
comment|// Dump the table to be bootstrapped if required.
if|if
condition|(
name|shouldBootstrapDumpTable
argument_list|(
...
block|}
block|}
name|dumpTableListToDumpLocation
argument_list|(
name|tableList
argument_list|,
name|dumpRoot
...
name|LoginException
block|{
comment|// Empty list will create an empty file to distinguish it from db level replication. If no file is there, that means
comment|// db level replication. If empty file is there, means no table satisfies the policy.
if|if
condition|(
name|tableList
operator|==
...
name|debug
argument_list|(
literal|"Table list file is not created for db level replication."
argument_list|)
expr_stmt|;
return|return;
block|}
comment|// The table list is dumped in _tables/dbname file
name|Path
name|tableListFile
init|=
operator|new
...
expr_stmt|;
block|}
comment|// Close is called explicitly as close also calls the actual file system write,
comment|// so there is chance of i/o exception thrown by close.
name|writer
operator|.
name|close
argument_list|()
...
argument_list|()
operator|+
literal|" creation failed even after "
operator|+
name|FileUtils
operator|.
name|MAX_IO_ERROR_RETRY
...
argument_list|()
operator|+
literal|" is created for table list - "
operator|+
name|tableList
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// TODO : Currently we don't support separate table list for each database.
name|tableList
operator|=
name|work
operator|.
...
name|info
argument_list|(
literal|"Bootstrap Dump for table "
operator|+
name|tblName
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Scheduled a repl copy task from [{}] to [{}]"
argument_list|,
name|replPathMapping
operator|.
name|getSrcPath
...
return|;
block|}
comment|// Get list of valid transactions for Repl Dump. Also wait for a given amount of time for the
comment|// open transactions to finish. Abort any open transactions after the wait is over.
name|String
name|getValidTxnListForReplDump
parameter_list|(
name|Hive
...
name|HiveException
block|{
comment|// Key design point for REPL DUMP is to not have any txns older than current txn in which
comment|// dump runs. This is needed to ensure that Repl dump doesn't copy any data files written by
comment|// any open txns mainly for streaming ingest case where one delta file shall have data from
comment|// committed/aborted/open txns. It may also have data inconsistency if the on-going txns
comment|// doesn't have corresponding open/write events captured which means, catch-up incremental
comment|// phase won't be able to replicate those txns. So, the logic is to wait for the given amount
comment|// of time to see if all open txns< current txn is getting aborted/committed. If not, then
comment|// we forcefully abort those txns just like AcidHouseKeeperService.
name|ValidTxnList
name|validTxnList
init|=
name|getTxnMgr
