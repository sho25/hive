...
condition|)
block|{
comment|// Sanity check; restrictedConfig is always set in setup.
throw|throw
operator|new
name|AssertionError
argument_list|(
literal|"setupPool or setupNonPool needs to be called first"
argument_list|)
throw|;
block|}
if|if
...
name|info
argument_list|(
literal|"Updated tez session pool manager with triggers {} from active resource plan: {}"
argument_list|,
name|appliedTriggers
argument_list|,
name|resourcePlan
...
name|info
argument_list|(
literal|"No session user set; using the UGI user "
operator|+
name|userName
argument_list|)
expr_stmt|;
...
try|try
block|{
comment|// Note: this is not the calling user, but rather the user under which this session will
comment|//       actually run (which is a different under doAs=false). This seems to be intended.
name|String
name|userName
init|=
name|Utils
...
name|info
argument_list|(
literal|"Incoming queue null is reset to current queue "
operator|+
name|queueName
argument_list|)
expr_stmt|;
