...
specifier|private
name|void
name|startNextSessionFromQueue
parameter_list|()
throws|throws
name|Exception
block|{
...
control|)
block|{
comment|// The queue is FIFO, so if we cycle thru length items, we'd start each session once.
name|startNextSessionFromQueue
argument_list|()
expr_stmt|;
block|}
...
name|debug
argument_list|(
literal|"Starting session expiration threads; session lifetime is "
operator|+
name|sessionLifetimeMs
operator|+
literal|" + [0, "
