...
name|columnExprs
init|=
name|getColumnExprsFromASTNode
argument_list|(
name|whereClause
argument_list|,
name|reduceSinkInputRowResolver
...
return|;
block|}
comment|/**    * Given an ASTNode, it returns all of the descendant ASTNodes which represent column expressions    *    * @param node    * @param inputRR    * @return    * @throws SemanticException    */
specifier|private
name|List
argument_list|<
name|ASTNode
...
return|;
block|}
comment|// Expressions are not allowed currently in cluster/distribute/order/sort by.
comment|// It would be good to support them in the future, but till then it is better
comment|// to throw a good semantic error instead of some crpytic error.
specifier|private
name|void
...
argument_list|()
decl_stmt|;
comment|// Columns can only exist at the top
if|if
condition|(
name|input
operator|.
...
specifier|private
name|void
name|validateExpressionSkipParent
parameter_list|(
name|ASTNode
name|inputExpr
parameter_list|,
...
specifier|private
name|void
name|validateExpressionHandleTableQualifier
parameter_list|(
name|ASTNode
name|inputExpr
parameter_list|,
...
name|SemanticException
block|{
comment|// If the expression is tab.column, go to the columns
comment|// Same for value[3]
if|if
condition|(
operator|(
name|inputExpr
...
block|}
block|}
comment|// Validate that the expression only consists of constants and columns.
comment|// Expressions are not allowed in the cluster/distribute/order/sort by list
specifier|private
name|void
...
literal|true
decl_stmt|;
comment|// The first level of children is whether it is ascending/descending
comment|// for order by and sort by
if|if
condition|(
operator|(
name|clauseType
...
name|printInfo
argument_list|(
literal|"WARNING: could not auto-update stale indexes, indexes are not in of sync"
argument_list|)
expr_stmt|;
block|}
block|}
...
name|skewedColNames
operator|=
name|analyzeCreateTableSkewedColNames
argument_list|(
name|skewedColNames
argument_list|,
name|child
...
name|str
range|:
name|getSkewedColumnValuesFromASTNode
argument_list|(
name|vAstNode
argument_list|)
control|)
