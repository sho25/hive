...
return|;
block|}
comment|/*    * Phase1: hold onto any CTE definitions in aliasToCTE.    * CTE definitions are global to the Query.    */
specifier|private
name|void
name|processCTE
parameter_list|(
...
parameter_list|)
block|{
comment|/*      * When saving a view definition all table references in the AST are qualified; including CTE references.      * Where as CTE definitions have no DB qualifier; so we strip out the DB qualifier before searching in      *<code>aliasToCTEs</code> map.      */
name|String
name|currDB
init|=
name|SessionState
...
return|;
block|}
comment|/*    * If a CTE is referenced in a QueryBlock:    * - add it as a SubQuery for now.    *   - SQ.alias is the alias used in QB. (if no alias is specified,    *     it used the CTE name. Works just like table references)    *   - Adding SQ done by:    *     - copying AST of CTE    *     - setting ASTOrigin on cloned AST.    *   - trigger phase 1 on new QBExpr.    *   - update QB data structs: remove this as a table reference, move it to a SQ invocation.    */
specifier|private
name|void
name|addCTEAsSubQuery
parameter_list|(
name|QB
name|qb
parameter_list|,
...
argument_list|(
literal|"VALUES() clause is not fully supported yet..."
argument_list|)
throw|;
block|}
elseif|else
...
name|RuntimeException
argument_list|(
literal|"DELETE is not (yet) implemented..."
argument_list|)
throw|;
case|case
name|HiveParser
...
name|RuntimeException
argument_list|(
literal|"UPDATE is not (yet) implemented..."
argument_list|)
throw|;
default|default:
name|skipRecursion
...
argument_list|()
decl_stmt|;
comment|/*        * used to capture view to SQ conversions. This is used to check for        * recursive CTE invocations.        */
name|Map
argument_list|<
name|String
argument_list|,
...
condition|)
block|{
comment|/*            * if this s a CTE reference:            * Add its AST as a SubQuery to this QB.            */
name|ASTNode
name|cteNode
init|=
name|findCTEFromName
...
return|;
block|}
comment|/*    * for inner joins push a 'is not null predicate' to the join sources for    * every non nullSafe predicate.    */
specifier|private
name|Operator
name|genNotNullFilterForJoinSourcePlan
parameter_list|(
name|QB
name|qb
parameter_list|,
...
name|partnCols
operator|=
name|getPartitionColsFromBucketCols
argument_list|(
name|dest
argument_list|,
name|qb
...
argument_list|()
expr_stmt|;
comment|// If the query here is an INSERT_INTO and the target is an immutable table,
comment|// verify that our destination is empty before proceeding
if|if
condition|(
name|dest_tab
operator|.
...
name|warn
argument_list|(
literal|"Attempted write into an immutable table : "
operator|+
name|dest_tab
operator|.
name|getTableName
...
name|warn
argument_list|(
literal|"Error while trying to determine if immutable table has any data : "
operator|+
name|dest_tab
operator|.
name|getTableName
...
condition|)
block|{
comment|// set the root of the temporary path where dynamic partition columns will populate
name|dpCtx
operator|.
name|setRootPath
argument_list|(
...
name|warn
argument_list|(
literal|"Attempted write into an immutable table partition : "
operator|+
name|dest_tab
operator|.
name|getTableName
...
name|warn
argument_list|(
literal|"Error while trying to determine if immutable table partition has any data : "
operator|+
name|dest_tab
operator|.
name|getTableName
...
parameter_list|)
block|{
comment|// ptf node form is:
comment|// ^(TOK_PTBLFUNCTION $name $alias? partitionTableFunctionSource partitioningSpec? expression*)
comment|// guaranteed to have an alias here: check done in processJoin
if|if
condition|(
name|node
operator|.
...
argument_list|)
expr_stmt|;
comment|// set up WriteEntity for replication
name|outputs
operator|.
name|add
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/*          * if there is only one destination in Query try to push where predicates          * as Join conditions          */
name|Set
argument_list|<
name|String
argument_list|>
...
return|;
block|}
comment|// add dummy data for not removed by CombineHiveInputFormat, etc.
specifier|private
name|Path
name|createDummyFile
parameter_list|()
...
argument_list|)
expr_stmt|;
comment|// put accessed columns to readEntity
if|if
condition|(
name|HiveConf
operator|.
...
condition|)
block|{
comment|// if desired check we're not going over partition scan limits
name|enforceScanLimits
argument_list|(
name|pCtx
argument_list|,
name|origFetchTask
...
condition|)
block|{
comment|// a scan limit on the number of partitions has been set by the user
if|if
condition|(
name|fTask
operator|!=
...
condition|)
block|{
comment|// having a fetch task at this point means that we're not going to
comment|// launch a job on the cluster
if|if
condition|(
operator|!
name|fTask
...
block|{
comment|// At this point we've run the partition pruner for all top ops. Let's
comment|// check whether any of them break the limit
for|for
control|(
name|Operator
argument_list|<
...
parameter_list|)
block|{
comment|// should not occur since second parameter to getTableWithQN is false
throw|throw
operator|new
...
argument_list|)
decl_stmt|;
comment|// Do not allow view to be defined on temp table
name|Set
argument_list|<
name|String
argument_list|>
...
name|SemanticException
argument_list|(
literal|"View definition references temporary table "
operator|+
name|alias
argument_list|)
throw|;
...
parameter_list|)
block|{
comment|// Don't know the characteristics of non-native tables,
comment|// and don't have a rational way to guess, so assume the most
comment|// conservative case.
if|if
condition|(
name|isNonNativeTable
condition|)
