...
comment|// evaluate expressions that occur frequently. For a single groupby, no need to do so
if|if
condition|(
name|grpBy
operator|&&
...
comment|// if sample cols is not specified then default is bucket cols
name|boolean
name|colsEqual
init|=
literal|true
...
argument_list|)
expr_stmt|;
comment|// is it a top level QB, and can it be optimized ? For eg: select * from T does not need a map-reduce job
name|QBParseInfo
name|qbp
init|=
name|qb
