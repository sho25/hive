...
name|stringTypeInfo
argument_list|)
comment|// Script output is always a string
argument_list|)
expr_stmt|;
block|}
name|Operator
...
return|;
block|}
comment|/**    * Class to store UDAF related information.    */
specifier|static
class|class
name|UDAFInfo
block|{
...
decl_stmt|;
block|}
comment|/**    * Returns the UDAFInfo struct for the aggregation    * @param aggName  The name of the UDAF.    * @param mode     The mode of the aggregation. This affects the evaluate method.    * @param aggParameters  The actual exprNodeDesc of the parameters.    * @param aggTree   The ASTNode node of the UDAF in the query.    * @return UDAFInfo    * @throws SemanticException when the UDAF is not found or has problems.    */
name|UDAFInfo
name|getUDAFInfo
parameter_list|(
name|String
...
name|reason
init|=
literal|"Looking for UDAF Evaluator Iterator\""
operator|+
name|aggName
operator|+
literal|"\" with parameters "
...
name|aggName
operator|+
literal|"\" does not have terminate()/terminatePartial() methods."
decl_stmt|;
throw|throw
operator|new
name|SemanticException
...
argument_list|)
decl_stmt|;
comment|// If it is a outer most query and no sorting is specified, exact limit is applied by the fetch task
if|if
condition|(
name|isOuterQuery
operator|&&
...
return|;
block|}
comment|/*    * Is sorting reuired ?    * If there are no cluster by/sort by keys, then an additional map-reduce job is not needed.    * Else, sort the output by the relevant key (via another map-reduce job).    */
specifier|private
name|boolean
name|sortRequired
parameter_list|(
...
name|RuntimeException
argument_list|(
literal|"Internal exception: cannot convert from "
operator|+
name|typeInfoPassed
operator|+
literal|" to "
