...
expr_stmt|;
block|}
comment|// Detect a UDTF by looking up the function name in the registry.
comment|// Not as clean TRANSFORM due to the lack of a special token.
name|boolean
name|isUDTF
init|=
literal|false
...
throw|;
block|}
comment|//Require an AS for UDTFs
if|if
condition|(
operator|(
operator|(
...
argument_list|)
expr_stmt|;
comment|// For UDTF's, skip the function name
name|int
name|startPosn
init|=
name|isUDTF
...
throw|;
block|}
comment|// TODO: support partition for CTAS?
if|if
condition|(
name|partCols
operator|!=
...
comment|// check for existence of table. Throw an exception if it exists.
try|try
block|{
name|Table
name|tab
...
parameter_list|)
block|{
comment|// may be unable to get meta data
throw|throw
operator|new
name|SemanticException
argument_list|(
...
literal|false
assert|;
comment|// should never be unknown command type
block|}
return|return
literal|null
return|;
