...
specifier|private
name|OperatorInfo
name|genGroupByPlanGroupByOpForward
parameter_list|(
name|QBParseInfo
name|parseInfo
parameter_list|,
...
argument_list|()
decl_stmt|;
comment|// Spray on distinctText first
if|if
condition|(
name|distinctText
operator|!=
...
expr_stmt|;
block|}
comment|// copy the input row resolver
name|ArrayList
argument_list|<
name|exprNodeDesc
argument_list|>
...
argument_list|)
decl_stmt|;
comment|// send all the group by expressions
for|for
control|(
name|int
name|i
...
block|}
block|}
comment|// send all the aggregation expressions
name|HashMap
argument_list|<
name|String
argument_list|,
...
specifier|private
name|OperatorInfo
name|genGroupByPlanForwardOperator
parameter_list|(
name|QBParseInfo
name|parseInfo
parameter_list|,
...
argument_list|)
decl_stmt|;
comment|// set forward operator as child of each of input
name|List
argument_list|<
name|Operator
argument_list|<
...
return|;
block|}
comment|/**    * Generate a Group-By plan using a 2 map-reduce jobs. The first map-reduce    * job has already been constructed. Evaluate partial aggregates first,    * followed by actual aggregates. The first map-reduce stage will be     * shared by all groupbys.    */
annotation|@
name|SuppressWarnings
argument_list|(
literal|"nls"
...
argument_list|)
decl_stmt|;
comment|// ////// Generate GroupbyOperator2
name|OperatorInfo
name|groupByOperatorInfo2
init|=
name|genGroupByPlanGroupByOperator2MR
...
argument_list|)
decl_stmt|;
comment|// ////// Generate SelectOperator
name|OperatorInfo
name|selectOperatorInfo
init|=
name|genGroupByPlanSelectOperator
...
block|}
block|}
comment|/*     // We have the table object here - go over the row resolver     // and check all the types are the same     // Vector<ColumnInfo> srcOpns = input.get(0).getRowResolver().getColumnInfos();      Vector<ColumnInfo> insOpns = new Vector<ColumnInfo>();     try {       StructObjectInspector rowObjectInspector = (StructObjectInspector)dest_tab.getDeserializer().getObjectInspector();       List<? extends StructField> fields = rowObjectInspector.getAllStructFieldRefs();       for (int i=0; i<fields.size(); i++) {         insOpns.add(new ColumnInfo(             fields.get(i).getFieldName(),             TypeInfoUtils.getTypeInfoFromObjectInspector(fields.get(i).getFieldObjectInspector()),             false));       }     } catch (SerDeException e) {       throw new RuntimeException(e);     }     if (insOpns.size() != srcOpns.size()) {       // TODO: change this to error message later       throw new SemanticException("Number of columns in the select expression does not " +                                    "match the number of columns in table " + dest_tab.getName() +                                   " expected " + insOpns.size() + " but got " + srcOpns.size());     }      boolean needSelOperator = false;     for(int i=0; i< insOpns.size(); ++i) {       if (!insOpns.get(i).getType().equals(srcOpns.get(i).getType())) {         needSelOperator = true;         throw new SemanticException("insert and select column type do not match for table " + dest_tab.getName());       }     }     */
return|return
name|input
return|;
block|}
...
literal|false
decl_stmt|;
comment|// In case of a multiple group bys, all of them should have the same distinct key
for|for
control|(
name|String
name|dest
...
block|}
block|}
comment|// In the first stage, copy the input and all the group by expressions
comment|// and aggregate paramaters. This can be optimized in the future to only
comment|// evaluate expressions that occur frequently
if|if
condition|(
name|grpBy
condition|)
