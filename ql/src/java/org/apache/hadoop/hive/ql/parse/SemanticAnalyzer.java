...
expr_stmt|;
block|}
comment|/*      * remove any aggregation to be handled by Windowing.      */
if|if
condition|(
name|queryProperties
operator|.
...
expr_stmt|;
block|}
name|handleWindowingExprsInSelectList
argument_list|(
name|qb
argument_list|,
name|ctx_1
...
specifier|private
name|Operator
name|genGroupByPlan1MRMultiReduceGB
parameter_list|(
name|List
argument_list|<
name|String
...
condition|)
block|{
comment|// todo 8 way could be not enough number
if|if
condition|(
name|node
operator|.
...
return|;
block|}
comment|// Returns a list of the distinct exprs for a given clause name as
comment|// ExprNodeDesc.ExprNodeDescEqualityWrapper without duplicates
specifier|private
name|List
argument_list|<
name|ExprNodeDesc
...
condition|)
block|{
comment|// replaced by single fetch task
name|initParseCtx
argument_list|(
name|pCtx
argument_list|)
...
expr_stmt|;
block|}
comment|//--------------------------- Windowing handling -----------------------------------
comment|/*    * - A Select Item form is: ^(TOK_SELEXPR selectExpression Identifier* window_specification?)    * What makes a UDAF invocation a Windowing Function invocation:    * 1. It appears in a SelectExpr that as a WindowSpec    * 2. It is a UDAF that implies order (FunctionRegistry.impliesOrder)    * 3. It contains lead/lag UDF invocations in its args.    */
specifier|private
name|boolean
name|checkAndExtractWindowFunctionsInSelect
parameter_list|(
name|QB
name|qb
parameter_list|,
...
return|;
block|}
comment|/*    * return the UDAFs within the expressionTree.    * If implyOrder is true, then only return the invocations that:    * - are for UDAFs that implyOrder (FunctionRegistry.implyOrder)    * - or contain a Lead/Lag UDF invocation in their arguments    * If implyOrder is false, then return all UDAF invocations.    */
specifier|private
name|ArrayList
argument_list|<
name|ASTNode
...
return|;
block|}
comment|/*    * - Invoked during Phase1 when a TOK_SELECT is encountered.    * - Select tree form is: ^(TOK_SELECT ^(TOK_SELECTEXPR...) ^(TOK_SELECTEXPR...) ...)    * - A Select Item form is: ^(TOK_SELEXPR selectExpression Identifier* window_specification?)    *    * See checkAndExtractWindowFunctionsInSelect for rules on what makes a UDAF invocation    * a Windowing Function invocation    */
specifier|private
name|void
name|handleWindowingExprsInSelectList
parameter_list|(
...
expr_stmt|;
block|}
comment|/*      * In order to distinguish between different UDAF invocations on the same UDAF but different Windows      * add the WdwSpec node as a child of the Function Node.      * It is safe to do this after the function node has been converetd to a WdwFuncSpec.      */
if|if
condition|(
name|wsNode
operator|!=
...
expr_stmt|;
block|}
comment|/*      * We add the column to value columns or output column names      * only if it is not a virtual column      */
name|ArrayList
argument_list|<
name|ColumnInfo
argument_list|>
