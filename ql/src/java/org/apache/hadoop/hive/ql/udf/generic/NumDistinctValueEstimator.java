...
return|;
block|}
comment|/* We use two estimators - one due to Flajolet-Martin and a modification due to    * Alon-Matias-Szegedy. FM uses the location of the least significant zero as an estimate of    * log2(phi*ndvs).    * AMS uses the location of the most significant one as an estimate of the log2(ndvs).    * We average the two estimators with suitable modifications to obtain an estimate of ndvs.    */
specifier|public
name|long
