...
block|}
block|}
comment|/** Cache chunk which tracks whether it has been fully read. See       EncodedReaderImpl class comment about refcounts. */
comment|// TODO: these classes need some cleanup. Find every cast and field access and change to OO;
specifier|public
specifier|static
class|class
name|TrackedCacheChunk
...
block|}
block|}
comment|// TODO: should we pool these? only used for uncompressed.
specifier|private
specifier|static
class|class
...
name|buffer
operator|+
literal|" due to cache collision during pre-read"
argument_list|)
expr_stmt|;
block|}
name|cache
...
argument_list|)
expr_stmt|;
comment|// 7. It may happen that we only use new compression buffers once. Release initial refcounts.
for|for
control|(
name|ProcCacheChunk
name|chunk
