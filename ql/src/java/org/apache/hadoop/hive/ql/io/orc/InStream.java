...
decl_stmt|;
block|}
comment|/**    * Uncompresses part of the stream. RGs can overlap, so we cannot just go and decompress    * and remove what we have returned. We will keep iterator as a "hint" point.    * TODO: Java LinkedList and iter have a really stupid interface. Replace with own simple one?    */
specifier|public
specifier|static
name|void
...
name|IOException
block|{
comment|// TODO#: accpount for coffsets being -1 after finishing the normal methods.
name|colBuffer
operator|.
name|cacheBuffers
operator|=
...
name|AssertionError
argument_list|(
literal|"Compressed offset in the middle of cb"
argument_list|)
throw|;
name|CacheChunk
name|cc
...
name|current
decl_stmt|;
comment|// TODO#: DOUBLE check the iterator state.
name|int
name|chunkLength
init|=
...
comment|// Allocate the buffers, prepare cache kets.
name|LlapMemoryBuffer
index|[]
name|targetBuffers
init|=
...
name|chunk
expr_stmt|;
comment|// Relies on the fact that cache does not actually store DiskRange.
name|targetBuffers
index|[
name|ix
index|]
...
literal|null
expr_stmt|;
comment|// TODO#: are we supposed to release this to zcr in some cases
block|}
...
name|AssertionError
argument_list|(
literal|"We don't know what we are doing anymore"
argument_list|)
throw|;
block|}
name|long
