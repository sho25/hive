...
name|warn
argument_list|(
literal|"Skip runtime skew join as current task has multiple children."
argument_list|)
expr_stmt|;
return|return;
block|}
...
expr_stmt|;
block|}
comment|/**    * Currently, we only support the simplest cases where join is the last work    * of a spark work, i.e. the current ReduceWork is a leave work    * If the reduce work has follow-up work, e.g. an aggregation following the join,    * it's difficult to union the results of the original join and conditional map join    * and feed that to the follow up works. This is not an issue for MR, where ReduceWork    * is always a terminal work.    *    * TODO: can we relax this    */
specifier|private
specifier|static
name|boolean
name|supportRuntimeSkewJoin
