...
condition|)
block|{
comment|// For now, we only optimize the 2 value case.
return|return
literal|null
return|;
block|}
comment|/*      * When we have 2 simple values:      *                          CASE WHEN boolExpr THEN column | const ELSE column | const END      * then we can convert to:        IF (boolExpr THEN column | const ELSE column | const)      */
comment|// CONSIDER: Adding a version of IfExpr* than can handle a non-column/const expression in the
comment|//           THEN or ELSE.
name|ExprNodeDesc
name|exprNodeDesc1
init|=
name|childExpr
...
literal|null
return|;
comment|// Not handled by vector classes yet.
block|}
