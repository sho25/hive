...
block|}
block|}
comment|// TODO: When we support vectorized STRUCTs and can handle more in the reduce-side (MERGEPARTIAL):
comment|// TODO:   Write reduce-side versions of AVG. Currently, only map-side (HASH) versions are in table.
comment|// TODO:   And, investigate if different reduce-side versions are needed for var* and std*, or if map-side aggregate can be used..  Right now they are conservatively
comment|//         marked map-side (HASH).
specifier|static
name|ArrayList
argument_list|<
