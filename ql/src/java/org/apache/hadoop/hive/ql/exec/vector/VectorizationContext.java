...
name|expressions
operator|.
name|CastDateToVarCharWithFormat
import|;
end_import
...
name|expressions
operator|.
name|FilterDecimal64ColumnBetween
import|;
end_import
...
name|expressions
operator|.
name|FilterDecimal64ColumnInList
import|;
end_import
...
name|expressions
operator|.
name|FilterDecimal64ColumnNotBetween
import|;
end_import
...
name|gen
operator|.
name|Decimal64ColumnNotBetween
import|;
end_import
...
name|gen
operator|.
name|TimestampColumnNotBetween
import|;
end_import
...
specifier|private
name|boolean
name|testVectorAdaptorOverride
decl_stmt|;
specifier|public
enum|enum
name|HiveVectorIfStmtMode
block|{
name|ADAPTOR
block|,
name|GOOD
...
block|,
literal|100_000_000_000L
block|,
literal|1_000_000_000_000L
block|,
literal|10_000_000_000_000L
block|,
literal|100_000_000_000_000L
block|,
literal|1_000_000_000_000_000L
block|,
literal|10_000_000_000_000_000L
block|,
comment|// 16
literal|100_000_000_000_000_000L
block|,
literal|1_000_000_000_000_000_000L
block|,
comment|// 18
block|}
decl_stmt|;
...
name|boolean
index|[]
name|scratchColumnTrackWasUsed
init|=
operator|new
name|boolean
index|[
...
return|;
block|}
comment|/**    * Marks all actual scratch columns.    *    * They can be decomissioned with {@link #freeMarkedScratchColumns()}.    */
specifier|public
name|void
...
expr_stmt|;
block|}
comment|/**    * Frees up actually marked scract columns.    */
specifier|public
name|void
name|freeMarkedScratchColumns
parameter_list|()
...
specifier|private
name|VectorExpression
name|getFilterOnBooleanColumnExpression
parameter_list|(
name|ExprNodeColumnDesc
name|exprDesc
parameter_list|,
...
argument_list|)
decl_stmt|;
comment|// Are we forcing the usage of VectorUDFAdaptor for test purposes?
if|if
condition|(
operator|!
name|testVectorAdaptorOverride
...
condition|)
block|{
comment|// Skip boolean predicate.
name|childrenWithCasts
operator|.
name|add
argument_list|(
...
condition|)
block|{
comment|// Don't cast NULL using ConstantValueExpression but replace the NULL expression with the
comment|// desired type set. This lets the doGetIfExpression logic use IfExprCondExprNull, etc.
name|childrenWithCasts
operator|.
name|add
argument_list|(
...
condition|)
block|{
comment|// Some vector operators like VectorSelectOperator optimize out IdentityExpression out of
comment|// their vector expression list and don't evaluate the children, so just return the
comment|// child expression here instead of IdentityExpression.
return|return
name|getVectorExpression
argument_list|(
name|childExpr
...
argument_list|)
expr_stmt|;
comment|// DECIMAL_64 decimals must have same scale.
name|boolean
name|anyDecimal64Expr
init|=
literal|false
decl_stmt|;
name|boolean
name|isDecimal64ScaleEstablished
init|=
literal|false
decl_stmt|;
name|int
name|decimal64ColumnScale
init|=
literal|0
decl_stmt|;
name|boolean
...
argument_list|)
decl_stmt|;
comment|/*        * For columns, we check decimal columns for DECIMAL_64 DataTypePhysicalVariation.        * For UDFs, we check for @VectorizedExpressionsSupportDecimal64 annotation, etc.        */
specifier|final
name|boolean
name|isExprDecimal64
init|=
name|checkExprNodeDescForDecimal64
argument_list|(
name|childExpr
...
condition|)
block|{
comment|// Cannot handle NULL scalar parameter.
return|return
literal|null
return|;
block|}
...
condition|)
block|{
comment|// Currently, we don't have any vectorized expressions that take DECIMAL_64 inputs
comment|// and produce a regular decimal.  Or, currently, a way to express that in the
comment|// descriptor.
return|return
literal|null
return|;
...
argument_list|)
decl_stmt|;
comment|// Rewrite the operand with smaller scale, so we can process Decimal64 operations
comment|// on all the arithmetic and comparison operations.
if|if
condition|(
name|scaleMismatch
operator|&&
...
block|}
return|return
name|createDecimal64VectorExpression
argument_list|(
name|vectorClass
argument_list|,
name|childExprs
...
return|;
block|}
comment|// Intercept here for a possible Decimal64 vector expression class.
name|VectorExpression
name|result
init|=
name|getDecimal64VectorExpressionForUdf
...
return|;
block|}
comment|// Handle strange case of TO_DATE(date) or CAST(date to DATE)
specifier|private
name|VectorExpression
name|getIdentityForDateToDate
parameter_list|(
name|List
argument_list|<
name|ExprNodeDesc
...
condition|)
block|{
comment|// Wrap the PROJECTION IF expression output with a filter.
name|SelectColumnIsTrue
...
name|PROJECTION
argument_list|,
comment|/* isDecimal64ScaleEstablished */
literal|true
argument_list|,
comment|/* decimal64ColumnScale */
name|scale
argument_list|,
name|returnType
argument_list|,
...
name|HiveException
argument_list|(
literal|"Expected constant argument of string family but found "
operator|+
name|o
operator|.
name|getClass
...
parameter_list|)
block|{
comment|// Fall back to VectorUDFAdaptor.
return|return
literal|null
return|;
block|}
...
condition|)
block|{
comment|// STRING and VARCHAR types require no conversion, so use a no-op.
comment|// Also, CHAR is stored in BytesColumnVector with trimmed blank padding, so it also
comment|// requires no conversion;
return|return
name|getIdentityExpression
argument_list|(
name|childExpr
...
argument_list|)
expr_stmt|;
comment|// index 0 not needed since we know returnType
name|Class
argument_list|<
name|?
argument_list|>
name|veClass
init|=
name|getCastFormatVectorExpressionClass
argument_list|(
name|childExpr
argument_list|,
name|returnType
...
literal|" format not"
operator|+
literal|" vectorizable"
argument_list|)
throw|;
block|}
specifier|private
name|VectorExpression
name|tryDecimal64Between
parameter_list|(
name|VectorExpressionDescriptor
operator|.
name|Mode
...
condition|)
block|{
comment|// Projection mode is not applicable.
return|return
literal|null
return|;
block|}
...
condition|)
block|{
comment|// Do not to strict TypeInfo comparisons for DECIMAL -- just compare the category.
comment|// Otherwise, we generate unnecessary casts.
name|isNeedsCast
operator|=
operator|(
operator|(
...
block|}
block|}
comment|// Get a PROJECTION IF expression.
name|VectorExpression
name|ve
init|=
name|doGetIfExpression
argument_list|(
name|genericUDFIf
argument_list|,
name|childExpr
...
name|HiveException
block|{
comment|// Add HiveConf variable with 3 modes:
comment|//   1) adaptor: Always use VectorUDFAdaptor for IF statements.
comment|//
comment|//   2) good: Vectorize but don't optimize conditional expressions
comment|//
comment|//   3) better: Vectorize and Optimize conditional expressions.
comment|//
if|if
condition|(
name|hiveVectorIfStmtMode
...
return|;
block|}
comment|// Align the THEN/ELSE types.
name|childExpr
operator|=
name|getChildExpressionsWithImplicitCast
argument_list|(
...
condition|)
block|{
comment|// THEN NULL ELSE NULL: An unusual "case", but possible.
specifier|final
name|int
name|outputColumnNum
init|=
...
argument_list|)
decl_stmt|;
comment|// Only proceed if the THEN/ELSE types were aligned.
if|if
condition|(
name|thenExpr
operator|.
...
name|HiveException
argument_list|(
literal|"Unable to vectorize custom UDF. LIST, MAP, and UNION type constants not supported: "
operator|+
name|child
argument_list|)
throw|;
