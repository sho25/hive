...
name|gen
operator|.
name|VectorUDAFStdPopDecimalComplete
import|;
end_import
...
name|gen
operator|.
name|VectorUDAFStdPopDoubleComplete
import|;
end_import
...
name|gen
operator|.
name|VectorUDAFStdPopLongComplete
import|;
end_import
...
name|gen
operator|.
name|VectorUDAFStdPopTimestamp
import|;
end_import
...
name|gen
operator|.
name|VectorUDAFStdPopTimestampComplete
import|;
end_import
...
name|gen
operator|.
name|VectorUDAFStdSampDecimalComplete
import|;
end_import
...
name|gen
operator|.
name|VectorUDAFStdSampDoubleComplete
import|;
end_import
...
name|gen
operator|.
name|VectorUDAFStdSampLongComplete
import|;
end_import
...
name|gen
operator|.
name|VectorUDAFStdSampTimestampComplete
import|;
end_import
...
name|gen
operator|.
name|VectorUDAFVarPopDecimalComplete
import|;
end_import
...
name|gen
operator|.
name|VectorUDAFVarPopDoubleComplete
import|;
end_import
...
name|gen
operator|.
name|VectorUDAFVarPopLongComplete
import|;
end_import
...
name|gen
operator|.
name|VectorUDAFVarPopTimestampComplete
import|;
end_import
...
name|gen
operator|.
name|VectorUDAFVarSampDecimalComplete
import|;
end_import
...
name|gen
operator|.
name|VectorUDAFVarSampDoubleComplete
import|;
end_import
...
name|gen
operator|.
name|VectorUDAFVarSampLongComplete
import|;
end_import
...
name|gen
operator|.
name|VectorUDAFVarSampTimestampComplete
import|;
end_import
...
expr_stmt|;
block|}
comment|// Constructor useful making a projection vectorization context.
...
block|{
block|{
comment|// MIN, MAX, and SUM have the same representation for partial and full aggregation, so the
comment|// same class can be used for all modes (PARTIAL1, PARTIAL2, FINAL, and COMPLETE).
name|add
argument_list|(
operator|new
name|AggregateDefinition
...
argument_list|)
expr_stmt|;
comment|// COUNT(column) doesn't count rows whose column value is NULL.
name|add
argument_list|(
operator|new
name|AggregateDefinition
...
argument_list|)
expr_stmt|;
comment|// COUNT(*) counts all rows regardless of whether the column value(s) are NULL.
name|add
argument_list|(
operator|new
name|AggregateDefinition
...
argument_list|)
expr_stmt|;
comment|// Merge the counts produced by either COUNT(column) or COUNT(*) modes PARTIAL1 or PARTIAL2.
name|add
argument_list|(
operator|new
name|AggregateDefinition
...
argument_list|)
expr_stmt|;
comment|// TIMESTAMP SUM takes a TimestampColumnVector as input for PARTIAL1 and COMPLETE.
comment|// But the output is a double.
name|add
argument_list|(
operator|new
name|AggregateDefinition
...
argument_list|)
expr_stmt|;
comment|// Since the partial aggregation produced by AVG is a STRUCT with count and sum and the
comment|// STRUCT data type isn't vectorized yet, we currently only support PARTIAL1.  When we do
comment|// support STRUCTs for average partial aggregation, we'll need 4 variations:
comment|//
comment|//   PARTIAL1      Original data                        --> STRUCT Average Partial Aggregation
comment|//   PARTIAL2      STRUCT Average Partial Aggregation   --> STRUCT Average Partial Aggregation
comment|//   FINAL         STRUCT Average Partial Aggregation   --> Full Aggregation
comment|//   COMPLETE      Original data                        --> Full Aggregation
comment|//
comment|// NOTE: Since we do average of timestamps internally as double, we do not need a VectorUDAFAvgTimestampPartial2.
comment|//
name|add
argument_list|(
operator|new
...
argument_list|)
expr_stmt|;
comment|// (PARTIAL2 FLOAT_FAMILY covers INT_FAMILY and TIMESTAMP because it is:
comment|//    STRUCT Average Partial Aggregation   --> STRUCT Average Partial Aggregation
name|add
argument_list|(
operator|new
name|AggregateDefinition
...
argument_list|)
expr_stmt|;
comment|// (FINAL FLOAT_FAMILY covers INT_FAMILY and TIMESTAMP)
name|add
argument_list|(
operator|new
name|AggregateDefinition
...
argument_list|)
expr_stmt|;
comment|// We haven't had a chance to examine the VAR* and STD* area and expand it beyond PARTIAL1 and COMPLETE.
name|add
argument_list|(
operator|new
name|AggregateDefinition
...
argument_list|)
expr_stmt|;
comment|// (PARTIAL2L FLOAT_FAMILY covers INT_FAMILY, DECIMAL, and TIMESTAMP)
name|add
argument_list|(
operator|new
name|AggregateDefinition
...
argument_list|)
expr_stmt|;
comment|// UDAFBloomFilter. Original data is one type, partial/final is another,
comment|// so this requires 2 aggregation classes (partial1/complete), (partial2/final)
name|add
argument_list|(
operator|new
name|AggregateDefinition
...
condition|)
block|{
comment|// Must be AVG or one of the variance aggregations doing PARTIAL2 or FINAL.
comment|// E.g. AVG PARTIAL2 and FINAL accept struct<count:bigint,sum:double,input:double>
if|if
condition|(
name|udafEvaluatorMode
operator|!=
...
name|HiveException
argument_list|(
literal|"Input expression Hive type name "
operator|+
name|inputExpr
operator|.
name|getTypeString
...
argument_list|()
operator|+
literal|" -- expected PARTIAL2 or FINAL"
argument_list|)
throw|;
block|}
name|GenericUDAFEvaluator
...
argument_list|()
decl_stmt|;
comment|// UNDONE: What about AVG FINAL TIMESTAMP?
if|if
condition|(
name|evaluator
operator|instanceof
...
block|{
comment|// Nothing else supported yet...
throw|throw
operator|new
name|HiveException
argument_list|(
...
name|HiveException
argument_list|(
literal|"No vector argument type for Hive type name "
operator|+
name|inputExpr
operator|.
name|getTypeString
...
condition|)
block|{
comment|// A null means all modes are ok.
name|GenericUDAFEvaluator
operator|.
name|Mode
name|aggDefUdafEvaluatorMode
init|=
name|aggDef
operator|.
