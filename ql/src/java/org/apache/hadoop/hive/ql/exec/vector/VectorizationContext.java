...
return|;
block|}
comment|/**    * Create a filter expression for column IN (<list-of-constants> )    * @param childExpr    * @return    */
specifier|private
name|VectorExpression
name|getInFilterExpression
parameter_list|(
...
argument_list|)
decl_stmt|;
comment|// Remove nulls. This is safe because "value IN (<list> )" is never true for a NULL member
comment|// of<list>, under SQL semantics, because value = NULL is always false.
name|childrenForInList
operator|=
name|removeNullListEntries
argument_list|(
name|childrenForInList
argument_list|)
expr_stmt|;
...
name|colType
operator|+
literal|" not supported for IN in vectorized mode"
argument_list|)
throw|;
block|}
return|return
...
return|;
block|}
comment|// Return a version of the input IN list with the NULL entries removed.
specifier|private
name|List
argument_list|<
name|ExprNodeDesc
...
name|HiveException
argument_list|(
literal|"Unsuported type for vectorization: "
operator|+
name|javaType
argument_list|)
throw|;
...
literal|1000000
expr_stmt|;
comment|// shift left 6 digits to make room for nanos below ms precision
name|result
operator|+=
name|ts
operator|.
...
literal|1000000
expr_stmt|;
comment|// add in nanos, after removing the ms portion
return|return
name|result
return|;
block|}
