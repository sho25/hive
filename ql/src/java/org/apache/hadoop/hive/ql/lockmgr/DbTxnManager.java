...
argument_list|(
literal|"Unknown HiveOperation(null) for "
operator|+
name|getQueryIdWaterMark
argument_list|(
name|queryPlan
...
argument_list|)
expr_stmt|;
comment|// Re-checking as txn could have been closed, in the meantime, by a competing thread.
if|if
condition|(
name|isTxnOpen
argument_list|()
...
name|warn
argument_list|(
literal|"Transaction is already closed."
argument_list|)
expr_stmt|;
block|}
block|}
...
name|IllegalStateException
argument_list|(
literal|"Heartbeater is already started."
argument_list|)
throw|;
block|}
name|Heartbeater
...
condition|)
block|{
comment|// avoid unnecessary locking if the field is null
return|return;
block|}
name|boolean
name|isLockAcquired
...
try|try
block|{
comment|// The lock should not be held by other thread trying to stop the heartbeat for more than 31 seconds
name|isLockAcquired
operator|=
name|heartbeatTaskLock
operator|.
