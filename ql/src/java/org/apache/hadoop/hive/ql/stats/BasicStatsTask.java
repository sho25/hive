...
condition|)
block|{
comment|// TODO: this should also happen on any error. Right now this task will just fail.
name|StatsSetupConst
operator|.
...
condition|)
block|{
comment|// This may happen if ACID state is absent from config.
name|String
name|spec
init|=
name|partish
...
condition|)
block|{
comment|// Update stats for transactional tables (MM, or full ACID with overwrite), even
comment|// though we are marking stats as not being accurate.
if|if
condition|(
name|StatsSetupConst
operator|.
...
condition|)
block|{
comment|// Don't bother with aggregation in this case, it will probably be invalid.
name|parameters
operator|.
name|remove
argument_list|(
