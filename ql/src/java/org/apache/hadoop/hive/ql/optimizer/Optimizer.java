...
argument_list|()
expr_stmt|;
comment|// Add correlation optimizer for first phase query plan tree analysis.
comment|// The first phase will record original opColumnExprMap, opParseCtx, opRowResolver,
comment|// since these may be changed by other optimizers (e.g. entries in opColumnExprMap may be deleted).
comment|// If hive.groupby.skewindata is on, CorrelationOptimizer will not be applied.
comment|// TODO: Make correlation optimizer 1 phase.
...
condition|)
block|{
comment|// TODO: make CorrelationOptimizer compatible with SkewJoinOptimizer
name|transformations
operator|.
name|add
argument_list|(
...
expr_stmt|;
block|}
comment|// The second phase of correlation optimizer used for correlation detection and query plan tree transformation.
comment|// The second phase should be the last optimizer added into transformations.
if|if
condition|(
name|HiveConf
operator|.
