...
decl_stmt|;
comment|// whether Hybrid Grace Hash Join is enabled
specifier|private
specifier|transient
name|boolean
...
name|tag
expr_stmt|;
comment|// As we're calling processOp again to process the leftover triplets, we know the "row" is
comment|// coming from the on-disk matchfile. We need to recreate hashMapRowGetter against new hashtable
if|if
condition|(
name|hybridMapJoinLeftover
condition|)
...
name|pos
expr_stmt|;
comment|// record small table alias
name|JoinUtil
operator|.
name|JoinResult
name|joinResult
...
condition|)
block|{
comment|// Recursively process on-disk triplets (hash partition, sidefile, matchfile)
try|try
block|{
name|hybridMapJoinLeftover
operator|=
...
expr_stmt|;
block|}
comment|/**    * Continue processing each pair of spilled hashtable and big table row container,    * by bringing them back to memory and calling process() again.    * @param partition hash partition to process    * @param hybridHtContainer Hybrid hashtable container    * @throws HiveException    * @throws IOException    * @throws ClassNotFoundException    * @throws SerDeException    */
specifier|private
name|void
name|continueProcess
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// Iterate thru the on-disk matchfile, and feed processOp with leftover rows
name|ObjectContainer
name|bigTable
init|=
name|partition
...
argument_list|()
expr_stmt|;
comment|// Since there's only one hashmap to deal with, it's OK to create a MapJoinBytesTableContainer
name|currentSmallTable
operator|=
operator|new
name|MapJoinBytesTableContainer
