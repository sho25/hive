...
name|inputFileFormatClassName
operator|+
literal|", not using row deserialize"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|String
argument_list|>
name|dataAndPartColumnNameList
init|=
operator|new
name|ArrayList
argument_list|<
...
condition|)
block|{
comment|// Create needed in same order.
for|for
control|(
name|VirtualColumn
name|virtualColumn
...
name|className
operator|+
literal|" for row.deserialize checks"
argument_list|)
expr_stmt|;
block|}
block|}
...
block|{
comment|// Do complex input type checking...
name|boolean
name|inputIsPrimitive
decl_stmt|;
if|if
condition|(
name|parameters
...
literal|true
expr_stmt|;
comment|// Pretend for COUNT(*)
block|}
else|else
block|{
comment|// Multi-input should have been eliminated earlier.
comment|// Preconditions.checkState(parameters.size() == 1);
specifier|final
name|Category
name|inputCategory
init|=
...
name|setOperatorIssue
argument_list|(
literal|"Cannot vectorize GROUP BY with aggregation complex type inputs in "
operator|+
name|aggDesc
operator|.
name|getExprString
...
return|;
block|}
comment|// Now, look a the output.  If the output is complex, we switch to row-mode for all child
comment|// operators...
name|isVectorOutput
operator|=
operator|(
...
return|;
block|}
comment|/*    * Create the VectorPTFDesc data that is used during validation and that doesn't rely on    * VectorizationContext to lookup column names, etc.    */
specifier|private
specifier|static
name|VectorPTFDesc
name|createVectorPTFDesc
...
name|debug
argument_list|(
literal|"debugDisplayAllMaps rowColumnNames "
operator|+
name|Arrays
operator|.
name|toString
...
name|debug
argument_list|(
literal|"debugDisplayAllMaps rowColumnTypeInfos "
operator|+
name|Arrays
operator|.
name|toString
...
name|debug
argument_list|(
literal|"debugDisplayAllMaps virtualColumnCount "
operator|+
name|virtualColumnCount
argument_list|)
expr_stmt|;
