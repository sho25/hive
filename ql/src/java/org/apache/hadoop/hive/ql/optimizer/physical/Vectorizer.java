...
specifier|static
name|void
name|debugDisplayJoinOperatorTree
parameter_list|(
name|Operator
argument_list|<
name|?
...
decl_stmt|;
comment|// Always set the EXPLAIN conditions.
name|setMergeJoinWorkExplainConditions
argument_list|(
name|mergeJoinWork
argument_list|)
expr_stmt|;
name|logMergeJoinWorkExplainVectorization
argument_list|(
name|mergeJoinWork
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Unable to instantiate {} input format class. Cannot determine vectorization support."
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
operator|&&
operator|!
name|hasUnsupportedVectorizedParquetDataType
argument_list|(
name|inputFileFormatClass
argument_list|,
name|allTypeInfoList
...
return|;
block|}
comment|/*        * Currently, VectorizedParquetRecordReader cannot handle nested complex types.        */
for|for
control|(
name|TypeInfo
name|typeInfo
...
name|RuntimeException
argument_list|(
literal|"Unsupported complex type category "
operator|+
name|typeInfo
operator|.
name|getCategory
...
expr_stmt|;
block|}
comment|// HIVE-20419: Vectorization: Prevent mutation of VectorPartitionDesc after being used in a
comment|// hashmap key
specifier|final
name|boolean
...
name|functionName
operator|+
literal|" UNBOUNDED end frame is required for ROWS window type"
argument_list|)
expr_stmt|;
return|return
literal|false
...
comment|/*      * Similarly, we need a mapping since a value expression can be a calculation and the value      * will go into a scratch column.      *      * Value expressions include keys? YES.      */
name|boolean
...
condition|)
block|{
comment|// When the Big Key is not retained in the output result, we do need to copy the
comment|// Big Table key into the overflow batch so the projection of it (Big Table key) to
comment|// the Small Table key will work properly...
comment|//
name|nonOuterSmallTableKeyMapping
operator|.
name|add
...
block|{
comment|// For outer joins, since the small table key can be null when there for NOMATCH,
comment|// we must have a physical (scratch) column for those keys.  We cannot use the
comment|// projection optimization used by non-[FULL} OUTER joins above.
name|int
name|scratchColumn
init|=
name|vContext
...
argument_list|)
expr_stmt|;
comment|// Get the bucketing version
name|int
...
comment|// Various restrictions.
name|boolean
name|isVectorizationFileSinkArrowNativeEnabled
init|=
name|HiveConf
operator|.
name|getBoolVar
...
decl_stmt|;
name|boolean
name|isOkArrowFileSink
init|=
name|serdeClassName
operator|.
name|equals
argument_list|(
literal|"org.apache.hadoop.hive.ql.io.arrow.ArrowColumnarBatchSerDe"
argument_list|)
operator|&&
name|isVectorizationFileSinkArrowNativeEnabled
operator|&&
...
name|OperatorDesc
argument_list|>
name|specializeArrowFileSinkOperator
parameter_list|(
name|Operator
argument_list|<
name|?
...
name|info
argument_list|(
literal|"Vectorizer vectorizeOperator file sink class exception "
operator|+
name|opClass
operator|.
name|getSimpleName
...
condition|)
block|{
comment|// for now, disable operating on decimal64 column vectors for semijoin reduction as
comment|// we have to make sure same decimal type should be used during bloom filter creation
...
expr_stmt|;
block|}
comment|// Fix up the case where parent expression's output data type physical variations is DECIMAL whereas
...
control|)
block|{
comment|// we found at least one children with mismatch
if|if
condition|(
name|children
index|[
...
block|}
block|}
comment|// fix up the input column numbers and output column numbers
if|if
condition|(
name|inputArgsChanged
condition|)
...
operator|)
decl_stmt|;
comment|// VectorCoalesce receives arguments as an array.
comment|// Need to handle it as a special case to avoid instantiation failure.
if|if
condition|(
name|parent
operator|instanceof
...
block|}
block|}
comment|// retain output column number from parent
if|if
condition|(
name|parent
operator|.
...
expr_stmt|;
block|}
comment|// re-instantiate the parent expression with new arguments
name|VectorExpression
name|newParent
init|=
name|vContext
...
name|setOperatorIssue
argument_list|(
literal|"Vectorized& filtered full-outer joins not supported"
argument_list|)
expr_stmt|;
throw|throw
operator|new
...
argument_list|)
expr_stmt|;
comment|// These operators need to be linked to enable runtime statistics to be gathered/used correctly
name|planMapper
operator|.
name|link
argument_list|(
