...
name|warn
argument_list|(
literal|"Map work has more than 1 TableScanOperator. Fail validation!"
argument_list|)
expr_stmt|;
return|return
literal|null
...
argument_list|()
decl_stmt|;
comment|// Validate input format and schema evolution capability.
comment|// For the table, enter a null value in the multi-key map indicating no conversion necessary
comment|// if the schema matches the table.
name|HashMap
argument_list|<
name|ImmutablePair
argument_list|,
...
condition|)
block|{
comment|// We establish with the first one whether the table is partitioned or not.
name|LinkedHashMap
argument_list|<
name|String
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Add the table (non-partitioned) columns and types into the map as not needing
comment|// conversion (i.e. null).
name|conversionMap
operator|.
name|put
argument_list|(
...
block|}
name|ImmutablePair
name|columnNamesAndTypesCombination
init|=
operator|new
name|ImmutablePair
argument_list|(
...
argument_list|)
decl_stmt|;
comment|// Validate the column names that are present are the same.  Missing columns will be
comment|// implicitly defaulted to null.
if|if
condition|(
name|nextDataColumnList
operator|.
...
name|format
argument_list|(
literal|"Could not vectorize partition %s.  The partition column names %d is greater than the number of table columns %d"
argument_list|,
name|path
argument_list|,
name|nextDataColumnList
...
name|format
argument_list|(
literal|"Could not vectorize partition %s.  The partition column name %s is does not match table column name %s"
argument_list|,
name|path
argument_list|,
name|nextColumnName
...
block|}
block|}
comment|// The table column types might have been changed with ALTER.  There are restrictions
comment|// here for vectorization.
comment|// Some readers / deserializers take responsibility for conversion themselves.
comment|// If we need to check for conversion, the conversion object may come back null
comment|// indicating from a vectorization point of view the conversion is implicit.  That is,
comment|// all implicit integer upgrades.
if|if
condition|(
name|vectorPartDesc
operator|.
...
condition|)
block|{
comment|// The results will be in 2 members: validConversion and conversionFlags
name|partitionConversion
operator|.
...
expr_stmt|;
block|}
comment|// We enter this in our map so we don't have to check again for subsequent partitions.
name|conversionMap
operator|.
name|put
argument_list|(
...
name|ret
operator|=
name|validateHashAggregationDesc
argument_list|(
name|desc
operator|.
name|getAggregators
...
name|info
argument_list|(
literal|"Vectorized Reduce MergePartial GROUP BY does not support DISTINCT"
argument_list|)
expr_stmt|;
return|return
literal|false
...
operator|)
decl_stmt|;
comment|// Do we support merge-partial aggregation AND the output is primitive?
name|ret
operator|=
name|validateReduceMergePartialAggregationDesc
argument_list|(
name|desc
operator|.
name|getAggregators
...
name|info
argument_list|(
literal|"Vectorized Reduce MergePartial GROUP BY keys can only handle a key group when it is fed by reduce-shuffle"
argument_list|)
expr_stmt|;
return|return
literal|false
...
name|info
argument_list|(
literal|"Vectorized Reduce MergePartial GROUP BY will process key groups"
argument_list|)
expr_stmt|;
comment|// Primitive output validation above means we can output VectorizedRowBatch to the
comment|// children operators.
name|vectorDesc
operator|.
...
name|info
argument_list|(
literal|"Vectorized Reduce MergePartial GROUP BY will do global aggregation"
argument_list|)
expr_stmt|;
block|}
if|if
...
block|{
comment|// Reduce Hash GROUP BY or global aggregation.
name|ret
operator|=
name|validateHashAggregationDesc
argument_list|(
...
specifier|private
name|boolean
name|validateAggregationIsPrimitive
parameter_list|(
name|VectorAggregateExpression
name|vectorAggrExpr
parameter_list|)
...
name|debug
argument_list|(
literal|"debugDisplayAllMaps columnNames "
operator|+
name|Arrays
operator|.
name|toString
