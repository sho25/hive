...
name|CalciteSemanticException
argument_list|(
literal|"Group type not recognized"
argument_list|)
throw|;
block|}
name|HiveAggregate
...
condition|)
block|{
comment|// Aggregation columns may have been sorted in specific order
for|for
control|(
name|int
name|pos
...
block|{
comment|// Aggregation columns have not been reordered
for|for
control|(
name|int
name|i
...
name|rexBuilder
decl_stmt|;
comment|// this is to keep track of null literal which already has been visited
specifier|private
name|Map
argument_list|<
name|RexLiteral
...
argument_list|()
block|{
comment|// RexLiteral's equal only consider value and type which isn't sufficient
comment|// so providing custom comparator which distinguishes b/w objects irrespective
comment|// of value/type
annotation|@
name|Override
specifier|public
name|int
...
condition|)
block|{
comment|// Swap to get reference on the left side
return|return
name|visitCall
argument_list|(
operator|(
...
condition|)
block|{
comment|// cast for struct types can be ignored safely because explicit casting on struct
comment|// types are not possible, implicit casting e.g. CAST(ROW__ID as<...>) can be ignored
return|return
name|call
operator|.
name|getOperands
