...
control|)
block|{
comment|// If it is a GroupBy with grouping sets and grouping__id column
comment|// is selected, we reformulate to project that column from
comment|// the output of the GroupBy operator
name|boolean
name|reformulate
init|=
literal|false
...
argument_list|()
expr_stmt|;
comment|/*      * 7. Order Use in Order By from the block above. RelNode has no pointer to      * parent hence we need to go top down; but OB at each block really belong      * to its src/from. Hence the need to pass in sort for each block from      * its parent.      */
