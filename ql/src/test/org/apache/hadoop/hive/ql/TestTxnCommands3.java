...
block|,
literal|"s/delta_0000001_0000001_0000/bucket_00000_0"
block|}
block|,
block|{
...
block|,
literal|"warehouse/acid_uap/ds=today/delta_0000002_0000002_0000/bucket_00001_0"
block|}
block|,
block|{
literal|"{\"writeid\":2,\"bucketid\":536870912,\"rowid\":0}\t2\tyah\ttoday"
block|,
literal|"warehouse/acid_uap/ds=today/delta_0000002_0000002_0000/bucket_00000_0"
block|}
block|,
block|{
literal|"{\"writeid\":1,\"bucketid\":536936448,\"rowid\":0}\t1\tbah\ttomorrow"
block|,
literal|"warehouse/acid_uap/ds=tomorrow/delta_0000001_0000001_0000/bucket_00001_0"
block|}
block|,
block|{
literal|"{\"writeid\":1,\"bucketid\":536870912,\"rowid\":0}\t2\tyah\ttomorrow"
block|,
literal|"warehouse/acid_uap/ds=tomorrow/delta_0000001_0000001_0000/bucket_00000_0"
block|}
block|}
decl_stmt|;
name|checkResult
...
block|,
literal|"t/delta_0000002_0000002_0000/bucket_00000_0"
block|}
block|}
decl_stmt|;
name|checkResult
...
expr_stmt|;
comment|//makes delta_2_2 in T2
comment|//create failed compaction attempt so that compactor txn is aborted
name|HiveConf
operator|.
name|setBoolVar
argument_list|(
...
name|assertEquals
argument_list|(
literal|"Unexpected 0th compaction state"
argument_list|,
name|TxnStore
operator|.
...
argument_list|)
expr_stmt|;
comment|//check that the compactor txn is aborted
name|Assert
operator|.
name|assertTrue
argument_list|(
...
argument_list|)
expr_stmt|;
comment|//to make sure any (which is not in this case)
comment|// 'empty aborted' TXNS metadata is removed
name|openResp
operator|=
name|txnHandler
operator|.
...
argument_list|)
expr_stmt|;
comment|//we still have 1 aborted (compactor) txn
name|Assert
operator|.
name|assertTrue
argument_list|(
...
argument_list|)
expr_stmt|;
comment|//this returns 1 row since we only have 1 compaction executed
name|int
name|highestCompactWriteId
init|=
name|TxnDbUtil
operator|.
name|countQueryAgent
...
name|hiveConf
argument_list|,
literal|"select CC_HIGHEST_WRITE_ID from COMPLETED_COMPACTIONS"
argument_list|)
decl_stmt|;
comment|/**      * See {@link org.apache.hadoop.hive.metastore.txn.CompactionTxnHandler#updateCompactorState(CompactionInfo, long)}      * for notes on why CC_HIGHEST_WRITE_ID=TC_WRITEID      */
name|Assert
operator|.
name|assertEquals
argument_list|(
...
name|hiveConf
argument_list|,
literal|"select count(*) from TXN_COMPONENTS where TC_WRITEID="
operator|+
name|highestCompactWriteId
argument_list|)
argument_list|)
expr_stmt|;
comment|//now make a successful compactor run so that next Cleaner run actually cleans
name|HiveConf
operator|.
name|setBoolVar
argument_list|(
...
argument_list|)
expr_stmt|;
comment|//check both combinations - don't know what order the db returns them in
name|Assert
operator|.
name|assertTrue
argument_list|(
...
argument_list|)
expr_stmt|;
comment|//delete metadata about aborted txn from txn_components and files (if any)
name|runCleaner
argument_list|(
name|hiveConf
argument_list|)
...
argument_list|)
expr_stmt|;
comment|//to clean 'empty aborted'
name|openResp
operator|=
name|txnHandler
operator|.
...
argument_list|()
expr_stmt|;
comment|//now the aborted compactor txn is gone
name|Assert
operator|.
name|assertEquals
argument_list|(
...
expr_stmt|;
block|}
comment|/**    * Not enough deltas to compact, no need to clean: there is absolutely nothing to do.    */
annotation|@
name|Test
specifier|public
name|void
name|testNotEnoughToCompact
parameter_list|()
throws|throws
name|Exception
block|{
...
expr_stmt|;
block|}
comment|/**    * There aren't enough deltas to compact, but cleaning is needed because an insert overwrite    * was executed.    */
annotation|@
name|Test
specifier|public
name|void
name|testNotEnoughToCompactNeedsCleaning
parameter_list|()
throws|throws
name|Exception
block|{
