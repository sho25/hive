...
block|{
return|return
literal|"boolean:tinyint:smallint:int:bigint:float:double:string:decimal(38,18):date:timestamp"
return|;
block|}
block|}
specifier|public
...
specifier|public
name|void
name|testFSCallsVectorizedOrcAcidRowBatchReader
parameter_list|()
throws|throws
name|IOException
block|{
...
name|MockFile
argument_list|(
literal|"mock:/a/delta_0000002_0000002_0000/bucket_00002"
argument_list|,
literal|1000
argument_list|,
operator|new
...
argument_list|()
decl_stmt|;
comment|// marker comment to look at stats read ops in target/surefire-reports/*-output.txt
name|System
operator|.
name|out
operator|.
...
argument_list|)
expr_stmt|;
comment|// when creating the reader below there are 2 read ops per bucket file (listStatus and open).
comment|// HIVE-19588 removes listStatus from the code path so there should only be one read ops (open) after HIVE-19588
name|int
name|readsBefore
init|=
name|fs
...
parameter_list|)
block|{
comment|// this is expected as these mock files are not valid orc file
block|}
block|}
name|int
name|readsAfter
...
name|println
argument_list|(
literal|"STATS: listLocatedFileStatuses - "
operator|+
name|path
argument_list|)
expr_stmt|;
...
name|println
argument_list|(
literal|"STATS: getFileBlockLocationsImpl - "
operator|+
name|stat
operator|.
name|getPath
...
argument_list|)
expr_stmt|;
comment|// TODO: could we instead get FS from path here and add normal files for every UGI?
name|MockFileSystem
operator|.
name|clearGlobalFiles
argument_list|()
...
specifier|public
name|void
name|testSchemaEvolutionOldDecimal
parameter_list|()
throws|throws
name|Exception
block|{
...
specifier|public
name|void
name|testSchemaEvolutionDecimal64
parameter_list|()
throws|throws
name|Exception
block|{
...
specifier|public
name|void
name|testAcidReadPastLastStripeOffset
parameter_list|()
throws|throws
name|Exception
block|{
...
name|assertEquals
argument_list|(
literal|"Index length doesn't match number of stripes"
argument_list|,
name|stripes
operator|.
name|size
...
name|assertEquals
argument_list|(
literal|"1st Index entry mismatch"
argument_list|,
operator|new
name|RecordIdentifier
argument_list|(
...
name|assertEquals
argument_list|(
literal|"2nd Index entry mismatch"
argument_list|,
operator|new
name|RecordIdentifier
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Specify an OrcSplit that starts beyond the offset of the last stripe.
name|OrcSplit
name|split
init|=
operator|new
...
argument_list|()
decl_stmt|;
comment|// Iterate through any records.
comment|// Because our read offset was past the stripe offset, the rows from the last stripe will
comment|// not be read. Thus 0 records.
while|while
condition|(
name|reader
operator|.
