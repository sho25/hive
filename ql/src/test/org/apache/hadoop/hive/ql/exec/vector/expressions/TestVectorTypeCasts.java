...
name|b
operator|=
name|getBatchTimestampDecimalPrec5Scale2
argument_list|()
expr_stmt|;
name|expr
operator|.
...
return|;
block|}
comment|/*   @Test   public void testCastDecimalToDecimal() {      // test casting from one precision and scale to another.     VectorizedRowBatch b = getBatchDecimalDecimal();     VectorExpression expr = new CastDecimalToDecimal(0, 1);     expr.evaluate(b);     DecimalColumnVector r = (DecimalColumnVector) b.cols[1];     assertTrue(r.vector[0].getHiveDecimal().equals(HiveDecimal.create("10.00", (short) 2)));     assertFalse(r.noNulls);     assertTrue(r.isNull[1]);      // test an increase in precision/scale     b = getBatchDecimalDecimal();     expr = new CastDecimalToDecimal(1, 0);     expr.evaluate(b);     r = (DecimalColumnVector) b.cols[0];     assertTrue(r.vector[0].getHiveDecimal().equals(HiveDecimal.create("100.01", (short) 4)));     assertTrue(r.vector[1].getHiveDecimal().equals(HiveDecimal.create("-200.02", (short) 4)));     assertTrue(r.noNulls);   }   */
specifier|private
name|VectorizedRowBatch
