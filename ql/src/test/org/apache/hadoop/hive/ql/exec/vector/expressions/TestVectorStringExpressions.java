...
parameter_list|()
block|{
comment|// create a new batch with one char column (for input)
comment|// and one long column (for output)
name|VectorizedRowBatch
name|batch
init|=
operator|new
...
block|{
comment|// no nulls, not repeating
comment|// We don't test all the combinations because (at least currently)
comment|// the logic is inherited to be the same as testColLower, which checks all the cases).
name|VectorizedRowBatch
name|batch
init|=
name|makeStringBatchMixedCase
