...
specifier|public
name|void
name|testConsistentHashingFallback
parameter_list|()
throws|throws
name|IOException
block|{
...
name|locs
operator|+
literal|" locations aren't to our liking: "
operator|+
name|Arrays
operator|.
name|toString
...
argument_list|)
expr_stmt|;
comment|// TODO: the way we add hash fns does exhibit some irregularities.
comment|//       Seems like the 3rd iter has a better distribution in many cases, even better
comment|//       that the original hash. That trips the "above MA" criteria, even if the rest is flat.
name|assertTrue
argument_list|(
literal|"Found "
operator|+
...
parameter_list|)
block|{
comment|// This relies heavily on what method determineSplits ... calls and doesn't.
comment|// We could do a wrapper with only size() and get() methods instead of List, to be sure.
annotation|@
...
expr_stmt|;
block|}
comment|// All of this is completely bogus and mostly captures the following function:
comment|// f(output) = I-eyeballed-the(output) == they-look-ok.
comment|// It's pretty much a golden file...
comment|// The fact that stdev doesn't increase with increasing missCount is captured outside.
name|double
name|avg
init|=
name|ss
...
name|missCount
operator|+
literal|" misses isn't to "
operator|+
literal|"our liking: avg "
operator|+
name|avg
operator|+
literal|", stdev "
...
specifier|public
name|void
name|testDFSLocalityAwareAffinity
parameter_list|()
throws|throws
name|IOException
block|{
...
argument_list|)
decl_stmt|;
comment|// Different base localities
name|InputSplit
name|os1
init|=
name|createMockFileSplit
