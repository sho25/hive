...
name|info
argument_list|(
literal|"About to call get with "
operator|+
name|old
argument_list|)
expr_stmt|;
...
specifier|public
name|void
name|setNextWaitForAmRegistryFuture
parameter_list|(
name|SettableFuture
argument_list|<
name|Boolean
...
argument_list|()
expr_stmt|;
comment|// Test various combinations.
name|verifyMapping
argument_list|(
name|wm
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// User takes precendence over groups unless ordered explicitly.
name|verifyMapping
argument_list|(
name|wm
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Check explicit pool specifications - valid cases where priority is changed.
name|verifyMapping
argument_list|(
name|wm
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Explicit pool specification - invalid - there's no mapping that matches.
try|try
block|{
name|TezSessionState
name|r
...
name|fail
argument_list|(
literal|"Expected failure, but got "
operator|+
name|r
argument_list|)
expr_stmt|;
...
comment|// Expected.
block|}
comment|// Now allow the users to specify any pools.
name|conf
operator|.
name|set
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// If cluster info changes, qam should be called with the same fractions.
name|wm
operator|.
name|notifyOfClusterStateChange
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// Change the resource plan to use fifo policy.
name|plan
operator|=
operator|new
name|WMFullResourcePlan
...
argument_list|)
expr_stmt|;
comment|// Make sure the allocation is transfered correctly on return.
name|sessionA1
operator|.
name|returnToSessionManager
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// Make sure reuse changes the FIFO order of the session.
name|WmTezSession
name|sessionA4
init|=
operator|(
...
argument_list|)
expr_stmt|;
comment|// Remove the resource plan - disable WM. All the queries die.
name|wm
operator|.
name|updateResourcePlanAsync
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// No-op for session killed by WM.
name|assertEquals
argument_list|(
literal|0
argument_list|,
...
parameter_list|)
block|{
comment|// Ignore, this particular error is expected.
block|}
comment|// Apply the plan again - enable WM.
name|wm
operator|.
name|updateResourcePlanAsync
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// B is maxed out on capacity, so this move should fail the session
name|future
operator|=
name|wm
operator|.
...
name|assertEquals
argument_list|(
literal|"Destination pool B is full. Killing query."
argument_list|,
name|sessionA3
operator|.
...
specifier|public
name|void
name|testMoveSessionsMultiPool
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// [A: 0, B: 1, B.x: 0, B.y: 0, C: 0]
name|Future
argument_list|<
name|Boolean
argument_list|>
...
argument_list|)
expr_stmt|;
comment|// [A: 0, B: 0, B.x: 0, B.y: 0, C: 1]
name|future
operator|=
name|wm
operator|.
...
argument_list|)
expr_stmt|;
comment|// [A: 0, B: 0, B.x: 1, B.y: 0, C: 0]
name|future
operator|=
name|wm
operator|.
...
argument_list|)
expr_stmt|;
comment|// A is maxed out on capacity, so this move should fail the session
comment|// [A: 1, B: 0, B.x: 0, B.y: 0, C: 0]
name|future
operator|=
name|wm
...
name|assertEquals
argument_list|(
literal|"Destination pool A is full. Killing query."
argument_list|,
name|sessionA1
operator|.
name|getReasonForKill
...
argument_list|)
expr_stmt|;
comment|// return a loaned session goes back to tez am pool
comment|// [A: 0, B: 0, B.x: 0, B.y: 0, C: 0]
name|wm
operator|.
name|returnAfterUse
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Initialization fails with retry, no resource plan change.
name|SettableFuture
argument_list|<
name|Boolean
argument_list|>
...
name|retriedSession
expr_stmt|;
comment|// Initialization fails and so does the retry, no resource plan change.
name|theOnlySession
operator|.
name|setWaitForAmRegistryFuture
argument_list|(
