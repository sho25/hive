...
argument_list|()
expr_stmt|;
comment|//adds delta and delete_delta
name|ShowCompactResponse
name|rsp
init|=
name|txnHandler
...
argument_list|()
expr_stmt|;
comment|// since compaction was not run, state should not be "ready for cleaning" but "succeeded"
name|ShowCompactResponse
name|rsp
init|=
name|txnHandler
...
argument_list|)
expr_stmt|;
comment|// There should still be 4 directories in the location
name|FileSystem
name|fs
init|=
name|FileSystem
...
argument_list|)
expr_stmt|;
comment|// There should still now be 6 directories in the location
name|FileSystem
name|fs
init|=
name|FileSystem
...
argument_list|()
expr_stmt|;
comment|//this will create delta_20_24 and delete_delta_20_24. See MockRawReader
name|ShowCompactResponse
name|rsp
init|=
name|txnHandler
...
expr_stmt|;
block|}
comment|/**    * These tests are starting to be a hack.  The files writtern by addDeltaFile() are not proper    * Acid files and the {@link CompactorTest.MockRawReader} performs no merging of delta files and    * fakes isDelete() as a shortcut.  This makes files created on disk to not be representative of    * what they should look like in a real system.    * Making {@link org.apache.hadoop.hive.ql.txn.compactor.CompactorTest.MockRawReader} do proper    * delete event handling would be duplicating either OrcRawRecordMerger or VectorizedOrcAcidRowBatchReaer.    * @param type    * @throws Exception    */
specifier|private
name|void
name|compactNoBaseLotsOfDeltas
parameter_list|(
...
argument_list|)
decl_stmt|;
comment|/* delete_delta_21_23 and delete_delta_25_33 which are created as a result of compacting*/
name|int
name|numFilesExpected
init|=
literal|11
operator|+
operator|(
...
expr_stmt|;
comment|//streaming ingest
comment|//todo: this should have some _vXXXX suffix but addDeltaFile() doesn't support it
name|matchesNotFound
operator|.
name|add
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Drop table will clean the table entry from the compaction queue and hence worker have no effect
name|ms
operator|.
name|dropTable
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Drop partition will clean the partition entry from the compaction queue and hence worker have no effect
name|ms
operator|.
name|dropPartition
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// Compaction should not have run on a single delta file
name|FileSystem
name|fs
init|=
name|FileSystem
...
argument_list|)
expr_stmt|;
comment|// State should not be "ready for cleaning" because we skip cleaning
name|List
argument_list|<
name|ShowCompactResponseElement
argument_list|>
...
argument_list|)
expr_stmt|;
comment|// assert transaction with txnId=1 is still aborted after cleaner is run
name|startCleaner
argument_list|()
expr_stmt|;
name|List
