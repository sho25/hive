...
begin_comment
comment|/**  * Tests for CompactionTxnHandler.  */
end_comment
...
argument_list|)
expr_stmt|;
comment|// Set config so that TxnUtils.buildQueryWithINClauseStrings() will
comment|// produce multiple queries
name|conf
operator|.
name|setIntVar
argument_list|(
...
try|try
block|{
comment|// The first call to markFailed() should have removed the record from
comment|// COMPACTION_QUEUE, so a repeated call should fail
name|txnHandler
operator|.
name|markFailed
argument_list|(
...
name|fail
argument_list|(
literal|"The first call to markFailed() must have failed as this call did "
operator|+
literal|"not throw the expected exception"
argument_list|)
expr_stmt|;
block|}
catch|catch
...
expr_stmt|;
block|}
comment|// There are not enough failed compactions yet so checkFailedCompactions() should return false.
comment|// But note that any sql error will also result in a return of false.
name|assertFalse
argument_list|(
name|txnHandler
operator|.
...
argument_list|)
expr_stmt|;
comment|// Add more failed compactions so that the total is exactly COMPACTOR_INITIATOR_FAILED_THRESHOLD
for|for
control|(
name|int
name|i
...
expr_stmt|;
block|}
comment|// Now checkFailedCompactions() will return true
name|assertTrue
argument_list|(
name|txnHandler
operator|.
...
argument_list|)
expr_stmt|;
comment|// Check the output of show compactions
name|checkShowCompaction
argument_list|(
name|dbName
argument_list|,
name|tableName
...
argument_list|)
expr_stmt|;
comment|// Now add enough failed compactions to ensure purgeCompactionHistory() will attempt delete;
comment|// HiveConf.ConfVars.COMPACTOR_HISTORY_RETENTION_ATTEMPTED is enough for this.
comment|// But we also want enough to tickle the code in TxnUtils.buildQueryWithINClauseStrings()
comment|// so that it produces multiple queries. For that we need at least 290.
for|for
control|(
name|int
name|i
...
argument_list|)
expr_stmt|;
comment|//simulate auto-compaction interval
name|TimeUnit
operator|.
name|SECONDS
operator|.
...
argument_list|)
expr_stmt|;
comment|//simulate prev failed compaction
name|CompactionRequest
name|rqst
init|=
operator|new
