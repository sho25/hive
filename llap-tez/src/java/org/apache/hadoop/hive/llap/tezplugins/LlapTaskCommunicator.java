...
decl_stmt|;
comment|// TODO: this is an ugly hack because Tez plugin isolation does not make sense for LLAP plugins.
comment|//       We are going to register a thread-local here for now, so that the scheduler, initializing
comment|//       in the same thread after the communicator, will pick up. Or the other way around.
...
argument_list|()
decl_stmt|;
comment|/**    * @return true iff the error is fatal and we should give up on everything.    */
specifier|private
name|boolean
...
function_decl|;
block|}
comment|/**    * @param node    * @param callback    * @return if it was possible to attemp the registration. Sometimes it's    * not possible because is a dag is not running    */
specifier|public
name|boolean
name|registerDag
parameter_list|(
...
name|info
argument_list|(
literal|"Error registering dag with"
operator|+
literal|" appId="
operator|+
name|currentQueryIdentifierProto
...
name|warn
argument_list|(
literal|"Untracked node for "
operator|+
name|attemptId
operator|+
literal|"; NodeInfo points to "
operator|+
name|nodeId
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// TODO: danger of stack overflow... needs a retry limit?
return|return;
block|}
block|}
name|UpdateFragmentRequestProto
...
name|warn
argument_list|(
literal|"Failed to send update fragment request for {}"
argument_list|,
name|attemptId
operator|.
name|toString
...
index|]
decl_stmt|;
comment|// Check if the taskAttempt is present in AM view
if|if
condition|(
name|biMap
operator|.
...
name|info
argument_list|(
literal|"Sending a kill for attempt {}, due to a ping from "
operator|+
literal|"node with same host and same port but "
operator|+
