...
decl_stmt|;
comment|/** The fake job ID generated by HS2 to use for the job token. */
specifier|public
specifier|static
specifier|final
name|String
name|LLAP_PLUGIN_ENDPOINT_JOBID
init|=
literal|"llap.plugin.endpoint.jobid"
decl_stmt|;
comment|/** The job token generated by HS2 for the endpoint. See the comment at the place where    * this is used - the token will be generated by AM after we have AM registry. */
specifier|public
specifier|static
specifier|final
name|String
name|LLAP_PLUGIN_ENDPOINT_TOKEN
init|=
literal|"llap.plugin.endpoint.token"
decl_stmt|;
annotation|@
name|VisibleForTesting
specifier|public
...
argument_list|()
expr_stmt|;
comment|// Note: for HA/AMRegistry, this token will have to be published in a secure location (ZK).
comment|//       Otherwise, when HS2 dies, noone will be able to talk to this AM.
comment|//       We could generate it in AM in the first place, but then even the original HS2
comment|//       won't be able to talk to us before we register. Which might be ok.
comment|// For now, there's no mechanism to coordinate this, so we email the token from HS2.
name|byte
index|[]
name|tokenBytes
init|=
...
name|info
argument_list|(
literal|"Running with configuration: hosts={}, numSchedulableTasksPerNode={}, nodeBlacklistConf={}, localityConf={}"
argument_list|,
name|hostsString
argument_list|,
name|numSchedulableTasksPerNode
...
comment|// Whatever we were doing is irrelevant.
name|ti
operator|.
name|isPendingUpdate
operator|=
...
condition|)
block|{
comment|// Looks like we've succeeded at bringing the task state up to date with the local state.
name|ti
operator|.
name|isPendingUpdate
operator|=
