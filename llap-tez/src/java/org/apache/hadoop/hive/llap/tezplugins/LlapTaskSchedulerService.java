...
argument_list|()
operator|+
literal|" registered successfully for node "
operator|+
name|nodeInfo
operator|.
name|getHost
...
name|warn
argument_list|(
literal|"Error registering dag for node "
operator|+
name|nodeInfo
operator|.
name|getHost
...
argument_list|)
expr_stmt|;
comment|// In case we fail to register the dag we add the node anyway
comment|// We will try to register the dag when we schedule the first container
name|addNode
argument_list|(
name|nodeInfo
argument_list|,
...
block|}
block|}
comment|/// Shared singleton MetricsSource instance for all FileData locks
specifier|private
specifier|static
specifier|final
name|MetricsSource
...
static|static
block|{
comment|// create and register the MetricsSource for lock metrics
name|MetricsSystem
name|ms
init|=
name|LlapMetricsSystem
...
name|register
argument_list|(
literal|"LLAPTaskSchedulerLockMetrics"
argument_list|,
literal|"Lock metrics for R/W locks LLAP task scheduler"
argument_list|,
name|LOCK_METRICS
argument_list|)
expr_stmt|;
...
name|nodeInfo
operator|.
name|updateLlapServiceInstance
argument_list|(
name|serviceInstance
argument_list|,
name|numSchedulableTasksPerNode
...
name|info
argument_list|(
literal|"Updated node with identity: {} as a result of registry callback"
argument_list|,
name|serviceInstance
operator|.
name|getWorkerIdentity
