...
argument_list|)
expr_stmt|;
comment|//     Replacing NodeInfo means we end up discarding whatever state was known about that node.
comment|//     instanceToNodeMap.put(serviceInstance.getWorkerIdentity(), new NodeInfo(serviceInstance,
comment|//          nodeBlacklistConf, clock, numSchedulableTasksPerNode, metrics));
comment|//
comment|//
comment|//    LOG.info("Updated node with identity: {}", serviceInstance.getWorkerIdentity());
block|}
annotation|@
name|Override
specifier|public
...
condition|)
block|{
comment|// no locality-requested, iterate the available hosts in consistent order from the beginning
if|if
condition|(
name|LOG
operator|.
...
name|debug
argument_list|(
literal|"No-locality requested. Attempting to allocate next available host for task={}"
argument_list|,
name|request
operator|.
name|task
...
block|{
comment|// miss in locality request, try the next available host that can accept tasks (assume the consistent instances
comment|// list as a ring) from the index of first requested host
specifier|final
name|String
...
name|debug
argument_list|(
literal|"Locality miss. Attempting to allocate next available host from first requested host({}) for "
operator|+
literal|"task={}"
argument_list|,
name|firstRequestedHost
...
name|info
argument_list|(
literal|"Assigned task={} on node={}, to container={} on node={}"
argument_list|,
name|taskInfo
argument_list|,
name|nodeInfo
...
literal|"]: "
operator|+
literal|"canAcceptTask={}, numScheduledTasks={}, numSchedulableTasks={}, hadCommFailure={}, disabled={}"
argument_list|,
name|result
argument_list|,
name|numScheduledTasks
