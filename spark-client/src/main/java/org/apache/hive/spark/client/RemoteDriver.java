...
name|protocol
decl_stmt|;
comment|// a local temp dir specific to this driver
specifier|private
specifier|final
name|File
name|localTmpDir
...
argument_list|()
decl_stmt|;
comment|// jc is effectively final, but it has to be volatile since it's accessed by different
comment|// threads while the constructor is running.
specifier|private
specifier|volatile
name|JobContextImpl
name|jc
...
name|futureTimeout
decl_stmt|;
comment|// Rpc call timeout in milliseconds
specifier|private
name|RemoteDriver
parameter_list|(
name|String
...
argument_list|)
decl_stmt|;
comment|//set these only in mapConf and not in SparkConf,
comment|// as these are non-spark specific configs used by the remote driver
name|mapConf
operator|.
name|put
argument_list|(
...
name|info
argument_list|(
literal|"Received signal SIGTERM, attempting safe shutdown of Remote Spark Context"
argument_list|)
expr_stmt|;
name|protocol
operator|.
...
argument_list|(
literal|"Remote Spark Context was shutdown because it received a SIGTERM "
operator|+
literal|"signal. Most likely due to a kill request via YARN."
argument_list|)
expr_stmt|;
name|shutdown
argument_list|(
...
name|warn
argument_list|(
literal|"Failed to delete local tmp dir: "
operator|+
name|localTmpDir
argument_list|,
name|e
...
name|warn
argument_list|(
literal|"Failed to send out the error during RemoteDriver shutdown"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|Exception
block|{
comment|// In case the job context is not up yet, let's wait, since this is supposed to be a
comment|// "synchronous" RPC.
if|if
condition|(
name|jc
operator|==
...
name|IllegalStateException
argument_list|(
literal|"JobContext.monitor() is not available for synchronous jobs."
argument_list|)
throw|;
block|}
block|}
...
argument_list|)
decl_stmt|;
comment|// In case the job is empty, there won't be JobStart/JobEnd events. The only way
comment|// to know if the job has finished is to check the futures here ourselves.
for|for
control|(
name|JavaFutureAction
argument_list|<
...
name|debug
argument_list|(
literal|"Client job {}: {} of {} Spark jobs finished."
argument_list|,
name|req
operator|.
name|id
...
expr_stmt|;
block|}
comment|// If the job is not empty (but runs fast), we have to wait until all the TaskEnd/JobEnd
comment|// events are processed. Otherwise, task metrics may get lost. See HIVE-13525.
if|if
condition|(
name|sparkJobId
operator|!=
...
parameter_list|)
block|{
comment|// If the main thread throws an exception for some reason, propagate the exception to the
comment|// client and initiate a safe shutdown
if|if
condition|(
name|rd
operator|.
