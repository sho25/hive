...
literal|false
decl_stmt|;
comment|/**    * Time zone in which file was written, which may be stored in metadata.    */
specifier|private
name|ZoneId
name|writerTimezone
init|=
literal|null
decl_stmt|;
comment|/**    * Whether the file was written using proleptic Gregorian or hybrid calendar.    */
specifier|private
name|Boolean
...
throw|;
block|}
comment|// If a time zone is found in file metadata (property name: writer.time.zone), convert the
comment|// timestamp to that (writer) time zone in order to emulate time zone agnostic behavior.
comment|// If not, then the file was written by an older version of hive, so we convert the timestamp
comment|// to the server's (reader) time zone for backwards compatibility reasons - unless the
comment|// session level configuration hive.avro.timestamp.skip.conversion is set to true, in which
comment|// case we assume it was written by a time zone agnostic writer, so we don't convert it.
specifier|final
name|boolean
name|skipUTCConversion
decl_stmt|;
if|if
condition|(
name|configuration
