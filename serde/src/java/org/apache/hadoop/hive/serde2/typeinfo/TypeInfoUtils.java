...
comment|// if there are quotes, all the text between the quotes
comment|// is considered a single token (this can happen for
comment|// timestamp with local time-zone)
if|if
condition|(
name|begin
operator|>
...
condition|)
block|{
comment|// It's possible that old metadata still refers to "decimal" as a column type w/o
comment|// precision/scale. In this case, the default (10,0) is assumed. Thus, do nothing here.
block|}
elseif|else
if|if
condition|(
...
condition|)
block|{
comment|// only precision is specified
name|precision
operator|=
name|Integer
operator|.
...
condition|)
block|{
comment|// New metadata always have two parameters.
name|precision
operator|=
name|Integer
operator|.
...
name|IllegalArgumentException
argument_list|(
literal|"Type decimal only takes two parameter, but "
operator|+
name|params
operator|.
name|length
