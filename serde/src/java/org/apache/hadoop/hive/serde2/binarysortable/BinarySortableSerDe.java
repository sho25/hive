...
name|reuse
decl_stmt|;
comment|// Use internal text member to read value
name|deserializeText
argument_list|(
name|buffer
argument_list|,
...
name|reuse
decl_stmt|;
comment|// Use HiveVarchar's internal Text member to read the value.
name|deserializeText
argument_list|(
name|buffer
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// If we cache helper data for deserialization we could avoid having
comment|// to call getVarcharMaxLength() on every deserialize call.
name|r
operator|.
name|enforceMaxLength
argument_list|(
...
case|:
block|{
comment|// See serialization of decimal for explanation (below)
name|HiveDecimalWritable
name|bdw
init|=
operator|(
...
argument_list|)
decl_stmt|;
comment|// Trailing space should ignored for char comparisons.
comment|// So write stripped values for this SerDe.
name|Text
name|t
init|=
name|hc
...
argument_list|)
decl_stmt|;
comment|// use varchar's text field directly
name|Text
name|t
init|=
name|hc
...
argument_list|)
decl_stmt|;
name|serializeTimestampTZWritable
argument_list|(
name|buffer
argument_list|,
name|t
...
parameter_list|)
block|{
comment|// get the sign of the big decimal
name|int
name|sign
init|=
name|oldDec
...
argument_list|)
decl_stmt|;
comment|// we'll encode the absolute value (sign is separate)
name|oldDec
operator|=
name|oldDec
operator|.
...
argument_list|()
expr_stmt|;
comment|// get the scale factor to turn big decimal into a decimal< 1
comment|// This relies on the BigDecimal precision value, which as of HIVE-10270
comment|// is now different from HiveDecimal.precision()
name|int
name|factor
init|=
name|oldDec
...
name|factor
expr_stmt|;
comment|// convert the absolute big decimal to string
name|oldDec
operator|.
name|scaleByPowerOfTen
argument_list|(
...
argument_list|()
decl_stmt|;
comment|// finally write out the pieces (sign, scale, digits)
name|writeByte
argument_list|(
name|buffer
argument_list|,
...
expr_stmt|;
block|}
comment|// See comments for next method.
specifier|public
specifier|static
name|void
name|serializeHiveDecimal
...
parameter_list|)
block|{
comment|// Get the sign of the decimal.
name|int
name|signum
init|=
name|dec
...
argument_list|()
decl_stmt|;
comment|// Get the 10^N power to turn digits into the desired decimal with a possible
comment|// fractional part.
comment|// To be compatible with the OldHiveDecimal version, zero has factor 1.
name|int
name|factor
decl_stmt|;
if|if
...
expr_stmt|;
block|}
comment|// To make comparisons work properly, the "factor" gets the decimal's sign, too.
name|factor
operator|=
name|signum
operator|==
...
name|factor
expr_stmt|;
comment|// Convert just the decimal digits (no dot, sign, etc) into bytes.
comment|//
comment|// This is much faster than converting the BigInteger value from unscaledValue() which is no
comment|// longer part of the HiveDecimal representation anymore to string, then bytes.
name|int
name|index
init|=
name|dec
...
argument_list|)
decl_stmt|;
comment|/*      * Finally write out the pieces (sign, power, digits)      */
name|writeByte
argument_list|(
name|buffer
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// The toDigitsOnlyBytes stores digits at the end of the scratch buffer.
name|serializeBytes
argument_list|(
name|buffer
argument_list|,
...
expr_stmt|;
block|}
comment|// A HiveDecimalWritable version.
specifier|public
specifier|static
name|void
name|serializeHiveDecimal
