...
parameter_list|()
block|{   }
comment|/**    * Check if this inspector and all its field inspectors are initialized.    */
specifier|protected
name|boolean
name|isFullyInited
parameter_list|(
...
condition|)
block|{
comment|// This object should be the same as in cache, otherwise, it must be removed due to init error
throw|throw
operator|new
name|RuntimeException
argument_list|(
literal|"Cached object inspector is gone while waiting for it to initialize"
argument_list|)
throw|;
block|}
if|if
...
return|;
block|}
comment|// We don't want to check types already checked
name|checkedTypes
operator|.
name|add
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// This inspector is initialized, we still need
comment|// to check if all field inspectors are initialized
for|for
control|(
name|StructField
name|field
...
init|=
name|extractFieldObjectInspectors
argument_list|(
name|objectClass
argument_list|,
name|options
