...
parameter_list|()
block|{
comment|// Parse without using exceptions for better performance.
name|int
name|i
init|=
name|fieldStart
...
expr_stmt|;
block|}
comment|// We tolerate and ignore decimal places.
if|if
condition|(
name|bytes
index|[
...
literal|0
expr_stmt|;
comment|// Fall through below and verify trailing decimal digits.
block|}
else|else
block|{
if|if
...
condition|)
block|{
comment|// Use the old trick of comparing against number string to check for overflow.
if|if
condition|(
operator|!
name|negative
...
condition|)
block|{
comment|// Safe because of our number string comparision against min (negative) long.
name|saveLong
operator|=
operator|-
name|saveLong
...
block|}
block|}
comment|// Fall through to here if we detect the start of trailing decimal digits...
comment|// We verify trailing digits only.
while|while
condition|(
literal|true
condition|)
