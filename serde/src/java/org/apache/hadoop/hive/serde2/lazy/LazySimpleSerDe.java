...
throw|;
block|}
comment|// If the field that is passed in is a primitive, then we serialize the
comment|// primitive object.
if|if
condition|(
name|foi
operator|.
...
block|{
comment|// If the field is not declared (no schema was given at
comment|// initialization), or the field is declared as a primitive in
comment|// initialization, serialize the data to JSON string.
comment|// Otherwise serialize the data in the delimited way.
if|if
condition|(
name|declaredFields
operator|==
...
return|;
block|}
comment|/**    * Serialize the row into the StringBuilder.    * @param sb  The StringBuilder to store the serialized data.    * @param obj The object for the current field.    * @param objInspector  The ObjectInspector for the current Object.    * @param separators    The separators array.    * @param level         The current level of separator.    * @param nullString    The byte sequence representing the NULL value.    */
specifier|private
name|void
name|serialize
parameter_list|(
