...
literal|"10000"
decl_stmt|;
comment|// To parse the intermediate URI as a Java URI, we'll give a dummy authority(dummyhost:00000).
...
condition|)
block|{
name|configureConnParamsFromZooKeeper
argument_list|(
name|connParams
argument_list|)
expr_stmt|;
block|}
name|handleAllDeprecations
argument_list|(
name|connParams
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**    * This method handles the base parsing of the given jdbc uri. Some of JdbcConnectionParams    * returned from this method are updated if ZooKeeper is used for service discovery    *    * @param uri    * @param info    * @return    * @throws JdbcUriParseException    */
specifier|public
specifier|static
name|JdbcConnectionParams
name|extractURLComponents
...
name|String
name|authorityFromClientJdbcURL
init|=
name|getAuthorityFromJdbcURL
argument_list|(
name|uri
argument_list|)
decl_stmt|;
...
argument_list|()
decl_stmt|;
comment|// If we're using ZooKeeper, the final host, port will be read from ZooKeeper
comment|// (in a different method call). Therefore, we put back the original authority string
comment|// (which basically is the ZooKeeper ensemble) back in the uri
if|if
condition|(
name|ZooKeeperHiveClientHelper
operator|.
...
argument_list|()
decl_stmt|;
comment|// Replace ZooKeeper ensemble from the authority component of the JDBC Uri provided by the
comment|// client, by the host:port of the resolved server instance we will connect to
name|connParams
operator|.
name|setJdbcUriString
argument_list|(
