...
block|}
block|}
comment|/**    * Poll the result set status by checking if isSetHasResultSet is set    * @return    * @throws SQLException    */
specifier|private
name|TGetOperationStatusResp
name|waitForResultSetStatus
parameter_list|()
...
condition|)
block|{
comment|/**        * progress bar is completed if there is nothing we want to request in the first place.        */
name|inPlaceUpdateStream
operator|.
...
condition|)
do|;
comment|/*       we set progress bar to be completed when hive query execution has completed     */
name|inPlaceUpdateStream
operator|.
name|getEventNotifier
argument_list|()
...
argument_list|)
expr_stmt|;
comment|/**      * Poll on the operation status, till the operation is complete. We want to ensure that since a      * client might end up using executeAsync and then call this to check if the query run is      * finished.      */
name|long
...
name|warn
argument_list|(
literal|"Number of rows is greater than Integer.MAX_VALUE"
argument_list|)
expr_stmt|;
return|return
operator|-
...
name|SQLException
argument_list|(
literal|"Error building result set for query log: "
operator|+
name|e
argument_list|,
name|e
...
block|}
block|}
comment|/**    * Returns the Yarn ATS GUID.    * This method is a public API for usage outside of Hive, although it is not part of the    * interface java.sql.Statement.    * @return Yarn ATS GUID or null if it hasn't been created yet.    */
specifier|public
name|String
name|getYarnATSGuid
parameter_list|()
...
return|;
block|}
comment|/**    * This is only used by the beeline client to set the stream on which in place progress updates    * are to be shown    */
specifier|public
name|void
