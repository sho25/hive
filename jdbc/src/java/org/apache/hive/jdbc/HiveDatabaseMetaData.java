...
specifier|final
class|class
name|ClientInfoPropertiesResultSet
extends|extends
...
block|}
block|,
comment|// Note: other standard ones include e.g. ClientUser and ClientHostname,
comment|//       but we don't need them for now.
block|}
decl_stmt|;
specifier|private
name|int
...
name|SQLException
block|{
comment|// TODO: perhaps this could use a better implementation... for now even the Hive query result
comment|//       set doesn't support this, so assume the user knows what he's doing when calling us.
return|return
operator|(
name|T
operator|)
