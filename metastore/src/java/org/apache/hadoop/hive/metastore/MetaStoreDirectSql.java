...
name|runDbCheck
argument_list|(
literal|"SELECT version FROM v$instance"
argument_list|,
literal|"Oracle"
argument_list|)
condition|)
...
block|{
comment|// TODO: maybe we should use getProductName to identify all the DBs
name|String
name|productName
init|=
name|getProductName
...
name|name
operator|+
literal|" check failed, assuming we are not on "
operator|+
name|name
operator|+
literal|": "
...
block|}
comment|// Get full objects. For Oracle, do it in batches.
name|List
argument_list|<
name|Partition
argument_list|>
...
operator|+
literal|"min(\"BIG_DECIMAL_LOW_VALUE\"), max(\"BIG_DECIMAL_HIGH_VALUE\"), sum(\"NUM_NULLS\"), max(\"NUM_DISTINCTS\"), "
operator|+
literal|"max(\"AVG_COL_LEN\"), max(\"MAX_COL_LEN\"), sum(\"NUM_TRUES\"), sum(\"NUM_FALSES\") from \"PART_COL_STATS\""
operator|+
literal|" where \"DB_NAME\" = ? and \"TABLE_NAME\" = ? "
decl_stmt|;
name|String
...
name|queryText
operator|=
literal|"select \"COLUMN_NAME\", sum(\"NUM_NULLS\"), sum(\"NUM_TRUES\"), sum(\"NUM_FALSES\")"
operator|+
literal|" from \"PART_COL_STATS\""
operator|+
literal|" where \"DB_NAME\" = ? and \"TABLE_NAME\" = ? "
...
name|colType
expr_stmt|;
comment|// use linear extrapolation. more complicated one can be added in the future.
name|IExtrapolatePartStatus
...
argument_list|)
decl_stmt|;
comment|//if the colType is not the known type, long, double, etc, then get all index.
if|if
condition|(
name|index
operator|==
