...
return|;
block|}
comment|/**    * Drop an MPartition and cascade deletes (e.g., delete partition privilege grants,    *   drop the storage descriptor cleanly, etc.)    * @param part - the MPartition to drop    * @return whether the transaction committed successfully    */
specifier|private
name|boolean
name|dropPartitionCommon
parameter_list|(
...
return|;
block|}
comment|/**    * Makes a JDO query filter string    * if mtable is not null, generates the query to filter over partitions in a table.    * if mtable is null, generates the query to filter over tables in a database    */
specifier|private
name|String
...
specifier|private
name|String
name|makeTableQueryFilterString
parameter_list|(
name|String
name|filter
parameter_list|,
...
name|info
argument_list|(
literal|"Looking for location in DB_LOCATION_URI field in DBS table..."
argument_list|)
expr_stmt|;
for|for
control|(
...
name|error
argument_list|(
literal|"Encountered error while validating location URI"
operator|+
name|e
operator|.
...
expr_stmt|;
block|}
comment|// locationURI is a valid URI
if|if
condition|(
name|locationURI
operator|!=
...
specifier|private
name|int
name|updateMStorageDescriptorURI
parameter_list|(
name|URI
name|oldLoc
parameter_list|,
...
name|info
argument_list|(
literal|"Looking for location in LOCATION field in SDS table..."
argument_list|)
expr_stmt|;
for|for
control|(
...
name|info
argument_list|(
literal|"Looking for location in the value field of schema.url key in SERDES table..."
argument_list|)
expr_stmt|;
for|for
control|(
...
expr_stmt|;
block|}
comment|// schemaLocURI is a valid URI
if|if
condition|(
name|schemaLocURI
operator|!=
...
return|;
block|}
comment|/** The following APIs   *   *  - updateFSRootLocation   *   * is used by HiveMetaTool. This API **shouldn't** be exposed via Thrift.   *   */
annotation|@
name|SuppressWarnings
argument_list|(
literal|"finally"
...
name|info
argument_list|(
literal|"Updating FS root location..."
argument_list|)
expr_stmt|;
try|try
block|{
...
argument_list|()
expr_stmt|;
comment|// update locationURI in mDatabase
name|count
operator|=
name|updateMDatabaseURI
argument_list|(
...
name|count
expr_stmt|;
comment|// update location in mStorageDescriptor
name|count
operator|=
name|updateMStorageDescriptorURI
argument_list|(
...
name|count
expr_stmt|;
comment|// upgrade schema.url for avro serde
name|count
operator|=
name|updateAvroSerdeURI
argument_list|(
