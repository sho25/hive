...
decl_stmt|;
comment|/**    * A class to pass the Query object to the caller to let the caller release    * resources by calling QueryWrapper.query.closeAll() after consuming all the query results.    */
specifier|public
specifier|static
class|class
name|QueryWrapper
...
name|msg
init|=
literal|"commitTransactionExpectDeadlock cannot be called for a nested transaction"
decl_stmt|;
name|LOG
operator|.
name|error
...
name|info
argument_list|(
literal|"Deadlock exception during commit: "
operator|+
name|candidate
operator|.
name|getMessage
...
argument_list|)
expr_stmt|;
comment|// Add primary keys and foreign keys.
comment|// We need not do a deep retrieval of the Table Column Descriptor while persisting the PK/FK
comment|// since this transaction involving create table is not yet committed.
name|addPrimaryKeys
argument_list|(
name|primaryKeys
argument_list|,
...
expr_stmt|;
block|}
comment|// TODO# temporary; will be removed with ACID. Otherwise, need to do direct delete w/o get.
name|List
argument_list|<
name|MTableWrite
argument_list|>
...
argument_list|)
expr_stmt|;
comment|// CDs are reused; go thry partition SDs, detach all CDs from SDs, then remove unused CDs.
for|for
control|(
name|MColumnDescriptor
name|mcd
...
name|warn
argument_list|(
literal|"Error reporting Direct SQL errors to metrics system"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|debug
argument_list|(
literal|"Done retrieving all objects for getIndexes"
argument_list|)
expr_stmt|;
return|return
name|indexes
...
name|queryStr
init|=
literal|"select functionName from org.apache.hadoop.hive.metastore.model.MFunction "
operator|+
literal|"where database.name == dbName&& ("
decl_stmt|;
name|boolean
...
name|class
argument_list|,
literal|"table.tableName == t1&& table.database.name == t2&& writeId == t3"
argument_list|)
expr_stmt|;
name|query
operator|.
name|declareParameters
argument_list|(
literal|"java.lang.String t1, java.lang.String t2, java.lang.Long t3"
argument_list|)
expr_stmt|;
annotation|@
name|SuppressWarnings
...
name|MetaException
argument_list|(
literal|"More than one TableWrite for "
operator|+
name|dbName
operator|+
literal|"."
...
name|newQuery
argument_list|(
literal|"select writeId from org.apache.hadoop.hive.metastore.model.MTableWrite"
operator|+
literal|" where table.tableName == t1&& table.database.name == t2&& writeId> t3"
operator|+
literal|"&& writeId< t4"
operator|+
operator|(
...
name|class
argument_list|,
literal|"table.tableName == t1&& table.database.name == t2&& writeId> t3&& writeId< t4"
argument_list|)
expr_stmt|;
name|query
operator|.
name|declareParameters
argument_list|(
literal|"java.lang.String t1, java.lang.String t2, java.lang.Long t3, java.lang.Long t4"
argument_list|)
expr_stmt|;
name|query
operator|.
...
try|try
block|{
comment|// If the table had no MM writes, there's nothing to clean up
name|query
operator|=
name|pm
operator|.
...
name|q
init|=
literal|"select sd.location from org.apache.hadoop.hive.metastore.model.MPartition"
operator|+
literal|" where table.tableName == t1&& table.database.name == t2"
decl_stmt|;
name|query
operator|=
name|pm
