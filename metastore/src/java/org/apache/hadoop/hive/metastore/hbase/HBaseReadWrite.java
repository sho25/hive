...
return|;
block|}
comment|/**    * Given a role, find all users who are either directly or indirectly participate in this role.    * This is expensive, it should be used sparingly.  It scan the entire userToRole table and    * does a linear search on each entry.    * @param roleName name of the role    * @return set of all users in the role    * @throws IOException    * @throws NoSuchObjectException    */
name|Set
argument_list|<
name|String
argument_list|>
...
condition|)
block|{
comment|// Happens the first time a principal is added to a role
name|rolePrincipals
operator|=
operator|new
name|GrantInfoList
...
condition|)
block|{
comment|// Means there aren't any principals in this role, so probably not a problem.
return|return;
block|}
for|for
control|(
...
expr_stmt|;
block|}
comment|/**    * Remove all of the grants for a role.  This is not cheap.    * @param roleName    * @throws IOException    */
name|void
