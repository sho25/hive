...
argument_list|)
expr_stmt|;
comment|/*           List<byte[]> columnFamilies = new ArrayList<byte[]>();           columnFamilies.add(CATALOG_CF);           if (TABLE_TABLE.equals(name) || PART_TABLE.equals(name)) {             columnFamilies.add(STATS_CF);           }           self.get().conn.createHBaseTable(name, columnFamilies);           */
block|}
block|}
name|tablesCreated
operator|=
...
argument_list|()
decl_stmt|;
comment|// Initialize the list and build the Gets
for|for
control|(
name|int
name|pOff
...
control|)
block|{
comment|// Add an entry for this partition in the stats list
name|partitionStats
operator|=
operator|new
name|ColumnStatistics
...
argument_list|)
expr_stmt|;
comment|// Build the list of Gets
for|for
control|(
name|int
name|i
...
argument_list|)
decl_stmt|;
comment|// Get results from HBase
name|Result
index|[]
name|results
init|=
...
argument_list|)
decl_stmt|;
comment|// Deserialize the stats objects and add to stats list
for|for
control|(
name|int
name|pOff
...
return|;
block|}
comment|/**    * Get aggregate stats for a column from the DB and populate the bloom filter if it's not null    * @param dbName    * @param tblName    * @param partNames    * @param partVals    * @param colNames    * @return    * @throws IOException    */
name|AggrStats
name|getAggrStats
parameter_list|(
name|String
...
name|IOException
block|{
comment|// One ColumnStatisticsObj per column
name|List
argument_list|<
name|ColumnStatisticsObj
argument_list|>
...
argument_list|()
decl_stmt|;
comment|// TODO: Steal extrapolation logic from current MetaStoreDirectSql code
comment|// Right now doing nothing and keeping partitionsFound == partitionsRequested
name|int
name|partitionsFound
init|=
name|partitionsRequested
decl_stmt|;
for|for
...
condition|)
block|{
comment|// Read from HBase but don't add to cache since it doesn't qualify the criteria
name|colStatsAggr
operator|=
name|getAggrStatsFromDB
argument_list|(
name|dbName
argument_list|,
name|tblName
...
return|;
block|}
comment|/**    *    * @param dbName    * @param tblName    * @param partNames    * @param partVals    * @param colName    * @param bloomFilter    * @return    */
specifier|private
name|ColumnStatisticsObj
name|getAggrStatsFromDB
parameter_list|(
...
argument_list|)
decl_stmt|;
comment|// Build a list of Gets, one per partition
for|for
control|(
name|int
name|pOff
...
comment|// The results size and order is the same as the number and order of the Gets
comment|// If the column is not present in a partition, the Result object will be empty
for|for
control|(
name|int
name|pOff
...
condition|)
block|{
comment|// This is the 1st column stats object we got
name|colStatsAggr
operator|.
name|setColName
argument_list|(
...
block|{
comment|// Perform aggregation with whatever we've already aggregated
name|colStatsAggregator
operator|.
name|aggregate
argument_list|(
...
expr_stmt|;
block|}
comment|// Add partition to the bloom filter if it's requested
if|if
condition|(
name|bloomFilter
operator|!=
