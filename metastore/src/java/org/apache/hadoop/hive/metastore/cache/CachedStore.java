...
name|startTimer
argument_list|(
literal|"getColStatsForTablePartitions"
argument_list|)
expr_stmt|;
name|Map
argument_list|<
...
name|info
argument_list|(
literal|"CachedStore: starting cache update service (run every "
operator|+
name|cacheRefreshPeriod
operator|+
literal|"ms"
...
argument_list|)
expr_stmt|;
comment|// Update the partitions column stats for a table in cache
...
block|}
block|}
comment|// Update the cached partition col stats for a table
specifier|private
name|void
name|updateTablePartitionColStats
parameter_list|(
...
name|info
argument_list|(
literal|"Updating CachedStore: unable to read partitions column stats of table: "
operator|+
name|tblName
argument_list|,
name|e
...
block|{
comment|// TODO filter -> expr
comment|// SharedCache sharedCache = sharedCacheWrapper.get();
comment|// if (sharedCache == null) {
return|return
name|rawStore
operator|.
name|getNumPartitionsByFilter
...
return|;
comment|// }
comment|// Table table = sharedCache.getTableFromCache(HiveStringUtils.normalizeIdentifier(dbName),
comment|//     HiveStringUtils.normalizeIdentifier(tblName));
comment|// return 0;
block|}
annotation|@
name|Override
...
comment|// Need to see if it makes sense to do this as some col stats maybe out of date/missing on cache.
specifier|public
name|List
argument_list|<
name|ColumnStatistics
...
comment|// itself will tell whether
comment|// bitvector is null or not and aggr logic can automatically apply.
return|return
name|MetaStoreUtils
operator|.
