...
block|}
return|return
name|buildSerializedPartitionKey
argument_list|(
name|dbName
argument_list|,
name|tableName
...
name|RuntimeException
argument_list|(
literal|"Cannot serialize partition "
operator|+
name|StringUtils
operator|.
name|join
...
name|keyParts
init|=
name|desierliazeDbNameTableNameFromPartitionKey
argument_list|(
name|key
argument_list|,
name|callback
...
name|RuntimeException
argument_list|(
literal|"Can't have a null dbname or tablename"
argument_list|)
throw|;
block|}
else|else
...
name|RuntimeException
argument_list|(
literal|"Error when deserialize key"
argument_list|,
name|e
argument_list|)
throw|;
block|}
block|}
comment|// Deserialize a partition key and return _only_ the partition values.
specifier|private
specifier|static
name|List
argument_list|<
...
expr_stmt|;
block|}
comment|// Set only if table is temporary
if|if
condition|(
name|table
operator|.
...
return|;
block|}
comment|/**    * Serialize an index    * @param index index object    * @param sdHash hash that is being used as a key for the enclosed storage descriptor    * @return First element is the key, second is the serialized index    */
specifier|static
name|byte
index|[]
index|[]
...
return|;
block|}
comment|/**    * Deserialize an index.  This version should be used when the index key is not already    * known (eg a scan).    * @param key the key fetched from HBase    * @param serialized the value fetched from HBase    * @return A struct that contains the index plus parts of the storage descriptor    */
specifier|static
name|StorageDescriptorParts
...
return|;
block|}
comment|/**    * Deserialize an index.  This version should be used when the table key is    * known (eg a get).    * @param dbName database name    * @param origTableName original table name    * @param indexName index name    * @param serialized the value fetched from HBase    * @return A struct that contains the index plus parts of the storage descriptor    */
specifier|static
name|StorageDescriptorParts
name|deserializeIndex
parameter_list|(
...
return|;
block|}
comment|/**    * Serialize a delegation token    * @param tokenIdentifier    * @param delegationToken    * @return two byte arrays, first contains the key, the second the serialized value.    */
specifier|static
name|byte
index|[]
index|[]
...
return|;
block|}
comment|/**    * Deserialize a delegation token.    * @param value value fetched from hbase    * @return A delegation token.    * @throws InvalidProtocolBufferException    */
specifier|static
name|String
...
return|;
block|}
comment|/**    * Serialize a master key    * @param seqNo    * @param key    * @return two byte arrays, first contains the key, the second the serialized value.    */
specifier|static
name|byte
index|[]
index|[]
...
return|;
block|}
comment|/**    * Deserialize a master key.    * @param value value fetched from hbase    * @return A master key    * @throws InvalidProtocolBufferException    */
specifier|static
name|String
...
return|;
block|}
comment|/**    * Serialize the primary key for a table.    * @param pk Primary key columns.  It is expected that all of these match to one pk, since    *           anything else is meaningless.    * @return two byte arrays, first containts the hbase key, the second the serialized value.    */
specifier|static
name|byte
index|[]
index|[]
...
parameter_list|)
block|{
comment|// First, figure out the dbName and tableName.  We expect this to match for all list entries.
name|byte
index|[]
index|[]
name|result
...
argument_list|()
decl_stmt|;
comment|// Encode the primary key, if present
name|builder
operator|.
name|setPkName
argument_list|(
...
return|;
block|}
comment|/**    * Serialize the foreign key(s) for a table.    * @param fks Foreign key columns.  These may belong to multiple foreign keys.    * @return two byte arrays, first containts the key, the second the serialized value.    */
specifier|static
name|byte
index|[]
index|[]
...
argument_list|()
decl_stmt|;
comment|// Encode any foreign keys we find.  This can be complex because there may be more than
comment|// one foreign key in here, so we need to detect that.
name|Map
argument_list|<
name|String
argument_list|,
...
condition|)
block|{
comment|// We haven't seen this key before, so add it
name|fkBuilder
operator|=
name|HbaseMetastoreProto
operator|.
...
argument_list|)
operator|:
literal|"You switched databases on me!"
assert|;
name|fkBuilder
operator|.
...
argument_list|)
operator|:
literal|"You switched tables on me!"
assert|;
name|fkBuilder
operator|.
name|setReferencedPkName
argument_list|(
name|fkcol
operator|.
name|getPk_name
...
return|;
block|}
comment|/**    * Serialize the unique constraint(s) for a table.    * @param uks Unique constraint columns.  These may belong to multiple unique constraints.    * @return two byte arrays, first contains the key, the second the serialized value.    */
specifier|static
name|byte
index|[]
index|[]
...
return|;
block|}
comment|/**    * Serialize the not null constraint(s) for a table.    * @param nns Not null constraint columns.  These may belong to multiple constraints.    * @return two byte arrays, first contains the constraint, the second the serialized value.    */
specifier|static
name|byte
index|[]
index|[]
...
name|proto
operator|.
name|getEnableConstraint
argument_list|()
argument_list|,
name|proto
operator|.
name|getValidateConstraint
argument_list|()
argument_list|,
name|proto
operator|.
name|getRelyConstraint
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
