...
return|;
block|}
comment|// TODO: support for other ops for numbers to be handled in HIVE-4888.
return|return
name|isReverseOrder
condition|?
name|paramName
...
return|;
block|}
comment|/**      * Get partition column index in the table partition column list that      * corresponds to the key that is being filtered on by this tree node.      * @param table The table.      * @return The index.      */
specifier|public
name|int
name|getPartColIndexForFilter
parameter_list|(
...
return|;
block|}
comment|/**      * Validates and gets the query parameter for filter pushdown based on the column      * and the constant stored in this node.      * In future this may become different for SQL and JDOQL filter pushdown.      * @param table The table.      * @param partColIndex The index of the column to check.      * @return The parameter string.      */
specifier|public
name|String
name|getFilterPushdownParam
parameter_list|(
...
block|}
block|}
comment|/**    * For equals and not-equals, we can make the JDO query much faster by filtering    * based on the partition name. For a condition like ds="2010-10-01", we can see    * if there are any partitions with a name that contains the substring "ds=2010-10-01/"    * False matches aren't possible since "=" is escaped for partition names    * and the trailing '/' ensures that we won't get a match with ds=2010-10-011    * Note that filters on integral type equality also work correctly by virtue of    * comparing them as part of ds=1234 string.    *    * Two cases to keep in mind: Case with only one partition column (no '/'s)    * Case where the partition key column is at the end of the name. (no    * tailing '/')    *    * @param keyName name of the partition col e.g. ds    * @param value    * @param paramName name of the parameter to use for JDOQL    * @param params a map from the parameter name to their values    * @param isEq whether the operator is equals, or not-equals.    * @return    * @throws MetaException    */
specifier|private
specifier|static
name|String
name|makeFilterForEquals
