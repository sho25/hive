...
block|{
name|int
name|deadlockTryCount
init|=
literal|10
decl_stmt|;
name|int
name|deadlockRetryBackoffMs
init|=
literal|200
decl_stmt|;
while|while
...
argument_list|()
expr_stmt|;
comment|// Exception should propagate; don't override it by breaking out of the loop.
block|}
else|else
block|{
name|Boolean
...
condition|)
break|break;
comment|// Assume no exception; ok to break out of the loop.
throw|throw
operator|new
name|MetaException
argument_list|(
...
name|warn
argument_list|(
literal|"Getting the next write ID failed due to a deadlock; retrying"
argument_list|)
expr_stmt|;
name|Thread
operator|.
...
expr_stmt|;
block|}
comment|// Do a separate txn after we have reserved the number.
name|boolean
name|ok
init|=
literal|false
...
name|tblName
operator|+
literal|" does not exist or is not active"
argument_list|)
throw|;
block|}
name|tw
...
name|MetaException
argument_list|(
literal|"The write was aborted due to absolute timeout"
argument_list|)
throw|;
return|return
operator|new
...
condition|)
block|{
comment|// There may be some intermediate failed or active writes; get the valid ones.
name|List
argument_list|<
name|Long
argument_list|>
...
argument_list|)
decl_stmt|;
comment|// TODO: we could optimize here and send the smaller of the lists, and also use ranges
if|if
condition|(
operator|!
name|ids
...
argument_list|)
expr_stmt|;
name|startMmHousekeepingThread
argument_list|(
name|conf
argument_list|)
expr_stmt|;
