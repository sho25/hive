...
name|debug
argument_list|(
literal|"Admin role already created previously."
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|forName
argument_list|(
literal|"org.apache.hadoop.hive.ql.security.authorization.plugin.HiveAuthorizerFactory"
argument_list|)
expr_stmt|;
block|}
catch|catch
...
name|debug
argument_list|(
literal|"No auth manager specified"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|warn
argument_list|(
literal|"Configured auth manager "
operator|+
name|authCls
operator|.
name|getName
argument_list|()
operator|+
literal|" doesn't implement "
operator|+
name|ConfVars
operator|.
name|HIVE_AUTHENTICATOR_MANAGER
operator|+
literal|" admin role will not be created."
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|userStr
operator|+
literal|" is in incorrect format."
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|shouldAdd
assert|;
comment|// start would thrrow if it already existed here
name|boolean
name|madeDir
init|=
name|createLocationForAddedPartition
...
throw|;
block|}
comment|// Not sure if we can verify that the class is actually a UDF,
comment|// since from metastore we do not have access to ql where UDF classes live.
comment|// We can at least verify that the class name is a valid class.
try|try
block|{
name|Class
argument_list|<
