...
argument_list|()
decl_stmt|;
comment|// This gets all the partitions and then filters based on the specified
comment|// criteria. An alternative approach would be to get all the partition
comment|// names, do the filtering on the names, and get the partition for each
comment|// of the names. that match.
try|try
block|{
name|parts
operator|=
...
argument_list|()
decl_stmt|;
comment|// Since we are iterating through a LinkedHashMap, iteration should
comment|// return the partition values in the correct order for comparison.
for|for
control|(
name|String
name|val
...
name|incrementCounter
argument_list|(
literal|"get_delegation_token_with_signature"
argument_list|)
expr_stmt|;
name|logStartFunction
argument_list|(
...
expr_stmt|;
block|}
comment|/**    * Get a new delegation token.    * @param renewer the designated renewer    * @param token_signature an identifier that is set as the service on the generated token    */
specifier|public
specifier|static
name|String
name|getDelegationToken
