...
literal|null
decl_stmt|;
comment|// Need to get a Connector so we look up the user's authorizations if not otherwise specified
if|if
condition|(
name|accumuloParams
operator|.
...
argument_list|)
expr_stmt|;
comment|// In a YARN/Tez job, don't have the Kerberos credentials anymore, use the delegation token
name|AuthenticationToken
name|token
init|=
name|ConfiguratorBase
...
condition|)
block|{
comment|// Convert the stub from the configuration back into a normal Token
name|log
operator|.
name|info
argument_list|(
literal|"Found authentication token in Configuration: "
operator|+
name|token
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Converted authentication token from Configuration into: "
operator|+
name|unwrappedToken
argument_list|)
expr_stmt|;
comment|// It's possible that the Job doesn't have the token in its credentials. In this case, unwrapAuthenticatinoToken
comment|// will return back the original token (which we know is insufficient)
if|if
condition|(
name|unwrappedToken
operator|!=
...
name|info
argument_list|(
literal|"Creating Accumulo Connector with unwrapped delegation token"
argument_list|)
expr_stmt|;
name|connector
operator|=
...
name|info
argument_list|(
literal|"Job credentials did not contain delegation token, fetching new token"
argument_list|)
expr_stmt|;
block|}
block|}
...
name|info
argument_list|(
literal|"Obtaining Accumulo Connector using KerberosToken"
argument_list|)
expr_stmt|;
comment|// Construct a KerberosToken -- relies on ProxyUser configuration. Will be the client making
comment|// the request on top of the HS2's user. Accumulo will require proper proxy-user auth configs.
name|connector
operator|=
name|instance
operator|.
...
block|{
comment|// Still in the local JVM, use the username+password or Kerberos credentials
name|connector
operator|=
name|accumuloParams
operator|.
...
block|}
block|}
comment|/**    * Setup accumulo input format from conf properties. Delegates to final RecordReader from mapred    * package.    */
annotation|@
name|Override
specifier|public
name|RecordReader
...
expr_stmt|;
block|}
comment|// ACCUMULO-4670 RangeInputSplit doesn't preserve useSasl on the ClientConfiguration/ZooKeeperInstance
comment|// We have to manually re-set it in the JobConf to make sure it gets picked up.
if|if
condition|(
name|accumuloParams
operator|.
