...
init|=
literal|"llap.if.use.new.split.format"
decl_stmt|;
specifier|public
specifier|static
specifier|final
name|String
name|SESSION_QUERIES_FOR_GET_NUM_SPLITS
init|=
literal|"llap.session.queries.for.get.num.splits"
decl_stmt|;
specifier|public
specifier|static
specifier|final
name|Pattern
name|SET_QUERY_PATTERN
init|=
name|Pattern
operator|.
name|compile
...
name|SPLIT_QUERY
init|=
literal|"select get_llap_splits(\"%s\",%d)"
decl_stmt|;
specifier|public
specifier|static
specifier|final
...
expr_stmt|;
block|}
comment|//Exposed only for testing, clients should use LlapBaseInputFormat(boolean, BufferAllocator instead)
specifier|public
name|LlapBaseInputFormat
parameter_list|(
name|boolean
...
condition|)
block|{
comment|//Client provided their own allocator
name|recordReader
operator|=
operator|new
...
block|{
comment|//Client did not provide their own allocator, use constructor for global allocator
name|recordReader
operator|=
operator|new
name|LlapArrowBatchRecordReader
...
return|;
block|}
comment|/**    * Calling getSplits() will open a HiveServer2 connection which should be closed by the calling application    * using LlapBaseInputFormat.close() when the application is done with the splits.    */
annotation|@
name|Override
specifier|public
name|InputSplit
...
name|info
argument_list|(
literal|"Handle ID not specified - generated handle ID {}"
argument_list|,
name|handleId
argument_list|)
expr_stmt|;
...
control|)
block|{
comment|//allow only set queries
if|if
condition|(
name|SET_QUERY_PATTERN
operator|.
...
name|debug
argument_list|(
literal|"Executing session query: {}"
argument_list|,
name|q
argument_list|)
expr_stmt|;
...
name|warn
argument_list|(
literal|"Only SET queries are allowed, not executing this query: {}"
argument_list|,
name|q
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|// In case of USE_NEW_SPLIT_FORMAT=true, following format is used
comment|//       type                  split
comment|// schema-split             LlapInputSplit -- contains only schema
comment|// plan-split               LlapInputSplit -- contains only planBytes[]
comment|// 0                        LlapInputSplit -- actual split 1
comment|// 1                        LlapInputSplit -- actual split 2
comment|// ...                         ...
name|boolean
name|useNewSplitFormat
init|=
name|job
operator|.
name|getBoolean
...
block|{
comment|// to keep the old format, populate schema and planBytes[] in actual splits
if|if
condition|(
name|count
operator|==
...
name|error
argument_list|(
literal|"Closing connection due to error"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
throw|;
block|}
comment|// Keep connection open to hang on to associated resources (temp tables, locks).
comment|// Save to connectionMap so it can be closed at user's convenience.
name|addConnection
argument_list|(
name|handleId
argument_list|,
...
block|}
block|}
comment|/**    * Close the connection associated with the handle ID, if getSplits() was configured with a handle ID.    * Call when the application is done using the splits generated by getSplits().    * @param handleId Handle ID used in configuration for getSplits()    * @throws IOException    */
specifier|public
specifier|static
name|void
name|close
...
name|debug
argument_list|(
literal|"Closing {} connections for handle ID {}"
argument_list|,
name|handleConnections
operator|.
name|size
...
name|error
argument_list|(
literal|"Error while closing connection for "
operator|+
name|handleId
argument_list|,
name|err
...
name|debug
argument_list|(
literal|"No connection found for handle ID {}"
argument_list|,
name|handleId
argument_list|)
expr_stmt|;
block|}
block|}
comment|/**    * Close all outstanding connections created by getSplits() calls    */
specifier|public
specifier|static
name|void
name|closeAll
...
static|static
block|{
comment|// Shutdown hook to clean up resources at process end.
name|ShutdownHookManager
operator|.
...
name|IOException
argument_list|(
literal|"No service instances found in registry"
argument_list|)
throw|;
block|}
block|}
...
name|info
argument_list|(
literal|"Finding random live service instance"
argument_list|)
expr_stmt|;
name|Collection
argument_list|<
