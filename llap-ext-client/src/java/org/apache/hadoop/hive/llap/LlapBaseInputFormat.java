...
expr_stmt|;
block|}
comment|// Check if the handle has already been used in the registry
if|if
condition|(
name|connectionMap
operator|.
...
name|handleId
operator|+
literal|" has already been used. This must be unique."
argument_list|)
throw|;
block|}
try|try
...
argument_list|)
decl_stmt|;
comment|// Hopefully no one has used the same handle ID during the time that we executed the statement.
if|if
condition|(
name|putResult
operator|!=
...
name|debug
argument_list|(
literal|"Closing connection for handle ID {}"
argument_list|,
name|handleId
argument_list|)
expr_stmt|;
