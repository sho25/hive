...
block|}
block|}
comment|/**    * Interface for pluggable token store that can be implemented as shared store with external    * storage (for example with ZooKeeper for HA).    * Internal, store specific errors are translated into {@link TokenStoreError}.    */
specifier|public
specifier|static
interface|interface
name|TokenStore
...
name|TokenStoreError
function_decl|;
comment|/**      * Remove token. Ignores token does not exist.      * @param tokenIdentifier      */
name|boolean
name|removeToken
parameter_list|(
name|DelegationTokenIdentifier
