...
specifier|final
name|int
name|DELIMITED_CANDIDATE_THRESHOLD
init|=
literal|10
decl_stmt|;
specifier|public
...
name|debug
argument_list|(
literal|"CliDriver inited with classpath {}"
argument_list|,
name|System
operator|.
name|getProperty
...
name|printError
argument_list|(
literal|"Failed processing command "
operator|+
name|tokens
index|[
literal|0
...
expr_stmt|;
block|}
comment|// Set HDFS CallerContext to queryId and reset back to sessionId after the query is done
name|ShimLoader
operator|.
name|getHadoopShims
argument_list|()
...
throw|;
block|}
comment|// query has run capture the time
name|long
name|end
init|=
name|System
...
return|;
block|}
comment|/**    * Processes a line of semicolon separated commands    *    * @param line    *          The commands to process    * @param allowInterrupting    *          When true the function will handle SIG_INT (Ctrl+C) by interrupting the processing and    *          returning -1    * @return 0 if ok    */
specifier|public
name|CommandProcessorResponse
name|processLine
parameter_list|(
...
name|ret
decl_stmt|;
comment|// we can not use "split" function directly as ";" may be quoted
name|List
argument_list|<
name|String
argument_list|>
...
block|}
block|}
comment|/**    * Split the line by semicolon by ignoring the ones in the single/double quotes.    *    */
specifier|public
specifier|static
name|List
argument_list|<
...
name|printError
argument_list|(
literal|"Putting the global hiverc in "
operator|+
literal|"$HIVE_HOME/bin/.hiverc is deprecated. Please "
operator|+
literal|"use $HIVE_CONF_DIR/.hiverc instead."
argument_list|)
expr_stmt|;
block|}
block|}
...
block|}
block|}
comment|/**    * Execute the cli work    * @param ss CliSessionState of the CLI driver    * @param conf HiveConf for the driver session    * @param oproc Operation processor of the CLI invocation    * @return status of the CLI command execution    * @throws Exception    */
specifier|private
name|CommandProcessorResponse
name|executeDriver
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// use the specified database if specified
name|cli
operator|.
name|processSelectDatabase
argument_list|(
...
name|println
argument_list|(
literal|"WARNING: Directory for Hive history file: "
operator|+
name|historyDirectory
operator|+
literal|" does not exist.   History will not be available during this session."
argument_list|)
expr_stmt|;
block|}
block|}
...
name|println
argument_list|(
literal|"WARNING: Encountered an error while trying to initialize Hive's "
operator|+
literal|"history file.  History will not be available during this session."
argument_list|)
expr_stmt|;
name|System
operator|.
...
name|println
argument_list|(
literal|"WARNING: Failed to write command history file: "
operator|+
name|e
operator|.
name|getMessage
