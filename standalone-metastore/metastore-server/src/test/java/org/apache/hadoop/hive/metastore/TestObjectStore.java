...
name|Exception
block|{
comment|// Clear the SSL system properties before each test.
name|System
operator|.
name|clearProperty
argument_list|(
...
specifier|public
name|void
name|testConcurrentDropPartitions
parameter_list|()
throws|throws
name|MetaException
throws|,
...
argument_list|)
expr_stmt|;
comment|// Create some partitions
name|List
argument_list|<
name|List
argument_list|<
...
name|assertTrue
argument_list|(
literal|"Expect no active transactions."
argument_list|,
operator|!
name|objectStore
operator|.
...
begin_comment
comment|/**    * Test metastore configuration property METASTORE_MAX_EVENT_RESPONSE    */
end_comment
...
argument_list|)
expr_stmt|;
comment|// Verify if METASTORE_MAX_EVENT_RESPONSE will limit number of events to respond
for|for
control|(
name|int
name|i
...
begin_comment
comment|/**    * This test calls ObjectStore.setConf methods from multiple threads. Each threads uses its    * own instance of ObjectStore to simulate thread-local objectstore behaviour.    * @throws Exception    */
end_comment
...
specifier|public
name|void
name|testConcurrentPMFInitialize
parameter_list|()
throws|throws
name|Exception
block|{
...
decl_stmt|;
comment|// Barrier is used to ensure that all threads start race at the same time
specifier|final
name|int
name|numThreads
init|=
...
name|Exception
block|{
comment|// each thread gets its own ObjectStore to simulate threadLocal store
name|ObjectStore
name|objectStore
init|=
operator|new
...
control|)
block|{
comment|// set connectionPool to a random value to increase the likelihood of pmf
comment|// re-initialization
name|int
name|randomNumber
init|=
name|random
...
name|assertEquals
argument_list|(
literal|"Unexpected number of setConf calls"
argument_list|,
name|numIteration
operator|*
name|numThreads
...
begin_comment
comment|/**    * Test the SSL configuration parameters to ensure that they modify the Java system properties correctly.    */
end_comment
...
specifier|public
name|void
name|testDeprecatedConfigIsOverwritten
parameter_list|()
block|{
comment|// Different from the values in the safe config
name|MetastoreConf
operator|.
name|setVar
argument_list|(
