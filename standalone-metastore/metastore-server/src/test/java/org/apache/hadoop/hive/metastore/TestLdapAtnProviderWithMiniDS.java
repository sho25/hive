...
begin_comment
comment|/**  * TestSuite to test Metastore's LDAP Authentication provider with an  * in-process LDAP Server (Apache Directory Server instance).  *  */
end_comment
...
block|}
argument_list|)
comment|// standalone-metastore/metastore-server/src/test/resources/log4j2.properties sets root logger
comment|// level to debug. All the logs are captured by CapturingLogAppender which creates an ArrayList
comment|// out of those. The classes in org .apache.directory produce a lot of debug output. Hence
comment|// running this test causes Java Heap OOM. If we change the logging level to "info" in log4j2
comment|// .properties file, org.apache.hadoop.hive.metastore.metrics.TestMetrics fails since that relies
comment|// on DEBUG level logging being captured. Even we if disable CapturingLogAppender, the debug
comment|// output slows down the test and it takes a lot of time. So, for now disabling this test. This
comment|// test is copy of org.apache.hive.service.auth.TestLdapAtnProviderWithMiniDS, which is passing.
comment|// Once we deduplicate the LDAP code, this separate test will not be required. Till then we keep
comment|// it here in case somebody wants to run it.
annotation|@
name|Ignore
specifier|public
class|class
