...
specifier|public
name|void
name|testNoPartitionDiscoveryForReplTable
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// table property is set to true, but the table is marked as replication target. The new
comment|// partitions should not be created
name|table
operator|.
name|getParameters
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// Delete location of one of the partitions. The partition discovery task should not drop
comment|// that partition.
name|boolean
name|deleted
init|=
name|fs
...
specifier|public
name|void
name|testNoPartitionRetentionForReplTarget
parameter_list|()
throws|throws
name|TException
throws|,
...
argument_list|)
decl_stmt|;
comment|// Check for the existence of partitions 10 seconds after the partition retention period has
comment|// elapsed. Gives enough time for the partition retention task to work.
name|long
name|partitionRetentionPeriodMs
init|=
literal|20000
decl_stmt|;
name|long
name|waitingPeriodForTest
init|=
name|partitionRetentionPeriodMs
operator|+
literal|10
...
argument_list|)
expr_stmt|;
comment|// after 30s all partitions should remain in-tact for a table which is target of replication.
name|Thread
operator|.
name|sleep
argument_list|(
