...
parameter_list|()
block|{
comment|// allowing this to be increased via config breaks the merge impl
comment|// p=10 = ~1kb per vector or smaller
name|this
operator|.
name|numRegisterIndexBits
operator|=
...
name|IllegalArgumentException
argument_list|(
literal|"HyperLogLog cannot merge a smaller p into a larger one : "
operator|+
name|toString
argument_list|()
operator|+
...
expr_stmt|;
block|}
comment|/**    * Reduces the accuracy of the HLL provided to a smaller size    * @param p0     *         - new p size for the new HyperLogLog (smaller or no change)    * @return reduced (or same) HyperLogLog instance    */
specifier|public
name|HyperLogLog
name|squash
parameter_list|(
...
name|IllegalArgumentException
argument_list|(
literal|"HyperLogLog cannot be be squashed to be bigger. Current: "
operator|+
name|toString
argument_list|()
operator|+
