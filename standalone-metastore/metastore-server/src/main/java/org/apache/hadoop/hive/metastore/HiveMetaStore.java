...
name|transformer
decl_stmt|;
comment|// Variables for metrics
comment|// Package visible so that HMSMetricsListener can see them.
specifier|static
name|AtomicInteger
name|databaseCount
decl_stmt|,
...
block|}
decl_stmt|;
comment|/**      * Thread local HMSHandler used during shutdown to notify meta listeners      */
specifier|private
specifier|static
specifier|final
name|ThreadLocal
...
argument_list|()
decl_stmt|;
comment|/**      * Thread local Map to keep track of modified meta conf keys      */
specifier|private
specifier|static
specifier|final
name|ThreadLocal
...
argument_list|>
argument_list|>
name|threadLocalModifiedConfig
init|=
operator|new
name|ThreadLocal
argument_list|<>
...
block|}
decl_stmt|;
comment|/**      * Internal function to notify listeners for meta config change events      */
specifier|private
name|void
name|notifyMetaListeners
parameter_list|(
name|String
name|key
parameter_list|,
...
condition|)
block|{
comment|// All the fields of this event are final, so no reason to create a new one for each
comment|// listener
name|ConfigChangeEvent
name|cce
init|=
...
block|}
block|}
comment|/**      * Internal function to notify listeners to revert back to old values of keys      * that were modified during setMetaConf. This would get called from HiveMetaStore#cleanupRawStore      */
specifier|private
name|void
name|notifyMetaListenersOnShutDown
parameter_list|()
block|{
name|Map
argument_list|<
...
name|MetaException
argument_list|(
literal|"Unexpected: modifiedConf is non-null but conf is null"
argument_list|)
throw|;
block|}
comment|// Notify listeners of the changed value
for|for
control|(
name|Entry
argument_list|<
...
argument_list|()
decl_stmt|;
comment|// curr value becomes old and vice-versa
name|String
name|currVal
init|=
name|entry
...
name|error
argument_list|(
literal|"Failed to notify meta listeners on shutdown: "
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|// Make it possible for tests to check that the right type of PartitionExpressionProxy was
...
return|;
block|}
comment|/**      * Use {@link #getThreadId()} instead.      * @return thread id      */
annotation|@
name|Deprecated
specifier|public
specifier|static
...
block|}
block|}
comment|/**      * Use {@link #getConf()} instead.      * @return Configuration object      */
annotation|@
name|Deprecated
specifier|public
name|Configuration
...
name|info
argument_list|(
literal|"Begin calculating metadata count metrics."
argument_list|)
expr_stmt|;
name|Metrics
operator|.
...
block|}
name|boolean
name|canCachedStoreCanUseEvent
init|=
literal|false
decl_stmt|;
for|for
...
name|MetaException
argument_list|(
literal|"CahcedStore can not use events for invalidation as there is no "
operator|+
literal|" TransactionalMetaStoreEventListener to add events to notification table"
argument_list|)
throw|;
block|}
name|endFunctionListeners
...
expr_stmt|;
block|}
comment|// We only initialize once the tasks that need to be run periodically. For remote metastore
comment|// these threads are started along with the other housekeeping threads only in the leader
comment|// HMS.
name|String
name|leaderHost
init|=
...
name|info
argument_list|(
literal|"Not starting tasks specified by "
operator|+
name|ConfVars
operator|.
...
name|leaderHost
operator|+
literal|" is configured to run these tasks."
argument_list|)
expr_stmt|;
block|}
name|expressionProxy
...
name|IllegalArgumentException
argument_list|(
literal|"Illegal implementation for metadata transformer. Constructor is private"
argument_list|)
throw|;
name|transformer
operator|=
...
argument_list|)
expr_stmt|;
comment|// For backwards compatibility, since some threads used to be hard coded but only run if
comment|// frequency was> 0
if|if
condition|(
name|freq
operator|>
...
block|}
block|}
comment|/**      *      * Filter is actually enabled only when the configured filter hook is configured, not default, and      * enabled in configuration      * @return      */
specifier|private
name|boolean
name|getIfServerFilterenabled
parameter_list|()
...
name|info
argument_list|(
literal|"HMS server filtering is disabled by configuration"
argument_list|)
expr_stmt|;
return|return
literal|false
...
name|MetaException
argument_list|(
literal|"HMS server filtering is enabled but no filter hook is configured"
argument_list|)
throw|;
block|}
if|if
...
name|MetaException
argument_list|(
literal|"HMS server filtering is enabled but the filter hook is DefaultMetaStoreFilterHookImpl, which does no filtering"
argument_list|)
throw|;
block|}
name|LOG
...
name|info
argument_list|(
literal|"HMS server filtering is enabled. The filter class is "
operator|+
name|filterHookClassName
argument_list|)
expr_stmt|;
...
name|errorMsg
init|=
literal|"Unable to load filter hook at HMS server. "
decl_stmt|;
name|String
name|filterHookClassName
init|=
...
return|;
block|}
comment|/**      * Set copy of invoking HMSHandler on thread local      */
specifier|private
specifier|static
name|void
name|setHMSHandler
...
argument_list|)
decl_stmt|;
comment|// Save prev val of the key on threadLocal
name|Map
argument_list|<
name|String
argument_list|,
...
expr_stmt|;
block|}
comment|// Set invoking HMSHandler on threadLocal, this will be used later to notify
comment|// metaListeners in HiveMetaStore#cleanupRawStore
name|setHMSHandler
argument_list|(
name|this
argument_list|)
...
name|info
argument_list|(
literal|"Direct SQL optimization = {}"
argument_list|,
name|value
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Created RawStore: "
operator|+
name|ms
operator|+
literal|" from thread id: "
operator|+
name|Thread
operator|.
name|currentThread
...
argument_list|)
decl_stmt|;
comment|// Null check because in some test cases we get a null from ms.getCatalog.
if|if
condition|(
name|defaultCat
operator|!=
...
condition|)
block|{
comment|// One time update issue.  When the new 'hive' catalog is created in an upgrade the
comment|// script does not know the location of the warehouse.  So we need to update it.
name|LOG
operator|.
name|info
argument_list|(
literal|"Setting location of default catalog, as it hasn't been done after upgrade"
argument_list|)
expr_stmt|;
name|defaultCat
operator|.
...
condition|)
block|{
comment|// Timer will be null we aren't using the metrics
name|timerContexts
operator|.
name|get
argument_list|()
...
argument_list|()
operator|+
literal|" is not a valid catalog name"
argument_list|)
throw|;
block|}
if|if
...
name|InvalidObjectException
argument_list|(
literal|"You must specify a path for the catalog"
argument_list|)
throw|;
block|}
name|RawStore
...
name|MetaException
argument_list|(
literal|"Unable to create catalog path "
operator|+
name|catPath
operator|+
literal|", failed to create catalog "
operator|+
name|catalog
operator|.
name|getName
...
expr_stmt|;
block|}
comment|// set the create time of catalog
name|long
name|time
init|=
name|System
...
argument_list|)
expr_stmt|;
comment|// Create a default database inside the catalog
name|Database
name|db
init|=
operator|new
...
name|DEFAULT_DATABASE_NAME
argument_list|,
literal|"Default database for catalog "
operator|+
name|catalog
operator|.
name|getName
...
argument_list|)
expr_stmt|;
comment|// Above should have thrown NoSuchObjectException if there is no such catalog
assert|assert
name|oldCat
operator|!=
literal|null
...
condition|)
block|{
comment|// It might just be the default, in which case we can drop that one if it's empty
if|if
condition|(
name|allDbs
operator|.
...
parameter_list|)
block|{
comment|// This means there are tables of something in the database
throw|throw
operator|new
name|InvalidOperationException
argument_list|(
literal|"There are still objects in the default "
operator|+
literal|"database for catalog "
operator|+
name|catName
argument_list|)
throw|;
...
name|MetaException
argument_list|(
literal|"Error attempt to drop default database for "
operator|+
literal|"catalog "
operator|+
name|catName
...
name|InvalidOperationException
argument_list|(
literal|"There are non-default databases in the catalog "
operator|+
name|catName
operator|+
literal|" so it cannot be dropped."
argument_list|)
throw|;
block|}
block|}
...
return|;
block|}
comment|// Assumes that the catalog has already been set.
specifier|private
name|void
name|create_database_core
parameter_list|(
...
name|warn
argument_list|(
literal|"Failed to get owner name for create database operation."
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|debug
argument_list|(
literal|"Creating database path "
operator|+
name|dbPath
argument_list|)
expr_stmt|;
...
argument_list|()
decl_stmt|;
comment|// Perform the same URI normalization as create_database_core.
if|if
condition|(
name|newDB
operator|.
...
argument_list|)
decl_stmt|;
comment|// We can replicate into an empty database, in which case newDB will have indication that
comment|// it's target of replication but not oldDB. But replication flow will never alter a
comment|// database so that oldDB indicates that it's target or replication but not the newDB. So,
comment|// relying solely on newDB to check whether the database is target of replication works.
name|boolean
name|isReplicated
init|=
name|isDbReplicationTarget
...
name|MetaException
argument_list|(
literal|"Could not alter database \""
operator|+
name|parsedDbName
index|[
name|DB_NAME
index|]
operator|+
literal|"\". Could not retrieve old definition."
argument_list|)
throw|;
block|}
name|firePreEvent
...
name|InvalidOperationException
argument_list|(
literal|"can not drop a database which is a source of replication"
argument_list|)
throw|;
block|}
name|firePreEvent
...
argument_list|)
decl_stmt|;
comment|// First pass will drop the materialized views
name|List
argument_list|<
name|String
argument_list|>
...
block|}
block|}
comment|// Drop the materialized view but not its data
name|drop_table
argument_list|(
name|name
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Remove from all tables
name|uniqueTableNames
operator|.
name|remove
argument_list|(
...
decl_stmt|;
name|boolean
name|tableDataShouldBeDeleted
init|=
name|checkTableDataShouldBeDeleted
argument_list|(
name|table
argument_list|,
name|deleteData
...
name|MetaException
block|{
comment|// get_databases filters results already. No need to filter here
return|return
name|get_databases
argument_list|(
name|MetaStoreUtils
...
expr_stmt|;
block|}
comment|// If the given table has column statistics, save it here. We will update it later.
comment|// We don't want it to be part of the Table object being created, lest the create table
comment|// event will also have the col stats which we don't want.
if|if
condition|(
name|tbl
operator|.
...
block|{
comment|// Check that constraints have catalog name properly set first
if|if
condition|(
name|primaryKeys
operator|!=
...
expr_stmt|;
block|}
comment|// Set constraint name if null before sending to listener
name|List
argument_list|<
name|String
argument_list|>
...
block|}
block|}
comment|// If the table has column statistics, update it into the metastore. We need a valid
comment|// writeId list to update column statistics for a transactional table. But during bootstrap
comment|// replication, where we use this feature, we do not have a valid writeId list which was
comment|// used to update the stats. But we know for sure that the writeId associated with the
comment|// stats was valid then (otherwise stats update would have failed on the source). So, craft
comment|// a valid transaction list with only that writeId and use it to update the stats.
if|if
condition|(
name|colStats
operator|!=
...
expr_stmt|;
block|}
name|updateTableColumnStatsInternal
argument_list|(
name|colStats
argument_list|,
name|validWriteIds
...
name|warn
argument_list|(
literal|"create_table_with_environment_context got "
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|endFunction
argument_list|(
literal|"create_table_with_constraints"
argument_list|,
name|success
argument_list|,
name|ex
...
argument_list|)
decl_stmt|;
comment|// Set primary key name if null before sending to listener
if|if
condition|(
name|primaryKeyCols
operator|!=
...
argument_list|)
decl_stmt|;
comment|// Set foreign key name if null before sending to listener
if|if
condition|(
name|foreignKeyCols
operator|!=
...
argument_list|)
decl_stmt|;
comment|// Set unique constraint name if null before sending to listener
if|if
condition|(
name|uniqueConstraintCols
operator|!=
...
name|startFunction
argument_list|(
literal|"add_not_null_constraint"
argument_list|,
literal|": "
operator|+
name|constraintName
...
argument_list|)
decl_stmt|;
comment|// Set not null constraint name if null before sending to listener
if|if
condition|(
name|notNullConstraintCols
operator|!=
...
name|startFunction
argument_list|(
literal|"add_default_constraint"
argument_list|,
literal|": "
operator|+
name|constraintName
...
condition|)
block|{
comment|//TODO: Even listener for default
comment|//AddDefaultConstraintEvent addDefaultConstraintEvent = new AddDefaultConstraintEvent(defaultConstraintCols, true, this);
comment|//for (MetaStoreEventListener transactionalListener : transactionalListeners) {
comment|// transactionalListener.onAddNotNullConstraint(addDefaultConstraintEvent);
comment|//}
block|}
block|}
name|success
...
control|)
block|{
comment|//AddNotNullConstraintEvent addDefaultConstraintEvent = new AddNotNullConstraintEvent(defaultConstraintCols, true, this);
comment|//listener.onAddDefaultConstraint(addDefaultConstraintEvent);
block|}
block|}
name|endFunction
argument_list|(
...
condition|)
block|{
comment|//TODO: Even listener for check
comment|//AddcheckConstraintEvent addcheckConstraintEvent = new AddcheckConstraintEvent(checkConstraintCols, true, this);
comment|//for (MetaStoreEventListener transactionalListener : transactionalListeners) {
comment|// transactionalListener.onAddNotNullConstraint(addcheckConstraintEvent);
comment|//}
block|}
block|}
name|success
...
control|)
block|{
comment|//AddNotNullConstraintEvent addCheckConstraintEvent = new AddNotNullConstraintEvent(checkConstraintCols, true, this);
comment|//listener.onAddCheckConstraint(addCheckConstraintEvent);
block|}
block|}
name|endFunction
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Drop any constraints on the table
name|ms
operator|.
name|dropConstraint
argument_list|(
...
condition|)
block|{
comment|// External table data can be deleted if EXTERNAL_TABLE_PURGE is true
return|return
...
return|;
block|}
comment|/**      * Deletes the data in a table's location, if it fails logs an error      *      * @param tablePath      * @param ifPurge completely purge the table (skipping trash) while removing      *                data from warehouse      * @param shouldEnableCm If cm should be enabled      */
specifier|private
name|void
name|deleteTableData
parameter_list|(
...
block|}
block|}
comment|/**      * Deletes the data in a table's location, if it fails logs an error.      *      * @param tablePath      * @param ifPurge completely purge the table (skipping trash) while removing      *                data from warehouse      * @param db Database      */
specifier|private
name|void
name|deleteTableData
parameter_list|(
...
block|}
block|}
comment|/**     * Give a list of partitions' locations, tries to delete each one     * and for each that fails logs an error.     *     * @param partPaths     * @param ifPurge completely purge the partition (skipping trash) while     *                removing data from warehouse     * @param shouldEnableCm If cm should be enabled     */
specifier|private
name|void
name|deletePartitionData
parameter_list|(
...
block|}
block|}
comment|/**      * Give a list of partitions' locations, tries to delete each one      * and for each that fails logs an error.      *      * @param partPaths      * @param ifPurge completely purge the partition (skipping trash) while      *                removing data from warehouse      * @param db Database      */
specifier|private
name|void
name|deletePartitionData
parameter_list|(
...
block|}
block|}
comment|/**      * Delete data from path excluding cmdir      * and for each that fails logs an error.      *      * @param path      * @param ifPurge completely purge the partition (skipping trash) while      *                removing data from warehouse      * @param shouldEnableCm If cm should be enabled      */
specifier|private
name|void
name|deleteDataExcludeCmroot
parameter_list|(
...
condition|)
block|{
comment|//Don't delete cmdir if its inside the partition path
name|FileStatus
index|[]
name|statuses
init|=
...
expr_stmt|;
block|}
comment|//Check if table directory is empty, delete it
name|FileStatus
index|[]
name|statusWithoutFilter
init|=
name|path
operator|.
name|getFileSystem
...
block|{
comment|//If no cm delete the complete table directory
name|wh
operator|.
name|deleteDir
argument_list|(
...
block|}
block|}
comment|/**      * Deletes the partitions specified by catName, dbName, tableName. If checkLocation is true, for      * locations of partitions which may not be subdirectories of tablePath checks to make sure the      * locations are writable.      *      * Drops the metadata for each partition.      *      * Provides a list of locations of partitions which may not be subdirectories of tablePath.      *      * @param ms RawStore to use for metadata retrieval and delete      * @param catName The catName      * @param dbName The dbName      * @param tableName The tableName      * @param tablePath The tablePath of which subdirectories does not have to be checked      * @param checkLocation Should we check the locations at all      * @return The list of the Path objects to delete (only in case checkLocation is true)      * @throws MetaException      * @throws IOException      * @throws NoSuchObjectException      */
specifier|private
name|List
argument_list|<
name|Path
...
condition|)
block|{
comment|// No more partitions left to drop. Return with the collected path list to delete.
return|return
name|partPaths
return|;
block|}
...
argument_list|)
decl_stmt|;
comment|// Double check here. Maybe Warehouse.getDnsPath revealed relationship between the
comment|// path objects
if|if
condition|(
name|tableDnsPath
operator|==
...
control|)
block|{
comment|//No drop part listener events fired for public listeners historically, for drop table case.
comment|//Limiting to internal listeners for now, to avoid unexpected calls for public listeners.
if|if
condition|(
name|listener
operator|instanceof
...
specifier|private
name|void
name|alterPartitionForTruncate
parameter_list|(
name|RawStore
name|ms
parameter_list|,
...
specifier|private
name|void
name|alterTableStatsForTruncate
parameter_list|(
name|RawStore
name|ms
parameter_list|,
...
expr_stmt|;
block|}
comment|// TODO: this should actually pass thru and set writeId for txn stats.
if|if
condition|(
name|writeId
operator|>
...
name|MetaException
block|{
comment|// Deprecated path, won't work for txn tables.
...
block|}
block|}
comment|// Alter the table/partition stats and also notify truncate table event
name|alterTableStatsForTruncate
argument_list|(
name|getMS
argument_list|()
...
name|debug
argument_list|(
literal|"get_tables_ext:getTables() returned "
operator|+
name|tables
operator|.
name|size
...
name|debug
argument_list|(
literal|"get_tables_ext:getTableObjectsByName() returned "
operator|+
name|tObjects
operator|.
name|size
...
name|info
argument_list|(
literal|"Skipping translation for processor with "
operator|+
name|processorId
argument_list|)
expr_stmt|;
...
name|add
argument_list|(
name|convertTableToExtendedTable
argument_list|(
name|entry
operator|.
name|getKey
...
return|;
block|}
comment|/**      * This function retrieves table from metastore. If getColumnStats flag is true,      * then engine should be specified so the table is retrieve with the column stats      * for that engine.      */
specifier|private
name|Table
name|getTableInternal
parameter_list|(
...
literal|null
argument_list|,
literal|"To retrieve column statistics with a table, engine parameter cannot be null"
argument_list|)
expr_stmt|;
if|if
condition|(
...
condition|)
block|{
name|assertClientHasCapability
argument_list|(
name|capabilities
argument_list|,
...
name|warn
argument_list|(
literal|"Unexpected resultset size:"
operator|+
name|ret
operator|.
name|size
...
name|MetaException
argument_list|(
literal|"Unexpected result from metadata transformer:return list size is "
operator|+
name|ret
operator|.
name|size
...
throw|;
block|}
comment|// If column statistics was requested and is valid fetch it.
if|if
condition|(
name|getColumnStats
condition|)
...
literal|"Hive tests"
argument_list|,
literal|"get_table_objects_by_name_req"
argument_list|)
expr_stmt|;
block|}
name|List
...
name|MetaException
argument_list|(
literal|"Your client does not appear to support "
operator|+
name|what
operator|+
literal|". To skip"
operator|+
literal|" capability checks, please set "
operator|+
name|ConfVars
operator|.
...
argument_list|()
operator|+
literal|" to false. This setting can be set globally, or on the client for the current"
operator|+
literal|" metastore session. Note that this may lead to incorrect results, data loss,"
operator|+
literal|" undefined behavior, etc. if your client is actually incompatible. You can also"
operator|+
literal|" specify custom client capabilities via "
operator|+
name|call
operator|+
literal|" API."
...
control|)
block|{
comment|// Collect partition column stats to be updated if present. Partition objects passed down
comment|// here at the time of replication may have statistics in them, which is required to be
comment|// updated in the metadata. But we don't want it to be part of the Partition object when
comment|// it's being created or altered, lest it becomes part of the notification event.
if|if
condition|(
name|part
operator|.
...
expr_stmt|;
block|}
comment|// Iterate through the partitions and validate them. If one of the partitions is
comment|// incorrect, an exception will be thrown before the threads which create the partition
comment|// folders are submitted. This way we can be sure that no partition and no partition
comment|// folder will be created if the list contains an invalid partition.
if|if
condition|(
...
expr_stmt|;
block|}
comment|// Notification is generated for newly created partitions only. The subset of partitions
comment|// that already exist (existingParts), will not generate notifications.
if|if
condition|(
operator|!
name|transactionalListeners
...
block|}
block|}
comment|// Update partition column statistics if available. We need a valid writeId list to
comment|// update column statistics for a transactional table. But during bootstrap replication,
comment|// where we use this feature, we do not have a valid writeId list which was used to
comment|// update the stats. But we know for sure that the writeId associated with the stats was
comment|// valid then (otherwise stats update would have failed on the source). So, craft a valid
comment|// transaction list with only that writeId and use it to update the stats.
name|int
name|cnt
init|=
literal|0
...
expr_stmt|;
block|}
name|updatePartitonColStatsInternal
argument_list|(
name|tbl
argument_list|,
name|partColStats
...
return|;
block|}
comment|/**      * Remove the newly created partition folders. The values in the addedPartitions map indicates      * whether or not the location of the partition was newly created. If the value is false, the      * partition folder will not be removed.      * @param addedPartitions      * @throws MetaException      * @throws IllegalArgumentException      */
specifier|private
name|void
name|cleanupPartitionFolders
parameter_list|(
...
block|}
block|}
comment|/**      * Validate a partition before creating it. The validation checks      *<ul>      *<li>if the database and table names set in the partition are not null and they are matching      * with the expected values set in the tblName and dbName parameters.</li>      *<li>if the partition values are set.</li>      *<li>if none of the partition values is null.</li>      *<li>if the partition values are matching with the pattern set in the      * 'metastore.partition.name.whitelist.pattern' configuration property.</li>      *<li>if the partition doesn't already exist. If the partition already exists, an exception      * will be thrown if the ifNotExists parameter is false, otherwise it will be just ignored.</li>      *<li>if the partsToAdd set doesn't contain the partition. The partsToAdd set contains the      * partitions which are already validated. If the set contains the current partition, it means      * that the partition is tried to be added multiple times in the same batch. Please note that      * the set will be updated with the current partition if the validation was successful.</li>      *</ul>      * @param part      * @param catName      * @param tblName      * @param dbName      * @param partsToAdd      * @param ms      * @param ifNotExists      * @return      * @throws MetaException      * @throws TException      */
specifier|private
name|boolean
name|validatePartition
parameter_list|(
...
name|MetaException
argument_list|(
literal|"The database and table name must be set in the partition."
argument_list|)
throw|;
block|}
if|if
...
name|MetaException
argument_list|(
literal|"The partition values cannot be null or empty."
argument_list|)
throw|;
block|}
if|if
...
name|info
argument_list|(
literal|"Not adding partition {} as it already exists"
argument_list|,
name|part
argument_list|)
expr_stmt|;
...
parameter_list|)
block|{
comment|// If an exception is thrown in the execution of a task, set the failureOccurred flag to
comment|// true. This flag is visible in the tasks and if its value is true, the partition
comment|// folders won't be created.
comment|// Then iterate through the remaining tasks and wait for them to finish. The tasks which
comment|// are started before the flag got set will then finish creating the partition folders.
comment|// The tasks which are started after the flag got set, won't create the partition
comment|// folders, to avoid unnecessary work.
comment|// This way it is sure that all tasks are finished, when entering the finally part where
comment|// the partition folders are cleaned up. It won't happen that a task is still running
comment|// when cleaning up the folders, so it is sure we won't have leftover folders.
comment|// Canceling the other tasks would be also an option but during testing it turned out
comment|// that it is not a trustworthy solution to avoid leftover folders.
name|failureOccurred
operator|.
name|compareAndSet
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// Restore interruption status of the corresponding thread
name|Thread
operator|.
name|currentThread
argument_list|()
...
expr_stmt|;
block|}
comment|// Make sure all of the partitions have the catalog set as well
name|request
operator|.
name|getParts
argument_list|()
...
name|MetaException
argument_list|(
literal|"Partition list cannot be null."
argument_list|)
throw|;
block|}
if|if
...
argument_list|()
decl_stmt|;
comment|// If the catalog name isn't set, we need to go through and set it.
name|String
name|catName
decl_stmt|;
if|if
...
name|MetaException
argument_list|(
literal|"The database and table name cannot be null."
argument_list|)
throw|;
block|}
comment|// Ensures that the list doesn't have dups, and keeps track of directories we have created.
...
comment|// Iterate through the partitions and validate them. If one of the partitions is
comment|// incorrect, an exception will be thrown before the threads which create the partition
comment|// folders are submitted. This way we can be sure that no partition or partition folder
comment|// will be created if the list contains an invalid partition.
specifier|final
name|Partition
name|part
init|=
...
begin_comment
comment|/**      * Verify if update stats while altering partition(s)      * For the following three cases HMS will not update partition stats      * 1) Table property 'DO_NOT_UPDATE_STATS' = True      * 2) HMS configuration property 'STATS_AUTO_GATHER' = False      * 3) Is View      */
end_comment
...
argument_list|)
expr_stmt|;
comment|// Wouldn't it make more sense to return the first element of the list returned by the
...
argument_list|)
decl_stmt|;
comment|// No need to check catalog for null as parseDbName() will never return null for the catalog.
if|if
condition|(
name|partitionSpecs
operator|==
...
name|MetaException
argument_list|(
literal|"The DB and table name for the source and destination tables,"
operator|+
literal|" and the partition specs must not be null."
argument_list|)
throw|;
block|}
if|if
...
name|MetaException
argument_list|(
literal|"You cannot move a partition across catalogs"
argument_list|)
throw|;
block|}
name|boolean
...
expr_stmt|;
block|}
comment|// Passed the unparsed DB name here, as get_partitions_ps expects to parse it
name|List
argument_list|<
name|Partition
argument_list|>
...
argument_list|>
argument_list|>
name|transactionalListenerResponsesForDropPartition
init|=
name|Lists
operator|.
...
argument_list|)
decl_stmt|;
comment|// Check if any of the partitions already exists in destTable.
name|List
argument_list|<
name|String
argument_list|>
name|destPartitionNames
init|=
name|ms
operator|.
...
name|MetaException
argument_list|(
literal|"The DB name cannot be null."
argument_list|)
throw|;
block|}
if|if
...
name|MetaException
argument_list|(
literal|"The table name cannot be null."
argument_list|)
throw|;
block|}
if|if
...
name|MetaException
argument_list|(
literal|"The partition values cannot be null."
argument_list|)
throw|;
block|}
try|try
...
begin_comment
comment|/**      * Fire a pre-event for read table operation, if there are any      * pre-event listeners registered      */
end_comment
...
try|try
block|{
name|checkLimitNumberOfPartitionsByFilter
argument_list|(
name|parsedDbName
index|[
name|CAT_NAME
...
if|if
condition|(
name|isPartitionLimitEnabled
argument_list|()
condition|)
block|{
name|checkLimitNumberOfPartitions
argument_list|(
name|tblName
argument_list|,
...
specifier|private
name|void
name|checkLimitNumberOfPartitionsByExpr
parameter_list|(
name|String
name|catName
parameter_list|,
...
name|tblName
argument_list|,
name|get_num_partitions_by_expr
argument_list|(
name|catName
argument_list|,
name|dbName
...
argument_list|)
decl_stmt|;
comment|// get_partitions will parse out the catalog and db names itself
name|List
argument_list|<
name|Partition
argument_list|>
...
condition|)
block|{
comment|// if catName is not provided in the request use the catName parsed from the dbName
name|catName
operator|=
name|parsedDbName
index|[
...
name|startTableFunction
argument_list|(
literal|"get_partitions_with_specs"
argument_list|,
name|catName
argument_list|,
name|parsedDbName
...
argument_list|)
expr_stmt|;
comment|// This is serious black magic, as the following 2 lines do nothing AFAICT but without them
comment|// the subsequent call to listPartitionValues fails.
name|List
argument_list|<
name|FieldSchema
argument_list|>
...
name|format
argument_list|(
literal|"Unable to get partition for %s.%s.%s"
argument_list|,
name|catName
argument_list|,
name|dbName
...
argument_list|)
decl_stmt|;
comment|// TODO: this method name is confusing, it actually does full alter (sortof)
name|rename_partition
argument_list|(
name|parsedDbName
index|[
...
block|}
block|}
comment|// Adds the missing scheme/authority for the new partition location
if|if
condition|(
name|new_part
operator|.
...
block|}
block|}
comment|// Make sure the new partition has the catalog value set
if|if
condition|(
operator|!
name|new_part
...
begin_comment
comment|// The old API we are keeping for backward compat. Not used within Hive.
end_comment
...
control|)
block|{
comment|// Make sure the catalog name is set in the new partition
if|if
condition|(
operator|!
name|tmpPart
...
expr_stmt|;
block|}
comment|// Adds the missing scheme/authority for the new table location
if|if
condition|(
name|newTable
operator|.
...
block|}
block|}
comment|// Set the catalog name if it hasn't been set in the new table
if|if
condition|(
operator|!
name|newTable
...
name|Table
argument_list|>
name|get_all_materialized_view_objects_for_rewriting
parameter_list|()
throws|throws
name|MetaException
block|{
name|startFunction
argument_list|(
literal|"get_all_materialized_view_objects_for_rewriting"
argument_list|)
expr_stmt|;
name|List
argument_list|<
...
name|startFunction
argument_list|(
literal|"get_materialized_views_for_rewriting"
argument_list|,
literal|": db="
operator|+
name|dbname
...
throw|;
block|}
comment|// Pass unparsed db name here
name|List
argument_list|<
name|FieldSchema
argument_list|>
...
name|matches
argument_list|(
literal|"(hive|hdfs|mapred|metastore).*"
argument_list|,
name|name
argument_list|)
condition|)
...
argument_list|)
expr_stmt|;
comment|// Don't send the parsedDbName, as this method will parse itself.
name|ret
operator|=
...
argument_list|)
decl_stmt|;
comment|// Note: stats compliance is not propagated to the client; instead, we just return nothing
comment|//       if stats are not compliant for now. This won't work for stats merging, but that
comment|//       is currently only done on metastore size (see set_aggr...).
comment|//       For some optimizations we might make use of incorrect stats that are "better than
comment|//       nothing", so this may change in future.
name|result
operator|=
operator|new
name|TableStatsResult
...
name|TException
block|{
comment|// Note: this method appears to be unused within Hive.
comment|//       It doesn't take txn stats into account.
name|dbName
operator|=
name|dbName
operator|.
...
name|TException
block|{
comment|// Deprecated API, won't work for transactional tables
return|return
name|updateTableColumnStatsInternal
argument_list|(
name|colStats
...
name|InvalidInputException
argument_list|(
literal|"Only one stats object expected"
argument_list|)
throw|;
block|}
if|if
...
name|InvalidInputException
argument_list|(
literal|"Merge is not supported for non-aggregate stats"
argument_list|)
throw|;
block|}
name|ColumnStatistics
...
name|MetaException
block|{
comment|// TODO: is this really needed? this code is propagated from HIVE-1362 but most of it is useless.
...
argument_list|)
decl_stmt|;
comment|// This API looks unused; if it were used we'd need to update stats state and write ID.
comment|// We cannot just randomly nuke some txn stats.
if|if
condition|(
name|TxnUtils
operator|.
...
name|MetaException
argument_list|(
literal|"Cannot delete stats via this API for a transactional table"
argument_list|)
throw|;
block|}
name|ret
...
argument_list|)
decl_stmt|;
comment|// Don't pass the parsed db name, as get_partitions_by_filter will parse it itself
name|List
argument_list|<
name|Partition
argument_list|>
...
name|MetaException
argument_list|(
literal|"The DB and table name cannot be null."
argument_list|)
throw|;
block|}
name|startTableFunction
argument_list|(
literal|"get_num_partitions_by_filter"
argument_list|,
name|parsedDbName
index|[
name|CAT_NAME
...
name|endFunction
argument_list|(
literal|"get_num_partitions_by_expr"
argument_list|,
name|ret
operator|!=
operator|-
...
name|Override
specifier|public
name|GetPartitionsByNamesResult
name|get_partitions_by_names_req
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// If requested add column statistics in each of the partition objects
if|if
condition|(
name|getColStats
condition|)
block|{
comment|// Since each partition may have stats collected for different set of columns, we
comment|// request them separately.
for|for
control|(
name|Partition
name|part
...
name|startFunction
argument_list|(
literal|"get_all_token_identifiers."
argument_list|)
expr_stmt|;
name|List
argument_list|<
...
name|MetaException
argument_list|(
literal|"Function name cannot be null."
argument_list|)
throw|;
block|}
if|if
...
name|MetaException
argument_list|(
literal|"Database name in Function cannot be null."
argument_list|)
throw|;
block|}
if|if
...
name|MetaException
argument_list|(
literal|"Function owner type cannot be null."
argument_list|)
throw|;
block|}
if|if
...
name|MetaException
argument_list|(
literal|"Function type cannot be null."
argument_list|)
throw|;
block|}
block|}
...
parameter_list|)
block|{
comment|// Throwing MetaException instead of InvalidObjectException as the InvalidObjectException
comment|// is not defined for the alter_function method in the Thrift interface.
name|throwMetaException
argument_list|(
name|e
argument_list|)
...
name|MetaException
argument_list|(
literal|"Database and function name cannot be null."
argument_list|)
throw|;
block|}
try|try
...
parameter_list|)
block|{
comment|// The validateFunctionInfo method is used by the create and alter function methods as well
comment|// and it can throw InvalidObjectException. But the InvalidObjectException is not defined
comment|// for the alter_function method in the Thrift interface, therefore a TApplicationException
comment|// will occur at the caller side. Re-throwing the InvalidObjectException as MetaException
comment|// would eliminate the TApplicationException at caller side.
throw|throw
name|newMetaException
argument_list|(
name|e
...
try|try
block|{
comment|// Leaving this as the 'hive' catalog (rather than choosing the default from the
comment|// configuration) because all the default UDFs are in that catalog, and I think that's
comment|// would people really want here.
name|allFunctions
operator|=
name|ms
operator|.
...
name|TException
block|{
comment|// in replication flow, the write notification log table will be updated here.
if|if
condition|(
name|rqst
operator|.
...
argument_list|)
expr_stmt|;
comment|// The files in the commit txn message during load will have files with path corresponding to source
comment|// warehouse. Need to transform them to target warehouse using table or partition object location.
for|for
control|(
name|String
name|file
...
name|error
argument_list|(
literal|"failed to get checksum for the file "
operator|+
name|newPath
operator|+
literal|" with error: "
...
expr_stmt|;
block|}
name|addTxnWriteNotificationLog
argument_list|(
name|tbl
argument_list|,
name|ptnObj
...
literal|""
decl_stmt|;
comment|//Empty string is an invalid partition name. Can be used for non partitioned table.
if|if
condition|(
name|ptnObj
operator|!=
...
return|;
block|}
comment|// figure out if it is table level or partition level
name|ColumnStatistics
name|firstColStats
init|=
name|csNews
...
condition|)
block|{
comment|// there should be only one ColumnStatistics
if|if
condition|(
name|request
operator|.
...
name|MetaException
argument_list|(
literal|"Expecting only 1 ColumnStatistics for table's column stats, but find "
operator|+
name|request
operator|.
name|getColStatsSize
...
block|{
return|return
name|updateTableColumnStatsWithMerge
argument_list|(
name|catName
argument_list|,
name|dbName
...
block|{
comment|// This is the overwrite case, we do not care about the accuracy.
return|return
name|updateTableColumnStatsInternal
argument_list|(
name|firstColStats
...
block|{
comment|// partition level column stats merging
comment|// note that we may have two or more duplicate partition names.
comment|// see autoColumnStats_2.q under TestMiniLlapLocalCliDriver
name|Map
argument_list|<
name|String
argument_list|,
...
name|ret
operator|=
name|updatePartColumnStatsWithMerge
argument_list|(
name|catName
argument_list|,
name|dbName
...
control|)
block|{
comment|// We don't short-circuit on errors here anymore. That can leave acid stats invalid.
name|ret
operator|=
name|updatePartitonColStatsInternal
argument_list|(
...
try|try
block|{
comment|// a single call to get all column stats for all partitions
name|List
argument_list|<
name|String
argument_list|>
...
condition|)
block|{
comment|// some of the partitions miss stats.
name|LOG
operator|.
name|debug
argument_list|(
literal|"Some of the partitions miss stats."
argument_list|)
expr_stmt|;
block|}
name|Map
...
expr_stmt|;
block|}
comment|// another single call to get all the partition objects
name|List
argument_list|<
name|Partition
argument_list|>
...
condition|)
block|{
comment|// No columns can be merged; a shortcut for getMergableCols.
name|csNew
operator|.
name|setStatsObj
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// we merge those that can be merged
if|if
condition|(
name|csOld
operator|!=
...
condition|)
block|{
comment|// For now because the stats state is such as it is, we will invalidate everything.
comment|// Overall the sematics here are not clear - we could invalide only some columns, but does
comment|// that make any physical sense? Could query affect some columns but not others?
name|part
operator|.
name|setWriteId
argument_list|(
...
block|{
comment|// TODO: why doesn't the original call for non acid tables invalidate the stats?
name|LOG
operator|.
name|debug
argument_list|(
literal|"All the column stats "
operator|+
name|csNew
operator|.
name|getStatsDesc
...
argument_list|()
operator|+
literal|" are not accurate to merge."
argument_list|)
expr_stmt|;
block|}
block|}
...
argument_list|)
decl_stmt|;
comment|// we first use the valid stats list to prune the stats
name|boolean
name|isInvalidTxnStats
init|=
name|csOld
...
name|debug
argument_list|(
literal|"All the column stats are not accurate to merge."
argument_list|)
expr_stmt|;
name|result
operator|=
...
name|error
argument_list|(
literal|"Not authorized to make the get_next_notification call. You can try to disable "
operator|+
name|ConfVars
operator|.
...
name|error
argument_list|(
literal|"Not authorized to make the get_current_notificationEventId call. You can try to disable "
operator|+
name|ConfVars
operator|.
name|EVENT_DB_NOTIFICATION_API_AUTH
...
name|error
argument_list|(
literal|"Not authorized to make the get_notification_events_count call. You can try to disable "
operator|+
name|ConfVars
operator|.
name|EVENT_DB_NOTIFICATION_API_AUTH
...
name|Exception
block|{
comment|// Skip the auth in embedded mode or if the auth is disabled
if|if
condition|(
operator|!
name|isMetaStoreRemote
...
name|user
operator|+
literal|" is not allowed to perform this API call"
argument_list|)
throw|;
block|}
block|}
...
argument_list|)
decl_stmt|;
comment|/*          * The transactional listener response will be set already on the event, so there is not need          * to pass the response to the non-transactional listener.          */
name|MetaStoreListenerNotifier
operator|.
name|notifyEvent
argument_list|(
...
name|MetaException
argument_list|(
literal|"Table is not partitioned"
argument_list|)
throw|;
block|}
if|if
...
name|MetaException
argument_list|(
literal|"Table does not have storage location; this operation is not supported on views"
argument_list|)
throw|;
block|}
name|FileMetadataExprType
...
name|MetaException
argument_list|(
literal|"The operation is not supported for "
operator|+
name|tableInputFormat
argument_list|)
throw|;
...
name|MetaException
argument_list|(
literal|"Partition does not have storage location;"
operator|+
literal|" this operation is not supported on views"
argument_list|)
throw|;
block|}
name|String
...
literal|true
expr_stmt|;
comment|// Regardless of the following exception
if|if
condition|(
name|failCount
operator|>
...
name|errorMsg
init|=
literal|"The operation failed for "
operator|+
name|failCount
operator|+
literal|" partitions and "
operator|+
literal|"succeeded for "
operator|+
name|successCount
operator|+
literal|" partitions; unsupported formats: "
decl_stmt|;
name|boolean
name|isFirst
init|=
...
name|startTableFunction
argument_list|(
literal|"get_unique_constraints"
argument_list|,
name|catName
argument_list|,
name|db_name
...
name|startTableFunction
argument_list|(
literal|"get_not_null_constraints"
argument_list|,
name|catName
argument_list|,
name|db_name
...
name|Override
specifier|public
name|DefaultConstraintsResponse
...
name|startTableFunction
argument_list|(
literal|"get_default_constraints"
argument_list|,
name|catName
argument_list|,
name|db_name
...
name|error
argument_list|(
literal|"Exception thrown while querying metastore db uuid"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// If the default pool is not disabled, override the size with the specified parallelism.
name|defaultPoolSize
operator|=
name|plan
operator|.
...
name|error
argument_list|(
literal|"Exception while trying to persist resource plan"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Exception while trying to retrieve resource plan"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|Override
specifier|public
name|WMGetAllResourcePlanResponse
...
name|error
argument_list|(
literal|"Exception while trying to retrieve resource plans"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|MetaException
argument_list|(
literal|"Invalid request; multiple flags are set"
argument_list|)
throw|;
block|}
name|WMAlterResourcePlanResponse
...
argument_list|()
decl_stmt|;
comment|// This method will only return full resource plan when activating one,
comment|// to give the caller the result atomically with the activation.
name|WMFullResourcePlan
name|fullPlanAfterAlter
init|=
name|getMS
argument_list|()
operator|.
...
name|error
argument_list|(
literal|"Exception while trying to alter resource plan"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Exception while trying to get active resource plan"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Exception while trying to validate resource plan"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Exception while trying to drop resource plan"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Exception while trying to create trigger"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Exception while trying to alter trigger"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Exception while trying to drop trigger."
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Exception while trying to retrieve triggers plans"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Exception while trying to alter WMPool"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Exception while trying to create WMPool"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Exception while trying to drop WMPool"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|Override
specifier|public
name|WMCreateOrUpdateMappingResponse
...
name|error
argument_list|(
literal|"Exception while trying to create or update WMMapping"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Exception while trying to drop WMMapping"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Exception while trying to create or drop pool mappings"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Caught exception creating schema"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Caught exception altering schema"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Caught exception getting schema"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
try|try
block|{
comment|// look for any valid versions.  This will also throw NoSuchObjectException if the schema
comment|// itself doesn't exist, which is what we want.
name|SchemaVersion
name|latest
init|=
name|ms
...
name|schemaName
operator|+
literal|" cannot be dropped, it has"
operator|+
literal|" at least one valid version"
argument_list|)
expr_stmt|;
throw|throw
operator|(
...
name|error
argument_list|(
literal|"Caught exception dropping schema"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Caught exception adding schema version"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Caught exception getting schema version"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|startFunction
argument_list|(
literal|"get_latest_schema_version"
argument_list|,
literal|": "
operator|+
name|schemaName
...
name|error
argument_list|(
literal|"Caught exception getting latest schema version"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|startFunction
argument_list|(
literal|"get_all_schema_versions"
argument_list|,
literal|": "
operator|+
name|schemaName
...
name|error
argument_list|(
literal|"Caught exception getting all schema versions"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Caught exception dropping schema version"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Caught exception doing schema version query"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|startFunction
argument_list|(
literal|"map_schema_version_to_serde, :"
operator|+
name|rqst
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Caught exception mapping schema version to serde"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|startFunction
argument_list|(
literal|"set_schema_version_state, :"
operator|+
name|rqst
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Caught exception changing schema version state"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Caught exception creating serde"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Caught exception getting serde"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
specifier|public
name|void
name|scheduled_query_maintenance
parameter_list|(
...
block|}
block|}
comment|// Remove from zookeeper if it's configured
try|try
block|{
if|if
condition|(
...
name|error
argument_list|(
literal|"Error removing znode for this metastore instance from ZooKeeper."
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// we are in secure mode. Login using keytab
name|String
name|kerberosName
init|=
name|SecurityUtil
...
name|info
argument_list|(
literal|"Binding host "
operator|+
name|msHost
operator|+
literal|" for metastore server"
argument_list|)
expr_stmt|;
block|}
if|if
...
argument_list|)
decl_stmt|;
comment|// enable SSL support for HMS
name|List
argument_list|<
name|String
argument_list|>
...
comment|// cleaned up thread local RawStore. Otherwise, do it now.
name|cleanupRawStore
argument_list|()
expr_stmt|;
block|}
...
name|startedServing
argument_list|,
name|isMetastoreHousekeepingLeader
argument_list|(
name|conf
argument_list|,
...
expr_stmt|;
block|}
comment|// If dynamic service discovery through ZooKeeper is enabled, add this server to the ZooKeeper.
if|if
condition|(
name|MetastoreConf
operator|.
...
name|info
argument_list|(
literal|"Metastore server instance with URL "
operator|+
name|serverInstanceURI
operator|+
literal|" added to "
operator|+
literal|"the zookeeper"
argument_list|)
expr_stmt|;
block|}
catch|catch
...
name|error
argument_list|(
literal|"Error adding this metastore instance to ZooKeeper: "
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Compaction HMS parameters:"
argument_list|)
expr_stmt|;
name|HMSHandler
operator|.
...
name|warn
argument_list|(
literal|"Compactor Initiator is turned Off. Automatic compaction will not be triggered."
argument_list|)
expr_stmt|;
block|}
if|if
...
name|warn
argument_list|(
literal|"Invalid number of Compactor Worker threads({}) on HMS"
argument_list|,
name|numThreads
argument_list|)
expr_stmt|;
...
argument_list|)
decl_stmt|;
comment|// For the sake of backward compatibility, when the current HMS becomes the leader when no
comment|// leader is specified.
if|if
condition|(
name|leaderHost
operator|==
...
name|METASTORE_HOUSEKEEPING_LEADER_HOSTNAME
operator|+
literal|" is empty. Start all the "
operator|+
literal|"housekeeping threads."
argument_list|)
expr_stmt|;
return|return
literal|true
...
begin_comment
comment|/**    * @param port where metastore server is running    * @return metastore server instance URL. If the metastore server was bound to a configured    * host, return that appended by port. Otherwise return the externally visible URL of the local    * host with the given port    * @throws Exception    */
end_comment
...
begin_comment
comment|/**    * Start threads outside of the thrift service, such as the compactor threads.    * @param conf Hive configuration object    * @param isLeader true if this metastore is a leader. Most of the housekeeping threads are    *                 started only in a leader HMS.    */
end_comment
...
argument_list|)
expr_stmt|;
name|startRemoteOnlyTasks
argument_list|(
name|conf
argument_list|)
expr_stmt|;
name|startStatsUpdater
argument_list|(
name|conf
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|// The leader HMS may not necessarily have sufficient compute capacity required to run
comment|// actual compaction work. So it can run on a non-leader HMS with sufficient capacity
comment|// or a configured HS2 instance.
if|if
condition|(
name|MetastoreConf
operator|.
...
argument_list|(
literal|"org.apache.hadoop.hive.ql.stats.StatsUpdaterThread"
argument_list|)
decl_stmt|;
...
name|info
argument_list|(
literal|"This HMS instance will act as a Compactor Initiator."
argument_list|)
expr_stmt|;
block|}
block|}
...
name|info
argument_list|(
literal|"This HMS instance will act as a Compactor Worker with {} threads"
argument_list|,
name|numWorkers
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"This HMS instance will act as a Compactor Cleaner."
argument_list|)
expr_stmt|;
block|}
block|}
