...
block|}
block|}
comment|// If the table has column statistics, update it into the metastore. This feature is used
comment|// by replication to replicate table level statistics.
if|if
condition|(
name|tbl
operator|.
...
condition|)
block|{
comment|// We do not replicate statistics for a transactional table right now and hence we do not
comment|// expect a transactional table to have column statistics here. So passing null
comment|// validWriteIds is fine for now.
...
block|}
block|}
comment|// Update partition column statistics if available
for|for
control|(
name|Partition
name|newPart
