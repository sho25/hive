...
name|info
argument_list|(
literal|"Txn {} not present for repl policy {}"
argument_list|,
name|sourceTxnId
argument_list|,
name|replPolicy
...
name|MetaException
argument_list|(
literal|"Unable to get target transaction id "
operator|+
name|StringUtils
operator|.
name|stringifyException
...
name|query
init|=
literal|"select \"DB_ID\" from \"DBS\" where \"NAME\" = ?  and \"CTLG_NAME\" = ?"
decl_stmt|;
name|List
argument_list|<
name|String
...
name|MetaException
argument_list|(
literal|"DB with name "
operator|+
name|db
operator|+
literal|" does not exist in catalog "
operator|+
name|catalog
argument_list|)
throw|;
...
name|debug
argument_list|(
literal|"Updating repl id for db<"
operator|+
name|query
operator|.
name|replaceAll
...
condition|)
block|{
comment|//only one row insert or update should happen
throw|throw
operator|new
name|RuntimeException
argument_list|(
literal|"DATABASE_PARAMS is corrupted for db "
operator|+
name|db
argument_list|)
throw|;
...
condition|)
block|{
comment|// if only database last repl id to be updated.
return|return;
block|}
name|query
operator|=
...
argument_list|()
expr_stmt|;
comment|// select for update is not required as only one task will update this during repl load.
name|rs
operator|=
name|stmt
operator|.
...
name|debug
argument_list|(
literal|"Updating repl id for table<"
operator|+
name|query
operator|.
name|replaceAll
...
name|RuntimeException
argument_list|(
literal|"TABLE_PARAMS is corrupted for table "
operator|+
name|table
argument_list|)
throw|;
...
name|debug
argument_list|(
literal|"Updating repl id for part<"
operator|+
name|query
operator|.
name|replaceAll
...
name|RuntimeException
argument_list|(
literal|"PARTITION_PARAMS is corrupted for partition "
operator|+
name|partId
argument_list|)
throw|;
...
name|partCount
operator|+
literal|" Number of partition among "
operator|+
name|partList
operator|+
...
argument_list|)
expr_stmt|;
comment|// update the key/value associated with the transaction if it has been
comment|// set
if|if
condition|(
name|rqst
...
name|errorMsg
init|=
literal|"Error updating key/value in the sql backend with"
operator|+
literal|" txnId="
operator|+
name|rqst
...
block|}
name|boolean
name|foundValidUncompactedWrite
init|=
literal|false
decl_stmt|;
...
block|}
block|}
comment|// If we have compacted writes and some invalid writes on the table,
comment|// return the lowest invalid write as a writeIdHwm and set it as invalid.
if|if
condition|(
operator|!
name|foundValidUncompactedWrite
...
argument_list|)
expr_stmt|;
comment|//since this is on conversion from non-acid to acid, NEXT_WRITE_ID should not have an entry
comment|//for this table.  It also has a unique index in case 'should not' is violated
comment|// First allocation of write id should add the table to the next_write_id meta table
comment|// The initial value for write id should be 1 and hence we add 1 with number of write ids
comment|// allocated here
name|String
name|s
init|=
literal|"INSERT INTO \"NEXT_WRITE_ID\" (\"NWI_DATABASE\", \"NWI_TABLE\", \"NWI_NEXT\") VALUES (?, ?, "
...
name|e
argument_list|,
literal|"seedWriteIdOnAcidConversion("
operator|+
name|rqst
operator|+
literal|")"
...
try|try
block|{
comment|//Idempotent case is handled by notify Event
name|lockInternal
argument_list|()
expr_stmt|;
name|dbConn
...
condition|)
block|{
comment|// in case of key duplicate error, retry as it might be because of race condition
if|if
condition|(
name|waitForRetry
argument_list|(
literal|"addWriteNotificationLog("
operator|+
name|acidWriteEvent
operator|+
literal|")"
...
name|MetaException
argument_list|(
literal|"Unable to add write notification event "
operator|+
name|StringUtils
operator|.
name|stringifyException
...
block|}
block|}
comment|/**    * Get invalidation info for the materialization. Currently, the materialization information    * only contains information about whether there was update/delete operations on the source    * tables used by the materialization since it was created.    */
annotation|@
name|Override
annotation|@
name|RetrySemantics
...
name|warn
argument_list|(
literal|"Materialization creation metadata does not contain any table"
argument_list|)
expr_stmt|;
return|return
literal|null
return|;
block|}
comment|// We are composing a query that returns a single row if an update happened after
comment|// the materialization was created. Otherwise, query returns 0 rows.
name|Connection
name|dbConn
init|=
literal|null
...
argument_list|)
expr_stmt|;
comment|// Parse validReaderWriteIdList from creation metadata
specifier|final
name|ValidTxnWriteIdList
name|validReaderWriteIdList
init|=
...
argument_list|)
decl_stmt|;
comment|// Get the valid write id list for the tables in current state
specifier|final
name|List
argument_list|<
name|TableValidWriteIds
argument_list|>
name|currentTblValidWriteIdsList
init|=
operator|new
name|ArrayList
argument_list|<>
...
specifier|final
name|ValidTxnWriteIdList
name|currentValidReaderWriteIdList
init|=
name|TxnCommonUtils
operator|.
...
argument_list|()
decl_stmt|;
comment|// compose a query that select transactions containing an update...
name|query
operator|.
name|append
argument_list|(
...
return|;
block|}
comment|// First, we check whether the low watermark has moved for any of the tables.
comment|// If it has, we return true, since it is not incrementally refreshable, e.g.,
comment|// one of the commits that are not available may be an update/delete.
name|ValidWriteIdList
name|currentTblValidWriteIdList
init|=
name|currentValidReaderWriteIdList
operator|.
name|getTableValidWriteIdList
...
name|warn
argument_list|(
literal|"Current ValidWriteIdList for table {} not present in creation metadata, this should not happen"
argument_list|,
name|fullyQualifiedName
argument_list|)
expr_stmt|;
...
name|debug
argument_list|(
literal|"Minimum open write id do not match for table {}"
argument_list|,
name|fullyQualifiedName
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|// ...for each of the tables that are part of the materialized view,
comment|// where the transaction had to be committed after the materialization was created...
if|if
condition|(
name|i
operator|!=
...
expr_stmt|;
block|}
comment|// ... and where the transaction has already been committed as per snapshot taken
comment|// when we are running current query
name|query
operator|.
name|append
argument_list|(
...
name|warn
argument_list|(
literal|"getMaterializationInvalidationInfo failed due to "
operator|+
name|getMessage
argument_list|(
name|ex
...
name|MetaException
argument_list|(
literal|"Unable to retrieve materialization invalidation information due to "
operator|+
name|StringUtils
operator|.
name|stringifyException
...
name|debug
argument_list|(
literal|"Acquiring lock for materialization rebuild with txnId={} for {}"
argument_list|,
name|txnId
argument_list|,
name|Warehouse
...
argument_list|()
expr_stmt|;
comment|/**        * MUTEX_KEY.MaterializationRebuild lock ensures that there is only 1 entry in        * Initiated/Working state for any resource. This ensures we do not run concurrent        * rebuild operations on any materialization.        */
name|handle
operator|=
name|getMutexAPI
argument_list|()
...
name|info
argument_list|(
literal|"Ignoring request to rebuild "
operator|+
name|dbName
operator|+
literal|"/"
...
name|tableName
operator|+
literal|" since it is already being rebuilt"
argument_list|)
expr_stmt|;
return|return
operator|new
...
name|warn
argument_list|(
literal|"lockMaterializationRebuild failed due to "
operator|+
name|getMessage
argument_list|(
name|ex
...
name|info
argument_list|(
literal|"No lock found for rebuild of "
operator|+
name|Warehouse
operator|.
name|getQualifiedName
...
argument_list|)
operator|+
literal|" when trying to heartbeat"
argument_list|)
expr_stmt|;
comment|// It could not be renewed, return that information
return|return
literal|false
return|;
block|}
...
argument_list|()
expr_stmt|;
comment|// It could be renewed, return that information
return|return
literal|true
return|;
block|}
...
name|e
argument_list|,
literal|"heartbeatLockMaterializationRebuild("
operator|+
name|Warehouse
operator|.
name|getQualifiedName
...
name|MetaException
argument_list|(
literal|"Unable to heartbeat rebuild lock due to "
operator|+
name|StringUtils
operator|.
name|stringifyException
...
condition|)
block|{
comment|// The heartbeat has timeout, double check whether we can remove it
name|long
name|txnId
init|=
name|rs
...
condition|)
block|{
comment|// Txn was committed (but notification was not received) or it was aborted.
comment|// Either case, we can clean it up
name|txnIds
operator|.
name|add
argument_list|(
...
name|e
argument_list|,
literal|"cleanupMaterializationRebuildLocks"
argument_list|)
expr_stmt|;
throw|throw
operator|new
name|MetaException
argument_list|(
literal|"Unable to clean rebuild locks due to "
operator|+
name|StringUtils
operator|.
name|stringifyException
...
name|MetaException
block|{
comment|// cleanup should be done only for objecdts belonging to default catalog
specifier|final
name|String
name|defaultCatalog
init|=
...
name|debug
argument_list|(
literal|"Skipping cleanup because db: "
operator|+
name|dbName
operator|+
literal|" belongs to catalog "
operator|+
literal|"other than default catalog: "
operator|+
name|db
operator|.
name|getCatalogName
...
name|debug
argument_list|(
literal|"Skipping cleanup because table: "
operator|+
name|tblName
operator|+
literal|" belongs to catalog "
...
name|debug
argument_list|(
literal|"Skipping cleanup because partitions belong to catalog "
operator|+
literal|"other than default catalog: "
operator|+
name|table
