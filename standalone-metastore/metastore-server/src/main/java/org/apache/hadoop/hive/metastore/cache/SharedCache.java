...
name|MemberName
block|{
name|TABLE_COL_STATS_CACHE
block|,
name|PARTITION_CACHE
block|,
name|PARTITION_COL_STATS_CACHE
block|,
name|AGGR_COL_STATS_CACHE
block|}
static|static
block|{
try|try
...
control|)
block|{
name|refreshTableWrapperInCache
argument_list|(
name|s
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|//cache will re-weigh the TableWrapper and record new weight.
name|cache
operator|.
name|put
argument_list|(
...
block|}
block|}
comment|//concurrency level of table cache. Set to -1 to let Guava use default.
specifier|public
name|void
...
expr_stmt|;
block|}
comment|//number of miliseconds between size updates.
specifier|public
name|void
...
expr_stmt|;
block|}
comment|//set the table size map to fake table size. This is for testing only.
specifier|public
name|void
...
name|debug
argument_list|(
literal|"Eviction happened for table "
operator|+
name|notification
operator|.
name|getKey
...
name|debug
argument_list|(
literal|"current table cache contains "
operator|+
name|tableCache
operator|.
name|size
...
name|setName
argument_list|(
literal|"SharedCache table size updater: Thread-"
operator|+
name|t
operator|.
name|getId
...
name|error
argument_list|(
literal|"Error while getting object size."
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
specifier|private
name|int
name|tableColStatsCacheSize
decl_stmt|;
specifier|private
name|int
name|partitionCacheSize
decl_stmt|;
specifier|private
name|int
name|partitionColStatsCacheSize
decl_stmt|;
specifier|private
name|int
name|aggrColStatsCacheSize
decl_stmt|;
specifier|private
name|ReentrantReadWriteLock
name|tableLock
...
name|otherSize
operator|=
name|getTableWrapperSizeWithoutMaps
argument_list|()
expr_stmt|;
block|}
specifier|private
...
name|error
argument_list|(
literal|"Not able to estimate size."
argument_list|,
name|ex
argument_list|)
expr_stmt|;
...
parameter_list|()
block|{
comment|//facilitate testing only. In production we won't use tableSizeMap at all.
if|if
condition|(
name|tableSizeMap
operator|!=
...
name|partVals
operator|+
literal|" is not present in the cache."
argument_list|)
expr_stmt|;
return|return
literal|null
...
name|partVals
operator|+
literal|" is missing from cache. Cannot update the partition stats in cache."
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|partVal
operator|+
literal|" is not present in the cache. Cannot update stats in cache."
argument_list|)
expr_stmt|;
return|return
literal|null
...
name|debug
argument_list|(
literal|"Write id list "
operator|+
name|writeIdList
operator|+
literal|" is not compatible with write id "
operator|+
name|writeId
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|//TODO : Need to get last analysed. This is not being used by anybody now.
name|List
argument_list|<
name|ColumnStatisticsObj
argument_list|>
...
name|info
argument_list|(
literal|"Stats not available in cachedStore for col "
operator|+
name|colName
operator|+
...
name|debug
argument_list|(
literal|"The current cached store transactional partition column statistics for {}.{}.{} "
operator|+
...
specifier|public
name|void
name|removeAllPartitionColStats
parameter_list|()
block|{
try|try
block|{
...
name|info
argument_list|(
literal|"Could not refresh the aggregate stat as partition "
operator|+
name|partValues
operator|+
...
return|return;
block|}
comment|// for txn tables, if the write id is modified means the partition is updated post fetching of stats. So
comment|// skip updating the aggregate stats in the cache.
name|long
name|writeId
init|=
name|partition
...
name|tblName
operator|+
literal|" is missing from cache. Cannot update table stats in cache"
argument_list|)
expr_stmt|;
return|return;
block|}
...
argument_list|)
expr_stmt|;
comment|//tblWrapper.updateTableObj(newTable, this);
name|String
name|newDbName
init|=
name|StringUtils
...
name|CacheUtils
operator|.
name|buildDbKeyWithDelimiterSuffix
argument_list|(
name|catName
argument_list|,
name|dbName
...
name|warn
argument_list|(
literal|"Failed to get partition column statistics"
argument_list|)
expr_stmt|;
block|}
finally|finally
