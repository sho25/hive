...
init|=
literal|"Default catalog, for Hive"
decl_stmt|;
specifier|public
specifier|static
specifier|final
...
return|;
block|}
comment|/**    * Build the database path based on catalog name and database name.  This should only be used    * when a database is being created or altered.  If you just want to find out the path a    * database is already using call {@link #getDatabasePath(Database)}.  If the passed in    * database already has a path set that will be used.  If not the location will be built using    * catalog's path and the database name.    * @param cat catalog the database is in    * @param db database object    * @return Path representing the directory for the database    * @throws MetaException when the file path cannot be properly determined from the configured    * file system.    */
specifier|public
name|Path
...
return|;
block|}
comment|/**    * Get the path specified by the database.  In the case of the default database the root of the    * warehouse is returned.    * @param db database to get the path of    * @return path to the database directory    * @throws MetaException when the file path cannot be properly determined from the configured    * file system.    */
specifier|public
name|Path
...
name|MetaException
block|{
comment|// TODO CAT - I am fairly certain that most calls to this are in error.  This should only be
comment|// used when the database location is unset, which should never happen except when a
comment|// new database is being created.  Once I have confirmation of this change calls of this to
comment|// getDatabasePath(), since it does the right thing.  Also, merge this with
comment|// determineDatabasePath() as it duplicates much of the logic.
if|if
condition|(
name|dbName
operator|.
...
return|;
block|}
comment|/**    * Returns the default location of the table path using the parent database's location    * @param db Database where the table is created    * @param tableName table name    * @return    * @throws MetaException    */
annotation|@
name|Deprecated
specifier|public
name|Path
...
return|;
block|}
comment|// A few situations where we need the default table path, without a DB object
specifier|public
name|Path
name|getDefaultTablePath
parameter_list|(
...
return|;
block|}
comment|/**    * Get table name in cat.db.table format.    * @param table table object    * @return fully qualified name.    */
specifier|public
specifier|static
name|String
...
condition|)
block|{
comment|// Copy the source files to cmroot. As the client will move the source files to another
comment|// location, we should make a copy of the files to cmroot instead of moving it.
name|cm
operator|.
name|recycle
argument_list|(
...
name|debug
argument_list|(
literal|"Exception when checking if path ("
operator|+
name|path
operator|+
literal|")"
...
name|warn
argument_list|(
literal|"Cannot create partition spec from "
operator|+
name|currPath
operator|+
...
return|;
block|}
comment|/**    * Returns the path object for the given partition key-value pairs and the base location    *    * @param tblPath - the base location for the partitions. Typically the table location    * @param pm - Partition key value pairs    * @return    * @throws MetaException    */
specifier|public
name|Path
name|getPartitionPath
parameter_list|(
...
name|MetaException
argument_list|(
literal|"Invalid number of partition keys found for "
operator|+
name|table
operator|.
name|getTableName
