...
name|fail
argument_list|(
literal|"MetaException should have occurred."
argument_list|)
expr_stmt|;
block|}
catch|catch
...
comment|// pool when the exception occurs in the third one.
comment|// When the exception occurs, we go to the finally part, but the map can be empty
comment|// (it depends on the progress of the other threads) so the folders won't be deleted.
comment|// Assert.assertTrue(metaStore.isPathExists(new Path(partition1.getSd().getLocation())));
comment|// Assert.assertTrue(metaStore.isPathExists(new Path(partition2.getSd().getLocation())));
comment|// Assert.assertTrue(metaStore.isPathExists(new Path(partition3.getSd().getLocation())));
block|}
