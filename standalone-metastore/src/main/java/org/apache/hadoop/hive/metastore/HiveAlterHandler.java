...
name|String
argument_list|>
name|txnDropTableEventResponses
init|=
name|Collections
operator|.
name|emptyMap
...
name|String
argument_list|>
name|txnCreateTableEventResponses
init|=
name|Collections
operator|.
name|emptyMap
...
name|String
argument_list|>
name|txnAddPartitionEventResponses
init|=
name|Collections
operator|.
name|emptyMap
...
condition|)
block|{
comment|// An ALTER_TABLE event will be created for any alter table operation happening inside the same
comment|// database, otherwise a rename between databases is considered a DROP_TABLE from the old database
comment|// and a CREATE_TABLE in the new database plus ADD_PARTITION operations if needed.
if|if
condition|(
operator|!
name|success
...
comment|// make this call whether the event failed or succeeded. To make this behavior consistent, then
comment|// this call will be made also for failed events even for renaming the table between databases
comment|// to avoid a large list of ADD_PARTITION unnecessary failed events.
name|MetaStoreListenerNotifier
operator|.
name|notifyEvent
argument_list|(
...
parameter_list|)
block|{
comment|// Just log the error but not throw an exception as this post-commit event should
comment|// not cause the HMS operation to fail.
name|LOG
operator|.
name|error
argument_list|(
literal|"ADD_PARTITION events for ALTER_TABLE rename operation cannot continue because the following "
operator|+
literal|"table was not found on the metastore: "
operator|+
name|newDbName
operator|+
literal|"."
