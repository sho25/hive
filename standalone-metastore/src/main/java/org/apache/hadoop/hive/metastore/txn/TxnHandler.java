...
argument_list|()
operator|+
literal|" are already present for repl policy "
operator|+
name|rqst
operator|.
name|getReplPolicy
...
name|s
init|=
literal|"select RTM_TARGET_TXN_ID from REPL_TXN_MAP where RTM_SRC_TXN_ID = "
operator|+
name|sourceTxnId
operator|+
literal|" and RTM_REPL_POLICY = "
...
name|info
argument_list|(
literal|"Target txn is missing for the input source txn for ReplPolicy: "
operator|+
name|quoteString
argument_list|(
name|replPolicy
...
name|s
operator|=
literal|"select ctc_database, ctc_table, ctc_timestamp from COMPLETED_TXN_COMPONENTS where ctc_txnid = "
operator|+
name|txnid
expr_stmt|;
name|rs
...
name|debug
argument_list|(
literal|"Going to register table modification in invalidation cache<"
operator|+
name|s
operator|+
literal|">"
...
block|}
block|}
comment|// If all the txns in the list have already allocated write ids, then just skip new allocations
name|long
name|numOfWriteIds
init|=
name|txnIds
...
name|s
init|=
literal|"select ctc_timestamp, ctc_txnid, ctc_database, ctc_table "
operator|+
