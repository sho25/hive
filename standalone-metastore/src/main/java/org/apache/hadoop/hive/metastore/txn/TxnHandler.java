...
literal|"(ctc_txnid,"
operator|+
literal|" ctc_database, ctc_table, ctc_partition, ctc_writeid)"
argument_list|,
name|rows
argument_list|)
decl_stmt|;
...
name|s
operator|=
literal|"select ctc_database, ctc_table, ctc_writeid, ctc_timestamp from COMPLETED_TXN_COMPONENTS"
operator|+
literal|" where ctc_txnid = "
operator|+
name|txnid
expr_stmt|;
name|LOG
...
condition|)
block|{
comment|// We only enter in this loop if the transaction actually affected any table
name|txnComponents
operator|.
name|add
argument_list|(
