...
argument_list|)
decl_stmt|;
comment|// If there are any open txns, then the minimum of min_open_txnid from MIN_HISTORY_LEVEL table
comment|// could be the min_uncommitted_txnid if lesser than NEXT_TXN_ID.ntxn_next.
name|s
operator|=
...
argument_list|()
decl_stmt|;
comment|// Turn off COLUMN_STATS_ACCURATE for txnids' components in TBLS and PARTITIONS
for|for
control|(
name|Long
name|txnId
...
control|)
block|{
comment|// Get table ids for the current txnId.
name|s
operator|=
literal|"select tbl_id from TBLS where txn_id = "
operator|+
name|txnId
expr_stmt|;
name|LOG
...
continue|continue;
block|}
comment|// Update COLUMN_STATS_AcCURATE.BASIC_STATS to false for each tableId.
name|prefix
operator|.
name|append
argument_list|(
literal|"delete from TABLE_PARAMS "
operator|+
literal|" where param_key = '"
operator|+
...
name|rc
operator|+
literal|" COLUMN_STATE_ACCURATE.BASIC_STATS states from TBLS"
argument_list|)
expr_stmt|;
block|}
name|queries
...
argument_list|)
expr_stmt|;
comment|// Get partition ids for the current txnId.
name|s
operator|=
literal|"select part_id from PARTITIONS where txn_id = "
operator|+
name|txnId
expr_stmt|;
name|LOG
...
continue|continue;
block|}
comment|// Update COLUMN_STATS_AcCURATE.BASIC_STATS to false for each ptnId.
name|prefix
operator|.
name|append
argument_list|(
literal|"delete from PARTITION_PARAMS "
operator|+
literal|" where param_key = '"
operator|+
literal|"COLUMN_STATS_ACCURATE"
...
name|rc
operator|+
literal|" COLUMN_STATE_ACCURATE.BASIC_STATS states from PARTITIONS"
argument_list|)
expr_stmt|;
block|}
name|queries
