...
block|}
block|}
comment|// Summary for column statistics
specifier|public
name|Integer
name|summaryStat
parameter_list|(
...
literal|11
decl_stmt|;
comment|// Define summary metrics for each column
for|for
control|(
name|Column
name|c
...
name|append
argument_list|(
literal|"PERCENTILE_APPROX(CAST("
operator|+
name|col
operator|+
literal|" AS DOUBLE),0.05),"
argument_list|)
expr_stmt|;
name|sql
operator|.
...
argument_list|)
expr_stmt|;
comment|// The summary query returns only one row
if|if
condition|(
name|rs
operator|.
...
name|format
argument_list|(
literal|"%%-%ds\t%%-11s\t%%-11s\t%%-11s\t%%-11s\t%%-11s\t%%-11s\t%%-11s\t%%-11s"
operator|+
literal|"\t%%-11s\t%%-11s\t%%-11s\t%%-11s\t%%-11s\n"
argument_list|,
name|maxColName
operator|+
literal|1
...
return|;
block|}
comment|// Summary for top column values
specifier|public
name|Integer
name|summaryTop
parameter_list|(
...
name|StringBuilder
argument_list|(
literal|"SELECT id, col, cnt FROM ("
operator|+
literal|"SELECT id, col, cnt, ROW_NUMBER() OVER (PARTITION BY id ORDER BY cnt DESC) rn "
operator|+
literal|"FROM (SELECT CAST(GROUPING__ID AS DECIMAL) id, COALESCE("
argument_list|)
decl_stmt|;
comment|// CAST AS INT does not work as expected (ID is still considered as STRING in ORDER BY for some reason)
name|int
name|topNum
init|=
name|evalPop
...
name|append
argument_list|(
literal|")) t) t WHERE rn<= "
operator|+
name|topNum
operator|+
literal|" ORDER BY id, cnt DESC"
argument_list|)
expr_stmt|;
comment|// Add LIMIT as Order by-s without limit can disabled for safety reasons
name|sql
operator|.
name|append
argument_list|(
...
expr_stmt|;
block|}
comment|// First value for next column
else|else
block|{
comment|// Pad with empty rows if the number of values in group is less than TOP num
for|for
control|(
name|int
name|j
