...
return|;
block|}
comment|/**    * Get data types for all columns of the SELECT statement    */
name|Row
...
argument_list|)
decl_stmt|;
comment|// Hive does not support ResultSetMetaData on PreparedStatement, and Hive DESCRIBE
comment|// does not support queries, so we have to execute the query with LIMIT 1
if|if
condition|(
name|connType
operator|==
...
expr_stmt|;
block|}
comment|// Hive DESCRIBE outputs "empty_string NULL" row before partition information
if|if
condition|(
name|typ
operator|==
