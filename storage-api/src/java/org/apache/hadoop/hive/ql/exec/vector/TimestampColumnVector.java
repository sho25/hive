...
expr_stmt|;
block|}
comment|/**    * Set a field from a Timestamp.    *    * This is a FAST version that assumes the caller has checked to make sure elementNum    * is correctly adjusted for isRepeating.  And, that the isNull entry    * has been set.  Only the output entry fields will be set by this method.    *    * For backward compatibility, this method does check if the timestamp is null and set the    * isNull entry appropriately.    *    * @param elementNum    * @param timestamp    */
specifier|public
name|void
name|set
parameter_list|(
...
expr_stmt|;
block|}
comment|/**    * Set a field from the current value in the scratch timestamp.    *    * This is a FAST version that assumes the caller has checked to make sure the current value in    * the scratch timestamp is valid and elementNum is correctly adjusted for isRepeating.  And,    * that the isNull entry has been set.  Only the output entry fields will be set by this method.    *    * @param elementNum    */
specifier|public
name|void
