...
expr_stmt|;
block|}
comment|/**    * A constructor to support rebuilding the BloomFilter from a serialized representation.    * @param bits - bits are used as such for bitset and are NOT copied, any changes to bits will affect bloom filter    * @param numFuncs - number of hash functions    */
specifier|public
name|BloomFilter
parameter_list|(
name|long
...
argument_list|()
expr_stmt|;
comment|// input long[] is set as such without copying, so any modification to the source will affect bloom filter
name|this
operator|.
name|bitSet
operator|=
...
expr_stmt|;
block|}
comment|/**    * Serialize a bloom filter    * @param out output stream to write to    * @param bloomFilter BloomFilter that needs to be seralized    */
specifier|public
specifier|static
name|void
name|serialize
...
name|IOException
block|{
comment|/**      * Serialized BloomFilter format:      * 1 byte for the number of hash functions.      * 1 big endian int(That is how OutputStream works) for the number of longs in the bitset      * big endian longs in the BloomFilter bitset      */
...
block|}
block|}
comment|/**    * Deserialize a bloom filter    * Read a byte stream, which was written by {@linkplain #serialize(OutputStream, BloomFilter)}    * into a {@code BloomFilter}    * @param in input bytestream    * @return deserialized BloomFilter    */
specifier|public
specifier|static
name|BloomFilter
name|deserialize
...
name|IOException
argument_list|(
literal|"Unable to deserialize BloomFilter"
argument_list|)
decl_stmt|;
name|io
operator|.
...
block|}
block|}
comment|// Given a byte array consisting of a serialized BloomFilter, gives the offset (from 0)
comment|// for the start of the serialized long values that make up the bitset.
comment|// NumHashFunctions (1 byte) + NumBits (4 bytes)
specifier|public
specifier|static
specifier|final
name|int
...
literal|5
decl_stmt|;
comment|/**    * Merges BloomFilter bf2 into bf1.    * Assumes 2 BloomFilters with the same size/hash functions are serialized to byte arrays    * @param bf1Bytes    * @param bf1Start    * @param bf1Length    * @param bf2Bytes    * @param bf2Start    * @param bf2Length    */
specifier|public
specifier|static
name|void
