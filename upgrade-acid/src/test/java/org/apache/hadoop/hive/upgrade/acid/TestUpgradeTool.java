...
argument_list|)
expr_stmt|;
comment|/*     todo: parse     target/tmp/org.apache.hadoop.hive.upgrade.acid.TestUpgradeTool-1527286256834/compacts_1527286277624.sql     make sure it's the only 'compacts' file and contains     ALTER TABLE default.tacid COMPACT 'major'; ALTER TABLE default.tacidpart PARTITION(p=10Y) COMPACT 'major';     * */
name|TxnStore
name|txnHandler
init|=
name|TxnUtils
...
argument_list|)
expr_stmt|;
comment|/*     * todo: parse     * convertToAcid_1527286288784.sql make sure it has     * ALTER TABLE default.tflat SET TBLPROPERTIES ('transactional'='true');     * convertToMM_1527286288784.sql make sure it has     * ALTER TABLE default.tflattext SET TBLPROPERTIES ('transactional'='true', 'transactional_properties'='insert_only');     * */
block|}
comment|/**    * includes 'execute' for postUpgrade    * @throws Exception    */
annotation|@
name|Test
specifier|public
name|void
...
name|runStatementOnDriver
argument_list|(
literal|"create table TAcid (a int, b int) clustered by (b) into 2 buckets stored as orc TBLPROPERTIES ('transactional'='false')"
argument_list|)
expr_stmt|;
name|runStatementOnDriver
argument_list|(
literal|"create table TAcidPart (a int, b int) partitioned by (p int)  clustered by (b) into 2 buckets  stored"
operator|+
...
argument_list|)
expr_stmt|;
comment|//todo: to test these need to link against 3.x libs - maven profiles?
comment|//runStatementOnDriver("create table TFlat (a int, b int) stored as orc tblproperties('transactional'='false')");
comment|//runStatementOnDriver("create table TFlatText (a int, b int) stored as textfile tblproperties('transactional'='false')");
name|Hive
name|db
init|=
name|Hive
...
argument_list|)
expr_stmt|;
comment|/**     todo: parse      target/tmp/org.apache.hadoop.hive.upgrade.acid.TestUpgradeTool-1527286026461/convertToAcid_1527286063065.sql      make sure it has:     ALTER TABLE default.tacid SET TBLPROPERTIES ('transactional'='true');     ALTER TABLE default.tacidpart SET TBLPROPERTIES ('transactional'='true');      */
block|}
specifier|private
specifier|static
name|void
