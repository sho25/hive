...
init|=
literal|"Strict-Transport-Security:max-age=31536000; includeSubDomains"
decl_stmt|;
specifier|private
specifier|static
specifier|final
...
return|;
block|}
comment|/**    * Same as {@link HttpServer#isInstrumentationAccessAllowed(ServletContext, HttpServletRequest, HttpServletResponse)}    * except that it returns true only if<code>hadoop.security.instrumentation.requires.admin</code> is set to true.    */
annotation|@
name|InterfaceAudience
operator|.
name|LimitedPrivate
...
expr_stmt|;
block|}
comment|/**    * Setup cross-origin requests (CORS) filter.    * @param b - builder    */
specifier|private
name|void
name|setupCORSFilter
parameter_list|(
name|Builder
name|b
parameter_list|)
...
return|;
block|}
comment|/**    * Secure the web server with PAM.    */
name|void
name|setupPam
parameter_list|(
name|Builder
...
condition|)
block|{
name|disableDirectoryListingOnServlet
argument_list|(
name|webAppContext
argument_list|)
expr_stmt|;
...
name|IOException
block|{
comment|// Set handling for low resource conditions.
specifier|final
name|LowResourceMonitor
name|low
init|=
...
name|info
argument_list|(
literal|"ASYNC_PROFILER_HOME env or -Dasync.profiler.home not specified. Disabling /prof endpoint.."
argument_list|)
expr_stmt|;
block|}
for|for
