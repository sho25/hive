...
block|}
block|}
comment|/**    * Construct a new instance.    *    * @param pattern Pattern to use for parsing or formatting    * @param forParsing Flag to indicate use of pattern    * @throws IllegalArgumentException if pattern is invalid    */
specifier|public
name|HiveSqlDateTimeFormatter
parameter_list|(
specifier|final
...
name|value
operator|+
literal|" could not be cast to string."
argument_list|,
name|e
argument_list|)
throw|;
...
block|}
comment|// Set case.
comment|// If the first letter is lowercase then the output is lowercase: 'mONTH' -> 'may'
comment|// If the first two letters are uppercase then the output is uppercase: 'MOnth' -> 'MAY'
comment|// If the first letter is uppercase and the second is lowercase then the output is capitalized:
comment|//     'Month' -> 'May'.
if|if
condition|(
name|Character
...
return|;
block|}
comment|/**    * Anything left unparsed at end of input string? Throw error.    * @param fullInput full input String    * @param index where we left off parsing    */
specifier|private
name|void
name|checkForLeftoverInput
parameter_list|(
...
block|}
block|}
comment|/**    * Create a timestamp from the list of values parsed from the input and the list of tokens    * parsed from the pattern input.    *    * We need to be able to parse input like "29.02.2000" (with pattern "dd.mm.yyyy")    * correctly – if we assigned the day value to the timestamp before the year value, then    * output would be 2000-02-28. So before creating the Timestamp we have to:    *    *  - Make a list of pairs.    *    Left value: only the Tokens that represent a temporal value    *    Right value: their corresponding int values parsed from the input    *  - Sort this list by length of base unit, in descending order (years before months, etc.).    *  - Then create the parsed output Timestamp object by creating a LocalDateTime object using the    *    token's TemporalField and the value.    *    * @param tokens list of tokens of any type, in order of pattern input    * @param temporalValues list of integer values parsed from the input, in order of input    * @return the parsed Timestamp    * @throws IllegalStateException if temporal values list and tokens in tokens list sizes are not    * equal    */
specifier|private
name|Timestamp
name|getTimestampFromValues
parameter_list|(
...
parameter_list|)
block|{
comment|// Get list of temporal Tokens
name|List
argument_list|<
name|Token
argument_list|>
...
argument_list|()
argument_list|,
literal|"temporalTokens list length ("
operator|+
name|temporalTokens
operator|.
name|size
argument_list|()
operator|+
literal|") differs from that of temporalValues (length: "
operator|+
name|temporalValues
operator|.
name|size
...
argument_list|)
expr_stmt|;
comment|// Get sorted list of temporal Token/value Pairs
name|List
argument_list|<
name|ImmutablePair
argument_list|<
...
name|separatorsFound
operator|+
literal|" does not match expected separator: "
operator|+
name|token
operator|.
name|string
