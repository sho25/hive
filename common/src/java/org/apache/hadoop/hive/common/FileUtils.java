...
condition|)
block|{
comment|// S3A file system has an optimized recursive directory listing implementation however it doesn't support filtering.
comment|// Therefore we filter the result set afterwards. This might be not so optimal in HDFS case (which does a tree walking) where a filter could have been used.
name|listS3FilesRecursive
argument_list|(
name|fileStatus
argument_list|,
name|fs
...
block|{
name|generalListStatusRecursively
argument_list|(
name|fs
argument_list|,
name|fileStatus
...
decl_stmt|;
return|return
name|checkIsOwnerOfFileHierarchy
argument_list|(
name|fsAsUser
argument_list|,
name|fileStatus
...
name|debug
argument_list|(
literal|"Skipping child access check since the directory is already removed"
argument_list|)
expr_stmt|;
return|return
literal|true
...
return|;
block|}
comment|/**    * Creates the directory and all necessary parent directories.    * @param fs FileSystem to use    * @param f path to create.    * @param conf Hive configuration    * @return true if directory created successfully.  False otherwise, including if it exists.    * @throws IOException exception in creating the directory    */
specifier|public
specifier|static
name|boolean
name|mkdir
...
name|debug
argument_list|(
literal|"copying srcPaths : {}, to DestPath :{} ,with doAs: {}"
argument_list|,
name|StringUtils
operator|.
name|join
...
name|error
argument_list|(
literal|"The jar file path {} does not exist"
argument_list|,
name|path
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**    * Returns the incremented sleep time in milli seconds.    * @param repeatNum number of retry done so far.    */
specifier|public
specifier|static
name|int
name|getSleepTime
