...
block|}
block|}
comment|/**    * Writes the default ConfVars out to a temporary File and returns    * a URL pointing to the temporary file.    * We need this in order to initialize the ConfVar properties    * in the underling Configuration object using the addResource(URL)    * method.    *    * Using Configuration.addResource(InputStream) would be a preferable    * approach, but it turns out that method is broken since Configuration    * tries to read the entire contents of the same InputStream repeatedly.    */
specifier|private
specifier|static
