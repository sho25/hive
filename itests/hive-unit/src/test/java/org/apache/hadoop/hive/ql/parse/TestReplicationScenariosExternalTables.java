...
specifier|public
name|void
name|retryIncBootstrapExternalTablesFromDifferentDumpWithoutCleanTablesConfig
parameter_list|()
throws|throws
name|Throwable
block|{
...
name|run
argument_list|(
literal|"create table t2 as select * from t1"
argument_list|)
operator|.
name|dump
argument_list|(
...
operator|.
name|Tuple
name|tupleBootstrapManagedTable
init|=
name|primary
operator|.
name|run
...
name|run
argument_list|(
literal|"create table t2 (id int) partitioned by (key int)"
argument_list|)
operator|.
name|run
argument_list|(
...
operator|.
name|Tuple
name|tupleIncConvertToExternalTbl
init|=
name|primary
operator|.
name|run
...
name|run
argument_list|(
literal|"select id from t2 where key=10"
argument_list|)
operator|.
name|verifyResult
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Check if the table type is set correctly in target.
name|replicaTable
operator|=
name|hiveForReplica
operator|.
...
argument_list|)
expr_stmt|;
comment|// Verify if new table location is set inside the base directory.
name|assertTablePartitionLocation
argument_list|(
name|primaryDbName
operator|+
...
argument_list|)
expr_stmt|;
comment|// Old location should be removed and set to new location.
name|assertFalse
argument_list|(
name|replica
operator|.
