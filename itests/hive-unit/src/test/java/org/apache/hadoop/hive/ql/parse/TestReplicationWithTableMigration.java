...
specifier|public
name|void
name|testBootstrapConvertedExternalTableAutoPurgeDataOnDrop
parameter_list|()
throws|throws
name|Throwable
block|{
...
name|run
argument_list|(
literal|"create table avro_tbl partitioned by (country string) ROW FORMAT SERDE "
operator|+
...
name|run
argument_list|(
literal|"insert into avro_tbl partition (country='india') values ('another', 13)"
argument_list|)
operator|.
name|dump
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// After drop, the external table data location should be auto deleted as it is converted one.
name|assertFalse
argument_list|(
name|replica
operator|.
...
specifier|public
name|void
name|testIncConvertedExternalTableAutoDeleteDataDirOnDrop
parameter_list|()
throws|throws
name|Throwable
block|{
...
name|run
argument_list|(
literal|"create table avro_tbl ROW FORMAT SERDE "
operator|+
literal|"'org.apache.hadoop.hive.serde2.avro.AvroSerDe' stored as avro "
operator|+
literal|"tblproperties ('avro.schema.url'='"
...
name|run
argument_list|(
literal|"insert into avro_tbl values ('str', 13)"
argument_list|)
operator|.
name|dump
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Data location is valid and is under default external warehouse directory.
name|Table
name|avroTable
init|=
name|replica
...
name|run
argument_list|(
literal|"create table texternal (id int) "
argument_list|)
operator|.
name|run
argument_list|(
literal|"insert into texternal values (1)"
argument_list|)
operator|.
name|dump
argument_list|(
...
name|run
argument_list|(
literal|"select id from texternal"
argument_list|)
operator|.
name|verifyResult
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// forcefully (setting db property) alter the table type. For acid table, set the bootstrap acid table to true. For
comment|// external table, the alter event should alter the table type at target cluster and then distcp should copy the
comment|// files. This is done to mock the upgrade done using HiveStrictManagedMigration.
name|HiveConf
name|hiveConf
init|=
name|primary
...
try|try
block|{
comment|//Set the txn config required for this test. This will not enable the full acid functionality in the warehouse.
name|hiveConf
operator|.
name|setBoolVar
argument_list|(
...
name|run
argument_list|(
literal|"insert into texternal values (2)"
argument_list|)
operator|.
name|run
argument_list|(
literal|"alter table tacid set tblproperties ('transactional'='true')"
argument_list|)
operator|.
name|run
argument_list|(
literal|"alter table texternal SET TBLPROPERTIES('EXTERNAL'='TRUE')"
argument_list|)
operator|.
name|run
argument_list|(
literal|"insert into texternal values (3)"
argument_list|)
operator|.
name|run
argument_list|(
...
name|add
argument_list|(
literal|"'hive.repl.bootstrap.acid.tables'='true'"
argument_list|)
expr_stmt|;
name|withConfigs
operator|.
...
name|add
argument_list|(
literal|"'hive.repl.include.external.tables'='true'"
argument_list|)
expr_stmt|;
name|withConfigs
operator|.
