...
name|String
argument_list|>
name|EXTRA_ZOOKEEPER_CONFIG
init|=
name|Arrays
operator|.
name|asList
argument_list|(
literal|"4lw.commands.whitelist=*"
argument_list|,
literal|"admin.enableServer=false"
argument_list|)
decl_stmt|;
specifier|private
name|MiniAccumuloCluster
...
expr_stmt|;
block|}
comment|/**    * Update ZooKeeper config file with the provided lines    *    * (unfortunately Accumulo Mini Cluster does not allow to customize the ZooKeeper config    * or to add additional system properties for the JVM of ZooKeeper, so we have to append the    * ZooKeeper config file directly)    */
specifier|private
name|void
name|appendZooKeeperConf
parameter_list|(
