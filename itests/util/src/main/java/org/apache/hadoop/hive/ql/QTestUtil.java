...
literal|false
decl_stmt|;
comment|// Parameters which help tracking cached data generation.
specifier|private
specifier|final
name|String
name|driverName
...
name|Exception
block|{
comment|// For now, to avoid changing multiple test templates, a null driver name avoids
comment|// data generation optimizations.
name|this
argument_list|(
name|outDir
argument_list|,
...
name|info
argument_list|(
literal|"Creating QTestUtil with settings: "
operator|+
literal|"driverName="
operator|+
name|driverName
operator|+
literal|", attemptingCacheUsage="
operator|+
name|attemptingCacheUsage
operator|+
literal|", test.build.dir="
...
condition|)
block|{
name|cleanupNonCacheUsage
argument_list|()
expr_stmt|;
block|}
else|else
block|{
name|cleanupCacheUsage
argument_list|()
expr_stmt|;
block|}
name|FunctionRegistry
...
name|IOException
block|{
comment|// Remove the Warehouse and metastore directories completely.
comment|// Also close the current db, since files are going to come in to replace it soon.
name|Preconditions
operator|.
name|checkState
argument_list|(
...
name|info
argument_list|(
literal|"Cleaning up metastore Dir: {}"
argument_list|,
name|metaStorePath
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Creating sources without data caching. attemptingCacheUsage={}, forceCreate={}"
argument_list|,
name|attemptingCacheUsage
argument_list|,
name|forceCreate
...
name|info
argument_list|(
literal|"Creating sources with data caching"
argument_list|)
expr_stmt|;
name|createSourcesCached
argument_list|(
name|scriptFile
argument_list|)
expr_stmt|;
...
name|HiveException
block|{
comment|// First check if the cache already exists. If it does just copy it over.
name|Path
name|cachedWarehousePath
init|=
operator|new
name|Path
argument_list|(
...
name|info
argument_list|(
literal|"Cached data found in {}. Attempting to use it"
argument_list|,
name|cachedDataPath
argument_list|)
expr_stmt|;
comment|// Data is alredy cached
comment|// Copy the files over to where they should be
name|Path
name|warehousePath
init|=
operator|new
...
parameter_list|)
block|{
comment|// Does not matter if it does not exist.
block|}
name|warehouseFs
operator|.
...
block|{
comment|// Something is missing. Cleanup. Re-generate and cache
name|LOG
operator|.
name|info
argument_list|(
literal|"Partial or no cached data found at {}. Cache will be created"
argument_list|,
name|cachedDataPath
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"No cached data found at {}. Cache will be created"
argument_list|,
name|cachedDataPath
argument_list|)
expr_stmt|;
comment|// No caching. Re-generate the data and cache it.
block|}
comment|// Generate and cache the data
name|setupDbsEtc
argument_list|(
literal|true
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Close the DB so that contents can be copied out safely.
name|Hive
operator|.
name|closeCurrent
argument_list|()
expr_stmt|;
comment|// Cache the sources
name|localFs
operator|.
name|mkdirs
argument_list|(
...
specifier|final
name|Pattern
name|metaStoreUriPattern
init|=
name|Pattern
operator|.
name|compile
argument_list|(
literal|"derby.*?databaseName=(.*?)(;|$)"
argument_list|)
decl_stmt|;
specifier|private
name|String
...
name|info
argument_list|(
literal|"TestWarehouseDir set to: [{}]"
argument_list|,
name|testWarehouse
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// The derby path comes from METASTORECONNECTURLKEY. Default ends up being target/junit_metastore_db
name|String
name|metaStoreConnectUrl
init|=
name|conf
operator|.
name|getVar
...
name|warn
argument_list|(
literal|"Disabling attempted cache usage since metastore path cannot be determined from {}"
argument_list|,
name|metaStoreConnectUrl
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// Turn this on only if the path is absolute, and is at least 3 deep - since we'll be deleting files later.
name|localFs
operator|=
name|FileSystem
operator|.
...
name|warn
argument_list|(
literal|"Disableing attempted cache usage since metastore path may not be absolute, or depth is< 3. MetaStorePath={}"
argument_list|,
name|metaStorePathString
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Generating file failed"
argument_list|,
name|e
argument_list|)
expr_stmt|;
