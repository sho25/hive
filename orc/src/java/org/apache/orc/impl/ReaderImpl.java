...
block|}
block|}
comment|/**    * Ensure this is an ORC file to prevent users from trying to read text    * files or RC files as ORC files.    * @param psLen the postscript length    * @param buffer the tail of the file    * @throws IOException    */
specifier|protected
specifier|static
name|void
name|ensureOrcFooter
...
name|FileFormatException
argument_list|(
literal|"Malformed ORC file. Invalid postscript length "
operator|+
name|psLen
argument_list|)
throw|;
...
condition|)
block|{
comment|// if it isn't there, this may be 0.11.0 version of the ORC file.
comment|// Read the first 3 bytes from the buffer to check for the header
if|if
condition|(
operator|!
name|Text
...
expr_stmt|;
block|}
comment|/**    * Get the WriterVersion based on the ORC file postscript.    * @param writerVersion the integer writer version    * @return the version of the software that produced the file    */
specifier|public
specifier|static
name|OrcFile
operator|.
