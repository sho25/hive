...
block|}
block|}
comment|/**    * Fetches kafka supervisor status report from druid overlod.    * @param table    * @return kafka supervisor report or null when druid overlord is unreachable.    */
annotation|@
name|Nullable
specifier|private
name|KafkaSupervisorReport
...
block|}
block|}
comment|/**    * Creates metadata moves then commit the Segment's metadata to Druid metadata store in one TxN    *    * @param table Hive table    * @param overwrite true if it is an insert overwrite table    *    * @throws MetaException if errors occurs.    */
specifier|protected
name|List
argument_list|<
name|DataSegment
...
comment|// e.g. Total size of segments in druid, loadstatus of table on historical nodes etc.
return|return
literal|null
return|;
block|}
