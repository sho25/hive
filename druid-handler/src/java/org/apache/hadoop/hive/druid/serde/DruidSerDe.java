...
comment|// SELECT YEAR(Calcs.date0) AS yr_date0_ok FROM druid_tableau.calcs Calcs WHERE (YEAR(Calcs.date0) IS NULL) LIMIT 1
comment|// is planed in a way where we only push a filter down and keep the project of null as hive project. Thus empty columns
name|columnNames
operator|=
name|Collections
operator|.
