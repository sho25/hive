...
return|;
block|}
comment|// Properties from configuration
specifier|final
name|int
name|numConnection
init|=
...
argument_list|)
decl_stmt|;
comment|// Create request to obtain nodes that are holding data for the given datasource and intervals
specifier|final
name|Lifecycle
name|lifecycle
init|=
...
return|;
block|}
comment|/* Method that splits Select query depending on the threshold so read can be    * parallelized. We will only contact the Druid broker to obtain all results. */
specifier|private
specifier|static
name|HiveDruidSplit
index|[]
...
condition|)
block|{
comment|// There are no rows for that time range, we can submit query as it is
return|return
operator|new
name|HiveDruidSplit
index|[]
