...
begin_comment
comment|/**  * Base record reader for given a Druid query. This class contains the logic to  * send the query to the broker and retrieve the results. The transformation to  * emit records needs to be done by the classes that extend the reader.  *<p>  * The key for each record will be a NullWritable, while the value will be a  * DruidWritable containing the timestamp as well as all values resulting from  * the query.  */
end_comment
...
name|queryResultsIterator
operator|=
name|createQueryResultsIterator
argument_list|()
expr_stmt|;
block|}
block|}
...
name|e
argument_list|,
literal|"Failure getting results for query[%s] url[%s] because of [%s]"
argument_list|,
name|query
argument_list|,
name|url
