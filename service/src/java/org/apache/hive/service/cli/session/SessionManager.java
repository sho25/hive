...
name|hiveConf
decl_stmt|;
comment|/** The lock that synchronizes the allowSessions flag and handleToSession map.       Active-passive HA first disables the connections, then closes existing one, making sure       there are no races between these two processes. */
specifier|private
specifier|final
name|Object
name|sessionAddLock
...
condition|)
block|{
name|registerOpenSesssionMetrics
argument_list|(
name|metrics
argument_list|)
expr_stmt|;
name|registerActiveSesssionMetrics
argument_list|(
name|metrics
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Connections limit are user: {} ipaddress: {} user-ipaddress: {}"
argument_list|,
name|userLimit
argument_list|,
name|ipAddressLimit
...
expr_stmt|;
block|}
comment|// in case of an overflow return -1
return|return
operator|(
name|int
operator|)
...
name|HiveSQLException
block|{
comment|// Check the flag opportunistically.
synchronized|synchronized
init|(
name|sessionAddLock
init|)
...
block|}
block|}
comment|// Do the expensive operations outside of any locks; we'll recheck the flag again at the end.
comment|// if client proxies connection, use forwarded ip-addresses instead of just the gateway
specifier|final
name|List
argument_list|<
name|String
...
name|warn
argument_list|(
literal|"Failed to close the session opened during an HA state change; ignoring"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
argument_list|)
decl_stmt|;
comment|// increment the counters only when there are no violations
if|if
condition|(
name|violation
operator|==
...
if|if
condition|(
name|trackConnectionsPerIpAddress
argument_list|(
name|clientIpAddress
argument_list|)
condition|)
...
if|if
condition|(
name|trackConnectionsPerUserIpAddress
argument_list|(
name|username
argument_list|,
name|clientIpAddress
...
return|;
block|}
comment|// order of forwarded ips per X-Forwarded-For http spec (client, proxy1, proxy2)
return|return
name|forwardedAddresses
operator|.
name|get
...
block|{
return|return
literal|"Connection limit per user reached (user: "
operator|+
name|username
operator|+
literal|" limit: "
...
block|{
return|return
literal|"Connection limit per ipaddress reached (ipaddress: "
operator|+
name|ipAddress
operator|+
literal|" limit: "
...
block|{
return|return
literal|"Connection limit per user:ipaddress reached (user:ipaddress: "
operator|+
name|username
operator|+
literal|":"
