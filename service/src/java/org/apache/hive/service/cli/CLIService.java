...
argument_list|)
decl_stmt|;
comment|// need to reset the monitor, as operation handle is not available down stream, Ideally the
comment|// monitor should be associated with the operation handle.
name|session
operator|.
...
parameter_list|)
block|{
comment|// Note: Hive ops do not use the normal Future failure path, so this will not happen
comment|//       in case of actual failure; the Future will just be done.
...
name|debug
argument_list|(
literal|"timed out and hence returning progress log as NULL"
argument_list|)
expr_stmt|;
return|return
operator|new
...
name|warn
argument_list|(
literal|"Error while getting progress update"
argument_list|,
name|e
argument_list|)
expr_stmt|;
