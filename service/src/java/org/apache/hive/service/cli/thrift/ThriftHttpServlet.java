...
condition|)
block|{
comment|// general format of XFF header is 'X-Forwarded-For: client, proxy1, proxy2' where left most being the client
name|clientIpAddress
operator|=
name|SessionManager
operator|.
...
name|info
argument_list|(
literal|"Trusted domain authN is enabled. clientIp from X-Forwarded-For header: {}"
argument_list|,
name|clientIpAddress
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|// Wait until all the data is received and then respond with 401
if|if
condition|(
name|request
operator|.
