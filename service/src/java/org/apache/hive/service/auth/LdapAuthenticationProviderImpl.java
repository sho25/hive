...
specifier|static
name|String
name|groupMembership_attr
decl_stmt|;
specifier|private
specifier|static
name|String
name|groupClass_attr
decl_stmt|;
name|LdapAuthenticationProviderImpl
parameter_list|(
name|HiveConf
...
name|debug
argument_list|(
literal|"Connected using principal="
operator|+
name|user
operator|+
literal|" at url="
...
expr_stmt|;
block|}
comment|// if a custom LDAP query is specified, it takes precedence over other configuration properties.
comment|// if the user being authenticated is part of the resultset from the custom query, it succeeds.
if|if
condition|(
name|customQuery
operator|!=
...
name|debug
argument_list|(
literal|"Authentication failed: User is not a member of configured groups"
argument_list|)
expr_stmt|;
throw|throw
operator|new
...
name|info
argument_list|(
literal|"Authentication succeeded using ldap user search"
argument_list|)
expr_stmt|;
return|return;
block|}
comment|// Ideally we should not be here. Indicates partially configured LDAP Service.
comment|// We allow it for now for backward compatibility.
name|LOG
operator|.
name|info
argument_list|(
...
literal|null
decl_stmt|;
comment|// we are using the first part of the userDN in the search criteria.
comment|// We know the DN is legal as we are able to bind with it, this is to confirm that its a user.
name|String
name|searchFilter
init|=
literal|"(&(|(objectClass=person)(objectClass=user)(objectClass=inetOrgPerson))("
...
name|debug
argument_list|(
literal|"Searching for groups under "
operator|+
name|groupBase
argument_list|)
expr_stmt|;
...
name|warn
argument_list|(
literal|"Exception searching for user groups"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// retrieve the attributes that are meant to desginate user DNs
name|returnAttributes
operator|=
operator|new
name|String
