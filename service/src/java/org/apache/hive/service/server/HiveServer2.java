...
try|try
block|{
name|logCompactionParameters
argument_list|(
name|hiveConf
argument_list|)
expr_stmt|;
name|maybeStartCompactorThreads
argument_list|(
name|hiveConf
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Compaction HS2 parameters:"
argument_list|)
expr_stmt|;
name|String
name|runWorkerIn
...
name|warn
argument_list|(
literal|"Invalid number of Compactor Worker threads({}) on HS2"
argument_list|,
name|numWorkers
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**    * The watcher class shuts down the server if there are no more active client    * sessions at the time of receiving a 'NodeDeleted' notification from ZooKeeper.    */
specifier|public
class|class
...
block|}
block|}
comment|/**    * @return true if the server instance was deregistered from ZooKeeper, else false. The function might    * be called even when the instance is not registered with the ZooKeeper (See    * SessionManage.closeSessionInternal()). In that case, return false since the deregistration has    * not really happened.    */
specifier|public
name|boolean
...
expr_stmt|;
block|}
comment|// Add a Znode to the specified ZooKeeper with name: serverUri=host:port;
comment|// version=versionInfo; sequence=sequenceNumber
name|zooKeeperHelper
operator|=
name|hiveConf
operator|.
...
name|info
argument_list|(
literal|"This HS2 instance will act as Compactor Worker with {} threads"
argument_list|,
name|numWorkers
argument_list|)
expr_stmt|;
