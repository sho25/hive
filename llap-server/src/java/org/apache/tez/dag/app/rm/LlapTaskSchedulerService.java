...
argument_list|()
decl_stmt|;
comment|// TODO Track resources used by this application on specific hosts, and make scheduling decisions
comment|// accordingly.
...
comment|// For non-random allocations - Walk through all pending tasks to get local assignments, then
comment|// start assigning them to non local hosts.
...
argument_list|()
expr_stmt|;
comment|// TODO Consider un-blacklisting the node since at least 1 slot should have become available
comment|// on the node.
block|}
elseif|else
if|if
...
block|}
block|}
comment|/* do not allocate nodes from this process, as then the pending tasks will get starved */
block|}
finally|finally
block|{
name|writeLock
