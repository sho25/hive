...
specifier|final
name|String
name|FAILED_TO_ALLOCATE_MSG
init|=
literal|"Failed to allocate [{}]X[{}] bytes after [{}] attempt, evicted [{}] bytes and partially allocated [{}] bytes"
decl_stmt|;
specifier|private
specifier|final
name|Arena
...
literal|0.01f
decl_stmt|;
comment|/**    * Max number of attempts to try allocations before giving up and move to forced evictions or/and freelist/brute    * force discards,    * Note to reader: why 5, i don't know, i run with 2 and 5 seems providing same results, thus the 5, better try more    * than start evictions, Also I am sure any value less than one will hurt.    */
specifier|private
specifier|static
specifier|final
name|int
name|MAX_FAST_ATTEMPT
init|=
literal|5
decl_stmt|;
comment|/**    * Default max amount of bytes to force evict on failed allocation attempts 16MB. used for testing, for production    * please adjust {@link ConfVars.LLAP_ALLOCATOR_MAX_FORCE_EVICTED}    */
specifier|private
specifier|static
specifier|final
name|int
name|MAX_FORCED_EVICTION_SIZE
init|=
literal|1024
operator|*
literal|1024
...
comment|// different allocation areas this is due to the fact that Allocations arrive in a burst fashion.
comment|// Simple case - say we have 2 arenas, 256Kb available in arena 2.
...
comment|// To mitigate that as of now:
comment|//  - First we try again (fast path):
comment|//  - Second try to push on the memoryManger/evictor to evict more.
comment|//  - Third bypass the memory manager and use discards method to brute force evict if needed.
name|int
...
condition|)
block|{
comment|// Try to evict more starting from dest.length - destAllocIx<< allocLog2, exponentially increase the eviction
comment|// request based on forceEvictAttempt counter. Why Exponentially increase, usually query fragments triggers a
comment|// burst of allocation at the same time, IMO it is worth to over evict thus next allocation will get free lunch.
name|currentEvicted
operator|=
name|memoryManager
operator|.
...
argument_list|)
expr_stmt|;
comment|// Note that memoryManager.evictMemory might evict 0 thus bail out after MAX_DISCARD_ATTEMPTS
name|totalForceEvictedBytes
operator|+=
name|currentEvicted
expr_stmt|;
...
literal|true
expr_stmt|;
comment|// Ensure all-or-nothing allocation.
for|for
control|(
name|int
name|i
...
name|info
argument_list|(
literal|"Failed to deallocate after a partially successful allocate: "
operator|+
name|dest
index|[
name|i
...
block|}
block|}
comment|//Need to release the un-allocated but reserved memory.
name|memoryManager
operator|.
name|releaseMemory
argument_list|(
