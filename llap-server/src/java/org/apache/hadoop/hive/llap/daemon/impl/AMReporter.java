...
name|debug
argument_list|(
literal|"Query complete received for {}"
argument_list|,
name|queryIdentifier
argument_list|)
expr_stmt|;
...
name|debug
argument_list|(
literal|"Removed following AMs due to query complete:"
argument_list|)
expr_stmt|;
for|for
control|(
...
expr_stmt|;
block|}
comment|// TODO: if there are more fields perhaps there should be an array of class.
name|TezAttemptArray
name|aw
init|=
operator|new
...
name|attemptId
operator|+
literal|" is no longer registered"
argument_list|)
expr_stmt|;
name|tasks
operator|.
...
return|;
block|}
comment|/**      * @return A snapshot of the tasks running at this daemon from this AM.      * Doesn't have to be consistent between multiple tasks; whether some task makes it into      * a given heartbeat when it's about to be started/about to finish is a timing issue anyway.      */
name|TaskSnapshot
name|getTasksSnapshot
parameter_list|()
block|{
