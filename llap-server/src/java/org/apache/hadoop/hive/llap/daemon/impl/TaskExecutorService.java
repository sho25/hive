...
begin_comment
comment|/**  * Task executor service provides method for scheduling tasks. Tasks submitted to executor service  * are submitted to wait queue for scheduling. Wait queue tasks are ordered based on the priority  * of the task. The internal wait queue scheduler moves tasks from wait queue when executor slots  * are available or when a higher priority task arrives and will schedule it for execution.  * When pre-emption is enabled, the tasks from wait queue can replace(pre-empt) a running task.  * The pre-empted task is reported back to the Application Master(AM) for it to be rescheduled.  *<br>  * Because of the concurrent nature of task submission, the position of the task in wait queue is  * held as long the scheduling of the task from wait queue (with or without pre-emption) is complete.  * The order of pre-emption is based on the ordering in the pre-emption queue. All tasks that cannot  * run to completion immediately (canFinish = false) are added to pre-emption queue.  *<br>  * When all the executor threads are occupied and wait queue is full, the task scheduler will  * return SubmissionState.REJECTED response  *<br>  * Task executor service can be shut down which will terminated all running tasks and reject all  * new tasks. Shutting down of the task executor service can be done gracefully or immediately.  */
end_comment
...
specifier|final
name|int
name|configuredMaxExecutors
decl_stmt|;
specifier|private
specifier|final
name|int
name|configuredWaitingQueueSize
decl_stmt|;
specifier|private
specifier|final
name|Clock
...
annotation|@
name|VisibleForTesting
comment|/**    * Accessed under the epic lock unless it's used for reporting.    * Added when calling schedule.    * Removed if evicted, killed, or finished with success or failure.    * In the former two cases, killTask is called on TaskRunnerCallable. In most cases, some    * cleanup is performed under the epic lock.    */
specifier|final
name|ConcurrentMap
argument_list|<
name|String
...
name|waitQueueComparatorClassName
argument_list|,
literal|"required argument 'waitQueueComparatorClassName' is null"
argument_list|)
expr_stmt|;
name|checkNotNull
argument_list|(
name|classLoader
argument_list|,
literal|"required argument 'classLoader' is null"
argument_list|)
expr_stmt|;
name|checkNotNull
argument_list|(
name|metrics
argument_list|,
literal|"required argument 'metrics' is null"
argument_list|)
expr_stmt|;
name|LOG
operator|.
...
expr_stmt|;
block|}
comment|/**    * Sets the TaskExecutorService capacity to the new values. Both the number of executors and the    * queue size should be smaller than that original values, so we do not mess up with the other    * settings. (For example: We do not allow higher executor number which could cause memory    * oversubscription since the container memory sizes are calculated based on the maximum memory    * and the maximum number of executors)    * Setting smaller capacity will not cancel or reject already executing or queued tasks in itself.    * @param newNumExecutors The new number of executors    * @param newWaitQueueSize The new number of wait queue size    */
annotation|@
name|Override
specifier|public
...
argument_list|(
literal|"Requested newNumExecutors="
operator|+
name|newNumExecutors
operator|+
literal|" is greater than the configured maximum="
operator|+
name|configuredMaxExecutors
argument_list|)
throw|;
...
name|IllegalArgumentException
argument_list|(
literal|"Requested newWaitQueueSize="
operator|+
name|newWaitQueueSize
operator|+
literal|" is greater than the configured maximum="
...
name|IllegalArgumentException
argument_list|(
literal|"Negative numExecutors is not allowed. Requested "
operator|+
literal|"newNumExecutors="
operator|+
name|newNumExecutors
argument_list|)
throw|;
...
name|IllegalArgumentException
argument_list|(
literal|"Negative waitQueueSize is not allowed. Requested "
operator|+
literal|"newWaitQueueSize="
operator|+
name|newWaitQueueSize
argument_list|)
throw|;
...
argument_list|)
expr_stmt|;
comment|// If there is no executor left so the queued tasks can not be finished anyway, kill them all.
if|if
condition|(
name|newNumExecutors
operator|==
...
name|task
operator|+
literal|"], since no executor left."
argument_list|)
expr_stmt|;
name|task
operator|.
...
name|info
argument_list|(
literal|"TaskExecutorService is setting capacity to: numExecutors="
operator|+
name|newNumExecutors
operator|+
literal|", waitQueueSize="
...
name|debug
argument_list|(
literal|"Fragment {} guaranteed state changed to {}; finishable {}, in wait queue {}, "
operator|+
literal|"in preemption queue {}"
argument_list|,
name|taskWrapper
...
argument_list|)
expr_stmt|;
comment|// Do the removal before we change the element, to avoid invalid queue ordering.
if|if
condition|(
name|isGuaranteed
operator|&&
...
name|taskWrapper
operator|.
name|updateIsGuaranteed
argument_list|(
name|isGuaranteed
argument_list|)
expr_stmt|;
name|forceReinsertIntoQueue
argument_list|(
name|taskWrapper
argument_list|,
name|isRemoved
...
condition|)
block|{
comment|// No need to check finishable here; if it was set it would already be in the queue.
...
argument_list|()
expr_stmt|;
comment|// TODO: this will probably send a message to AM. Is that needed here?
name|taskWrapper
operator|.
name|getTaskRunnerCallable
argument_list|()
...
decl_stmt|;
block|}
comment|/** A map where fragments live between when the TezTaskRunner2 calls taskSucceeded/Failed, or    *  an external caller calls killTask, and when the TaskRunnerCallable comes out of the    *  threadpool.    */
annotation|@
name|VisibleForTesting
specifier|final
name|ConcurrentMap
...
do|do
block|{
comment|// Tez internals may register the same task as completing multiple times.
name|val
operator|=
name|runningFragmentCount
operator|.
...
name|warn
argument_list|(
literal|"RunningFragmentCount is already 0. Multiple calls for the same completion."
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|debug
argument_list|(
literal|"{} scheduled for execution. canFinish={}, isGuaranteed={}"
argument_list|,
name|taskWrapper
operator|.
name|getRequestId
...
name|info
argument_list|(
literal|"Adding {} to pre-emption queue"
argument_list|,
name|taskWrapper
operator|.
name|getRequestId
...
parameter_list|)
block|{
comment|// TODO: is this check even needed given what the caller checks?
if|if
condition|(
operator|!
name|enablePreemption
...
argument_list|)
expr_stmt|;
comment|// This call checks under lock if we can actually preempt the task.
comment|// It is possible to have a race where the update (that's also under lock) makes the
comment|// task finishable or guaranteed between the remove and kill, but it's the same timing
comment|// issue as would happen is there was a tiny delay on the network, so we don't care.
name|TaskWrapper
name|victim
init|=
name|getSuitableVictimFromPreemptionQueue
argument_list|(
name|rejected
argument_list|)
decl_stmt|;
...
condition|)
block|{
comment|// No need to check guaranteed here; if it was false we would already be in the queue.
name|addToPreemptionQueue
argument_list|(
name|taskWrapper
argument_list|)
...
init|)
block|{
name|insertIntoPreemptionQueueOrFailUnlocked
argument_list|(
name|taskWrapper
argument_list|)
expr_stmt|;
...
argument_list|()
expr_stmt|;
comment|// Note that the code updating the state of the task does it when it's out of the queue.
comment|// So, the priorities in the queue should be correct; if the top task is not killable then
comment|// no task in the queue would be killable.
if|if
condition|(
name|taskWrapper
operator|==
...
literal|1
return|;
comment|// Then, non-finishable must always precede finishable.
name|v1
operator|=
name|o1
operator|.
