...
specifier|final
name|String
name|SHUFFLE_OS_CACHE_ALWAYS_EVICT
init|=
literal|"llap.shuffle.os.cache.always.evict"
decl_stmt|;
specifier|public
specifier|static
specifier|final
name|boolean
name|DEFAULT_SHUFFLE_OS_CACHE_ALWAYS_EVICT
init|=
literal|false
decl_stmt|;
specifier|public
...
name|info
argument_list|(
literal|"manageOsCache:{}, shouldAlwaysEvictOsCache:{}, readaheadLength:{}"
operator|+
literal|", maxShuffleConnections:{}, localDirs:{}"
operator|+
literal|", shuffleBufferSize:{}, shuffleTransferToAllowed:{}"
operator|+
literal|", connectionKeepAliveEnabled:{}, connectionKeepAliveTimeOut:{}"
operator|+
literal|", mapOutputMetaInfoCacheSize:{}, sslFileBufferSize:{}"
argument_list|,
name|manageOsCache
argument_list|,
name|shouldAlwaysEvictOsCache
...
return|;
block|}
comment|/**    * Register an application and it's associated credentials and user information.    *    * This method and unregisterDag must be synchronized externally to prevent races in shuffle token registration/unregistration    * This method may be called several times but we can only set the registeredDirectories once which will be    * in the first call in which they are not null.    *    * @param applicationIdString    * @param dagIdentifier    * @param appToken    * @param user    * @param appDirs    */
specifier|public
name|void
name|registerDag
parameter_list|(
...
argument_list|()
decl_stmt|;
comment|// In case of KeepAlive, ensure that timeout handler does not close connection until entire
comment|// response is written (i.e, response headers + mapOutput).
name|ChannelPipeline
name|pipeline
init|=
name|ch
...
block|}
block|}
comment|// If Keep alive is enabled, do not close the connection.
if|if
condition|(
operator|!
name|keepAliveParam
...
condition|)
block|{
comment|// On error close the channel.
name|future
operator|.
name|getChannel
argument_list|()
...
return|return;
block|}
comment|// Entire response is written out. Safe to enable timeout handling.
name|timeoutHandler
operator|.
name|setEnabledTimeout
argument_list|(
