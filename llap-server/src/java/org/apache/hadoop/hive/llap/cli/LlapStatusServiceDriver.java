...
name|ret
operator|=
name|populateAppStatusFromSliderStatus
argument_list|(
name|appName
argument_list|,
name|sliderClient
...
name|ret
operator|=
name|populateAppStatusFromSliderDiagnostics
argument_list|(
name|appName
argument_list|,
name|sliderClient
...
name|LlapSliderUtils
operator|.
name|getApplicationDiagnosticsFromYarnDiagnostics
argument_list|(
name|appReport
argument_list|,
name|LOG
...
name|warn
argument_list|(
literal|"AppDiagnostics not available for YARN application report"
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|processAppDiagnostics
argument_list|(
name|appStatusBuilder
argument_list|,
name|appDiagnostics
...
block|}
block|}
comment|/**    * Populates information from SliderStatus.    *    * @param appName    * @param sliderClient    * @param appStatusBuilder    * @return an ExitCode. An ExitCode other than ExitCode.SUCCESS implies future progress not possible    * @throws LlapStatusCliException    */
specifier|private
name|ExitCode
name|populateAppStatusFromSliderStatus
parameter_list|(
...
block|}
block|}
comment|/**    * Populates information based on the slider diagnostics call. Must be invoked    * after populating status from slider status.    * @param appName    * @param sliderClient    * @param appStatusBuilder    * @return    * @throws LlapStatusCliException    */
specifier|private
name|ExitCode
name|populateAppStatusFromSliderDiagnostics
parameter_list|(
...
name|sliderClient
operator|.
name|actionDiagnosticContainers
argument_list|(
name|args
argument_list|)
expr_stmt|;
...
name|SLIDER_CLIENT_ERROR_OTHER
argument_list|,
literal|"Failed to get container diagnostics from slider"
argument_list|,
name|e
argument_list|)
throw|;
...
name|info
argument_list|(
literal|"Slider container diagnostics not available"
argument_list|)
expr_stmt|;
return|return
name|ExitCode
...
return|;
block|}
comment|/**    * Populate additional information for containers from the LLAP registry. Must be invoked    * after Slider status. Also after slider-diagnostics.    * @param appStatusBuilder    * @return an ExitCode. An ExitCode other than ExitCode.SUCCESS implies future progress not possible    * @throws LlapStatusCliException    */
specifier|private
name|ExitCode
name|populateAppStatusFromLlapRegistry
parameter_list|(
...
comment|// launched between slider-status/slider-diagnostics. Skip for this iteration.
name|LOG
operator|.
name|debug
argument_list|(
...
parameter_list|)
block|{
comment|// For a running app this should be empty.
name|String
name|finalMessage
init|=
name|appDiagnostics
operator|.
name|getFinalMessage
argument_list|()
decl_stmt|;
name|Collection
argument_list|<
...
condition|)
block|{
comment|// New launch. Not available during slider status, but available now.
name|instance
operator|=
operator|new
name|LlapInstance
...
name|warn
argument_list|(
literal|"Unexpected containerstate={}, for container={}"
argument_list|,
name|containerInformation
operator|.
name|getState
...
name|debug
argument_list|(
literal|"ContainerInfos is null"
argument_list|)
expr_stmt|;
block|}
block|}
...
block|{
comment|// TODO HIVE-15865 Handle additional reasons like OS launch failed (Slider needs to give this info)
name|sb
operator|.
name|append
argument_list|(
...
expr_stmt|;
block|}
comment|// TODO HIVE-15865: Include information about pending requests, and last allocation time
comment|// once Slider provides this information.
break|break;
case|case
name|RUNNING_ALL
case|:
...
condition|)
block|{
comment|// slider has started llap application, now if for some reason state changes to COMPLETE then fail fast
if|if
condition|(
name|launchingState
operator|==
...
name|warn
argument_list|(
literal|"Application stopped while launching. COMPLETE state reached while waiting for RUNNING state."
operator|+
literal|" Failing "
operator|+
literal|"fast.."
...
name|warn
argument_list|(
literal|"Watch mode enabled and slider client creation failed. Retrying.."
argument_list|)
expr_stmt|;
name|numAttempts
operator|--
...
name|warn
argument_list|(
literal|"Watch mode enabled and got slider client error. Retrying.."
argument_list|)
expr_stmt|;
name|numAttempts
operator|--
...
block|{
comment|// TODO Remove this before commit.
block|}
block|}
