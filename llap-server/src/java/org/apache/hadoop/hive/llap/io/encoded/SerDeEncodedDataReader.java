...
throw|;
block|}
comment|// TODO: move to base class?
annotation|@
name|Override
specifier|protected
name|Void
...
literal|0
return|;
comment|// Always 0, no memory checks.
block|}
annotation|@
name|Override
specifier|public
...
literal|0
return|;
comment|// We never ever use any memory.
block|}
annotation|@
name|Override
specifier|public
...
literal|1
return|;
comment|// Meaningless for this writer.
block|}
annotation|@
name|Override
specifier|public
...
condition|)
block|{
comment|// TODO: will 0-length split work? should we assume 1+ chars and add 1 for isUnfortunate?
name|FileSplit
name|splitPart
init|=
operator|new
...
condition|)
block|{
comment|// This initializes cacheWriter with data.
name|readSplitFromFile
argument_list|(
name|split
argument_list|,
name|splitIncludes
...
condition|)
block|{
comment|// If we had a cache range already, it should not have been split.
assert|assert
name|cacheWriter
operator|==
literal|null
...
continue|continue;
block|}
comment|// TODO: We write each slice using a separate writer, so we don't share dictionaries. Fix?
name|ColumnStreamData
name|cb
init|=
name|CSD_POOL
...
argument_list|()
expr_stmt|;
comment|// Column IDs are only used to verify eventual writer includes.
name|cacheWriter
operator|=
operator|new
name|CacheWriter
...
expr_stmt|;
block|}
comment|// Close the writer to finalize the metadata. No catch since we cannot go on if this throws.
name|orcWriter
operator|.
name|close
argument_list|()
...
block|{
comment|// We don't need the source reader anymore.
if|if
condition|(
name|offsetReader
operator|!=
...
throw|;
block|}
comment|// TODO: ideally, we want to transform the rows to only have the included columns, and
comment|//       only write those to the writer, with modified schema; then map back to full set later.
return|return
name|OrcFile
operator|.
name|writerOptions
...
return|;
block|}
comment|// TODO: move to a superclass?
annotation|@
name|Override
specifier|public
name|void
