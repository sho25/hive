...
begin_comment
comment|/**  * Checks the status of the Llap.  */
end_comment
...
parameter_list|)
block|{
comment|// In case of failure, send back whatever is constructed so far - which would be from the AppReport
name|logError
argument_list|(
name|e
argument_list|)
...
block|}
block|}
comment|/**    * Populates parts of the AppStatus.    *    * @return an ExitCode. An ExitCode other than ExitCode.SUCCESS implies future progress not possible    * @throws LlapStatusCliException    */
specifier|private
name|ExitCode
name|processAppReport
parameter_list|(
...
block|}
block|}
comment|/**    * Populates information from YARN Service Status.    *    * @return an ExitCode. An ExitCode other than ExitCode.SUCCESS implies future progress not possible    * @throws LlapStatusCliException    */
specifier|private
name|ExitCode
name|populateAppStatusFromServiceStatus
parameter_list|(
...
return|;
block|}
comment|/**    * Populate additional information for containers from the LLAP registry. Must be invoked    * after YARN Service status and diagnostics.    * @return an ExitCode. An ExitCode other than ExitCode.SUCCESS implies future progress not possible    * @throws LlapStatusCliException    */
specifier|private
name|ExitCode
name|populateAppStatusFromLlapRegistry
parameter_list|(
...
name|error
argument_list|(
literal|"LLAP did not start. Check the application log for more info:\n"
operator|+
literal|"\tyarn logs --applicationId {} -out<path>"
argument_list|,
name|statusServiceDriver
operator|.
name|applicationId
...
argument_list|)
expr_stmt|;
comment|// HACK: due to the System.exit some log messages may not be present.
try|try
block|{
name|Thread
operator|.
...
return|;
block|}
comment|/**    * Helper method to construct a diagnostic message from a complete AppStatusBuilder.    */
specifier|private
specifier|static
name|String
name|constructDiagnostics
