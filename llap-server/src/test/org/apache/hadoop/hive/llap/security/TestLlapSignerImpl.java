...
parameter_list|()
block|{
comment|// We cannot synchronize properly with the internal thread (or something is weird about
comment|// the parent class internal state) and occasionally get null keys; loop until we can get
comment|// the key.
name|long
name|endTimeMs
init|=
...
literal|1000000000L
decl_stmt|;
comment|// Wait for at most 3 sec.
name|int
name|keyId
init|=
operator|-
...
name|AssertionError
argument_list|(
literal|"The ID didn't change but we couldn't get the key "
operator|+
name|keyId
argument_list|)
throw|;
...
name|AssertionError
argument_list|(
literal|"Cannot get a key from the base class; the last ID was "
operator|+
name|keyId
argument_list|)
throw|;
