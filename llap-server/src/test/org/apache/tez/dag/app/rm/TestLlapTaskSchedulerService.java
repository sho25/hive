...
block|{
comment|// TODO Fix the races and the broken scheduler control in the tests
specifier|private
specifier|static
specifier|final
name|String
...
decl_stmt|;
name|int
name|schedulerRunNumber
init|=
name|tsWrapper
operator|.
name|getSchedulerRunNumber
argument_list|()
decl_stmt|;
name|tsWrapper
operator|.
...
argument_list|)
expr_stmt|;
comment|// Limited allocations per node. So better locality when nodes come out of the blacklist
comment|// TODO This is flaky, since multiple nodes can get enabled at roughly the same time,
...
name|LlapTaskSchedulerServiceForTest
operator|.
name|LLAP_TASK_SCHEDULER_IN_TEST
argument_list|,
literal|true
argument_list|)
expr_stmt|;
...
argument_list|()
expr_stmt|;
comment|// One shceduler pass from the nodes that are added at startup
name|awaitSchedulerRunNumber
argument_list|(
literal|1
argument_list|)
...
name|ts
operator|.
name|forTestGetSchedulerRunNumber
argument_list|()
return|;
block|}
name|void
...
name|ts
operator|.
name|forTestSetupSchedulerStartWait
argument_list|(
name|val
argument_list|)
expr_stmt|;
...
name|ts
operator|.
name|forTestSignalSchedulerStart
argument_list|()
expr_stmt|;
block|}
name|void
...
name|LLAP_TASK_SCHEDULER_IN_TEST
init|=
literal|"llap.task.scheduler.in-test"
decl_stmt|;
specifier|private
specifier|final
name|boolean
...
specifier|final
name|Condition
name|forTestSchedulerRunCondition
init|=
name|forTestSchedulerLock
operator|.
name|newCondition
...
specifier|final
name|Condition
name|forTestSchedulerRunStartCondition
init|=
name|forTestSchedulerLock
operator|.
name|newCondition
...
specifier|final
name|AtomicBoolean
name|forTestControlledScheduleStart
init|=
operator|new
name|AtomicBoolean
argument_list|(
...
try|try
block|{
name|forTestAwaitSchedulerStartSignal
argument_list|()
expr_stmt|;
name|super
operator|.
...
block|{
name|forTestMaybeSignalSchedulerRun
argument_list|()
expr_stmt|;
block|}
block|}
