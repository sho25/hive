...
begin_comment
comment|/**  * Test HBaseDirectOUtputFormat and HBaseStorageHandler using a MiniCluster  */
end_comment
...
argument_list|)
expr_stmt|;
comment|// why we need to set all the 3 properties??
name|job
operator|.
...
name|testName
init|=
literal|"TestHiveHBaseTableOutputFormat"
decl_stmt|;
name|Path
name|methodTestDir
init|=
...
argument_list|)
expr_stmt|;
comment|//assertEquals(1l,result.getColumn(familyNameBytes,Bytes.toBytes(pair[0])).get(0).getTimestamp());
block|}
name|index
operator|++
expr_stmt|;
...
try|try
block|{
comment|//now setting the schema
name|HiveConf
name|hiveConf
init|=
name|HCatUtil
...
argument_list|)
expr_stmt|;
comment|//job.setOutputFormatClass(HiveHBaseTableOutputFormat.class);
name|job
operator|.
