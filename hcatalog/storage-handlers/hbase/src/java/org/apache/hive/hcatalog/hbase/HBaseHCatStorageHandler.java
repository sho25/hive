...
name|HBaseHCatStorageHandler
operator|.
name|getFullyQualifiedHBaseTableName
argument_list|(
name|tableInfo
argument_list|)
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|//Getting hbase delegation token in getInputSplits does not work with PIG. So need to
comment|//do it here
if|if
condition|(
name|jobConf
...
expr_stmt|;
block|}
comment|//This adds it directly to the jobConf. Setting in jobProperties does not get propagated
comment|//to JobConf as of now as the jobProperties is maintained per partition
comment|//TODO: Remove when HCAT-308 is fixed
...
argument_list|)
expr_stmt|;
comment|//Set up table in revision manager.
name|RevisionManager
name|rm
init|=
name|HBaseRevisionManagerUtil
...
parameter_list|)
block|{
comment|//setConf is called both during DDL operations and  mapred read/write jobs.
comment|//Creating a copy of conf for DDL and adding hbase-default and hbase-site.xml to it.
comment|//For jobs, maintaining a reference instead of cloning as we need to
comment|//  1) add hbase delegation token to the Credentials.
comment|//  2) set tmpjars on it. Putting in jobProperties does not get propagated to JobConf
comment|//     in case of InputFormat as they are maintained per partition.
comment|//Not adding hbase-default.xml and hbase-site.xml to jobConf as it will override any
comment|//hbase properties set in the JobConf by the user. In configureInputJobProperties and
comment|//configureOutputJobProperties, we take care of adding the default properties
comment|//that are not already present. TODO: Change to a copy for jobs after HCAT-308 is fixed.
name|jobConf
operator|=
name|conf
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|//Drop table in revision manager.
name|rm
operator|=
name|HBaseRevisionManagerUtil
operator|.
...
name|class
argument_list|,
comment|//hcat hbase storage handler jar
name|HBaseHCatStorageHandler
operator|.
name|class
argument_list|,
