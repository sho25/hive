...
return|return;
block|}
comment|// Set proxy user privilege and initialize the global state of ProxyUsers
name|Configuration
name|conf
init|=
operator|new
...
block|}
block|}
comment|/**    * Test for event-based replication scenario    *    * Does not test if replication actually happened, merely tests if we're able to consume a repl task    * iter appropriately, calling all the functions expected of the interface, without errors.    */
annotation|@
name|Test
specifier|public
name|void
name|testReplicationTaskIter
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// set really low batch size to ensure batching
name|cfg
operator|.
name|set
argument_list|(
...
name|dbName
init|=
literal|"testReplicationTaskIter"
decl_stmt|;
name|long
name|baseId
init|=
...
decl_stmt|;
block|{
comment|// Perform some operations
comment|// 1: Create a db after dropping if needed => 1 or 2 events
name|sourceMetastore
operator|.
name|dropDatabase
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// 2: Create an unpartitioned table T1 => 1 event
name|String
name|tblName1
init|=
literal|"T1"
...
argument_list|)
expr_stmt|;
comment|// 3: Create a partitioned table T2 => 1 event
name|String
name|tblName2
init|=
literal|"T2"
...
argument_list|)
expr_stmt|;
comment|// 4: Add a partition P1 to T2 => 1 event
name|HCatTable
name|table2Created
init|=
name|sourceMetastore
...
argument_list|)
expr_stmt|;
comment|// 5 : Create and drop partition P2 to T2 10 times => 20 events
for|for
control|(
name|int
name|i
...
expr_stmt|;
block|}
comment|// 6 : Drop table T1 => 1 event
name|sourceMetastore
operator|.
name|dropTable
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// verify that the number of events since we began is at least 25 more
name|long
name|currId
init|=
name|sourceMetastore
...
name|currId
operator|+
literal|"] must be more than 25 greater than baseId["
operator|+
name|baseId
operator|+
literal|"]"
...
expr_stmt|;
block|}
comment|// do rest of tests on db we just picked up above.
name|List
argument_list|<
...
name|append
argument_list|(
literal|"cleanupLocationsPerRetry entries :"
operator|+
name|locns
operator|.
name|size
...
name|append
argument_list|(
literal|"cleanupLocationsAfterEvent entries :"
operator|+
name|locns
operator|.
name|size
...
name|info
argument_list|(
literal|"task was not actionable."
argument_list|)
expr_stmt|;
block|}
block|}
