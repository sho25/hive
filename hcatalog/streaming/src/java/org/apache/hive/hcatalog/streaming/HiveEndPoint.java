...
begin_comment
comment|/**  * Information about the hive end point (i.e. table or partition) to write to.  * A light weight object that does NOT internally hold on to resources such as  * network connections. It can be stored in Hashed containers such as sets and hash tables.  * @deprecated as of Hive 3.0.0, replaced by org.apache.hive.streaming.HiveStreamingConnection  */
end_comment
...
parameter_list|)
block|{
comment|//ignore this - multiple clients may be trying to create the same partition
comment|//AddPartitionDesc has ifExists flag but it's not propagated to
comment|// HMSHnalder.add_partitions_core() and so it throws...
block|}
catch|catch
parameter_list|(
name|HiveException
...
name|INACTIVE
expr_stmt|;
comment|// The Write Ids returned for the transaction batch is also sequential
name|recordWriter
operator|.
name|newBatch
argument_list|(
