...
begin_comment
comment|/**  * Pig {@link LoadFunc} to read data from Howl  */
end_comment
...
comment|// call setInput on OwlInputFormat only in the frontend because internally
comment|// it makes calls to the owl server - we don't want these to happen in
comment|// the backend
...
comment|// OwlInputFormat - we have to get the RequiredFields information
...
comment|// OwlInputFormat needs to know about pruned projections - so doing it
comment|// here will ensure we communicate to OwlInputFormat about pruned
...
condition|)
block|{
comment|// convert to owlschema and pass to OwlInputFormat
try|try
block|{
name|outputSchema
operator|=
...
comment|// setOutputSchema on OwlInputFormat
if|if
condition|(
name|HCatUtil
operator|.
...
name|PigHCatUtil
operator|.
name|validateHowlTableSchemaFollowsPigRules
argument_list|(
name|howlTableSchema
argument_list|)
expr_stmt|;
...
name|PigException
argument_list|(
literal|"Table schema incompatible for reading through HowlLoader :"
operator|+
name|e
operator|.
name|getMessage
...
comment|// howl and pass it in setLocation()
name|partitionFilterString
operator|=
name|getHowlComparisonString
argument_list|(
name|partitionFilter
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// Howl will always prune columns based on what we ask of it - so the
...
condition|)
block|{
comment|// call getOwlComparisonString on lhs and rhs, and and join the
