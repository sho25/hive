...
condition|)
block|{
comment|// TODO: Hack! Pig messes up mapred.output.dir, when 2 Storers are used in the same Pig script.
comment|// Workaround: Set mapred.output.dir from OutputJobInfo.
name|resetMapRedOutputDirFromJobInfo
argument_list|(
name|context
operator|.
name|getConfiguration
...
condition|)
block|{
comment|//TODO fix this hack, something wrong with pig
comment|//running multiple storers in a single job, the real output dir got overwritten or something
comment|//the location in OutputJobInfo is still correct so we'll use that
comment|//TestHCatStorer.testMultiPartColsInData() used to fail without this
name|resetMapRedOutputDirFromJobInfo
argument_list|(
name|context
operator|.
...
block|}
block|}
comment|/**    * TODO: Clean up this Hack! Resetting mapred.output.dir from OutputJobInfo.    * This works around PIG-2578, where Pig messes up output-directory    * if multiple storers are used in the same pig-script.    * @param config The configuration whose mapred.output.dir is to be reset.    */
specifier|private
name|void
name|resetMapRedOutputDirFromJobInfo
parameter_list|(
