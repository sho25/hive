...
argument_list|)
decl_stmt|;
comment|//Pig makes 3 calls to HCatOutputFormat.setOutput(HCatStorer) with different JobConf
comment|//which leads to creating 2 transactions.
comment|//So apart from fixing HCatStorer to pass same OutputJobInfo, making the call idempotent for other
comment|//cases which might call multiple times but with same JobConf.
name|Transaction
name|txn
init|=
literal|null
...
parameter_list|)
block|{
comment|//Not cloning as we want to set tmpjars on it. Putting in jobProperties does not
comment|//get propagated to JobConf in case of InputFormat as they are maintained per partition.
comment|//Also we need to add hbase delegation token to the Credentials.
name|hbaseConf
operator|=
name|conf
expr_stmt|;
