...
expr_stmt|;
block|}
comment|/**    * Logically split the set of input files for the job. Returns the    * underlying InputFormat's splits    * @param jobContext the job context object    * @return the splits, an HCatInputSplit wrapper over the storage    *         driver InputSplits    * @throws IOException or InterruptedException    */
annotation|@
name|Override
specifier|public
name|List
...
argument_list|()
decl_stmt|;
comment|//If running through a Pig job, the InputJobInfo will not be available in the
comment|//backend process context (since HCatLoader works on a copy of the JobContext and does
comment|//not call HCatInputFormat.setInput in the backend process).
comment|//So this function should NOT attempt to read the InputJobInfo.
name|HCatInputStorageDriver
name|storageDriver
decl_stmt|;
try|try
...
argument_list|)
decl_stmt|;
comment|//Create the underlying input formats record record and an HCat wrapper
name|RecordReader
name|recordReader
init|=
name|inputFormat
...
name|Exception
argument_list|(
literal|"job information not found in JobContext. HCatInputFormat.setInput() not called?"
argument_list|)
throw|;
block|}
return|return
