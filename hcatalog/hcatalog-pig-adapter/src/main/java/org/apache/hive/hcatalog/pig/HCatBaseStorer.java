...
decl_stmt|;
comment|/**    * Controls what happens when incoming Pig value is out-of-range for target Hive column    */
specifier|static
specifier|final
name|String
name|ON_OOR_VALUE_OPT
init|=
literal|"onOutOfRangeValue"
decl_stmt|;
comment|/**    * prop name in Configuration/context    */
specifier|static
specifier|final
name|String
name|ON_OORA_VALUE_PROP
init|=
literal|"hcat.pig.store.onoutofrangevalue"
decl_stmt|;
comment|/**    * valid values for ON_OOR_VALUE_OPT    */
specifier|public
specifier|static
enum|enum
...
name|sign
decl_stmt|;
comment|//it's key that this is a per HCatStorer instance object
specifier|private
specifier|final
name|DataLossLogger
name|dataLossLogger
...
name|debug
argument_list|(
literal|"convertPigSchemaToHCatSchema(pigSchema,tblSchema)=("
operator|+
name|pigSchema
operator|+
literal|","
...
argument_list|)
decl_stmt|;
comment|//if writing to a partitioned table, then pigSchema will have more columns than tableSchema
comment|//partition columns are not part of tableSchema... e.g. TestHCatStorer#testPartColsInData()
comment|//        HCatUtil.assertNotNull(hcatFieldSchema, "Nothing matching '" + fSchema.alias + "' found " +
comment|//                "in target table schema", LOG);
name|fieldSchemas
operator|.
name|add
argument_list|(
...
name|debug
argument_list|(
literal|"convertPigSchemaToHCatSchema(computed)=("
operator|+
name|s
operator|+
literal|")"
...
return|;
block|}
comment|/**    * Here we are processing HCat table schema as derived from metastore,     * thus it should have information about all fields/sub-fields, but not for partition columns    */
specifier|private
name|HCatFieldSchema
name|getHCatFSFromPigFS
parameter_list|(
...
name|debug
argument_list|(
literal|"hcatFieldSchema is null for fSchema '"
operator|+
name|fSchema
operator|.
name|alias
...
argument_list|)
expr_stmt|;
comment|//throw new IllegalArgumentException("hcatFiledSchema is null; fSchema=" + fSchema + " " +
comment|//      "(pigSchema, tableSchema)=(" + pigSchema + "," + tableSchema + ")");
block|}
block|}
name|byte
name|type
...
name|DATETIME
case|:
comment|//Pig DATETIME can map to DATE or TIMESTAMP (see HCatBaseStorer#validateSchema()) which
comment|//is controlled by Hive target table information
if|if
condition|(
name|hcatFieldSchema
operator|!=
...
name|BIGINTEGER
case|:
comment|//fall through; doesn't map to Hive/Hcat type; here for completeness
default|default:
throw|throw
operator|new
name|FrontendException
...
argument_list|)
return|;
comment|//toEpochMilli() returns UTC time regardless of TZ
case|case
name|DATE
case|:
comment|/**          * We ignore any TZ setting on Pig value since java.sql.Date doesn't have it (in any          * meaningful way).  So the assumption is that if Pig value has 0 time component (midnight)          * we assume it reasonably 'fits' into a Hive DATE.  If time part is not 0, it's considered          * out of range for target type.          */
name|DateTime
name|dateTime
init|=
operator|(
...
name|hcatFS
argument_list|,
literal|"Time component must be 0 (midnight) in local timezone; Local TZ val='"
operator|+
name|pigObj
operator|+
literal|"'"
...
expr_stmt|;
block|}
comment|/**    * depending on user config, throws an exception or logs a msg if the incoming Pig value is    * out-of-range for target type.    * @param additionalMsg may be {@code null}     */
specifier|private
name|void
name|handleOutOfRangeValue
parameter_list|(
...
literal|0
decl_stmt|;
comment|//helps with debug messages
for|for
control|(
name|FieldSchema
name|pigField
...
block|}
block|}
comment|/**    * This method encodes which Pig type can map (be stored in) to which HCat type.    * @throws HCatException    * @throws FrontendException    */
specifier|private
name|void
name|validateSchema
parameter_list|(
...
parameter_list|,
name|Schema
name|topLevelPigSchema
parameter_list|,
name|HCatSchema
name|topLevelHCatSchema
parameter_list|,
name|int
name|columnPos
parameter_list|)
...
condition|)
block|{
comment|//there is no point trying to validate further if we have no type info about target field
switch|switch
condition|(
name|type
condition|)
...
name|BIGDECIMAL
case|:
name|throwTypeMismatchException
argument_list|(
name|type
argument_list|,
name|Lists
...
name|type
operator|+
literal|"' Pig datatype in column "
operator|+
name|columnPos
operator|+
literal|"(0-based) is not supported by HCat"
argument_list|,
name|PigHCatUtil
operator|.
name|PIG_EXCEPTION_CODE
...
condition|)
block|{
comment|//see HIVE-6194
throw|throw
operator|new
name|FrontendException
argument_list|(
literal|"(pigSch,hcatSchema)=("
operator|+
name|pigField
operator|+
literal|","
...
name|Type
argument_list|>
name|hcatRequiredType
parameter_list|,
name|HCatFieldSchema
name|hcatActualField
parameter_list|,
name|int
name|columnPos
parameter_list|)
...
argument_list|)
operator|+
literal|"' type in column "
operator|+
name|columnPos
operator|+
literal|"(0-based) cannot map to HCat '"
operator|+
name|hcatActualField
operator|.
name|getType
argument_list|()
operator|+
literal|"'type.  Target filed must be of HCat type {"
operator|+
name|StringUtils
operator|.
name|join
...
name|IOException
block|{   }
comment|/**    * todo: when job is complete, should print the msgCount table to log     */
specifier|private
specifier|static
specifier|final
class|class
...
literal|" "
operator|+
literal|"Will write NULL instead.  Only 1 such message per type/column is emitted."
argument_list|)
expr_stmt|;
block|}
name|msgCount
