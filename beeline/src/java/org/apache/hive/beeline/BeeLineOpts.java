...
specifier|final
name|int
name|DEFAULT_INCREMENTAL_BUFFER_ROWS
init|=
literal|1000
decl_stmt|;
specifier|public
...
specifier|private
name|boolean
name|convertBinaryArrayToString
init|=
literal|true
decl_stmt|;
specifier|private
...
name|Ignore
block|{
comment|// marker annotations for functions that Reflector should ignore / pretend it does not exist
comment|// NOTE: BeeLineOpts uses Reflector in an extensive way to call getters and setters on itself
comment|// If you want to add any getters or setters to this class, but not have it interfere with
comment|// saved variables in beeline.properties, careful use of this marker is needed.
comment|// Also possible to get this by naming these functions obtainBlah instead of getBlah
comment|// and so on, but that is not explicit and will likely surprise people looking at the
comment|// code in the future. Better to be explicit in intent.
block|}
specifier|public
interface|interface
name|Env
block|{
comment|// Env interface to mock out dealing with Environment variables
comment|// This allows us to interface with Environment vars through
comment|// BeeLineOpts while allowing tests to mock out Env setting if needed.
name|String
name|get
parameter_list|(
name|String
...
argument_list|)
return|;
comment|// base env impl simply defers to System.getenv.
block|}
block|}
decl_stmt|;
specifier|public
...
argument_list|()
decl_stmt|;
comment|// make sure we initialize if necessary
return|return
name|names
operator|.
name|toArray
...
specifier|public
name|void
name|setConvertBinaryArrayToString
parameter_list|(
name|boolean
name|convert
parameter_list|)
...
return|;
block|}
comment|/**    * @param numRows - the number of rows to store in history file    */
specifier|public
name|void
name|setMaxHistoryRows
parameter_list|(
name|int
name|numRows
parameter_list|)
...
expr_stmt|;
block|}
comment|/**    * In beeline mode returns the beeline option provided by command line argument or config file    * In compatibility mode returns the value of the hive.cli.print.current.db config variable    * @return Should the current db displayed in the prompt    */
specifier|public
name|boolean
