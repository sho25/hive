...
argument_list|,
operator|new
name|LlapUmbilicalExternalPolicyProvider
argument_list|()
argument_list|)
expr_stmt|;
block|}
...
parameter_list|)
block|{
comment|// Maintain count of outstanding requests for tokenIdentifier.
name|int
index|[]
name|refCount
init|=
...
argument_list|)
expr_stmt|;
comment|// Should only need to insert the token the first time.
name|jobTokenSecretManager
operator|.
name|addTokenForJob
argument_list|(
...
block|{
comment|// Maintain count of outstanding requests for tokenIdentifier.
comment|// If count goes to 0, it is safe to remove the token.
name|int
index|[]
name|refCount
init|=
...
name|warn
argument_list|(
literal|"No refCount found for tokenIdentifier "
operator|+
name|tokenIdentifier
argument_list|)
expr_stmt|;
