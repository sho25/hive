...
operator|++
expr_stmt|;
comment|// Don't retry immediately - use delay with exponential backoff
name|long
name|retryDelay
init|=
name|determineRetryDelay
argument_list|()
decl_stmt|;
name|LOG
operator|.
name|info
argument_list|(
literal|"Queueing fragment for resubmission {}, attempt {}, delay {}"
argument_list|,
name|this
operator|.
name|requestInfo
...
argument_list|()
expr_stmt|;
comment|// Don't timeout because of retry delay
name|retryExecutor
operator|.
name|schedule
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// taskKill() should also be received during a rejected submission,
comment|// we will let that logic handle retries.
return|return;
block|}
block|}
if|if
...
parameter_list|()
block|{
comment|// Delay with exponential backoff
name|int
name|maxDelay
init|=
operator|(
...
name|append
argument_list|(
literal|"LlapTaskUmbilicalExternalClient"
argument_list|)
expr_stmt|;
if|if
condition|(
...
condition|)
block|{
comment|// A task kill while the request is still in PENDING state means the request should be retried.
name|LOG
operator|.
name|info
argument_list|(
literal|"Received task kill for {} which is still in pending state. Retry submission."
argument_list|,
name|taskAttemptIdString
argument_list|)
expr_stmt|;
