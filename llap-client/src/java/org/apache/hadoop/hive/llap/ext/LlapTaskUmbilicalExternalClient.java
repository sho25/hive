...
argument_list|(
literal|"Client has already been closed"
argument_list|)
throw|;
block|}
name|closed
...
literal|true
expr_stmt|;
comment|// Check if the request is registered - if so we can cancel the request
for|for
control|(
name|Map
operator|.
...
name|info
argument_list|(
literal|"Client has already been closed, but received heartbeat from "
operator|+
name|taskAttemptIdString
argument_list|)
expr_stmt|;
comment|// Set shouldDie response so the LLAP daemon closes this umbilical connection.
name|response
operator|.
name|setShouldDie
argument_list|()
...
parameter_list|)
block|{
comment|// Add a bit of delay in case the daemon has not closed the umbilical connection yet.
name|clientCleanupExecuter
operator|.
name|schedule
argument_list|(
operator|new
name|ClientCleanupTask
argument_list|(
name|client
argument_list|)
argument_list|,
...
condition|)
block|{
comment|// No more outstanding connections, ok to close.
try|try
block|{
name|LOG
operator|.
...
name|error
argument_list|(
literal|"Error cleaning up client"
argument_list|,
name|err
argument_list|)
expr_stmt|;
...
block|{
comment|// Reschedule this task for later.
name|LOG
operator|.
name|debug
argument_list|(
literal|"Client still has umbilical connection - rescheduling cleanup."
argument_list|)
expr_stmt|;
name|scheduleClientForCleanup
argument_list|(
