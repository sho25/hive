...
argument_list|)
expr_stmt|;
comment|// Save so we can verify end of stream
comment|// We need mark support - wrap with BufferedInputStream.
name|din
operator|=
operator|new
name|DataInputStream
...
block|{
comment|// End of input. Confirm we got end of stream indicator from server,
comment|// as well as DONE status from fragment execution.
if|if
condition|(
operator|!
name|cin
...
name|IOException
argument_list|(
literal|"Hit end of input, but did not find expected end of data indicator"
argument_list|)
throw|;
block|}
comment|// There should be a reader event available, or coming soon, so okay to be blocking call.
name|ReaderEvent
name|event
init|=
name|getReaderEvent
