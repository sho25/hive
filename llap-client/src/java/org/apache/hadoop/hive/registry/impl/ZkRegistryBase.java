...
argument_list|()
decl_stmt|;
comment|// there can be only one instance per path
specifier|private
specifier|final
name|Map
argument_list|<
...
decl_stmt|;
comment|// there can be multiple instances per node
specifier|private
specifier|final
name|Map
argument_list|<
...
name|aclProvider
decl_stmt|;
comment|// get acl provider for most outer path that is non-null
if|if
condition|(
name|userPathPrefix
operator|==
...
argument_list|(
operator|new
name|ZkConnectionStateListener
argument_list|()
argument_list|)
expr_stmt|;
block|}
...
return|;
block|}
comment|/**    * Get the ensemble server addresses from the configuration. The format is: host1:port,    * host2:port..    *    * @param conf configuration    **/
specifier|private
specifier|static
name|String
name|getQuorumServers
...
expr_stmt|;
block|}
comment|// waitForInitialCreate must have already been called in registerServiceRecord.
try|try
block|{
name|znode
operator|.
...
name|error
argument_list|(
literal|"Unable to update znode with new service record"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|warn
argument_list|(
literal|"Invalid LLAP worker node name: {} was {}"
argument_list|,
name|childData
operator|.
name|getPath
...
name|info
argument_list|(
literal|"instance is null for event: {} childData: {}"
argument_list|,
name|event
operator|.
name|getType
...
name|info
argument_list|(
literal|"Connection state change notification received. State: {}"
argument_list|,
name|connectionState
argument_list|)
expr_stmt|;
